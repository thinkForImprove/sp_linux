    ...
    ui(new Ui::CDevSIUTest)
{
    ...
    for(int i = 1; i < 16; i++){
        sprintf(szText, "%d", i);
        ui->flicker_idx_comboBox->addItem(szText);
    }

    for(int i = 1; i < 9; i++){
        sprintf(szText, "%d", i);
        ui->skim_idx_comboBox->addItem(szText);
    }
}
...
void CDevSIUTest::on_flicker_on_clicked()
{
    THISMODULE(__FUNCTION__);
    QString log;
    int iFlickerIdx = atoi(ui->flicker_idx_comboBox->currentText().toStdString().c_str());
    long lRet = m_pDev->SetFlickerLed(iFlickerIdx - 1, 1);
    log = log.asprintf("打开flicker灯%d:", iFlickerIdx);
    AddString("%s lRet = %d", log.toUtf8().constData(), lRet);
}

void CDevSIUTest::on_flicker_off_clicked()
{
    THISMODULE(__FUNCTION__);
    QString log;
    int iFlickerIdx = atoi(ui->flicker_idx_comboBox->currentText().toStdString().c_str());
    long lRet = m_pDev->SetFlickerLed(iFlickerIdx - 1, 0);
    log = log.asprintf("关闭flicker灯%d:", iFlickerIdx);
    AddString("%s lRet = %d", log.toUtf8().data(), lRet);
}

void CDevSIUTest::on_skim_on_clicked()
{
    THISMODULE(__FUNCTION__);
    QString log;
    int iSkimIdx = atoi(ui->skim_idx_comboBox->currentText().toStdString().c_str());
    long lRet = m_pDev->SetSkimLed(iSkimIdx - 1, 1);
    log = log.asprintf("打开SKIM灯%d:", iSkimIdx);
    AddString("%s lRet = %d", log.toUtf8().data(), lRet);
}

void CDevSIUTest::on_skim_off_clicked()
{
    THISMODULE(__FUNCTION__);
    QString log;
    int iSkimIdx = atoi(ui->skim_idx_comboBox->currentText().toStdString().c_str());
    long lRet = m_pDev->SetSkimLed(iSkimIdx - 1, 0);
    log = log.asprintf("关闭SKIM灯%d", iSkimIdx);
    AddString("%s lRet = %d", log.toUtf8().data(), lRet);
}
