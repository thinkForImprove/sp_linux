...
CDevCRM_CRT730B::CDevCRM_CRT730B() : m_devCRT730B(LOG_NAME_DEVCRM),
    m_clErrorDet((LPSTR)"1")      // INI设定CRT730B返卡模块类型编号为1
{
...
int CDevCRM_CRT730B::CMRetainCard(const char *szCardNo, const int nSlotNo)
{
    ...
    {
        if (nRet == IMP_ERR_DEVRET_SLOT_HAVECARD)
        {
...
#define CASE_SET_HW_DETAIL(IMP, DSTR, HSTR) \
    ...
    switch(nRet)
    {
        ...
        // 0~100: 硬件设备返回
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_CMD_INVALID, EC_DEV_Unsup_CMD, "0000000")     // 命令编码未定义
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_CMD_PARAM, EC_DEV_HWParInvalid, "0000001")    // 命令参数错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_CMD_DATA, EC_DEV_HWParInvalid, "0000002")     // 命令数据错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_CMD_UNABLE, EC_DEV_DevHWErr, "0000003")       // 命令无法执行
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_DOOR_ISOPEN, EC_DEV_DevHWErr, "0000006")      // 维护门已经开启（此时不能发送动作指令!）
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_POWER_VOLLOW, EC_DEV_DevHWErr, "0000011")     // 电源电压过低(低于20V)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_POWER_VOLHIGH, EC_DEV_DevHWErr, "0000012")    // 电源电压过高(高于28V)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_POWER_NEEDINIT, EC_DEV_DevHWErr, "0000013")   // 电源恢复正常但没有重新初始化
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_POWER_FRAM, EC_DEV_DevHWErr, "0000016")       // 模块内部FRAM错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_SOLT_SENSER, EC_DEV_DevHWErr, "0000019")      // 暂存仓传感器错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_SLOT_JAM, EC_DEV_DevHWErr, "0000020")         // 升降暂存仓阻塞
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_INCARD_JAM, EC_DEV_MedJammed, "0000022")      // 进卡阻塞
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_INCARD_TIMEOUT, EC_DEV_TimeOut, "0000023")    // 等待进卡超时
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_INCARD_TAKE, EC_DEV_DevHWErr, "0000024")      // 进卡时卡被拔走
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_EJECT_JAM, EC_DEV_MedJammed, "0000028")       // 退卡堵塞
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_POWER_LOCAT_JAM, EC_DEV_DevHWErr, "0000029")  // 电机定位堵塞
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_SLOT_NOTAIM_CK, EC_DEV_DevHWErr, "0000030")   // 暂存仓未对准进卡口
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_SLOT_HAVECARD, EC_DEV_MedIsHave, "0000031")   // 目标暂存仓有卡->指定卡已存在
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_SLOT_NOTHAVE, EC_DEV_MedNotFound, "0000032")  // 目标暂存仓无卡->指定卡不存在
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_CINFO_NOSAVE, EC_DEV_DevHWErr, "0000035")     // 卡片信息未保存->硬件故障
    }
