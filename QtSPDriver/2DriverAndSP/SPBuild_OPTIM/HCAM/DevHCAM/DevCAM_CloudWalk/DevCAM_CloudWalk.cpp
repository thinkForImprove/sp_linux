...
CDevCAM_CloudWalk::CDevCAM_CloudWalk() : CDevImpl_CloudWalk(LOG_NAME_DEVCAM)
{
    ...
    m_wViewMode = 0;                                    // 图像绘制标记(0无动作/1暂停,缺省0)
...
long CDevCAM_CloudWalk::Open(LPCSTR lpMode)
{
    ...
    bGetVersion(szVer1, 64);

    Log(ThisModule, __LINE__, "算法版本号:%s | ", szVer1);
...
long CDevCAM_CloudWalk::Display(DWORD hWnd, WORD wAction, WORD wX, WORD wY, WORD wWidth, WORD wHeight)
{
    ...
    if (wAction == CAMERA_PAUSE)    // 暂停
    {
        ...
        m_wViewMode = 1;                                    // 图像绘制标记(01暂停)

        /*if (m_stCamCwInitParam.wOpenType == CAM_OPEN_DEVIDX)
        {
            // 按序号休眠相机
            bSleepCamera(m_stCamCwInitParam.wVisCamIdx, TRUE);
            bSleepCamera(m_stCamCwInitParam.wNisCamIdx, TRUE);

        } else
        {
            // 按VID/PID休眠相机
            bSleepCameraEx(m_stCamCwInitParam.szVisVid, m_stCamCwInitParam.szVisPid, TRUE);
            bSleepCameraEx(m_stCamCwInitParam.szNisVid, m_stCamCwInitParam.szNisPid, TRUE);
        }*/
    } else
    if (wAction == CAMERA_RESUME)    // 继续
    {
        if (m_bDisplayOpenOK != TRUE)   // Display未打开窗口，直接return
        {
            return CAM_SUCCESS;
        ...
        m_wViewMode = 0;                                    // 图像绘制标记(0无动作)

        /*if (m_stCamCwInitParam.wOpenType == CAM_OPEN_DEVIDX)
        {
            // 按序号恢复休眠相机
            bSleepCamera(m_stCamCwInitParam.wVisCamIdx, FALSE);
            bSleepCamera(m_stCamCwInitParam.wNisCamIdx, FALSE);

        } else
        {
            // 按VID/PID恢复休眠相机
            bSleepCameraEx(m_stCamCwInitParam.szVisVid, m_stCamCwInitParam.szVisPid, FALSE);
            bSleepCameraEx(m_stCamCwInitParam.szNisVid, m_stCamCwInitParam.szNisPid, FALSE);
        }*/
    }
...
long CDevCAM_CloudWalk::TakePictureEx(LPSTR lpFileName, LPCSTR lpCamData, WORD wPicType, bool isContinue, DWORD dwTimeOut, WORD wCamara)
{
    ...
    m_wLiveStat = LIVE_ERR_OK;      // 初始化活体图像检查结果为正常
...
long CDevCAM_CloudWalk::SetData(void *vInitPar, WORD wDataType)
{
    switch(wDataType)
    {
        case DATATYPE_INIT:
        {
            ...
            SetSDKVersion(((LPST_CAM_DEV_INIT_PARAM)vInitPar)->stCamCwInitParam.wSDKVersion);
            break;
        }
        ...
        case DATATYPE_SET_IMAGEPAR:     // 设置图像参数
        {
            memcpy(&m_stCamImageMothod, (LPST_CAM_IMAGE_MOTHOD)vInitPar,
                   sizeof(ST_CAM_IMAGE_MOTHOD));
            break;
        }
        default:
            break;
...
long CDevCAM_CloudWalk::GetData(void *vInitPar, WORD wDataType)
{
    switch(wDataType)
    {
        case DATATYPE_INIT:
            break;
        case DATATYPE_LIVESTAT:     // 活检状态
            *((WORD*)vInitPar) = m_wLiveStat;
            break;
        default:
            break;
    }
...
void CDevCAM_CloudWalk::showLive(int errCode, const float* scores, const float* distances, int nFaceNumber)
{
    ...
    if (errCode == CW_FACE_LIVENESS_IS_LIVE) // 检测到活体
    {
        ...
        if (m_stCamImageMothod.wParam[0] == 1)  // 需要生成红外图像
        {
            CHAR m_pNisSaveFileName[MAX_PATH] = { 0x00 };   // 保存文件名
            CHAR szFileExt[12] = { 0x00 };                  // 保存扩展名
            CWLiveImage live_Nis = { 0x00 };                // 红外摄像数据

            if (GetNirImgSup() != TRUE)
            {
                Log(ThisFile, 1, "当前SDK版本不支持[获取人脸红外数据]功能,不生成红外图像");
            } else
            {
                if ((m_wTakePicMode & TAKEPIC_PERSON) == TAKEPIC_PERSON)    // 人脸摄像
                {
                    FileDir::_split_whole_name(m_pSaveFileName, m_pNisSaveFileName, szFileExt);
                    sprintf(m_pNisSaveFileName + strlen(m_pNisSaveFileName),
                            "%s%s", m_stCamImageMothod.szNirPicSuffix, szFileExt);
                }
                if ((m_wTakePicMode & TAKEPIC_ROOM) == TAKEPIC_ROOM)    // 全景摄像
                {
                    FileDir::_split_whole_name(m_pSaveFileNameRoom, m_pNisSaveFileName, szFileExt);
                    sprintf(m_pNisSaveFileName + strlen(m_pNisSaveFileName),
                            "%s%s", m_stCamImageMothod.szNirPicSuffix, szFileExt);
                }

                code = bGetBestFace(2.0f, FALSE, &live_Nis, 2);     // 人脸红外数据获取
                if (code == TRUE)
                {
                    Log(ThisFile, 1, "showLive()->bGetBestFace(2)获取人脸红外数据成功");
                } else
                {
                    Log(ThisFile, 1, "showLive()->bGetBestFace(2)获取人脸红外数据失败");
                    return;
                }

                bRet = showLiveToImage(live_Nis, m_pNisSaveFileName, m_wSaveFileType);
            }
        }
    ...
    {
        switch(errCode) // 活检图像检查错误记录到全局变量
        {
            case CW_FACE_LIVENESS_VIS_NO_FACE_ERROR:                // 26121086:可见光输入没有人脸
                m_wLiveStat = LIVE_ERR_VIS_NOFACE;                      // 可见光未检测到人脸
                break;
            case CW_FACE_LIVENESS_VIS_OCCLUSION_ERROR:              // 26121041:检测到人脸存在遮挡
            case CW_FACE_LIVENESS_VIS_BLACKSPEC_ERROR:              // 26121042:检测到黑框眼镜
            case CW_FACE_LIVENESS_VIS_SUNGLASS_ERROR:               // 26121043:检测到墨镜
            case CW_LIVENESS_LIV_VIS_PROCEDUREMASK_ERR:             // 26121044:检测到口罩
                m_wLiveStat = LIVE_ERR_FACE_SHELTER;                    // 人脸有遮挡(五官有遮挡,戴镜帽等)
                break;
            case CW_FACE_LIVENESS_DIST_TOO_CLOSE_ERROR:             // 26121032:人脸距离太近
                m_wLiveStat = LIVE_ERR_FACE_TOONEAR;                    // 人脸离摄像头太近
                break;
            case CW_LIVENESS_DISTANCE_TOO_FAR_ERR:                  // 26121033:人脸距离太远
                m_wLiveStat = LIVE_ERR_FACE_TOOFAR;                     // 人脸离摄像头太远
                break;
            case CW_FACE_LIVENESS_POSE_DETECT_FAIL_ERROR:           // 26121034:人脸角度不满足要求
                m_wLiveStat = LIVE_ERR_FACE_ANGLE_FAIL;                 // 人脸角度不满足要求(低头/抬头/侧脸等)
                break;
            case CW_LIVENESS_FACE_CLARITY_DET_FAIL_ERR:             // 26121035:人脸清晰度不满足要求
                m_wLiveStat = LIVE_ERR_FACE_SHAKE;                      // 检测到人脸晃动/模糊
                break;
            case CW_FACE_LIVENESS_VIS_EYE_CLOSE_ERROR:              // 26121036:检测到闭眼
                m_wLiveStat = LIVE_ERR_FACE_EYECLOSE;                   // 检测到闭眼
                break;
            case CW_FACE_LIVENESS_VIS_MOUTH_OPEN_ERROR:             // 26121037:检测到张嘴
                m_wLiveStat = LIVE_ERR_FACE_MOUTHOPEN;                  // 检测到张嘴
                break;
            case CW_FACE_LIVENESS_IS_UNLIVE_ERROR:                  // 26121082:非活体
                m_wLiveStat = LIVE_ERR_IS_UNLIVE;                       // 检测到非活体
                break;
            case CW_FACE_LIVENESS_NIS_NO_FACE_ERROR:                // 26121085:红外输入没有人脸
                m_wLiveStat = LIVE_ERR_NIS_NOFACE;                      // 红外光未检测到人脸
                break;
            case CW_LIVENESS_MULTIPLE_FACES:                        // 75000123:检测到多张人脸
                m_wLiveStat = LIVE_ERR_FACE_MULTIPLE;                   // 检测到多张人脸
                break;
            default:
                //m_wLiveStat = LIVE_ERR_UNKNOWN;                         // 其他/未知错误
                break;
        }
    }
}
