...
private: // XFS_CAM_DEC.cpp
    ...
    bool UpdateStatus(const DEVCAMSTATUS &stStatus,
                      const DEVCAMSTATUS &stStatusRoom,
                      const DEVCAMSTATUS &stStatusHigh);            // 更新状态
    ...
    HRESULT CameraMode_Display(WORD wCamMode, DWORD hWnd, WORD wAction,
                               WORD wX, WORD wY, WORD wHidth, WORD wHeight,
                               WORD wHpixel, WORD wVpixel);         // 按指定模式调用DevCAM接口
    HRESULT CameraMode_TakePictureEx(WORD wCamMode, LPSTR lpFileName, LPCSTR lpCamData,
                                     WORD wPicType, BOOL bIisContinue, DWORD wTimeOut);// 按指定模式调用DevCAM接口
...
private:
    ...
    CQtDLLLoader<IDevCAM>                   m_pDevRoom;             // 全景摄像DevCAM接口实例
    CQtDLLLoader<IDevCAM>                   m_pDevHigh;             // 高拍摄像DevCAM接口实例
    ...
    //BOOL                                    bDisplayOK;
    WORD                                    m_wDisplayOK;           // 当前摄像窗口模式
    WORD                                    m_wIsOpenOk;            // 设备打开
    //BOOL                                    m_bIsOpenOk;            // 30-00-00-00(FT#0031)

    MainWindow                              *showWin;               // 摄像窗口
    QSharedMemory                           *m_qSharedMemData;      // 摄像数据共享内存

    WORD                                    m_wDeviceModeSup;       // 摄像模式支持(INI指定)
    WORD                                    m_wDeviceType;          // 人脸摄像设备类型(INI指定)
    WORD                                    m_wDeviceRoomType;      // 全景摄像设备类型(INI指定)
    WORD                                    m_wDeviceHighType;      // 高拍摄像设备类型(INI指定)
