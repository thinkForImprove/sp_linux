...
* 文件描述: ZL-F1000A3高拍仪模块底层指令，提供控制接口 头文件
*
* 版本历史信息
* 变更说明：建立文件
* 变更日期：2022年2月17日
...
// 摄像类型定义
#define CAM_TYPE_DOC        0       // 文档摄像头
#define CAM_TYPE_PER        1       // 人脸摄像头
...
class CDevImpl_ZLF1000A3 : public CLogManage
{
public:
    CDevImpl_ZLF1000A3();
    CDevImpl_ZLF1000A3(LPSTR lpLog);
    CDevImpl_ZLF1000A3(LPSTR lpLog, LPSTR lpDevStr);
    virtual ~CDevImpl_ZLF1000A3();

public: // 动态库接口封装
    INT DeviceOpen(WORD wType);                                 // 1. 打开设备
...
private:
    ...
    INT             m_nRetErrOLD[12];                           // 处理错误值保存(0:库加载/1:设备连接/2:设备状态/3:Open标记检查/4介质吸入)
    BOOL            m_bDevOpenOk;
    WORD            m_wOpenCamType;
