...
CDevSIU_IOMC::CDevSIU_IOMC(LPCSTR lpDevType): m_cDev(lpDevType), m_cPWDev(lpDevType)
{
    ...
    m_bACR500SoftwareControlFlash = true;
}
...
long CDevSIU_IOMC::Open(LPCSTR lpMode, bool bSensorOnly/* = true*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    m_strOpenMode = lpMode;
    m_bSensorOnly = bSensorOnly;
    ...
    //扎把和强制退证灯常亮
    if(!m_bSensorOnly){
//        SetSKMLampCmd(GUIDLIGHT_CONTINUOUS, 6);
//        SetLightsCmd(GUIDlIGHT_FORCEEJECTIDX, GUIDLIGHT_CONTINUOUS);
    }

    //连接ACR500控制版
    if(!m_bSensorOnly){
        if(!m_strACR500ComParam.empty()){
            if(m_cDevACR500.Open(m_strACR500ComParam.c_str()), m_bACR500SoftwareControlFlash){
                //关闭所有灯
                m_cDevACR500.Reset();
            }
        }
...
//    if (!m_cDev.IsOpen())
    ...
    //关闭常量灯(扎把和强制退证)
    if(!m_bSensorOnly){
//        SetSKMLampCmd(GUIDLIGHT_OFF, 6);
//        SetLightsCmd(GUIDlIGHT_FORCEEJECTIDX, GUIDLIGHT_OFF);
    }
...
long CDevSIU_IOMC::GetStatus(DEVSIUSTATUS &stStatus)
{
    ...
    long lRet = GetSensStatus();
    if (0 != lRet)
    {
        if(lRet == ERR_IOMC_NOT_OPEN || lRet == ERR_IOMC_NO_DEVICE){
            UpdateStatus(DEVICE_OFFLINE, 1);
        } else {
            // 故障时，保持故障前的状态
    ...
    if(!m_bSensorOnly){
        if(!m_strACR500ComParam.empty()){
            unsigned char byLightStatus[7] = {0};
            if(!m_cDevACR500.GetGuidLightStatus(byLightStatus)){
                UpdateStatus(DEVICE_OFFLINE, 1);
                return -1;
            }
        }
...
long CDevSIU_IOMC::SetLightsCmd(WORD wID, WORD wCmd)
{
    ...
    // Set IOMC parameter
    // 支持的灯：0,1,2,3,4,5,6,8,9,10,11,12,13,31
    switch (wID)
    {
    case GUIDLIGHT_CARDUNIT: {                            // Card unit
        bool bTestMode = false;
        int iCardUnitFlickPortIndex = IOMC_FLK_ID_FL_CD + 1;
        CINIFileReader ini;
        if(ini.LoadINIFile("/usr/local/CFES/ETC/SIUConfig.ini")){
            CINIReader iniReader = ini.GetReaderSection("SIUInfo");            
            bTestMode = (int)iniReader.GetValue("PortIndexTestMode", 0) == 1;
            if(bTestMode){
                iCardUnitFlickPortIndex = (int)iniReader.GetValue("CardUnitFlickPortIndex", IOMC_FLK_ID_FL_CD + 1);
                if(iCardUnitFlickPortIndex < 1 || iCardUnitFlickPortIndex > 15){
                    iCardUnitFlickPortIndex = IOMC_FLK_ID_FL_CD + 1;
                }
            }
        }
        stFlickerSetData.Flk[iCardUnitFlickPortIndex - 1] = byNewStatus;
//        stFlickerSetData.Flk[IOMC_FLK_ID_FL_CD] = byNewStatus;
    ...
    case GUIDLIGHT_NOTESDISPENSER:                        // Note dispenser
        if(!m_strACR500ComParam.empty()){
            return m_cDevACR500.SetGuidLight(5, wCmd) ? ERR_IOMC_SUCCESS : ERR_IOMC_HARDWARE_ERROR;
        } else {
            return ERR_IOMC_NOT_SUPPORT;
        }
        break;
    case GUIDLIGHT_COINDISPENSER:                         //射频卡指示灯
    {
        bool bTestMode = false;
        int iFIDCSkmPortIndex = 1;                        //设置默认值
        CINIFileReader ini;
        if(ini.LoadINIFile("/usr/local/CFES/ETC/SIUConfig.ini")){
            CINIReader iniReader = ini.GetReaderSection("SIUInfo");
            bTestMode = (int)iniReader.GetValue("PortIndexTestMode", 0) == 1;
            if(bTestMode){
                iFIDCSkmPortIndex = iniReader.GetValue("FIDCSkmPortIndex", 1);
                if(iFIDCSkmPortIndex < 1 || iFIDCSkmPortIndex > 8){
                    iFIDCSkmPortIndex = 1;
                }
                return SetSKMLampCmd(wCmd, iFIDCSkmPortIndex);
            } else {
//                if(!m_strACR500ComParam.empty()){
//                    return m_cDevACR500.SetGuidLight(1, wCmd) ? ERR_IOMC_SUCCESS : ERR_IOMC_HARDWARE_ERROR;
//                } else {
//                    return ERR_IOMC_NOT_SUPPORT;
//                }
            }
    ...
    case GUIDLIGHT_RECEIPTPRINTER:                        // Receipt printer
        ...
        break;
    case GUIDLIGHT_PASSBOOKPRINTER:                       //存折打印机指示灯
        return SetSKMLampCmd(wCmd, 2);
        break;
    case GUIDLIGHT_ENVDEPOSITORY:                         //刷磁设备
    {
        bool bTestMode = false;
        int iLedIndex = 4;                              //设置默认值
        CINIFileReader ini;
        if(ini.LoadINIFile("/usr/local/CFES/ETC/SIUConfig.ini")){
            CINIReader iniReader = ini.GetReaderSection("SIUInfo");
            bTestMode = (int)iniReader.GetValue("PortIndexTestMode", 0) == 1;
            if(bTestMode){
                iLedIndex = (int)iniReader.GetValue("ACR500LedIndex", 1);
                if(iLedIndex < 1 || iLedIndex > 7){
                    iLedIndex = 1;
                }
            }
        }
        if(!m_strACR500ComParam.empty()){
            return m_cDevACR500.SetGuidLight(iLedIndex, wCmd) ? ERR_IOMC_SUCCESS : ERR_IOMC_HARDWARE_ERROR;
        } else {
            return ERR_IOMC_NOT_SUPPORT;
        }
    }
    ...
    case GUIDLIGHT_BILLACCEPTOR:                          // Note acceptor
        ...
        break;    
    case GUIDLIGHT_ENVDISPENSER:                          //UKey
//        if(!m_strACR500ComParam.empty()){
//            return m_cDevACR500.SetGuidLight(6, wCmd) ? ERR_IOMC_SUCCESS : ERR_IOMC_HARDWARE_ERROR;
//        } else {
//            return ERR_IOMC_NOT_SUPPORT;
//        }
        return SetSKMLampCmd(wCmd, 1);
        break;
    case GUIDLIGHT_DOCUMENTPRINTER:                       //A4 激光打印机灯
//        if(!m_strACR500ComParam.empty()){
//            return m_cDevACR500.SetGuidLight(7, wCmd) ? ERR_IOMC_SUCCESS : ERR_IOMC_HARDWARE_ERROR;
//        } else {
//            return ERR_IOMC_NOT_SUPPORT;
//        }
        return SetSKMLampCmd(wCmd, 2);
        break;
    case GUIDLIGHT_COINACCEPTOR:                          //身份证指示灯
        stFlickerSetData.Flk[IOMC_FLK_ID_FL_CS] = byNewStatus;
        break;
    case GUIDLIGHT_FINGERPRINT:                           // 指纹仪
        stFlickerSetData.Flk[IOMC_FLK_ID_FL_BAR] = byNewStatus;
        break;   
    case GUIDLIGHT_SCANNER:                               //扫描仪器
//        return SetSKMLampCmd(wCmd, 1);
        stFlickerSetData.Flk[IOMC_FLK_ID_LMP_CS] = byNewStatus;
        break;
    case GUIDlIGHT_FORCEEJECTIDX:                         //身份证强制退证灯
        stFlickerSetData.Flk[IOMC_FLK_ID_ICRW_FLK] = byNewStatus;
        break;
    case GUIDLIGHT_ENVSCANNER:                            //支票处理单元
        stFlickerSetData.Flk[IOMC_FLK_ID_ICRW_FLK] = byNewStatus;
        break;
    case GUIDLIGHT_DEPOSITRECPPRINTER:                    //存单打印
        return SetSKMLampCmd(wCmd, 3);
        break;
    case GUIDLIGHT_DEPOSITRECPREADER:                     //存单读取
    default:
        return ERR_IOMC_NOT_SUPPORT;
    }
...
void CDevSIU_IOMC::UpdateSensorsStatus()
{
    ...
    if(m_stSensInfo.byFUKA_SENSE_INF[4] & 0x02){
        m_stStatus.wSensors[SENSORS_PROXIMITY] = SENSORS_OFF;
    } else {
        m_stStatus.wSensors[SENSORS_PROXIMITY] = SENSORS_ON;
    }
}
...
bool CDevSIU_IOMC::SetSupportStatus()
{
    ...
    m_strACR500ComParam = (LPCSTR)cINI.GetValue("ACR500ComParam", "");
    m_bACR500SoftwareControlFlash = (int)cINI.GetValue("ACR500SoftwareControlFlash", 0) == 1;
...
long CDevSIU_IOMC::SetSKMLampCmd(WORD wCmd,  BYTE byPortNum)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    ...
    case GUIDLIGHT_CONTINUOUS:
    {
        ...
        if (lRet != 0)
        {
            Log(ThisModule, __LINE__, "SKM灯[%d]控制命令失败：wCmd:%d,lRet=%d", byPortNum, wCmd, lRet);
            return ERR_IOMC_HARDWARE_ERROR;
        }
        return ERR_IOMC_SUCCESS;
    }
...
void CDevSIU_IOMC::ExitSKMLampFlashThread()
{
    if(m_tid != 0){
        g_bExitThread = true;
...
//40-00-00-00(FT#0001)
long CDevSIU_IOMC::SetFlashRecycleParamer()
{
    THISMODULE(__FUNCTION__);

    STR_IOMC strIomc = {0};
    IOMCRespInfT stRespInf = {0};
    LPIOMCWRITECYCLE lpstIomcWriteCycle = (LPIOMCWRITECYCLE)&strIomc;

    WORD wLen = sizeof(IOMCWRITECYCLE);
    lpstIomcWriteCycle->wLEN = EXWORDHL(wLen);
    lpstIomcWriteCycle->wCNTL_ID = 0x0100;
    lpstIomcWriteCycle->wCNTL_LNG = 0x0400;
    lpstIomcWriteCycle->wCNTL_CMD = 0x0006;

    lpstIomcWriteCycle->wDATA_ID_FLK = 0x4007;
    lpstIomcWriteCycle->wDATA_LNG_FLK = 0x1200;
    memcpy((LPSTR)lpstIomcWriteCycle->FLK_Cycle1, m_byOnOffTime, sizeof(m_byOnOffTime));
...
bool CDevSIU_IOMC::IsDeviceNoraml()
{
    BYTE byLightStatus[7] = {0};
    if(!m_cDev.IsOpen() ||
       (!m_bSensorOnly && !m_strACR500ComParam.empty() && !m_cDevACR500.GetGuidLightStatus(byLightStatus))){
        return false;
    }
