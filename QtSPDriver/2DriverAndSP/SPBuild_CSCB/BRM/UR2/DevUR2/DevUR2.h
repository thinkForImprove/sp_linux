...
public:
    ...
    //返回：0 成功，>0 警告，<0 失败；见返回码定义
    //说明：执行命令的最大时间(秒)：120
    virtual int CloseCashInShutter(                             //test#11
    char pMaintenanceInfo[MAINTENANCE_INFO_LENGTH],             //test#11
    ...
    //返回：0 成功，>0 警告，<0 失败；见返回码定义
    //说明：执行命令的最大时间(秒)：120
    virtual int OpenCashInShutter(                              //test#11
    char pMaintenanceInfo[MAINTENANCE_INFO_LENGTH]              //test#11
    );                                                          //test#11
    ...
    //返回：0 门关闭，1 门打开
    //说明：执行命令的最大时间(秒)：120
    virtual int GetCashInShutterStatus();                   //test#13
    ...
    virtual int  SetUnfitNoteVerifyLevel(eCMDType CmdType, BOOL bSevereModel);
     ...
     //功能：获取入金口Com值
     virtual int  SetComVal(const char * strCashInShutterPort);
protected:
