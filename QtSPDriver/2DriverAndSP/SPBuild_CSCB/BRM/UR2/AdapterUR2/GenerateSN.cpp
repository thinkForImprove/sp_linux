...
CThreadGenerateSN::CThreadGenerateSN(EnumSaveSerialOperation nOper, ADP_LFS_CMD_ID eLfsCmdId, int iResult, BOOL bSNAddingMode)
...
bool CThreadGenerateSN::GenerateData()
{
    ...
    m_pSaveCusSNInfo->SetSNAdingMode(m_bSNAddingMode);
    ...
    for (int i = 0; ittrace != m_mapNotesTraceInfo.end(); ittrace++, i++)
    {
        ...
        if (IsNeedSaveSNIMG())
        {
            ...
            ReplaceChar(pSNInfo->cSerialNumber, '*', '$');
                                               ...
                                               const ST_MEDIA_INFORMATION_INFO *pTraceInfo, char lpImageName[MAX_PATH])
{
    ...
    case DENOMINATION_CODE_5_B:       memcpy(&lpImageName[25], "0051999", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_5_C:       memcpy(&lpImageName[25], "0052005", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_5_D:       memcpy(&lpImageName[25], "0052020", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_1_B:       memcpy(&lpImageName[25], "0011999", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_1_D:       memcpy(&lpImageName[25], "0012019", 7); break;        //30-00-00-00(FS#0018)
...
long CThreadGenerateSN::InitDllSaveFileSNInfo()
{
    ...
    m_pSaveCusSNInfo->GetSaveImgConfig(m_stIMGConfig);
    Log("InitDllSaveFileSNInfo", 1, "DefaultImgPath:%s", m_stIMGConfig.szDefImgPath);
    return WFS_SUCCESS;
}
