...
#define HARD_INFO_TCR       0x40              //test#13
...
CUR2Adapter::CUR2Adapter()
{
    ...
    cFirmHardInfo = HARD_INFO_TCR;                      //test#13

    memset(&stStackeNotesDenoInfoTemp, 0, sizeof(stStackeNotesDenoInfoTemp));          //test#13
    memset(&stStackeUnfitNotesDenoInfoTemp, 0, sizeof(stStackeUnfitNotesDenoInfoTemp));     //test#13
    memset(&stStackeRejectNotesDenoInfoTemp, 0, sizeof(stStackeRejectNotesDenoInfoTemp));    //test#13
...
long CUR2Adapter::Open()
{
    ...
    m_pUR2Dev->SetComVal(m_strCashInShutterPort.c_str());
...
long CUR2Adapter::InitADPConfig()
{
    ...
    szValue = (LPCSTR)cINI.GetValue("CashInShutterPort",           //test#11
                                              "COM:/dev/ttyUSB0:57600,N,8,1");           //test#11

    m_strCashInShutterPort = (string)szValue;
...
//test#13    iValue = (int)cINI.GetValue("CashOutMaxNum", "200");   //test#3
       iValue = (int)cINI.GetValue("CashOutMaxNum", "10000");    //test#13
//test#13    m_sCDMCapabilities.wMaxDispenseItems = ((iValue <= 200) && (iValue > 0)) ? iValue : 200;
          m_sCDMCapabilities.wMaxDispenseItems = ((iValue <= 10000) && (iValue > 0)) ? iValue : 10000;  //test#13
    m_sCDMCapabilities.bShutter = TRUE;
//test#12    m_sCDMCapabilities.bShutterControl = TRUE;//固定值 见SR001006
    m_sCDMCapabilities.bShutterControl = TRUE;                 //test#12
 ...
 //test#13   iValue = (int)cINI.GetValue("CashInMaxNum", "200");
   iValue = (int)cINI.GetValue("CashInMaxNum", "10000");            //test#13
    Log(ThisModule,  1, "CashInMaxNum=%d", iValue);
//test#13    m_sCIMCapabilities.wMaxCashInItems = ((iValue <= 300) && (iValue >= 0)) ? iValue : 300;
   m_sCIMCapabilities.wMaxCashInItems = ((iValue <= 10000) && (iValue >= 0)) ? iValue : 10000;    //test#13
...
long CUR2Adapter::SetUnitInfo()
{
    ...
    char usArryCassCashInPrioritySet[MAX_CASSETTE_NUM] ;
    char usArryCassCashOutPrioritySet[MAX_CASSETTE_NUM];

    for (int i = 0; i < MAX_CASSETTE_NUM; i++)
    {
        usArryCassCashInPrioritySet[i] = 1;
        usArryCassCashOutPrioritySet[i] = 1;
    }
...
//test by guojy        int iRet = m_pUR2Dev->SetUnitInfo(iTotalCashInURJB, /*tTime,*/ stOperationalInfo,   m_stHWConfig, stCassType, bArryAcceptDenoCode,
//test by guojy                                         stCashCountLevel, stStoreMoneyLevel, stDispenseLevel, usArryCassCashInPrioritySet, usArryCassCashOutPrioritySet,
//test by guojy                                          m_sDevConfigInfo.bURJBSupp);
//test by guojy        UpdateADPDevStatus(iRet);
    }
/*
...
long CUR2Adapter::GetStatus(ADPSTATUSINFOR &Status)
{
    ...
    if (!m_bConnectNormal || !m_bBRMInitialized)
    {
        ...
        m_sDevStatus.stInShutter = ADP_INSHUTTER_UNKNOWN;
        m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_UNKNOWN;
 //test#12       m_sDevStatus.stStacker = ADP_STACKER_UNKNOWN;
        m_sDevStatus.stStacker = ADP_STACKER_NOTSUPPORTED;           //test#12
        m_sDevStatus.stTransport = ADP_TRANSPORT_UNKNOWN;
    }
...
//返回：无
//说明：内部方法
void CUR2Adapter::ClearError()
{
    const char *ThisModule = "ClearError";

    m_bIsNeedReset = FALSE;
    m_bCloseShutterJammed = FALSE;
    m_bOpenShutterJammed = FALSE;
    SaveLastErrCode(ERR_UR_SUCCESS, ThisModule);
    SaveLastActionErrCode(ERR_UR_SUCCESS, "");
    UpdateADPDevStatus(ERR_UR_SUCCESS);
}
...
//test#13             || stDevStatusInfo.bESCOpen
...
//test#13             || !stDevStatusInfo.bESCInPos
        ...
        || m_sCIMCapabilities.bShutter)
    {
        ...
        if (m_bCloseShutterJammed || m_bOpenShutterJammed)
        {
            ...
            if(stDevStatusInfo.iOutShutterStatus == UR2SHUTTER_STATUS_CLOSED){                //test#13
                m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_CLOSED;                            //test#13
            }else{                                                                            //test#13
                m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_OPEN;                              //test#13
            }                                                                                 //test#13
            if(stDevStatusInfo.CashInShutterStatus == UR2SHUTTER_STATUS_CLOSED){              //test#13
                m_sDevStatus.stInShutter = ADP_INSHUTTER_CLOSED;                              //test#13
            }else{                                                                            //test#13
                m_sDevStatus.stInShutter = ADP_INSHUTTER_OPEN;                                //test#13
            }                                                                                 //test#13
        }                                                                                     //test#13
  //test#13       else if (stDevStatusInfo.iOutShutterStatus == UR2SHUTTER_STATUS_CLOSED
  //test#13                 /*stDevStatusInfo.iInnShutterStatus == UR2SHUTTER_STATUS_CLOSED*/)//门处于关闭状态并且 阻塞标识已清除
  //test#13         {
  //test#13             m_sDevStatus.stInShutter = ADP_INSHUTTER_CLOSED;
  //test#13            m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_CLOSED;
  //test#13        }
  //test#13         else //门处于打开状态并且 阻塞标识已清除
  //test#13        {
  //test#13            m_sDevStatus.stInShutter = ADP_INSHUTTER_OPEN;
  //test#13            m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_OPEN;
  //test#13        }
    }
    else
    {
    ...
    if (m_sDevConfigInfo.bSafeDoorStatusSupport)
    {
        ...
        if (dwIniValue == 2)
        {
            m_sDevStatus.stSafeDoor = ADP_SAFEDOOR_OPEN;
        }
        else if (dwIniValue == 1)
        {
            m_sDevStatus.stSafeDoor = ADP_SAFEDOOR_CLOSED;
        }
        else
        {
            m_sDevStatus.stSafeDoor = ADP_SAFEDOOR_UNKNOWN;
 ...
 /*   if (stDevStatusInfo.bCashAtCS
    ...
    if(stDevStatusInfo.bCashAtCSErrPos
       || stDevStatusInfo.bCashAtShutter){
       m_sDevStatus.stOutPutPosition = ADP_OUTPOS_NOTEMPTY;
    ...
    if(stDevStatusInfo.bCashAtOutCS){
       m_sDevStatus.stOutPutPosition = ADP_OUTPOS_NOTEMPTY; //test#28

    }else{
       m_sDevStatus.stOutPutPosition = ADP_OUTPOS_EMPTY;      //test#28
    }

    // 获取暂存区状态
//test#13    if (stDevStatusInfo.bESCEmpty)
//test#13    {

//            m_sDevStatus.stStacker = ADP_STACKER_EMPTY;   // 导致present无法执行

/*//test#13    }
    else if (stDevStatusInfo.bESCFull)
    {
...
long CUR2Adapter::QueryDevInfo()
{
    ...
    //追加入金口门状态取得处理
    m_pUR2Dev->GetCashInShutterStatus();        //test#13
                           ...
                           BOOL bSNAddingMode)
{
    ...
    iRet = DispenseFromAllCass(aryulItems, aryulDispenseCount, aryulRejects, arywCUError, iRejBillPerDenAll, bSNAddingMode);
...
//返回：XFS通用错误码及CIM错误码
//说明：
//test#13 long CUR2Adapter::ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject)
long CUR2Adapter::ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject, ADP_CUERROR arywCUError[],WORD wCashInCnt)  //test#13
{
    ...
    int iAmount = 0;                                            //test#27
    int iAmountLimit = 0;                                       //test#27
    int iValue = 0;                                             //test#27
    CINIReader cINI = m_ReaderConfigFile.GetReaderSection("BRMInfo");
    if(m_stCashInLimitInf.ulTotalItemsLimit != 0){
        iAmount = static_cast<int>(m_stCashInLimitInf.ulTotalItemsLimit);
    }else{
         iAmountLimit = static_cast<int>(cINI.GetValue("CashCountMaxAmount", "0"));

        if(iAmountLimit == 0){                                  //test#5
            iAmountLimit = iAmountLimitForAPTmp;                //test#5
        }                                                       //test#5
        iAmount = iAmountLimit;
    }

    if(m_stCashInLimitInf.lpAmountLimit.ulAmount != 0){                         //test#27
        iValue = static_cast<int>(m_stCashInLimitInf.lpAmountLimit.ulAmount);   //test#27
    }else{                                                                      //test#27
        iValue = static_cast<int>(cINI.GetValue("CashInMaxNum", "200"));
    }                                                                           //test#27
    NOTE_POWER_INDEX iPowerIndex = POWER_INDEX_1;
    Log(ThisModule, 0, "CashCount 最大张数：%d, 最大金额为:%d", iValue, iAmountLimit);

    BYTE ucProhibited = 0x00;                                       //test#13
    GetProhibitedCashUnitBit(m_arybEnableCassUnit, ucProhibited);   //test#13
    ...
    int iRet = m_pUR2Dev->CashCount(m_sDevConfigInfo.CashInNoteType,  iValue, iAmount, iPowerIndex,
                                    ...
                                    nRejectNotes, stStackeNotesDenoInfo,stStackeUnfitNotesDenoInfo, stStackeRejectNotesDenoInfo, ucProhibited,       //test#13
                                    usFedNoteCondition, pMaintenanceInfo);
    SaveLastErrCode(iRet, ThisModule);
    ...
    //test#13 start
    if(wCashInCnt == 1){
        stStackeNotesDenoInfoTemp = stStackeNotesDenoInfo;
        stStackeUnfitNotesDenoInfoTemp = stStackeUnfitNotesDenoInfo;
        stStackeRejectNotesDenoInfoTemp = stStackeRejectNotesDenoInfo;

    }
    if(wCashInCnt >= 2){
        AccumulatePerDenomCount(stStackeNotesDenoInfoTemp, stStackeNotesDenoInfo);              //40-00-00-00(FT#0016)
        AccumulatePerDenomCount(stStackeUnfitNotesDenoInfoTemp, stStackeUnfitNotesDenoInfo);    //40-00-00-00(FT#0016)
        AccumulatePerDenomCount(stStackeRejectNotesDenoInfoTemp, stStackeRejectNotesDenoInfo);  //40-00-00-00(FT#0016)
 /*40-00-00-00(FT#0016) del start
        for(int m = 0; m < MAX_DENOMINATION_NUM; m++){
            if(stStackeNotesDenoInfoTemp.stStackeNotesInfo[m].ucDENOCode ==
                    stStackeNotesDenoInfo.stStackeNotesInfo[m].ucDENOCode){
                    stStackeNotesDenoInfoTemp.stStackeNotesInfo[m].usNumberStack +=
                            stStackeNotesDenoInfo.stStackeNotesInfo[m].usNumberStack;
//遗留问题：相同NoteID的钞票收纳到不同抄箱时，收纳目标钞箱不能区分
            }
            if(stStackeUnfitNotesDenoInfoTemp.stStackeNotesInfo[m].ucDENOCode ==
                    stStackeUnfitNotesDenoInfo.stStackeNotesInfo[m].ucDENOCode){
                    stStackeUnfitNotesDenoInfoTemp.stStackeNotesInfo[m].usNumberStack +=
                            stStackeUnfitNotesDenoInfo.stStackeNotesInfo[m].usNumberStack;
            }
            if(stStackeRejectNotesDenoInfoTemp.stStackeNotesInfo[m].ucDENOCode ==
                    stStackeRejectNotesDenoInfo.stStackeNotesInfo[m].ucDENOCode){
                    stStackeRejectNotesDenoInfoTemp.stStackeNotesInfo[m].usNumberStack +=
                            stStackeRejectNotesDenoInfo.stStackeNotesInfo[m].usNumberStack;
            }
        }
40-00-00-00(FT#0016) del end*/
    }
    //test#13 end
...
//40-00-00-00(FT#0016) add start
    //统计金种对应的张数
    int iBoxIdx = 0;
    std::map<BYTE, int> mapNoteIdToCount;
    for(int i = 0; i < MAX_DENOMINATION_NUM; i++){
        if(stStackeNotesDenoInfo.stStackeNotesInfo[i].usNumberStack > 0){
            iBoxIdx = stStackeNotesDenoInfo.stStackeNotesInfo[i].iDest & 0xf0;
            if(iBoxIdx > 0){
                mapNoteIdToCount[stStackeNotesDenoInfo.stStackeNotesInfo[i].ucDENOCode] +=
                        stStackeNotesDenoInfo.stStackeNotesInfo[i].usNumberStack;
            }
        }
    }
    //损券
    for(int i = 0; i < MAX_DENOMINATION_NUM; i++){
        if(stStackeUnfitNotesDenoInfo.stStackeNotesInfo[i].usNumberStack > 0){
            iBoxIdx = stStackeUnfitNotesDenoInfo.stStackeNotesInfo[i].iDest & 0xf0;
            if(iBoxIdx > 0){
                mapNoteIdToCount[stStackeUnfitNotesDenoInfo.stStackeNotesInfo[i].ucDENOCode] +=
                        stStackeUnfitNotesDenoInfo.stStackeNotesInfo[i].usNumberStack;
            }
        }
    }

    std::map<BYTE, int>::iterator itr = mapNoteIdToCount.begin();
    for(itr; itr != mapNoteIdToCount.end(); itr++) {
        map<BYTE, WFSCIMNOTETYPE>::const_iterator it = m_mapSupportNotID.find(itr->first);
...
/*40-00-00-00(FT#0016) del start
    for (int j = 0; j < MAX_DENOMINATION_NUM ; j++)
    {
        ...
        if (it != m_mapSupportNotID.end() && it->second.bConfigured
   //test#13         && (stStackeNotesDenoInfo.stStackeNotesInfo[j].iDest == ID_ESC))
         && ((stStackeNotesDenoInfo.stStackeNotesInfo[j].iDest == ID_ROOM_1) ||     //test#13
             (stStackeNotesDenoInfo.stStackeNotesInfo[j].iDest == ID_ROOM_2) ||    //test#13
             (stStackeNotesDenoInfo.stStackeNotesInfo[j].iDest == ID_ROOM_3) ||     //test#13
             (stStackeNotesDenoInfo.stStackeNotesInfo[j].iDest == ID_ROOM_4) ||     //test#13
             (stStackeNotesDenoInfo.stStackeNotesInfo[j].iDest == ID_ROOM_5)))      //test#13
        {
            //形成验钞结果数据
            if ((ioffset + 1) * 12 > 256)
            {
...
40-00-00-00(FT#0016) del end*/
    ...
    //点钞数为0
 /*//test#13 高存机暂不支持ESC   if (iNumStackedToESC == 0)
    {
    ...
    if (m_pErrCodeTrans->IsTooManyItemInCSOfCashCount(m_szLastErrCode))
    {
        Log(ThisModule, WFS_ERR_CIM_TOOMANYITEMS, "CS钞票过多，验钞失败(ErrCode:%s)", m_szLastErrCode);
        iTooManeyItemsHappenFlg = 1;
    }
    ...
    for (int i = 0; i < MAX_CASSETTE_NUM; i++)                                                                        //test#13
    {                                                                                                                 //test#13
        if (pNumStackedToPerCass[i] > 0)                                                                              //test#13
        {                                                                                                             //test#13
            arywCUError[i] = ADP_CUERROR_OK;                                                                          //test#13
        }                                                                                                             //test#13
    }                                                                                                                 //test#13
    BOOL bCassErr = m_pErrCodeTrans->GetCUErr(m_szLastErrCode, m_arybEnableCassUnit, arywCUError);                    //test#13

    if (iRet < 0)                                                                                                     //test#13
    {                                                                                                                  //test#13
        Log(ThisModule, WFS_ERR_HARDWARE_ERROR, "存钞失败(ErrCode:%s)(ReturnValue:%d)", m_szLastErrCode, iRet);         //test#13
        return bCassErr ? WFS_ERR_CIM_CASHUNITERROR : WFS_ERR_HARDWARE_ERROR;                                          //test#13
    }                                                                                                                  //test#13
...
long CUR2Adapter::StoreCash(char ppNoteCounts[ADP_MAX_CU_SIZE][256], ADP_CUERROR arywCUError[ADP_MAX_CU_SIZE])
{
	...
	//test#13 start
    if(cFirmHardInfo == HARD_INFO_TCR){
      ...
      for (int j = 0; j < MAX_DENOMINATION_NUM ; j++)
      {
          stNoteDeInfo = stStackeNotesDenoInfoTemp.stStackeNotesInfo[j];
          if (stNoteDeInfo.usNumberStack > 0)
          {
              if(stNoteDeInfo.iDest == ID_CS) continue;  //test#13
              ...
              if ((iCassOffset[ucCassID - 1] + 1) * 12 > 256)             //40-00-00-00(FT#0016)
              {
      ...
      for (int j = 0; j < MAX_DENOMINATION_NUM ; j++)
      {
          stNoteUnfiDeInfo = stStackeUnfitNotesDenoInfoTemp.stStackeNotesInfo[j];
          if (stNoteUnfiDeInfo.usNumberStack > 0)
          {
              if(stNoteUnfiDeInfo.iDest == ID_CS) continue;  //test#13
...
long CUR2Adapter::Init(ADP_CUERROR arywCUError[ADP_MAX_CU_SIZE])
{
    ...
    iRet = m_pUR2Dev->CloseCashInShutter(pMaintenanceInfo);          //test#11
    if (iRet == 0)                                                  //test#11
    {                                                               //test#11
        m_bCloseShutterJammed = FALSE;                               //test#11
    }                                                               //test#11
    return WFS_SUCCESS;
}
                                      ...
                                      BOOL bSNAddingMode)
{
    ...
    for (int i = 0; i < MAX_CASSETTE_NUM; i++)  // 重置钞箱出空错误 lfg
    {
        if(stDispMissfeedRoom.bArryRoom[i] && stDispMissfeedRoom.bRoomEmpty){
            arywCUError[i] = ADP_CUERROR_EMPTY;
            break;
        }
    }
 ...
 //test#4    {
        CThreadGenerateSN SNWork(SAVESERIALOPERAT_D, m_eLfsCmdId, iRet, bSNAddingMode);     //test#8
        QueryNoteMediaInfo(&SNWork);
        QueryNoteSerialInfo(&SNWork);
  //test#4   }
                                ...
                                ADP_CUERROR arywCUError[ADP_MAX_CU_SIZE])
{
    ...
    ULONG iRejBillPerDenAll[ADP_MAX_DENOMINATION_NUM] = {0};
    iRet = DispenseFromAllCass(aryCassItems, aryulDispenseCount, aryulRejects, arywCUError, iRejBillPerDenAll, FALSE);
...
long CUR2Adapter::OpenShutter(BOOL bInShutter)
{
    ...
    VERIFYPOINTNOTEMPTY(m_pUR2Dev)
 //test#11   return OpenDeviceShutter();
  return OpenDeviceShutter(bInShutter);               //test#11
}
...
long CUR2Adapter::CloseShutter(BOOL bInShutter)
{
    ...
    VERIFYPOINTNOTEMPTY(m_pUR2Dev)
 //test#11   return CloseDeviceShutter();
    return CloseDeviceShutter(bInShutter);      //test#11
}
...
long CUR2Adapter::RetractNotes(USHORT usRetraceArea, USHORT usIndex, ADP_CUERROR arywCuError[ADP_MAX_CU_SIZE])
{
    ...
    if (m_sDevStatus.stStacker == ADP_STACKER_NOTEMPTY)
    {
        //将TS的钞票退回到CS
...
BOOL CUR2Adapter::QueryNoteMediaInfo(CThreadGenerateSN *pwork)
{
    ...
    while (TRUE)
    {
        lRet = m_pUR2Dev->GetMediaInformation(usIndex, usNumNotes, usNumTotalNotes, aryMedioInfo);
        if (lRet < 0)
        {
...
//返回：XFS通用错误码及CDM错误码
//说明：内部函数
//test#11    long CUR2Adapter::OpenDeviceShutter()
long CUR2Adapter::OpenDeviceShutter(BOOL bInShutter)                   //test#11
{
    ...
    if(bInShutter == TRUE){                                             //test#11
        iRet = m_pUR2Dev->OpenCashInShutter(pMaintenanceInfo);          //test#11
        if (iRet == 0)                                                  //test#11
        {                                                               //test#11
            m_bOpenShutterJammed = FALSE;                               //test#11
        }                                                               //test#11
    }else{
        iRet = m_pUR2Dev->OpenShutter(pMaintenanceInfo);                    //test#11
        if (iRet == 0)
        {
            m_bOpenShutterJammed = FALSE;
        }
...
//返回：XFS通用错误码及CDM错误码
//说明：内部函数
//test#11  long CUR2Adapter::CloseDeviceShutter()
long CUR2Adapter::CloseDeviceShutter(BOOL bInShutter)       //test#11
{
    ...
    if(bInShutter == TRUE){                                             //test#11
        iRet = m_pUR2Dev->CloseCashInShutter(pMaintenanceInfo, FALSE);  //test#11
        if (iRet == 0)                                                  //test#11
        {                                                               //test#11
            m_bCloseShutterJammed = FALSE;                              //test#11
        }                                                               //test#11
    }else{                                                              //test#11
        iRet = m_pUR2Dev->CloseShutter(pMaintenanceInfo, FALSE);        //test#11
        if (iRet == 0)
        {
            m_bCloseShutterJammed = FALSE;
        }
        ...
        {
            iRet = m_pUR2Dev->CloseShutter(pMaintenanceInfo, m_sDevConfigInfo.bCloseShutterForce);
            if (iRet == 0)
            {
                m_bCloseShutterJammed = FALSE;
            }
...
//返回：XFS通用错误码
//说明：内部函数
void CUR2Adapter::SaveLastErrCode(int iRet, LPCSTR lpModule)
{
...
//40-00-00-00(FT#0016)
void CUR2Adapter::AccumulatePerDenomCount(ST_TOTAL_STACKE_NOTES_DENO_INFO &stResult, ST_TOTAL_STACKE_NOTES_DENO_INFO &stAppend)
{
    THISMODULE(__FUNCTION__);
    for(int m = 0; m < MAX_DENOMINATION_NUM; m++){
        if(stAppend.stStackeNotesInfo[m].ucDENOCode == 0){
            break;
        }

        int n;
        bool bFind = false;
        for(n = 0; n < MAX_DENOMINATION_NUM; n++){
            if(stResult.stStackeNotesInfo[n].ucDENOCode == 0){
                break;
            }
            //当前存在NoteID和Dest均相同的，只增加计数
            if((stAppend.stStackeNotesInfo[m].ucDENOCode == stResult.stStackeNotesInfo[n].ucDENOCode) &&
               (stAppend.stStackeNotesInfo[m].iDest == stResult.stStackeNotesInfo[n].iDest)){
                stResult.stStackeNotesInfo[n].usNumberStack += stAppend.stStackeNotesInfo[m].usNumberStack;
                bFind = true;
                break;
            }
        }

        //当前未存在NoteID和Dest均相同的，直接追加
        if(!bFind){
            if(n < MAX_DENOMINATION_NUM){
                stResult.stStackeNotesInfo[n] = stAppend.stStackeNotesInfo[m];
                stResult.ucCount++;
            } else {
                Log(ThisModule, __LINE__, "ST_TOTAL_STACKE_NOTES_DENO_INFO->stStackeNotesInfo size not enougn[%d]", n);
            }
        }
    }
...
void CUR2Adapter::WriteNoteVersionInfoToIni()
{
    ...
    for(auto it : m_mapBVSupportNoteID){
        ...
        sprintf(szKeyValue, "%s %04lu %d", it.second.cCurrencyID,
...
//test#27
void  CUR2Adapter::vSetCimCashinLimit(ULONG ulTotalItemsLimit,ULONG ulAmount, LPCSTR lpCurrencyID)
{
    const char *ThisModule = "vSetCimCashinLimit";
    AutoLogFuncBeginEnd();

    int index;
    //only be called once after the WFS_CMD_CIM_CASH_IN_START command
    //and before the first WFS_CMD_CIM_CASH_IN command, otherwise it will fail with the WFS_ERR_SEQUENCE_ERROR error
    //Any command that completes the cash-in transaction (i.e. WFS_CMD_CIM_CASH_IN_END,
    //WFS_CMD_CIM_CASH_IN_ROLLBACK, WFS_CMD_CIM_RETRACT and WFS_CMD_CIM_RESET commands) will clear the limit
    if(ulTotalItemsLimit != 0 || ulAmount != 0){
         //入金限额枚数
        if (ulTotalItemsLimit <= 0xFFFF) {						//STD40-03-01-18(FS#00040)
            //UR-TCR最大收纳枚数（0xFFFF）以下
            m_stCashInLimitInf.ulTotalItemsLimit = ulTotalItemsLimit;
        } else {
            //指定金额大于0x7FFF*pow(10,5)时，不设定。
            m_stCashInLimitInf.ulTotalItemsLimit = 0;
            Log(ThisModule, 1, "CIMCashInLimit is warning! CashInLimit=%d ",m_stCashInLimitInf.ulTotalItemsLimit);
        }

        //入金限额
        if(ulAmount == 0){
            m_stCashInLimitInf.lpAmountLimit.ulAmount		= 0;
            m_stCashInLimitInf.lpAmountLimit.byPowerIndex	= 0;
          } else {
              for(index=0; index<=5; index++){
                if(ulAmount <= static_cast<ULONG>(0x7FFF * (10^index))){
                    m_stCashInLimitInf.lpAmountLimit.ulAmount = (ulAmount / static_cast<ULONG>(10^index));
                    m_stCashInLimitInf.lpAmountLimit.byPowerIndex = static_cast<BYTE>(index);
                    break;
                }
            }
            if(index > 5){
    ...
    string strAdpConfigUR2File = string(SPETCPATH) + "/" + URIniName;
    CINIFileReader AdpConfigUR2Reader;
    AdpConfigUR2Reader.LoadINIFile(strAdpConfigUR2File);
    CINIWriter cINIUR2 = AdpConfigUR2Reader.GetWriterSection("BRMInfo");
    char cBuf[1024];
    ...
    cINIUR2.SetValue("CashCountMaxAmount", static_cast<DWORD>(m_stCashInLimitInf.lpAmountLimit.ulAmount));
    cINIUR2.SetValue("CashInMaxNum", static_cast<DWORD>(m_stCashInLimitInf.ulTotalItemsLimit));
}
