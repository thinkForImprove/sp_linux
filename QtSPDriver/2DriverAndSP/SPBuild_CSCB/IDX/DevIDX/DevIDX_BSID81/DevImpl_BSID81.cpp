...
BOOL CDevImpl_BSID81::bLoadLibrary()
{
    ...
    if (m_bLoadIntfFail)
    {
        ...
        {
            Log(ThisFile, 1, "加载动态库函数接口<%s> fail. ReturnCode:%s.",
...
void CDevImpl_BSID81::vUnLoadLibrary()
{
    if (m_LoadLibrary.isLoaded())
    {
        ...
        {
            Log(ThisFile, 1, "卸载动态库<%s> fail. ReturnCode:%s.",
                m_szLoadDllPath, m_LoadLibrary.errorString().data());
        }
        m_bLoadIntfFail = TRUE;
    }
...
BOOL CDevImpl_BSID81::OpenDevice(WORD wType)
{
    ...
    {
        Log(ThisFile, 1, "加载动态库: OpenDevice()->bLoadLibrary() fail. ");
        return FALSE;
    }
    ...
    {
        Log(ThisFile, 1, "枚举扫描设备: OpenDevice()->bEnumScannerDevice() fail. ");
        return FALSE;
    }
    ...
    {
        Log(ThisFile, 1, "打开设备: OpenDevice()->bOpenConnection() fail. ");
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::CloseDevice()
{
    ...
    Log(ThisFile, 1, "设备关闭,释放动态库: Close Device(%d), unLoad Library.", m_bsDeviceId);
...
BOOL CDevImpl_BSID81::bEnumScannerDevice()
{
    ...
    {
        Log(ThisFile, 1, "枚举扫描设备: bEnumScannerDevice()->EnumScannerDevice() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bOpenConnection()
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "打开设备: bOpenConnection()->OpenConnection(%d) fail. ReturnCode:%s", m_bsDeviceId, GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
    Log(ThisFile, 1, "打开设备: bOpenConnection()->OpenConnection(%d) succ. ", m_bsDeviceId);
...
BOOL CDevImpl_BSID81::bCloseConnection()
{
    ...
    {
        Log(ThisFile, 1, "关闭设备: bCloseConnectio()->CloseConnection(%d) fail. CloseConnection is NULL, Not LoadLibrary", m_bsDeviceId);
        return FALSE;
    }
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "关闭设备: bCloseConnectio()->CloseConnection(%d) fail. ReturnCode:%s", m_bsDeviceId, GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bCheckHaveIdCard(UINT unChkTime)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        //Log(ThisFile, 1, "检测是否放入卡: bCheckHaveIdCard()->CheckHaveIdCard(%d, %d) fail. ReturnCode:%s",
...
BOOL CDevImpl_BSID81::bStartScanIdCard()
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "启动扫描: bStartScanIdCard()->StartScanIdCard(%d) fail. ReturnCode:%s",
...
BOOL CDevImpl_BSID81::bSavePicToMemory(LPSTR lpFrontImgBuf, LPSTR lpRearImgBuf, LPINT lpFrontLen, LPINT lpRearLen)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "读取当前图像数据块到内存: bSavePicToMemory()->SavePicToMemory(%d) fail. Result: %d|%d, ReturnCode:%s",
...
BOOL CDevImpl_BSID81::bSavePicToFile(LPSTR lpImgBuf, UINT uBufLen, LPSTR lpFileName, INT Format)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "保存图像数据块到文件: bSavePicToFile()->SavePicToFile(%d, Data, %d, %s, %d|%s) fail. ReturnCode:%s",
...
BOOL CDevImpl_BSID81::bSavePicToFileII(LPSTR lpImgBuf, UINT uBufLen, LPSTR lpFileName, INT Format, FLOAT fZoomScale)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "保存图像数据块到文件(指定缩放比例): bSavePicToFileII()->SavePicToFileII(%d, Data, %d, %s, %d|%s, %.2f) fail. ReturnCode:%s",
...
BOOL CDevImpl_BSID81::bRetainIdCard()
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "吞卡: bRetainIdCard()->RetainIdCard() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bBackIdCard()
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "退卡: bBackIdCard()->BackIdCard() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bBackAndHoldIdCard()
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "出卡并持卡: bBackAndHoldIdCard()->BackAndHoldIdCard() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bGetID2Info(IDInfo &stIDCard, LPSTR lpHeadImageName)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "获取二代证信息(国内): bGetID2Info()->GetID2Info() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bGetID2InfoEx(IDInfoEx &stIDCard, LPSTR lpHeadImageName)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "获取二代证信息(国内,带指纹): bGetID2InfoEx()->GetID2InfoEx() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bGetIDInfoForeign(IDInfoForeign &stIDCard, LPSTR lpHeadImageName)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "获取二代证信息(国外): bGetIDInfoForeign()->GetIDInfoForeign() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bGetIDInfoGAT(IDInfoGAT &stIDCard, LPSTR lpHeadImageName)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "获取二代证信息(卡类型): bGetIDInfoGAT()->GetIDInfoGAT() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bGetIDCardType(LPINT lpCardType)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "获取二代证信息(卡类型): bGetIDCardType()->bGetIDCardType(%d) fail. ReturnCode:%s",
            m_bsDeviceId, GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bGetFWVersion(LPSTR lpFwVer)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "获取固件版本信息: bGetFWVersion()->GetFWVersion(%d) fail. ReturnCode:%s",
            m_bsDeviceId, GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bGetSWVersion(LPSTR lpSwVer)
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "获取软件版本信息: bGetSWVersion()->GetSWVersion() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bGetDevStatus(DEVSTATUS &devStatus)
{
    ...
    {
        Log(ThisFile, 1, "获取设备状态: bGetDevStatus()->GetDevStatus() fail. ReturnCode:%s", GetErrorStr(m_nStatErrCode));
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bResetDevice()
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "硬复位设备: bResetDevice()->ResetDevice() fail. ReturnCode:%s", GetErrorStr(m_nDevErrCode));
        return FALSE;
    }

    if (OpenDevice() != TRUE)
    {
        Log(ThisFile, 1, "硬复位设备后重新连接: bResetDevice()->OpenDevice() fail. ");
        return FALSE;
    }
...
BOOL CDevImpl_BSID81::bSoftResetDevice(int iMode)
{
    ...
    {
        Log(ThisFile, 1, "软复位设备: bSoftResetDevice() input mode invalid fail. mode<%d> != <%d|%d|%d|%d>",
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "软复位设备: bSoftResetDevice()->SoftResetDevice(%d, %d) fail. ReturnCode:%s",
...
BOOL CDevImpl_BSID81::bBackIdCardToRerec()
{
    ...
    if (m_nDevErrCode != IDDIGITALCOPIER_NO_ERROR)
    {
        Log(ThisFile, 1, "退卡到可识别位置: bBackIdCardToRerec()->BackIdCardToRerec(%d) fail. ReturnCode:%s",
            m_bsDeviceId, GetErrorStr(m_nDevErrCode));
        return FALSE;
    }
