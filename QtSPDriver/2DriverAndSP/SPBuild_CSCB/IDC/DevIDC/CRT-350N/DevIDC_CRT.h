...
#define TIMEOUT_WAIT_ACTION                                     (5*1000 * 6)   // 延长硬件命令返回时间,避免超时
...
public:
    ...
    ************************************************************/
    ...
    // CRD: 发卡模块相关命令 如下:
    virtual int GetDevStat(STCRDDEVSTATUS &stStat); // 读取设备状态
    virtual int DispenseCard(const int nUnitNo);    // 发卡
    virtual void GetVersion(char* szVer, long lSize, ushort usType);// 取版本号
...
private:
    ...
    int GetRetainNum();             //30-00-00-00(FT#0009)
    //禁止进卡
    ...
    // CRD: 发卡模块相关处理 如下:
    INT GetFWVersion(LPSTR lpVer);         // 取发卡机固件版本
    INT GetSerialNumer(LPSTR lpVer);       // 取发卡机序列号
    INT SendSinglePacket_COM(const char* pszCmd, const char *lpData, int nLen, const char *pszCallFunc);
    INT GetResponse_COM(char *pszReponse, int nLen, int nTimeout, const char *pszCallFunc);
    INT RcvDataCheck(LPCSTR lpcSndCmd, LPCSTR lpcRcvData, INT &nRcvDataLen);    // 应答数据Check
    CHAR* CmdToStr(LPSTR lpCmd);                                                // 命令返回中文含义
    CHAR* ConvertErrCodeToStr(INT nRet);                                        // DevXXX错误码返回中文含义
    INT ConvertErrCode_CRT2Dev(CHAR szDeviceErr[3]);                            // CRT报文错误码返回中文含义
    INT ConvertErrCode_DevIDC2CRD(INT nRet);                                    // DevIDC错误码转DevCRD
    USHORT GetDataCRC(UCHAR *ucData, USHORT usDataSize, USHORT usInitCrc = INIT);// 计算CRC
    USHORT Calc_CRC(USHORT usCrc, USHORT usCh);                                 // 计算CRC
...
private:
    ...
    INT                             m_nRetainNum;              //回收计数值30-00-00-00(FT#0009)
...
private:
    ...
    string                          m_strMode;
    USHORT                          m_IDCInterProtocal;         // 读卡器通信协议
...
private:
    ...
    WORD                            m_wSignalCom;               // 通讯方式(0USB/1COM,缺省0)

private:    // CRD: 发卡模块相关变量 如下:
    CHAR                            m_szCRDCmdStr[1024];
    CHAR                            m_szCRDErrStr[1024];
};
#endif // DEVIDC_CRT_H
