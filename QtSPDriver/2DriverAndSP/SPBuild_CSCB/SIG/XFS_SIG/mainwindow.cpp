    ...
    ui(new Ui::MainWindow)
{
    ...
    qSignatureBrowser = new QTextBrowser();
    qSignatureTitle = new QLabel(tr("电子签名窗口"));
...
void MainWindow::ImagePathToHtml(QString &qImagePath)
{
    qImagePath = QString("<img src=\"%1\"/>").arg(qImagePath);
}

void MainWindow::ShowWin(int x, int y, int w, int h/*, char* pszImagePath*/)
{
    //QString qImgPath = QString(QLatin1String(pszImagePath));
    ...
    qSignatureBrowser->resize(w, h);
    qSignatureBrowser->move(x, y);
//    if (!qImgPath.isEmpty())
//    {
//        ImagePathToHtml(qImgPath);
//        qSignatureBrowser->insertHtml(qImgPath);
//    }

    qSignatureTitle->resize(w / 2, h / 10);
    qSignatureTitle->move(x / 4, 0);
    qSignatureTitle->setScaledContents(true);
    this->show();
}
