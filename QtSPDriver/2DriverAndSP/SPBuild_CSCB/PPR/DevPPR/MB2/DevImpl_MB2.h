...
#define DLL_DEVLIB_NAME  "libPRx.so.1"           // 缺省动态库名
...
*************************************************************/
//DRV CMD
typedef long (*OPEN)(const char *pszCommPort,
                     long lCurEmulation,
                     long lDestEmulation,
                     char *pszCommCfgFilePathr,
                     long *plErrorCode,
                     long lBaudrate);
typedef long (*CLOSE)(long hComm);
typedef long (*PRTFLUSHPRT)(long hComm);
typedef long (*EJECTMEDIA)(long hComm, int iDirection);
typedef long (*INSERTMEDIA)(long hComm, long lInPosV, long lTimeout);
typedef long (*SWALLOWPASSBOOK)(long hComm);
typedef long (*GETMEDIALENGTH)(long hComm);
typedef long (*GETMEDIAWIDTH)(long hComm);
typedef long (*GETMEDIAPOSH)(long hComm);
typedef long (*BEEP)(long hComm,long lBeepCounts,long lIntervalTime);
typedef long (*GETFIRMWAREVERSION)(long hComm, char* szFirmwareVersion);
typedef long (*SETPAGELENGTH)(long hComm, long PageLength);
typedef long (*PRTWRITEDATA)(long hComm, unsigned char* command, long dataLength, long writeTimeout);
typedef long (*PRTCHANGECODEDISTANCE)(long hComm, float fDistance);
typedef long (*PRTSELWESTCODE)(long hComm, int iSel);
typedef long (*PRTSETFONTWIDTH)(long hComm,float fWidth);
typedef long (*SCANSETCTRL)(ScanCtrl &scanCtrl, long lDpi, long lEjectMode);
typedef long (*SCANSETIMAGECTRL)(ScanImageCtrl &scanImageCtrl,
                                 ...
                                 long lEndX,
                                 long lEndY);
typedef long (*PRTPRINTTEXT)(long hComm, char* strData);
typedef long (*PRTMOVEABSPOS)(long hComm, long lX, long lY);
typedef long (*PRTMOVEABSRELPOS)(long hComm, long lY);
typedef long (*GETSTATUS)(long hComm, unsigned char* devCode);
typedef long (*GETMEDIASTATUS)(long hComm, unsigned char* devCode);
typedef long (*RESET)(long hComm);
typedef long (*PRTSETPTRPROPERTY)(long hComm, int iType, int iProperty);
typedef long (*CLEANERROR)(long hComm);
typedef long (*PRTFLUSHPRT)(long hComm);
typedef long (*MSREAD)(long hComm, char* szData, long lMagneticType);
typedef long (*PRXPRTPRINTBMP)( long hComm, long lX, long lY, char *szBmpFile, char *szPrtDataFile, char *szPrintType);
typedef long  (*SCAN)(long hComm,ScanCtrl scanCtrl,ScanImageCtrl *frontCtrl,ScanImageCtrl *rearCtrl);
typedef long (*PRXMSCONFIGHMS)(long hComm,long lMagneticType, long lMagnPos,unsigned char charOfEnd,bool bDuplicData,long lRetryCounts);
typedef long (*PRXMSREADEX)(long hComm, char* szData);
...
public:
    LONG     DeviceOpen(LPSTR lpMode);                       // 打开设备
...
public: // 接口函数封装
 ...
 //   INT     nGetDevStatus(DEVICESTATUS &stDevicestatus);                            // 8. 查询设备状态
    ...
    LONG    nGetFirmwareVersion(LPSTR lpVersion);                     // 12. 获取固件版本号
    ...
    INT     LineModePrintText(LPSTR lpPrintString, unsigned long wStringSize, LONG wStartX, LONG wStartY);
    INT     PrintFrontSetFormat();     // 打印前格式设置到打印机
    LONG    nGetDevStatus(unsigned char *);
    LONG    nGetMediaStatus(unsigned char *);
    INT     nControlMedia(MEDIA_ACTION2 enMediaAct);
    LONG    nMsRead(LPSTR szData, LONG lMagneticType);
    INT     nRetractMediaToBox();                                                        // 回收
    LONG    nPrintImage(char *szImagePath, unsigned long ulWidth, unsigned long ulHeight);
    LONG    lMediaInsertCheck(LONG);                    //test#30
    LONG    lSetCancelFlg(BOOL bCancelFlg);                        //TEST#30
...
private: // 动态库接口定义
    ...
    PRTMOVEABSPOS		PrtMoveAbsPos;
    PRTPRINTTEXT		PrtPrintText;
    GETMEDIALENGTH      GetMediaLenth;
    GETMEDIAWIDTH       GetMediaWidth;
    GETMEDIAPOSH        GetMeidaPosH;
    BEEP                Beep;
    GETFIRMWAREVERSION  GetFirmwareVersion;
    SETPAGELENGTH       SetPageLength;
    PRTWRITEDATA        PrtWriteData;
    PRTCHANGECODEDISTANCE  PrtChangeCodeDistance;
    PRTSELWESTCODE      PrtSelWestCode;
    PRTMOVEABSRELPOS    PrtMoveAbsRelPos;
    PRTSETFONTWIDTH     PrtSetFontWidth;
    SCAN                Scan;               //按指定扫描参数扫描图像
    SCANSETIMAGECTRL    ScanSetImageCtrl;
    SWALLOWPASSBOOK     SwallowPassBook;
    RESET				Reset;
    CLEANERROR			CleanError;
    PRTSETPTRPROPERTY	PrtSetPtrProperty;
    GETMEDIASTATUS 		GetMediaStatus;
    GETSTATUS 			GetStatus;
    PRTFLUSHPRT 		PrtFlushPrt;
    MSREAD				MsRead;
    PRXPRTPRINTBMP	    PrxPrtPrintBmp;
    SCANSETCTRL         sCanSetCtrl;
    PRXMSCONFIGHMS      PrxMsconfigHMS;
    PRXMSREADEX        MsReadEx;
