...
#define READFORMFILEVALUENAME       "ptrreadform"               // ReadForm值名
...
#define FORMFILEDEFAULTPPR          "/usr/local/CFES/DATA/FORM/PPR/PassBookPrint.wfm"
#define MEDIAFILEDEFAULTPPR         "/usr/local/CFES/DATA/FORM/PTR/PassBookMedia.wfm"
#define READFORMFILEDDEFAULTPPR     "/usr/local/CFES/DATA/FORM/PTR/PassBookRead.wfm"
#endif
...
public:
    ...
    void InitData()
    {
        memset((WFSPTRSTATUS *)this, 0, sizeof(WFSPTRSTATUS));  //清空本地数据

        memset(&m_RetractBins, 0, sizeof(m_RetractBins));
        m_lpRetractBins[0] = &m_RetractBins;      //只第一个元素指向一个有效值
        m_lpRetractBins[1] = NULL;         //第二个值总是NULL
        this->lppRetractBins = m_lpRetractBins;
     }
    ...
    virtual long AddRetractBins(const WFSPTRRETRACTBINS &RetractBin)
    {
        m_RetractBins = RetractBin;
        return 0;
    }
private:
        WFSPTRRETRACTBINS m_RetractBins;     //回收箱信息
        LPWFSPTRRETRACTBINS m_lpRetractBins[16]; //
};
...
public:
    CWfsPtrCaps()
    {
        ...
        this->fwType                = bJournal ? WFS_PTR_TYPEJOURNAL : WFS_PTR_TYPERECEIPT;     // 设备类型
        this->bCompound             = FALSE;
        this->wResolution           = WFS_PTR_RESMED;                                           // 打印清晰度:中等
        this->fwReadForm             = 0;                                                       // 是否支持读介质:不支持
        this->fwWriteForm           = WFS_PTR_WRITETEXT | WFS_PTR_WRITEGRAPHICS | (bJournal ? 0 : WFS_PTR_WRITEBARCODE);
        this->fwExtents               = 0;                                                      // 是否支持测量介质:不支持
        this->fwControl             = (bJournal ? 0 : (WFS_PTR_CTRLCUT | WFS_PTR_CTRLEJECT)) | WFS_PTR_CTRLFLUSH;
        this->usMaxMediaOnStacker   = 0;
        this->bAcceptMedia          = FALSE;
        this->bMultiPage            = FALSE;
        this->fwPaperSources         = WFS_PTR_PAPERUPPER;
        this->bMediaTaken           = bJournal ? FALSE : TRUE;;
...
public:
    ...
    BOOL SetData(DWORD dwLen, LPBYTE pData)
    {
        ...
        if (0 < dwLen)
        {
            memcpy(this->lpbData, lpbData, dwLen);
        }
        this->ulSize = dwLen;
...
public:
    ...
    // 查找指定的FORM
    inline CSPPrinterForm *FindReadForm(LPCSTR lpszFormName)
    {
        return (CSPPrinterForm *)m_ReadFormList.Find(lpszFormName);
    }
    ...
    inline LPCSTR GetReadFormNames(BOOL bForm = TRUE)
    {
        if (FALSE != bForm)
        {
            m_ReadFormNames = m_ReadFormList.GetNames();
            return m_ReadFormNames;
        }
        else
        {
            m_ReadMediaNames = m_ReadFormList.GetNames();
            return m_ReadMediaNames;
        }
    }
    ...
    CSPPrinterFormList *GetReadFormList()
    {
        return &m_ReadFormList;
    }
...
public:
    ...
    CMultiString        m_ReadFormNames;
    CMultiString        m_ReadMediaNames;
