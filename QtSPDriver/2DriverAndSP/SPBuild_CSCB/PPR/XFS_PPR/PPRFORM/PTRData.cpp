...
CSPPtrData::CSPPtrData(LPCSTR lpLogicalName)
{
    ...
    m_strReadFormName  = m_cXfsReg.GetValue("CONFIG", READFORMFILEVALUENAME, READFORMFILEDDEFAULTPPR);
...
BOOL CSPPtrData::LoadReadForms()
{
    const char *const ThisModule = "DATA.LoadForms";
    //从注册表中读取FORM文件名
    string strFile = m_strReadFormName;
    BOOL bIsDir = FALSE;
    if (strFile.empty())
    {
        strFile = FORMPATH;
    ...
    {
        m_ReadFormList.Clear();
        m_ReadFormList.Load(strFile.c_str(), true);
    }
