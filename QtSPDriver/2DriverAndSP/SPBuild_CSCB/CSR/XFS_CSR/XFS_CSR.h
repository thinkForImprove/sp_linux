...
typedef struct NoteBox_List
{
    ...
    INT GetRetraceFull(USHORT usBox)   // 获取物理票箱Full状态
    {
        for (INT i = 0; i < BOX_COUNT; i ++)
        {
            if (stNoteBox[i].bIsHave == TRUE)
    ...
    INT SetRetraceFull(USHORT usBox, BOOL bIsStat)   // 设置物理票箱Full状态
    {
        for (INT i = 0; i < BOX_COUNT; i ++)
        {
            if (stNoteBox[i].bIsHave == TRUE)
...
//-----------------处理类 定义-------------------------
class CXFS_CSR : public ICmdFunc, public CTextPrinter
{
