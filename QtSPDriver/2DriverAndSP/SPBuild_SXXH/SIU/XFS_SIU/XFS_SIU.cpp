...
HRESULT CXFS_SIU::OnOpen(LPCSTR lpLogicalName)
{
    ...
    // 加载DEV
    long lRet = m_pDev.Load(strDevDll.c_str(), "CreateIDevSIU", DEVTYPE);
    if (0 != lRet)
    {
    ...
    // 打开连接
    lRet = m_pDev->Open(strPort.c_str(), false);
    if (lRet != 0)
    {
...
HRESULT CXFS_SIU::OnStatus()
{
    ...
    // 更新状态和发状态事件
    UpdateStatus(stSiuStatus);
    if(stDevStatus.wDevice == DEVICE_OFFLINE){
        m_pDev->Close();
        CQtTime::Sleep(500);
        m_pDev->Open("", false);
    }
...
//40-00-00-00(FT#0002)    stSiuStatus.fwSensors[WFS_SIU_OPERATORSWITCH] = WFS_SIU_SUPERVISOR;
    stSiuStatus.fwSensors[WFS_SIU_OPERATORSWITCH] =
            (m_iMaintenanceStatusValue == 0) ? WFS_SIU_SUPERVISOR : WFS_SIU_MAINTENANCE;
    ...
    {
        if(i == WFS_SIU_PROXIMITY){
            switch (stDevStatus.wSensors[i])
            {
            case SENSORS_OFF:           wStatus = WFS_SIU_NOT_PRESENT;      break;
            case SENSORS_ON:            wStatus = WFS_SIU_PRESENT;          break;
            default:                    wStatus = WFS_SIU_NOT_AVAILABLE;    break;
            }
        } else {
            switch (stDevStatus.wSensors[i])
...
//40-00-00-00(FT#0002)    m_stCaps.fwSensors[WFS_SIU_OPERATORSWITCH] = WFS_SIU_RUN | WFS_SIU_SUPERVISOR;
    m_stCaps.fwSensors[WFS_SIU_OPERATORSWITCH] = WFS_SIU_RUN |
            ((m_iMaintenanceStatusValue == 0) ? WFS_SIU_SUPERVISOR : WFS_SIU_MAINTENANCE);
