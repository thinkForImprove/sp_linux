...
#define VERIFY_NOT_IN_CASH_IN()     VERIFY_ERR_CONDITION((m_CashInInfor.GetState() == WFS_CIM_CIACTIVE), WFS_ERR_CIM_CASHINACTIVE, "It is in CashIn transaction")

//校验处于CASH_IN事件中
//#define VERIFY_IN_CASH_IN()         VERIFY_ERR_CONDITION(!m_bCashInActive, WFS_ERR_CIM_NOCASHINACTIVE, "It is NOT in CashIn transaction")
#define VERIFY_IN_CASH_IN()         VERIFY_ERR_CONDITION((m_CashInInfor.GetState() != WFS_CIM_CIACTIVE), WFS_ERR_CIM_NOCASHINACTIVE, "It is NOT in CashIn transaction")
...
HRESULT XFS_CRSImp::ReOpen(BOOL bReset/* = FALSE*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
...
HRESULT XFS_CRSImp::OnOpen(LPCSTR lpLogicalName)
{
    ...
    //启动后第一次动作时删除
    m_pAdapter->SetLFSCmdId(ADP_OTHER);            //test#28
    m_pAdapter->vDeleteSnrinfoForce();                  //test#28
...
HRESULT XFS_CRSImp::OnStatus()
{
    ...
    // 空闲更新状态
    UpdateStatus();
    if ((m_pAdapter->IsConnected() == FALSE) &&
            (m_Param.GetEPDownAutoReset())) {
        if (ReOpen(TRUE) == WFS_SUCCESS) {
            UpdateStatus();
        }
    }
    return WFS_SUCCESS;
...
HRESULT XFS_CRSImp::OnCancelAsyncRequest()
{
    ...
    //AutoLogFuncBeginEnd();();
    m_bCanceled = TRUE;
    if (m_Param.GetCashInStatusCancelToUnknown()) {
        WORD wCashInState = m_CashInInfor.GetAvailable() ? m_CashInInfor.GetState() : WFS_CIM_CIUNKNOWN;
        if (wCashInState == WFS_CIM_CIACTIVE) {
        ...
        else
        {
            if (ADPStatus.stOutPutPosition == ADP_OUTPOS_EMPTY) //钞口无钞
            {
                ...
                else
                {
                    ...
                    //发送Taken事件
                    if(m_eWaitTaken == WTF_CDM){
                        if(m_Param.GetTakenEventAfterCloseShutterFlag() && m_Param.GetPresentSPControlCloseShutter()){
                            ...
                            //                        CDMFireItemTaken(CDMCaps.fwPositions);
                        }
                    } else {
                        //不做处理
                    }
...
HRESULT XFS_CRSImp::CDMDispense(const LPWFSCDMDISPENSE lpDisData, LPWFSCDMDENOMINATION &lpDenoOut)
{
    ...
    ADPSTATUSINFOR ADPStatus;
    GetUpdatedStatus(ADPStatus);
    VERIFY_SAFEDOOR_OPEN(ADPStatus.stSafeDoor, TRUE);          //40-00-00-00(FS#0004)
    ...
    HRESULT hRes = OnDenominate(lpDisData->usTellerID, lpDisData->usMixNumber, pDenoInOut);
    if (hRes != WFS_SUCCESS)
    {
        Log(ThisModule, hRes,
    ...
    ULONG ulDispenseCountsTemp[ADP_MAX_CU_SIZE];
    NOTEID_COUNT_ARRAY NoteIDCountArray[ADP_MAX_CU_SIZE];   //test#13
    CLEAR_ARRAY(ulDispenseCountsTemp);                      //test#13
    ...
    do {                                                     //test#13
        ...
        iRet = m_pAdapter->Dispense(ulCount2, ulDispenseCounts, ulRejectCounts, wCUErrors);           //test by guojy
...
//    CAutoFireDeviceStatus::IgnoreItemTakenInsertedOnce(TRUE, FALSE); //忽略一次ITEM插入取出事件
    ...
    //设置检视钞票取走关门标志
//40-00-00-00(FT#0005)    if (SHUTTER_CONTROL_BY_SP(CDMCaps) &&
//40-00-00-00(FT#0005)        CDMCaps.bItemsTakenSensor)
    if(CDMCaps.bItemsTakenSensor && m_Param.GetPresentSPControlCloseShutter())           //40-00-00-00(FT#0005)
    {
...
HRESULT XFS_CRSImp::CDMOpenShutter(WORD wPos)
{
    ...
    if (ADPStatus.stOutShutter == ADP_OUTSHUTTER_OPEN)
    {
       ...
       return WFS_ERR_CDM_SHUTTEROPEN;                 //40-00-00-00(FT#0006)
    }
...
//wb    VERIFY_NOT_IN_EXCHANGE(TRUE);  //TEST#28
...
HRESULT XFS_CRSImp::CIMGetCashInStatus(LPWFSCIMCASHINSTATUS &lpCashInStatus)
{
    ...
    if ((m_CashInInfor.GetAvailable()) &&
            ((m_Param.GetCashInStatusNoteNumberListMode() == 0) || (m_CashInInfor.GetNoteNumberCount() > 0)))
    {
    ...
    {
        switch (m_Param.GetCashInStatusNoteNumberListMode()) {
        case 0:
            break;
        case 1:
            lpStatus->SetZeroCountNoteNumberList();
            break;
        default:
            {
                LPWFSCIMNOTETYPELIST lpNumberList = m_Param.GetNoteTypeList();
                for (USHORT i = 0; i < lpNumberList->usNumOfNoteTypes; i++) {
                    lpStatus->SetCountByID(lpNumberList->lppNoteTypes[i]->usNoteID, 0);
                }
            }
            break;
        }
...
HRESULT XFS_CRSImp::CIMCashInStart(const LPWFSCIMCASHINSTART lpData)
{
    ...
    ADPSTATUSINFOR ADPStatus;
    GetUpdatedStatus(ADPStatus);
    VERIFY_SAFEDOOR_OPEN(ADPStatus.stSafeDoor, FALSE);          //40-00-00-00(FS#0004)
...
HRESULT XFS_CRSImp::CIMCashIn(LPWFSCIMNOTENUMBERLIST &lpResult)
{
    ...
    if (ADPStatus.stInPutPosition == ADP_INPOS_EMPTY)
    {
        Log(ThisModule, -1, "No Items InPutPosition:%s",
            ADPStatus.stInPutPosition == 1 ? "Empty" : "");
...
//test#13 end
  ...
  //test#13   for (NOTEID_COUNT_ARRAY::iterator it = NoteIDCount.begin(); it != NoteIDCount.end(); it++)
   for (NOTEID_COUNT_ARRAY::iterator it = NoteIDCountTemp.begin(); it != NoteIDCountTemp.end(); it++)      //test#13
    {
//test#13        lpCashInRes->SetCountByID(it->first, it->second);
...
//    CAutoFireDeviceStatus::IgnoreItemTakenInsertedOnce(FALSE, TRUE); //忽略一次ITEM插入取出事件
    ...
    if ((ADPStatus.stDevice != ADP_DEVICE_ONLINE) && (ADPStatus.stDevice != ADP_DEVICE_BUSY)) {   //test#29
        return WFS_ERR_HARDWARE_ERROR;              //test#29
    }                                               //test#29

    DWORD dwTimeout = m_Param.GetCashInEndCheckStatusTimeout();
    if (dwTimeout > 0) {
        do {
            CQtTime::Sleep(dwTimeout * 1000);
            iRet = UpdateStatus(ThisModule, ADPStatus, TRUE);
            if (iRet < 0) {
                m_CashInInfor.SetState(WFS_CIM_CIOK);
                Log(ThisModule, iRet,
            ...
            if ((ADPStatus.stDevice != ADP_DEVICE_ONLINE) && (ADPStatus.stDevice != ADP_DEVICE_BUSY)) {
                m_CashInInfor.SetState(WFS_CIM_CIOK);
                return WFS_ERR_HARDWARE_ERROR;
            }
            dwTimeout--;
...
40-00-00-00(FT#0006) del end*/
    ...
    if(wPos > WFS_CIM_POSINREAR){
        //打开出钞口门
        if(ADPStatus.stOutShutter == ADP_OUTSHUTTER_OPEN){
            return WFS_ERR_CIM_SHUTTEROPEN;
        }
...
HRESULT XFS_CRSImp::CIMStartEXChange(const LPWFSCIMSTARTEX lpStartEx, LPWFSCIMCASHINFO &lpCUInfor)
{
    ...
    {
        //TEST#29
        ICIMStatus *pIStatus = (ICIMStatus *)&m_CIMStatus;//TEST#29
        pIStatus->SetDeviceSt(WFS_CIM_DEVONLINE);//TEST#29
...
//wb    VERIFY_NOT_IN_EXCHANGE(FALSE);  //TEST#28
...
void XFS_CRSImp::GetUpdatedStatus(ADPSTATUSINFOR &ADPStatus)
{
    ...
    //    !m_bCashInActive && !m_bCashOutActive &&
        (m_CashInInfor.GetState() != WFS_CIM_CIACTIVE) && !m_bCashOutActive &&
...
//    else if (bCDM && m_Param.IsCDMStatusBusyWhenCashInActive() && m_bCashInActive)  // 在处于进钞事务时才设置为BUSY
    else if (bCDM && m_Param.IsCDMStatusBusyWhenCashInActive() && (m_CashInInfor.GetState() == WFS_CIM_CIACTIVE))  // 在处于进钞事务时才设置为BUSY
    {
...
WORD XFS_CRSImp::ComputeDispenseState(const ADPSTATUSINFOR &ADPStatus, BOOL *pbStopByHardwareError, BOOL bLog)
{
    ...
    //处理安全门打开的情况
    CHECK_DISPENSE_ACCEPTOR_STATE_ERROR(ADPStatus.stSafeDoor == ADP_SAFEDOOR_OPEN, WFS_CDM_DISPCUSTOP);         //40-00-00-00(FS#0004)
...
WORD XFS_CRSImp::ComputeAcceptorState(const ADPSTATUSINFOR &ADPStatus, BOOL bLog)
{
    ...
    //处理安全门打开的情况
    CHECK_DISPENSE_ACCEPTOR_STATE_ERROR(ADPStatus.stSafeDoor == ADP_SAFEDOOR_OPEN, WFS_CIM_ACCCUSTOP);  //40-00-00-00(FS#0004)
    //处理STACKER不正常的情况
...
long XFS_CRSImp::DenominateIsDispensable(ICDMDenomination *pDenoIn, ULONG &ulCountAlloced)
{
    ...
    for (USHORT i = 0; i < usCount; i++)
    {
        ...
        if (CUCanDispense(pCU))
        {
            if (pCU->GetCount() - m_Param.GetCassMiniGuarantee() < pValues[i])   //不够出      //wb
            {
                Log(ThisModule, -1,
...
long XFS_CRSImp::VerifyDenominateCurrency(ICDMDenomination *pDenoIn) const
{
    ...
    return WFS_ERR_CDM_INVALIDCURRENCY;         //test#28
}
...
//    if (!m_bCashInActive && !m_bCashOutActive)
    if ((m_CashInInfor.GetState() != WFS_CIM_CIACTIVE) && !m_bCashOutActive)
    {
...
HRESULT XFS_CRSImp::OpenShutter(const char *ThisModule, BOOL bInShutter, BOOL bCDM)
{
 ...
 //       return WFS_ERR_HARDWARE_ERROR;
        ...
        if((lRet == WFS_ERR_CDM_SHUTTERNOTOPEN) && !bCDM){//test#29
            return  WFS_ERR_CIM_SHUTTERNOTOPEN;   //test#29
        }else if((lRet == WFS_ERR_CIM_SHUTTERNOTOPEN) && bCDM){//test#29
            return WFS_ERR_CDM_SHUTTERNOTOPEN;//test#29
        }else{
            return lRet;  //test#29
        }
...
HRESULT XFS_CRSImp::CloseShutter(const char *ThisModule, BOOL bInShutter, BOOL bCDM)
{
 ...
 //test#29      return WFS_ERR_HARDWARE_ERROR;
        if((iRet == WFS_ERR_CDM_SHUTTERNOTCLOSED) && !bCDM){//test#29
            return  WFS_ERR_CIM_SHUTTERNOTCLOSED;   //test#29
        }else if((iRet == WFS_ERR_CIM_SHUTTERNOTCLOSED) && bCDM){//test#29
...
//    VERIFY_NOT_IN_EXCHANGE(pCUInterface->IsCDM());
    //wb add start
    if(pCUInterface->IsCDM() && m_bCDMExchangeActive){
        return WFS_ERR_CDM_EXCHANGEACTIVE;
    }

    if(!pCUInterface->IsCDM() && m_bCIMExchangeActive){
    ...
    if ((ADPStatus.stDevice != ADP_DEVICE_ONLINE) && (ADPStatus.stDevice != ADP_DEVICE_BUSY)) {
        iRet = Reset(ThisModule, NULL,
                     pCUInterface->IsCDM() ? m_pCUManager->GetCUInterface_CDM() : m_pCUManager->GetCUInterface_CIM(),
                     TRUE);
        if (iRet < 0) {
            Log(ThisModule, iRet, "Reset()失败");
            return iRet;
...
//wb    VERIFY_IN_EXCHANGE();
    if(pCUInterface->IsCDM() && !m_bCDMExchangeActive){         //wb
        return WFS_ERR_CDM_NOEXCHANGEACTIVE;                    //wb
    }                                                           //wb
    if(!pCUInterface->IsCDM() && !m_bCIMExchangeActive){        //wb
        return WFS_ERR_CIM_NOEXCHANGEACTIVE;                    //wb
    }                                                           //wb

    m_bCDMExchangeActive = FALSE;
    m_bCIMExchangeActive = FALSE;

    //判断机芯是否故障状态，故障则复位,复位失败则不做钞箱信息设定
    if(m_pAdapter->IsNeedReset()){
        int iRet = Reset(ThisModule, NULL, pCUInterface, TRUE);
        if (iRet < 0)
        {
            return iRet;
        }
    ...
    LPWFSCDMCUINFO lpCDMCuInfoTemp = pCUInterface->BuildByXFSCDMFormat();
    LPWFSCIMCASHINFO lpCIMCuInfoTemp = pCUInterface->BuildByXFSCIMFormat();
    ...
    if (m_pAdapter->IsNeedReset())
    {
        long iRet_Reset = Reset(ThisModule, NULL, pCUInterface, TRUE);
        if (iRet_Reset < 0)
        {
            Log(ThisModule, iRet, "Reset()失败");
            ...
            //恢复数据 //test#28 start
            if (pCUInterface->IsCDM())
                iRet = pCUInterface->SetByXFSCDMFormat(lpCDMCuInfoTemp);
            else
                iRet = pCUInterface->SetByXFSCIMFormat(lpCIMCuInfoTemp);
            if (iRet < 0)
            {
                Log(ThisModule, iRet, "pCUInterface->SetByXFSFormat失败");
            }
...
//    CAutoFireDeviceStatus::IgnoreItemTakenInsertedOnce(); //忽略一次ITEM插入取出事件
    ...
    if ((m_Param.GetRetractFromInShutterForReset()) &&      //入钞口回收
            (lpResetIn == NULL) && (!bInnerCall)) {
        ADPSTATUSINFOR ADPStatus;
        UpdateStatus(ThisModule, ADPStatus, TRUE);
        if (ADPStatus.stInPutPosition == ADP_INPOS_NOTEMPTY) {
            ULONG ulRejectCount = 0;
            ...
            ADP_CUERROR arywCUError[ADP_MAX_CU_SIZE];
            CLEAR_ARRAY(arywCUError);
            iRet = m_pAdapter->ValidateAndCounting(arycResult, ulRejectCount, eRejectCause, arywCUError, 1, TRUE);
            if (iRet < 0)
            {
                Log(ThisModule, iRet,
                    "m_pAdapter->ValidateAndCounting()失败");
