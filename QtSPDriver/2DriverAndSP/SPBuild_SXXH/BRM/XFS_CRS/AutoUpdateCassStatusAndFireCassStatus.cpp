        ...
        for (i = 0; i < pInterface->GetCUCount(); i++)
        {
            ...
            if (it != m_mapCU2Status.end())
            {
                if ((eStatus == ADP_CASHUNIT_HIGH && it->second == ADP_CASHUNIT_OK) ||
                    ...
                    (eStatus == ADP_CASHUNIT_EMPTY  && (it->second == ADP_CASHUNIT_OK ||
                      it->second == ADP_CASHUNIT_LOW || it->second == ADP_CASHUNIT_HIGH))|| //test#28//wb
                    (eStatus == ADP_CASHUNIT_FULL  && (it->second == ADP_CASHUNIT_OK ||
                     it->second == ADP_CASHUNIT_HIGH || it->second == ADP_CASHUNIT_LOW))  //test#28
