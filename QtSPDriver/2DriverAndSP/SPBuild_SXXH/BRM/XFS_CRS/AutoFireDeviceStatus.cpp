...
//        ADPStatus.stShutterPosition == ADP_SHUTPOS_EMPTY)
        if ((!m_bIgnoreItemTaken &&
            ...
            (m_pSP->m_eWaitTaken != WTF_NONE || ADPStatus.stOutShutter != ADP_OUTSHUTTER_CLOSED)) ||
             ((m_ADPStatus.stInPutPosition == ADP_INPOS_NOTEMPTY) && (ADPStatus.stInPutPosition == ADP_INPOS_EMPTY) &&
              (ADPStatus.stInShutter != ADP_INSHUTTER_CLOSED) && m_pSP->m_eWaitTaken != WTF_NONE))
    {
        ADPCDMCAPS CDMCaps;
        ADPCIMCAPS CIMCaps;
        m_pSP->m_pAdapter->GetCDMCapabilities(&CDMCaps);
        m_pSP->m_pAdapter->GetCIMCapabilities(&CIMCaps);
        if(m_pSP->m_bCdmCmd){
            if(m_pSP->m_Param.GetTakenEventAfterCloseShutterFlag()){
