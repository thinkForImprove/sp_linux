...
CASHUNIT_STATUS CCashUnit::ComputeStatusFromCfg(CASHUNIT_STATUS eStatus) const
{
    ...
    else if (m_usType == ADP_CASSETTE_BILL)
    {
        ...
        ULONG ulLowStatusCount = (ULONG)pCfg->GetCUValue(0, "CDMBoxLowStatusCount");
        if (ulLowStatusCount > 0) {
            if(m_ulCount <= ulLowStatusCount){
                if(eStatus == ADP_CASHUNIT_OK || eStatus == ADP_CASHUNIT_FULL || eStatus == ADP_CASHUNIT_HIGH){
                    eStatus = ADP_CASHUNIT_LOW;
                }
            } else {
    ...
    else if (eStatus != ADP_CASHUNIT_MISSING && m_usType == ADP_CASSETTE_RECYCLING && m_bCDM)
    {
        ...
        //test#28
//wb        if ((eStatus == ADP_CASHUNIT_OK || eStatus == ADP_CASHUNIT_FULL || eStatus == ADP_CASHUNIT_HIGH) &&
//wb            m_ulCount <= (ULONG)pCfg->GetCUValue(0, "CDMBoxLowStatusCount"))
//wb        {
//wb            return ADP_CASHUNIT_LOW;
//wb        }
        //wb add start
        ULONG ulLowStatusCount = (ULONG)pCfg->GetCUValue(0, "CDMBoxLowStatusCount");
        if (ulLowStatusCount > 0) {
