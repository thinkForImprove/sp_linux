...
using namespace std;
#include <string.h>
#include "IDevSIU.h"                        //40-00-00-00(FS#0004)
...
typedef struct _DevConfigInfo
{
    ...
    std::map<BYTE, BYTE> mapNoteIDGroups;
...
public:
    ...
    //实现IBRMAdapter接口
public:
    virtual BOOL IsConnected() {return (m_bConnectNormal && m_bBVConnectNormal);}
...
//test#13    virtual long ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject);
    virtual long ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject, ADP_CUERROR arywCUError[],WORD wCashInCnt, BOOL bForceMode = FALSE);         //test#13
    ...
    void vDeleteSnrinfoForce();  //test#28
    //内部成员函数
private:
    //----------- 初始化类函数 ---------------
    //功能：打开机芯连接
...
private:
    ...
    void DeleteSnrForce(CThreadGenerateSN *pwork);  //test#28
    //------------ 状态更新类函数 --------------
...
private:
    ...
    BOOL m_bOpenShutterJammedCDM;
    BOOL m_bCloseShutterJammedCDM;
    //USB通信是否正常
    //USB通信是否正常
	...
	CQtDLLLoader<IDevSIU>             m_pDevSIU;            //40-00-00-00(FS#0004)
    BOOL                              m_bSIUOpen;           //40-00-00-00(FS#0004)
