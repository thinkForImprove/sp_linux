...
CUR2Adapter::CUR2Adapter()
{
    ...
    m_bOpenShutterJammedCDM = FALSE;
    m_bCloseShutterJammedCDM = FALSE;
    m_bConnectNormal = FALSE;
    m_bSIUOpen = FALSE;                             //40-00-00-00(FS#0004)
...
long CUR2Adapter::Close()
{
    ...
    if(m_pDevSIU != nullptr){                   //40-00-00-00(FS#0004)
        m_pDevSIU->Close();                     //40-00-00-00(FS#0004)
        m_pDevSIU->Release();                   //40-00-00-00(FS#0004)
        m_bSIUOpen = FALSE;                     //40-00-00-00(FS#0004)
    }                                           //40-00-00-00(FS#0004)
    return WFS_SUCCESS;
}
...
//test#13             || !stDevStatusInfo.bESCInPos
    ...
    {
        //处理门阻塞状态
      /* //test#29  if (m_pErrCodeTrans->IsOpenShutterJammed(m_szLastErrCode))
        {
        ...
        if (m_bCloseShutterJammed || m_bOpenShutterJammed)
        {
            ...
            m_sDevStatus.stDevice = ADP_DEVICE_HWERROR;
        }else{ */ //test#29                                                                              //test#13
        if (m_bOpenShutterJammedCDM || m_bCloseShutterJammedCDM) {
            m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_JAMMED;
        } else {
            if(stDevStatusInfo.iOutShutterStatus == UR2SHUTTER_STATUS_CLOSED){                //test#13
            ...
            }else if(stDevStatusInfo.iOutShutterStatus == UR2SHUTTER_STATUS_OPEN){//test#29
                m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_OPEN;
            }else if(stDevStatusInfo.iOutShutterStatus == UR2SHUTTER_STATUS_UNKNOWN){//test#29
                if(m_bOpenShutterJammedCDM || m_bCloseShutterJammedCDM){
                    m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_JAMMED;
                }else {
                ...
                m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_JAMMED;  //test#29                            //test#13
            }                                                                                 //test#13
        }
        if (m_bOpenShutterJammed || m_bCloseShutterJammed) {
            m_sDevStatus.stInShutter = ADP_INSHUTTER_JAMMED;
        } else {
            if(stDevStatusInfo.CashInShutterStatus == UR2SHUTTER_STATUS_CLOSED){              //test#13
            ...
            }else if(stDevStatusInfo.CashInShutterStatus == UR2SHUTTER_STATUS_OPEN){
                 m_sDevStatus.stInShutter = ADP_INSHUTTER_OPEN;
            }else if(stDevStatusInfo.CashInShutterStatus == UR2SHUTTER_STATUS_UNKNOWN){                //test#13
                if(m_bOpenShutterJammed || m_bCloseShutterJammed){
                    m_sDevStatus.stInShutter = ADP_INSHUTTER_JAMMED;
                }else {
                    m_sDevStatus.stInShutter = ADP_INSHUTTER_CLOSED;                              //test#13
                ...
                m_sDevStatus.stInShutter = ADP_INSHUTTER_JAMMED;                                //test#13
            }                                                                                 //test#13
        }
   //     }                                                                                     //test#13
...
//40-00-00-00(FS#0004)        DWORD dwIniValue = 1;

//40-00-00-00(FS#0004)         if (dwIniValue == 2)
//40-00-00-00(FS#0004)         {
//40-00-00-00(FS#0004)             m_sDevStatus.stSafeDoor = ADP_SAFEDOOR_OPEN;
//40-00-00-00(FS#0004)         }
//40-00-00-00(FS#0004)         else if (dwIniValue == 1)
//40-00-00-00(FS#0004)         {
//40-00-00-00(FS#0004)             m_sDevStatus.stSafeDoor = ADP_SAFEDOOR_CLOSED;
//40-00-00-00(FS#0004)         }
//40-00-00-00(FS#0004)         else
//40-00-00-00(FS#0004)         {
//40-00-00-00(FS#0004)             m_sDevStatus.stSafeDoor = ADP_SAFEDOOR_UNKNOWN;
//40-00-00-00(FS#0004)         }
 ...
 /*   if (stDevStatusInfo.bCashAtCS
    ...
    if(stDevStatusInfo.bCashAtCSErrPos
       || stDevStatusInfo.bCashAtShutter){
 //test#28      m_sDevStatus.stOutPutPosition = ADP_OUTPOS_NOTEMPTY;
      m_sDevStatus.stInPutPosition = ADP_INPOS_NOTEMPTY; //test#28

    }else{
  //test#28           m_sDevStatus.stOutPutPosition = ADP_OUTPOS_EMPTY;
           m_sDevStatus.stInPutPosition = ADP_INPOS_EMPTY;      //test#28
...
long CUR2Adapter::QueryDevInfo()
{
    ...
    iRet = m_pUR2Dev->GetUnitInfo(iTotalCashInURJB, bArryDenoCodeBySet, bArryAcceptDenoCode, stOperationalInfo,
                                  stHWConfig, stCassType, stCashCountLevel, stStoreMoneyLevel, stDispenseLevel);
    if ((iRet == ERR_UR_DRV_DRVHND_DIFFER) || (iRet == ERR_UR_DRV_DRV_REMOVE)) {    // EPDown
        if (m_bBVConnectNormal) {
            m_pUR2Dev->CloseBVConn();
            m_bBVConnectNormal = FALSE;
        }
        if (m_bConnectNormal) {
            m_pUR2Dev->CloseURConn();
            m_bConnectNormal = FALSE;
        }
    }
...
long CUR2Adapter::ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject, ADP_CUERROR arywCUError[],WORD wCashInCnt, BOOL bForceMode/* = FALSE*/)  //test#13
{
    ...
    int iRet = m_pUR2Dev->CashCount(m_sDevConfigInfo.CashInNoteType,  iValue, iAmount, iPowerIndex,
                                    ...
                                    usFedNoteCondition, pMaintenanceInfo, bForceMode);
    SaveLastErrCode(iRet, ThisModule);
...
long CUR2Adapter::Init(ADP_CUERROR arywCUError[ADP_MAX_CU_SIZE])
{
    ...
    {                                                               //test#11
        ...
        SaveLastErrCode(iRet, ThisModule);//test#29
        SaveLastActionErrCode(iRet, "CloseCashInShutter Reset"); //test#29
        UpdateADPDevStatus(iRet);   //test#29
        m_bCloseShutterJammed = TRUE;//test#29
        iRet = WFS_ERR_HARDWARE_ERROR;//test#29
    }                                                               //test#11
      return iRet;  //test#29
//    return WFS_SUCCESS;
}
...

{
    ...
    {
        bMediaDetected = TRUE;
        if ((usRetraceArea != ADP_RETRACTEARE_NOSET) &&
                (usRetraceArea != ADP_RETRACTEARE_CASHSLOT) &&
                (usRetraceArea != ADP_RETRACTEARE_TRANSPORT)) {
...
long CUR2Adapter::OpenDeviceShutter(BOOL bInShutter)                   //test#11
{
    ...
    char pMaintenanceInfo[MAINTENANCE_INFO_LENGTH] = {0};
    m_bCashOutBeforePresent = FALSE;
    m_bOpenShutterJammed = FALSE;  //TEST#29
    m_bOpenShutterJammedCDM = FALSE;
    if(bInShutter == TRUE){                                             //test#11
        ...
        if (iRet != 0)
        {
            Log(ThisModule, iRet, "关闭设备入金口钞门失败(ErrCode:%s)(ReturnValue:%d)", m_szLastErrCode, iRet);
            return WFS_ERR_CIM_SHUTTERNOTOPEN;  //test#29
        }
    }else{
...
long CUR2Adapter::CloseDeviceShutter(BOOL bInShutter)       //test#11
{
    ...
    char pMaintenanceInfo[MAINTENANCE_INFO_LENGTH] = {0};
    int iRet = 0;                                                       //test#11
    m_bCloseShutterJammed = FALSE;   //TEST#29
    m_bCloseShutterJammedCDM = FALSE;
...
//        BOOL bCloseShutterJammed = m_pErrCodeTrans->IsCloseShutterJammed(m_szLastErrCode);
        ...
        {
            Log(ThisModule, iRet, "关闭设备入金口钞门失败(ErrCode:%s)(ReturnValue:%d)", m_szLastErrCode, iRet);
            return WFS_ERR_CIM_SHUTTERNOTCLOSED;  //test#29
 //           return bCloseShutterJammed ? WFS_ERR_CDM_SHUTTERNOTCLOSED : WFS_ERR_HARDWARE_ERROR;
        ...
        if (iRet != 0)
        {
            ...
            return WFS_ERR_CDM_SHUTTERNOTCLOSED; //test#29
//test#29            return bCloseShutterJammed ? WFS_ERR_CDM_SHUTTERNOTCLOSED : WFS_ERR_HARDWARE_ERROR;
        }
    }                                                                   //test#11
...
//test#28
void CUR2Adapter::vDeleteSnrinfoForce()
{
    int iRet = 0;
    CThreadGenerateSN SNWork(SAVESERIALOPERAT_S, m_eLfsCmdId, iRet);    //test#8
    DeleteSnrForce(&SNWork);
}
void CUR2Adapter::DeleteSnrForce(CThreadGenerateSN *pwork){

    pwork->DeleteSnrinfomation();
}
