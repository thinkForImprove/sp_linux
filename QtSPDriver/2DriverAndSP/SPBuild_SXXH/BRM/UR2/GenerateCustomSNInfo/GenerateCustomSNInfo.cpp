...
void CSaveCusSNInfo::OnSNInfoGeneration(const STSNINFO &stSNInfo)
{
    ...
    if (m_uFGMode == FSNGENMODE_NORMAL)
    {
        ...
        LPFSNFileData pdata;
        pdata = new FSNFileData;
        memset(&pdata->stFSNHead, 0, 32);
        pdata->stFSNHead.Counter = stSNInfo.dwCount;
        ...
        if (pdata != nullptr)
        {
            for (Uint32 i = 0; i < pdata->stFSNHead.Counter; i++)
            {
...
void CSaveCusSNInfo::GenerateSNFile()
{
    ...
    {
        sprintf(szSection, "LEVEL4_%03d", i);
        ConfigFile.SetNewSection(szSection);
    ...
    {
        sprintf(szSection, "LEVEL3_%03d", i);
        ConfigFile.SetNewSection(szSection);
        cINI = ConfigFile.GetWriterSection(szSection);
        cINI.AddValue("Index", (*it).usIndex);
        cINI.AddValue("Currency", (*it).szCurrency);
    ...
    {
        sprintf(szSection, "LEVEL2_%03d", i);
        ConfigFile.SetNewSection(szSection);
        cINI = ConfigFile.GetWriterSection(szSection);
        cINI.AddValue("Index", (*it).usIndex);
        cINI.AddValue("Currency", (*it).szCurrency);
...
void CSaveCusSNInfo::GenerateFSNInfo(const LPSTSNINFODETAILS lpSNInfoDetails, LPSTFSNBody lpFSNBody)
{
...

    {
        ...
        sprintf(szMachineSNo, "%s/CFES/UR2", &szYear[2]);
...
void CSaveCusSNInfo::GenerateFSNFileHeader(LPFSNFileData pdata)
{
    ...
    //以下为固定值
    pdata->stFSNHead.HeadStart[0] = 20;
    pdata->stFSNHead.HeadStart[1] = 10;
    pdata->stFSNHead.HeadStart[2] = 7;
    pdata->stFSNHead.HeadStart[3] = 26;

    pdata->stFSNHead.HeadString[0] = 0;
    pdata->stFSNHead.HeadString[1] = 1;
    pdata->stFSNHead.HeadString[2] = 0x2E;
    pdata->stFSNHead.HeadString[3] = 'S';
    pdata->stFSNHead.HeadString[4] = 'N';
    pdata->stFSNHead.HeadString[5] = 'o';

    pdata->stFSNHead.HeadEnd[0] = 0;
    pdata->stFSNHead.HeadEnd[1] = 1;
    pdata->stFSNHead.HeadEnd[2] = 2;
    pdata->stFSNHead.HeadEnd[3] = 3;
}
...
void CSaveCusSNInfo::GenerateFSNFile(LPFSNFileData pdata)
{
    ...
    if (m_uFGMode == FSNGENMODE_NORMAL)
    {
        ...
        sprintf(szFileName, "sp_%04d%02d%02d_%02d%02d%02d.fsn", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
        sprintf(szFileNameBak, "%04d_%02d_%02d_%02d_%02d_%02d.fsn", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
        ...
        int iLen;
        iLen = fwrite(&pdata->stFSNHead, sizeof(STFSNHEAD) /*32*/, 1, pf);
        for (DWORD j = 0; j < pdata->stFSNHead.Counter; j++)
        {
            iLen = fwrite(pdata->pstFSNBody[j], sizeof(STFSNBody) /*1644*/, 1, pf);
        }
        fclose(pf);
...
void CSaveCusSNInfo::InitConfig()
{
    ...
    m_strConfigFilePathForXH = SPETCPATHFORXH;       //test#28
    m_strConfigFilePathForXH += "/";                 //test#28
    m_strConfigFilePathForXH += "SPSerialNoCfg.ini"; //test#28
    ...
    //test#28 start
    QFileInfo infoforxh(m_strConfigFilePathForXH.c_str());   //for 陕西信合
    if (!info.exists())
    {
        Log("InitConfig", __LINE__, "SPSerialNoCfg.ini not exist");
        return;
    }

    CINIFileReader ReaderConfigFileForXH;
    ReaderConfigFileForXH.LoadINIFile(m_strConfigFilePathForXH.c_str());
    CINIReader cINIRead = ReaderConfigFileForXH.GetReaderSection("SNRINFO");
    m_dwSaveSNDays = (DWORD)cINIRead.GetValue("SaveDays", 90);
    //test#28 end
...
//test#28   m_dwSaveSNDays = (DWORD)cINI.GetValue("SaveDays", 90);
    ...
    if (m_strSNInfoSaveDir.empty() == false) {
        QFile qFile(m_strSNInfoSaveDir.c_str());
    ...
    string strSerChar = (LPCSTR)cINI.GetValue("SerialNoChar", '$');
    m_cSerialNoChar = strSerChar.at(0);  //test#29
...
void CSaveCusSNInfo::GetSaveImgConfig(STIMGCONFIG &stImgConfig)
{
    ...
    stImgConfig.cSerialNoChar = m_cSerialNoChar;    //test#29
}
...
//test#28
void CSaveCusSNInfo::vDeleteSnrinfoForOpen()
{
    CheckAndDeleteImage(m_strImageSaveDir.c_str());
    CheckAndDeleteDailyCumulativeFile();
