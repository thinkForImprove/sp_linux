...
HRESULT CXFS_CAM::TakePictureEx(const WFSCAMTAKEPICTEX &stTakePict, DWORD dwTimeout)
{
    ...
    if (hRet != WFS_SUCCESS)
    {
        Log(ThisModule, __LINE__, "摄像命令(TakePictureEx)失败: ->InnerTakePictureEx() Fail. ReturnErr: %d, ReturnCode:%d",
            hRet, hRet);
        return hRet;
    }
...
HRESULT CXFS_CAM::Display(const WFSCAMDISP &stTakePict, DWORD dwTimeout)
{
    ...
    HRESULT hRet = 0;
    hRet = InnerDisplay(stTakePict, dwTimeout);
    if (hRet != WFS_SUCCESS)
    {
...
HRESULT CXFS_CAM::DisplayEx(const WFSCAMDISPEX &stDisplayEx, DWORD dwTimeout)
{
    ...
    if (hRet != WFS_SUCCESS)
    {
        Log(ThisModule, __LINE__, "摄像(DisplayEx)失败: ->InnerDisplayEx() Fail. ReturnErr: %d, ReturnCode:%d",
            hRet, hRet);
        return hRet;
    }
...
HRESULT CXFS_CAM::GetSignature(const WFSCAMGETSIGNATURE &stSignature, WFSCAMSIGNDATA &stSignData, DWORD dwTimeout)
{
...
void CXFS_CAM::InitConifig()
{
    ...
    {
        // 是否改变Display入参宽高(0不改变/1以宽为基准/2以高为基准/3固定宽高,缺省0)        // 30-00-00-00(FT#0055)
        m_sCamIniConfig.usDispWHChange =                                          // 30-00-00-00(FT#0055)
                (WORD)m_cXfsReg.GetValue(szKeyName, "DispWHChange", (DWORD)0);    // 30-00-00-00(FT#0055)

        // Display定宽,缺省640                                                     // 30-00-00-00(FT#0055)
        m_sCamIniConfig.usDispFixWidth =                                          // 30-00-00-00(FT#0055)
                (WORD)m_cXfsReg.GetValue(szKeyName, "DispFixWidth", (DWORD)640);  // 30-00-00-00(FT#0055)

        // Display定高,缺省480                                                      // 30-00-00-00(FT#0055)
        m_sCamIniConfig.usDispFixHeight =                                          // 30-00-00-00(FT#0055)
                (WORD)m_cXfsReg.GetValue(szKeyName, "DispFixHeight", (DWORD)480);  // 30-00-00-00(FT#0055)
