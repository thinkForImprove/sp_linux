...
HRESULT CXFS_IDC::CRD_DispenseCard(const LPWFSCRDDISPENSE lpDispense)
{
    ...
    if (m_CRDStatus.fwDevice == WFS_CRD_DEVOFFLINE) // 断线时返回HWErr                // 40-00-00-00(FT#0013)
    {                                                                               // 40-00-00-00(FT#0013)
        Log(ThisModule, -1, "DispenseCard: Device Is OFFLINE. Return: %d.",         // 40-00-00-00(FT#0013)
            WFS_ERR_HARDWARE_ERROR);                                                // 40-00-00-00(FT#0013)
        return WFS_ERR_HARDWARE_ERROR;                                              // 40-00-00-00(FT#0013)
    }                                                                               // 40-00-00-00(FT#0013)
    ...
    // 检查回收箱,FULL不允许发卡
    if (m_stCRDBoxList.nGetDispBoxData(m_stCRDBoxList.GetRetainNum(), DISPRWT_STATUS) == UNITINFO_STAT_FULL)
    {
        Log(ThisModule, -1, "DispenseCard: 回收箱Full, Return: %d.", WFS_ERR_CRD_RETAINBINFULL);
    ...
    if (nStatRet >= 0)
    {
        if (enCardstatus != IDCSTAUTS_NOCARD)
        {
            ...
            //return WFS_ERR_DEV_NOT_READY;                                 // 40-00-00-00(FT#0009)
            return WFS_ERR_CRD_DEVICE_OCCUPIED; // 设备内有卡,第2张卡不能发    // 40-00-00-00(FT#0009)
        }
    } else
    {
    ...
    // 发卡到出口                                                                    // 40-00-00-00(FT#0008)
    if (lpDispense->bPresent == TRUE)                                               // 40-00-00-00(FT#0008)
    {                                                                               // 40-00-00-00(FT#0008)
        Log(ThisModule, -1, "lpDispense->bPresent[%d] == TRUE, 调用CRD_EjecdCard()发卡到出卡口.", // 40-00-00-00(FT#0008)
            lpDispense->bPresent);                                                  // 40-00-00-00(FT#0008)
        return CRD_EjecdCard();                                                     // 40-00-00-00(FT#0008)
    }                                                                               // 40-00-00-00(FT#0008)
...
void CXFS_IDC::UpdateDevCRDStatus(LPSTCRDDEVSTATUS lpStat)
{
    ...
    if((nCRDRet = m_pDev->GetDevStat(stStatus)) == ERR_CRD_UNSUP_CMD) // 不支持
    {
        ...
        {
            stStatus.wDevice = WFS_CRD_DEVOFFLINE;
            //return;                                                   // 40-00-00-00(FT#0014)
        }
        nCRDRet = m_pCRDDev->GetDevStat(stStatus);
    }

    if (nCRDRet == ERR_CRD_READTIMEOUT) // 接收数据超时认同为OFFLINE       // 40-00-00-00(FT#0014)
    {                                                                   // 40-00-00-00(FT#0014)
        stStatus.Clear();                                               // 40-00-00-00(FT#0014)
    }                                                                   // 40-00-00-00(FT#0014)
    ...
    {
        if (stStatus.GetUnitType(i) == UNIT_NOTSUPP)
        {
            continue;
        }
...
LONG CXFS_IDC::ConvertCRDUnitInfoStatus(WORD wStat)
{
    switch (wStat)
    {
        case UNITINFO_STAT_OK       : return WFS_CRD_STATCUOK;          // 正常
