...
//    InitStatus();                                                         //30-00-00-00(FT#0019)
    ...
    // 读卡器Open前先设置读卡器的通信协议类型
    m_pDev->SetData((void*)&m_stConfig.usInterProtocal, 99);
    ...
    //----------发卡模块处理----------
    if (m_stCRDINIConfig.bCRDDeviceSup == TRUE)     // 支持启用发卡模块
    {
        ...
        // 加载发卡模块动态库
        if (!LoadCRDDevDll(ThisModule))
        {
            return WFS_ERR_HARDWARE_ERROR;
        }

        // Open发卡模块
        nCRDRet = m_pCRDDev->Open(m_stCRDINIConfig.szCRDDeviceConList);
        if (nCRDRet != CRM_SUCCESS)
        {
            Log(ThisModule, -1, "Open CRD Device[%s] Fail, CRD RetCode: %d, Return: %d",
                m_stCRMINIConfig.szCRMDeviceConList, nCRDRet, CRMErr2XFSErrCode(nCRDRet));
            return CRDErr2XFSErrCode(nCRDRet);
        } */

        // 更新扩展状态_版本参数
        CHAR szCRDFWVer[MAX_PATH] = { 0x00 };     // CRD固件版本
        m_pDev->GetVersion(szCRDFWVer, sizeof(szCRDFWVer) - 1, 2);

        m_cCRDExtra.AddExtra("LastErrorCode", "00000000");
        m_cCRDExtra.AddExtra("LastErrorDetail", "00000000");
        m_cCRDExtra.AddExtra("FirmwareVersion", szCRDFWVer);
    }
...
HRESULT CXFS_IDC::OnStatus()
{
    ...
    if (nRet == ERR_IDC_DEVICE_REOPEN)                  //30-00-00-00(FT#0019)
        UpdateDevStatus(ERR_IDC_SUCCESS);               //30-00-00-00(FT#0019)
    else                                                // 40-00-00-00(FT#0014)
        UpdateDevStatus(nRet);                          // 40-00-00-00(FT#0014)
    if (m_Status.fwDevice == WFS_IDC_DEVOFFLINE){       //30-00-00-00(FT#0019)
        ...
        {                                               //30-00-00-00(FT#0019)
            //m_Status.fwDevice = WFS_IDC_DEVONLINE;    //30-00-00-00(FT#0019)
        }                                               //30-00-00-00(FT#0019)
    }                                                   //30-00-00-00(FT#0019)

    if (m_stCRDINIConfig.bCRDDeviceSup == TRUE)
    {
        UpdateDevCRDStatus();   // 更新发卡机状态
    }
...
HRESULT CXFS_IDC::EjectCard(DWORD dwTimeOut)
{
    ...
    else if (m_Cardstatus == IDCSTAUTS_ENTERING)//30-00-00-00(FT#0044) 卡在读卡器口
    {                                           //30-00-00-00(FT#0044)
        //解除jam状态                            //30-00-00-00(FT#0044)
        m_bJamm = FALSE;                        //30-00-00-00(FT#0044)
        m_WaitTaken = WTF_TAKEN;                //30-00-00-00(FT#0044)
        return WFS_SUCCESS;                     //30-00-00-00(FT#0044)
    }                                           //30-00-00-00(FT#0044)
    else if (/*m_Cardstatus == IDCSTAUTS_ENTERING || */               //30-00-00-00(FT#0044)
             m_Cardstatus == IDCSTAUTS_INTERNAL)//卡在读卡器口
    {
        iRet = m_pDev->EjectCard();             //30-00-00-00(FT#0044)
        UpdateDevStatus(iRet);                  //30-00-00-00(FT#0044)
        if (iRet >= 0)                          //30-00-00-00(FT#0044)
        {
            //解除jam状态
            m_bJamm = FALSE;
...
//            nRet = UpdateCardStatus();                                        //30-00-00-00(FT#0044)
//            if (nRet < 0)                                                     //30-00-00-00(FT#0044)
//            {                                                                 //30-00-00-00(FT#0044)
//                Log(ThisModule, -1, "检测卡失败:%s", ProcessReturnCode(nRet));  //30-00-00-00(FT#0044)
//                return WFS_ERR_HARDWARE_ERROR;                                //30-00-00-00(FT#0044)
//            }                                                                 //30-00-00-00(FT#0044)
                                                                                //30-00-00-00(FT#0044)
//            m_Status.fwMedia = WFS_IDC_MEDIAENTERING;                         //30-00-00-00(FT#0044)
//            if (LatestCardstatus != IDCSTAUTS_NOCARD)                         //30-00-00-00(FT#0044)
//                m_WaitTaken = WTF_TAKEN;                                      //30-00-00-00(FT#0044)

            return WFS_SUCCESS;
        }
        else if (iRet == ERR_IDC_JAMMED)                                        //30-00-00-00(FT#0044)
        {
            //m_Status.fwMedia = WFS_IDC_MEDIAJAMMED;                           //30-00-00-00(FT#0044)
            iRet = UpdateCardStatus();                                          //30-00-00-00(FT#0044)
            if (iRet < 0)                                                       //30-00-00-00(FT#0044)
            {                                                                   //30-00-00-00(FT#0044)
                Log(ThisModule, -1, "检测卡失败:%s", ProcessReturnCode(iRet));    //30-00-00-00(FT#0044)
                return WFS_ERR_HARDWARE_ERROR;
            }
            Log(ThisModule, -1, "退卡时:%s", ProcessReturnCode(iRet));           //30-00-00-00(FT#0044)
    ...
    {
        // 先去掉激活状态
        //int nRet = 0;                                                             //30-00-00-00(FT#0044)
        if (m_Cardstatus == IDCSTAUTS_ICC_PRESS ||
            m_Cardstatus == IDCSTAUTS_ICC_ACTIVE)
        {
            iRet = m_pDev->ICCDeActivation();                                       //30-00-00-00(FT#0044)
            UpdateDevStatus(iRet);                                                  //30-00-00-00(FT#0044)
            if (0 != iRet)                                                          //30-00-00-00(FT#0044)
            {
                Log(ThisModule, -1, "IC卡退卡去激活时:%s", ProcessReturnCode(iRet));   //30-00-00-00(FT#0044)
                return WFS_ERR_HARDWARE_ERROR;
            }
        }

        // 退卡                                                                       //30-00-00-00(FT#0044)
//        nRet = m_pDev->ICCRelease();                                               //30-00-00-00(FT#0044)
//        UpdateDevStatus(nRet);                                                     //30-00-00-00(FT#0044)
//        if (0 != nRet)                                                             //30-00-00-00(FT#0044)
//        {                                                                          //30-00-00-00(FT#0044)
//            Log(ThisModule, -1, "IC卡退卡释放卡时:%s", ProcessReturnCode(nRet));     //30-00-00-00(FT#0044)
//            return WFS_ERR_HARDWARE_ERROR;                                         //30-00-00-00(FT#0044)
//        }                                                                          //30-00-00-00(FT#0044)

        // 释放IC后再退卡
        iRet = m_pDev->EjectCard();
        UpdateDevStatus(iRet);
        if (iRet >= 0)
        {
            //解除jam状态
            m_bJamm = FALSE;
...
//            int nRet = UpdateCardStatus();                                    //30-00-00-00(FT#0044)
//            if (nRet < 0)                                                     //30-00-00-00(FT#0044)
//            {                                                                 //30-00-00-00(FT#0044)
//                Log(ThisModule, -1, "检测卡失败:%s", ProcessReturnCode(nRet));  //30-00-00-00(FT#0044)
//                return WFS_ERR_HARDWARE_ERROR;                                //30-00-00-00(FT#0044)
...
//                m_WaitTaken = WTF_TAKEN;                                      //30-00-00-00(FT#0044)
        ...
        else if (iRet == ERR_IDC_JAMMED)                                        //30-00-00-00(FT#0044)
        {
            ...
            Log(ThisModule, -1, "IC卡退卡时:%s", ProcessReturnCode(iRet));        //30-00-00-00(FT#0044)
            return WFS_ERR_IDC_MEDIAJAM;
        }
        else
        {
...
int CXFS_IDC::MoveCardToMMPosition()
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();

    int nRet = m_pDev->ICCMove();
    if (nRet < 0)
    {
        ...
        Log(ThisModule, -1, "吞卡失败_2:(%d)%s", nRet, ProcessReturnCode(nRet));
        return WFS_ERR_HARDWARE_ERROR;
    }
...
HRESULT CXFS_IDC::CMEjectCard(LPCSTR lpszCardNo)
{
    ...
    if (m_stCRMINIConfig.bCRMDeviceSup == TRUE) // 支持启用CRM
    {
        ...
        {
            Log(ThisModule, -1, "CMEject: CardNo[%s] Is Invalid(==NULL||<12||>20). Return: %d.",
...
HRESULT CXFS_IDC::CMRetainCard()
{
    ...
    if (m_stCRMINIConfig.bCRMDeviceSup == TRUE) // 支持启用CRM
    {
        ...
        if (m_Status.fwMedia == WFS_IDC_MEDIAPRESENT)                       //30-00-00-00(FT#0044)
        {                                                                   //30-00-00-00(FT#0044)
            nIDCRet = MoveCardToMMPosition(); // 卡移动到读卡器内部(读卡器命令)  //30-00-00-00(FT#0044)
            if (nIDCRet != WFS_SUCCESS)                                     //30-00-00-00(FT#0044)
            {                                                               //30-00-00-00(FT#0044)
                Log(ThisModule, 1, "卡移动到读卡器内部, Fail.");               //30-00-00-00(FT#0044)
                return nIDCRet;                                             //30-00-00-00(FT#0044)
            }                                                               //30-00-00-00(FT#0044)
        }                                                                   //30-00-00-00(FT#0044)
...
HRESULT CXFS_IDC::CMSetCount(LPWORD lpwCount)
{
    ...
    if (m_stCRMINIConfig.bCRMDeviceSup == TRUE) // 支持启用CRM
    {
        ...
        cINI.SetValue(RETAIN_CARD_MAXCOUNT, (DWORD)*lpwCount);
    } else
    {
        return WFS_ERR_UNSUPP_COMMAND;
    }
...
HRESULT CXFS_IDC::CMGetCardInfo(LPCSTR lpszQuery, char lpszCardInfo[1024])
{
    ...
    if (m_stCRMINIConfig.bCRMDeviceSup == TRUE) // 支持启用CRM
    {
        ...
        for (int i = 0; i < 5; i ++)
        {
            memset(szCardNo, 'F', sizeof(szCardNo));
            memset(szInTime, 'F', sizeof(szInTime));
...
HRESULT CXFS_IDC::CMReset()
{
    ...
    INT nCRMRet = CRM_SUCCESS;
    INT nIDCRet = WFS_SUCCESS;              //30-00-00-00(FT#0044)
    ...
    if (m_stCRMINIConfig.bCRMDeviceSup == TRUE) // 支持启用CRM
    {
        ...
        UpdateCardStatus();                                                 //30-00-00-00(FT#0044)
        if (m_Status.fwMedia == WFS_IDC_MEDIAPRESENT)                       //30-00-00-00(FT#0044)
        {                                                                   //30-00-00-00(FT#0044)
            nIDCRet = MoveCardToMMPosition(); // 卡移动到读卡器内部(读卡器命令)  //30-00-00-00(FT#0044)
            if (nIDCRet != WFS_SUCCESS)                                     //30-00-00-00(FT#0044)
...
//        else                                                                      //30-00-00-00(FT#0044)
//        {                                                                         //30-00-00-00(FT#0044)
//            int nIndex = 0;                                                       //30-00-00-00(FT#0044)
//            while((m_Cardstatus != IDCSTAUTS_INTERNAL) && (nIndex < 20))          //30-00-00-00(FT#0044)
//            {                                                                     //30-00-00-00(FT#0044)
//                nRet = UpdateCardStatus();                                        //30-00-00-00(FT#0044)
//                if (nRet < 0)                                                     //30-00-00-00(FT#0044)
//                {                                                                 //30-00-00-00(FT#0044)
...
//                    return WFS_ERR_HARDWARE_ERROR;                                //30-00-00-00(FT#0044)
//                }                                                                 //30-00-00-00(FT#0044)
//                CQtTime::Sleep(100);                                              //30-00-00-00(FT#0044)
//                nIndex++;                                                         //30-00-00-00(FT#0044)
//            }                                                                     //30-00-00-00(FT#0044)
                                                                                    //30-00-00-00(FT#0044)
//            if (m_Cardstatus == IDCSTAUTS_INTERNAL)                               //30-00-00-00(FT#0044)
//            {                                                                     //30-00-00-00(FT#0044)
//                Log(ThisModule, 1, "进卡成功");                                    //30-00-00-00(FT#0044)
//                m_Status.fwMedia = WFS_IDC_MEDIAPRESENT;                          //30-00-00-00(FT#0044)
//                FireCardInserted();                                               //30-00-00-00(FT#0044)
//            } else                                                                //30-00-00-00(FT#0044)
//            {                                                                     //30-00-00-00(FT#0044)
//                m_Status.fwDevice = WFS_IDC_DEVHWERROR;                           //30-00-00-00(FT#0044)
//                return WFS_ERR_HARDWARE_ERROR;                                    //30-00-00-00(FT#0044)
//            }                                                                     //30-00-00-00(FT#0044)
//        }                                                                         //30-00-00-00(FT#0044)
    } else {
    ...
    int nIndex = 0;                                         //30-00-00-00(FT#0044)
    while(true)                                             //30-00-00-00(FT#0044)
    {                                                       //30-00-00-00(FT#0044)
        if (m_Cardstatus == IDCSTAUTS_INTERNAL)             //30-00-00-00(FT#0044)
        {                                                   //30-00-00-00(FT#0044)
            Log(ThisModule, 1, "进卡成功");                  //30-00-00-00(FT#0044)
            m_Status.fwMedia = WFS_IDC_MEDIAPRESENT;        //30-00-00-00(FT#0044)
            //FireCardInserted();                             //30-00-00-00(FT#0044)
            break;                                          //30-00-00-00(FT#0044)
        }                                                   //30-00-00-00(FT#0044)
        else if (nIndex >= 15)                              //30-00-00-00(FT#0044)
        {                                                   //30-00-00-00(FT#0044)
            m_Status.fwDevice = WFS_IDC_DEVHWERROR;         //30-00-00-00(FT#0044)
            return WFS_ERR_HARDWARE_ERROR;                  //30-00-00-00(FT#0044)
        } else {                                            //30-00-00-00(FT#0044)
            UpdateCardStatus();                             //30-00-00-00(FT#0044)
            nIndex++;                                       //30-00-00-00(FT#0044)
            CQtTime::Sleep(10);                             //30-00-00-00(FT#0044)
        }                                                   //30-00-00-00(FT#0044)
    }                                                       //30-00-00-00(FT#0044)
...
//    if(m_stCRMINIConfig.bCRMDeviceSup){                                   //30-00-00-00(FT#0044)
//        INT nRet = MoveCardToMMPosition(); // 卡移动到读卡器内部(读卡器命令)   //30-00-00-00(FT#0044)
//        if (nRet != WFS_SUCCESS)                                          //30-00-00-00(FT#0044)
//        {                                                                 //30-00-00-00(FT#0044)
//            Log(ThisModule, 1, "卡移动到读卡器内部, Fail.");                 //30-00-00-00(FT#0044)
//            return nRet;                                                  //30-00-00-00(FT#0044)
//        }                                                                 //30-00-00-00(FT#0044)
//    }                                                                     //30-00-00-00(FT#0044)
...
long CXFS_IDC::Convert2XFSErrCode(long lIDCErrCode)
{
    ...
    case ERR_IDC_NOCHIP:                  return WFS_ERR_IDC_INVALIDMEDIA;  //30-00-00-00(FT#0058)
...
void CXFS_IDC::UpdateRetainCards(MODIFY_CARDS_MODE mode, bool fire_threshold/* = true*/)
{
    ...
    // 回收箱计数及状态更新到DispBox发卡箱结构体                                                             // 40-00-00-00(FT#0013)
    m_stCRDBoxList.nSetDispBoxData(m_stCRDBoxList.GetRetainNum(), m_Status.usCards, DISPRWT_COUNT);     // 40-00-00-00(FT#0013)
    INT nRetainStat = UNITINFO_STAT_OK;                                                                 // 40-00-00-00(FT#0013)
    switch(m_Status.fwRetainBin)                                                                        // 40-00-00-00(FT#0013)
    {                                                                                                   // 40-00-00-00(FT#0013)
        case WFS_IDC_RETAINBINOK: nRetainStat = UNITINFO_STAT_OK; break;                                // 40-00-00-00(FT#0013)
        case WFS_IDC_RETAINNOTSUPP: nRetainStat = UNITINFO_STAT_UNKNOWN; break;                         // 40-00-00-00(FT#0013)
        case WFS_IDC_RETAINBINFULL: nRetainStat = UNITINFO_STAT_FULL; break;                            // 40-00-00-00(FT#0013)
        case WFS_IDC_RETAINBINHIGH: nRetainStat = UNITINFO_STAT_HIGH; break;                            // 40-00-00-00(FT#0013)
    }                                                                                                   // 40-00-00-00(FT#0013)
    m_stCRDBoxList.nSetDispBoxData(m_stCRDBoxList.GetRetainNum(), nRetainStat, DISPRWT_STATUS);         // 40-00-00-00(FT#0013)
}
...
long CXFS_IDC::UpdateCardStatus()
{
    ...
    BOOL bBackRemoved = FALSE;                      // 从内部把卡移出
    ...
    if ((fwMedia == WFS_IDC_MEDIANOTPRESENT && m_WaitTaken == WTF_TAKEN) ||
            (fwMedia == WFS_IDC_MEDIANOTPRESENT && bBackRemoved))
    {
        FireMediaRemoved();
...
long CXFS_IDC::ReadChip()
{
    ...
    nRet = m_pDev->ICCActive(szATRInfo, nATRLen);                           //30-00-00-00(FT#0058)
    //if (ERR_IDC_HWERR == nRet)
    //{
        //UpdateDevStatus(nRet);
    //}
    if (nRet != 0)                                                          //30-00-00-00(FT#0058)
    {                                                                       //30-00-00-00(FT#0058)
        m_Status.fwChipPower  = WFS_IDC_CHIPNODEVICE;                       //30-00-00-00(FT#0058)
        Log("ICCActive", -1, "ReadChip失败:%s", ProcessReturnCode(nRet));    //30-00-00-00(FT#0058)
        return Convert2XFSErrCode(nRet);                                    //30-00-00-00(FT#0058)
    }                                                                       //30-00-00-00(FT#0058)
    //else
...
int CXFS_IDC::GetFWVersion()
{
    ...
    m_cExtra.AddExtra("FWDetail[00]", szFWVersion);
...
int CXFS_IDC::InitConfig()
{
    ...
    m_stConfig.usInterProtocal = (USHORT)m_cXfsReg.GetValue("CONFIG", "IDC_InterProtocal", (INT)0);

    //--------------------退卡模块INI配置参数获取--------------------
    // 是否支持启用退卡模块: 0不支持/1支持,缺省0
    m_stCRMINIConfig.bCRMDeviceSup = ((WORD)m_cXfsReg.GetValue("CRM_DEVICE_CFG", "CRMDeviceSup", (INT)0) == 1);
    ...
    if (m_stCRMINIConfig.bCRMDeviceSup == TRUE)
    {
        ...
        // 退卡模块设备类型(缺省0)
        m_stCRMINIConfig.wCRMDeviceType = (WORD)m_cXfsReg.GetValue("CRM_DEVICE_CFG", "DeviceType", (INT)0);
        ...
        if (m_stCRMINIConfig.wCRMDeviceType == CRM_DEV_CRT730B)
        {
            ...
            m_stCRMINIConfig.wDeviceInitAction = (WORD)m_cXfsReg.GetValue(szKeyName, "DeviceInitAction", (INT)0);
            ...
            // 3. 退卡后卡位置(0读卡器内部;1读卡器前入口,缺省0）
            m_stCRMINIConfig.wEjectCardPos = (WORD)m_cXfsReg.GetValue(szKeyName, "EjectCardPos", (INT)0);
            ...
            m_stCRMINIConfig.wEnterCardRetainSup = (WORD)m_cXfsReg.GetValue(szKeyName, "EnterCardRetainSup", (INT)0);
...
void CXFS_IDC::ThreadEject_Wait()
{
    ...
    while(wCount < (WORD)(1000 * 3 / 20))
    {
        ...
        wCount ++;
        m_CardReaderEjectFlag = MoveCardToMMPosition(); // 卡移动到读卡器内部(读卡器命令)
        if (m_CardReaderEjectFlag == WFS_SUCCESS)
        {
            break;
        }
