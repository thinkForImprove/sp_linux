...
#define LOG_NAME   "DevIDC_CRT.log"
...
CDevIDC_CRT::CDevIDC_CRT(LPCSTR lpDevType)
    : m_nCardStatus(IDCSTATUS_UNKNOWN),
      ...
      m_bICCRelease(TRUE),
      m_bICCJam(FALSE),
      m_nRetainNum(-1),         //30-00-00-00(FT#0009)
      m_bOpen(FALSE),           //30-00-00-00(FT#0019)
      m_bReOpen(FALSE),         //30-00-00-00(FT#0019)
      m_IDCInterProtocal(CRT_350NJ11_USB)
{
    SetLogFile(LOG_NAME, ThisFile, lpDevType);
    m_wSignalCom = 0;               // 通讯方式(0USB/1COM,缺省0)
}
...
LONG CDevIDC_CRT::ReadConfig()
{
    ...
    m_initParam.append("324100");                 //30-00-00-00(FT#0030)
    ...
    case 1:                                         //30-00-00-00(FT#0030)
        m_initParam.data()[5] = '1';                //30-00-00-00(FT#0030)
...
LONG CDevIDC_CRT::OpenDevice()
{
    ...
    if (iRet < 0)
    {
        Log(ThisModule, __LINE__, "Open dev[%s] failed[%d] ", m_strMode.c_str(), iRet);
...
//    }
    ...
    else if ((eActFlag == CARDACTION_EJECT))// 2 弹卡
    {
        ...
        else if (m_nCardStatus == IDCSTAUTS_ENTERING)         //30-00-00-00(FT#0044)
        {                                                     //30-00-00-00(FT#0044)
            m_bSendflag = TRUE;                               //30-00-00-00(FT#0044)
        }                                                     //30-00-00-00(FT#0044)
        else
        {
            if (EjectCard() < 0)
...
INT CDevIDC_CRT::EatCard()
{
    ...
    m_nRetainNum = GetRetainNum();          //30-00-00-00(FT#0009)
    char reply[2068] = {0};
    ...
    nRet = GetRetainNum();                                                  //30-00-00-00(FT#0009)
    if (nRet >= 0 && m_nRetainNum >= 0 && ((nRet - m_nRetainNum) <= 0))     //30-00-00-00(FT#0009)
    {                                                                       //30-00-00-00(FT#0009)
        nRet = CR_ERROR_BEFORE_RETRACT_PULL;                                //30-00-00-00(FT#0009)
        return HWErrToStdErr(nRet);                                         //30-00-00-00(FT#0009)
    }                                                                       //30-00-00-00(FT#0009)
...
//    if (nRet < 0) // 处理卡的错误状态           //30-00-00-00(FT#0044)
//    {                                        //30-00-00-00(FT#0044)
//        return ERR_IDC_HWERR;                //30-00-00-00(FT#0044)
//    }                                        //30-00-00-00(FT#0044)
    ...
    //nRet = SendCmd("C04", (LPCSTR)m_initParam.constData(), m_initParam.size(), ThisModule);      //30-00-00-00(FT#0030)//30-00-00-00(FT#0044)
    nRet = SendCmd("C30", nullptr, 0, ThisModule);                                                 //30-00-00-00(FT#0044)
    if (nRet != 0)
    {
        Log(ThisModule, __LINE__, "SendCmd()失败");
        return nRet;
    }
    nRet = GetResponse(reply, sizeof(reply), /*TIMEOUT_WAIT_ACTION*/30 * 1000, ThisModule);//40-00-00-00(FT#0008)
    if (nRet < 0)
    {
        /* 40-00-00-00(FT#0008) DELETE START
        CQtTime::Sleep(500);
        while (reply[3] == 0x00 || nIndex < 100)
        {
            ...
            if ((reply[6] == 0x30 && reply[7] == 0x31)) // 返回值6,7位=01为卡在卡口  // 40-00-00-00(FT#0008)
                break;
            nIndex++;
        }

        if (nRet < 0)
        {40-00-00-00(FT#0008) DELETE END*/
...
//    }
    ...
    {       
        ...
        while(TRUE){
            ...
            //CQtTime::Sleep(50);             //30-00-00-00(FT#0044)
...
INT CDevIDC_CRT::ReadTracks(STTRACK_INFO &stTrackInfo)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (reply[3] == 'P')
        {
            ...
            if (m_IDCInterProtocal == CRT_591HDR1_COM)
            {
                // CRT读卡发卡一体式设备返回信息多一位(发卡器状态位),去掉1位发卡器状态位
                memcpy(stTrackInfo.TrackData[i].szTrack, reply + 9, nLen - 5 - 1);
            } else
            {
                memcpy(stTrackInfo.TrackData[i].szTrack, reply + 8, nLen - 5);
            }
...
//    LOGDEVACTION();
    ...
    if (m_bReOpen)                      //30-00-00-00(FT#0019)
        return ERR_IDC_DEVICE_REOPEN;   //30-00-00-00(FT#0019)
    else                                //30-00-00-00(FT#0019)
        return iRet;                    //30-00-00-00(FT#0019)
}
...
INT CDevIDC_CRT::GetFWVersion(char pFWVersion[], unsigned int &uLen)
{
    ...
    if (m_IDCInterProtocal == CRT_591HDR1_COM)
    {
        INT nRcvDataLen = MAKEWORD(reply[2], reply[1]); // CMD(3BYTE)+stat(3位)+text
        memcpy(pFWVersion, reply + 9, nRcvDataLen - 6);  // CRT读卡发卡一体式设备返回信息多一位(发卡器状态位)
    } else
    {
        strcpy(pFWVersion, reply + 8);
    }
    uLen = strlen(pFWVersion);
    pFWVersion[uLen] = 0x00;
...
INT CDevIDC_CRT::ICCActive(char pATRInfo[], unsigned int &uATRLen)
{
    ...
    if (m_IDCInterProtocal == CRT_591HDR1_COM)
    {
        uATRLen = uATRLen - 1;                // 去掉1位发卡器状态位
        memcpy(pATRInfo, reply + 9, uATRLen); // CRT读卡发卡一体式设备返回信息多一位(发卡器状态位)
    } else
    {
        memcpy(pATRInfo, reply + 8, uATRLen); // 返回去掉了消息头、尾以及BCC校验数据的纯ATR信息数据
    }
...
INT CDevIDC_CRT::ICCChipIO(ICCardProtocol eProFlag, char *pInOutData, unsigned int &nInOutLen, DWORD dwBuffSz)
{
    ...
    int nRet = SendCmd(szCmd, pInOutData, nInOutLen, ThisModule);
    if (nRet != 0)
    {
        Log(ThisModule, __LINE__, "SendCmd()失败");
        return nRet;
    ...
    //返回61XX的特殊处理
    INT nAddCnt = 0;
    if (m_IDCInterProtocal == CRT_591HDR1_COM) // CRT591返回多一位发卡器状态位
    {
        nAddCnt = 1;
    }
    //40-00-00-00(FT#0005) add start
    if((reply[8 + nAddCnt] == 0x61) && ((MAKEWORD(reply[2], reply[1])) - 5 == (2 + nAddCnt))){
        BYTE byGetChipResCmd[5] = {0x00, 0xc0, 0x00, 0x00, 0x00};
        byGetChipResCmd[4] = reply[9 + nAddCnt];
    ...
    if (reply[5] == '5')
    {
        ...
        do {
            ...
            nLen = (MAKEWORD(reply[2], reply[1])) - 5 - nAddCnt;
            if(nLen + dwOffset > dwBuffSz){
                nLen = dwBuffSz - dwOffset;
                memcpy(pInOutData + dwOffset, reply + 8 + nAddCnt, nLen);
    ...
    } else if (reply[5] == '4')     //不超过1000字节
    {
        ...
        memcpy(pInOutData, reply + 8 + nAddCnt, nInOutLen);
    }
...
//30-00-00-00(FT#0009) add start
int CDevIDC_CRT::GetRetainNum()
{
    THISMODULE(__FUNCTION__);
    AutoMutex(m_MutexAction);
    VERTIFYISOPEN();
    ...
    int nNum = 0;
    int nRet = SendCmd("CC0", nullptr, 0, ThisModule);
    if (nRet < 0)
    {
        Log(ThisModule, __LINE__, "读取当前回收计数值命令CC0失败");
        return nRet;
    }
    ...
    nRet = GetResponse(reply, sizeof(reply), m_stIDC.bWaitInsertCardIntervalTime * 1000, ThisModule);
    if(nRet < 0){
        Log(ThisModule, __LINE__, "读取当前回收计数值命令CC0接收数据失败");
        return nRet;
    }
    if(reply[3] != 'P'){
        Log(ThisModule, __LINE__, "读取当前回收计数值命令执行失败");
        nRet = CardReaderError(reply + 6, ThisModule);
        return HWErrToStdErr(nRet);
    }
    ...
    if (m_IDCInterProtocal == CRT_591HDR1_COM)
    {
        memcpy(byte, reply + 9, 2); // CRT读卡发卡一体式设备返回信息多一位(发卡器状态位)
    } else
    {
        memcpy(byte, reply + 8, 2);
    }
    //memcpy(byte, reply + 8, 2);
    nNum = atoi(byte);
...
int CDevIDC_CRT::HWErrToStdErr(int iCardError)
{
    ...
    case CR_ERROR_ICC_OR_SAM_ACTIVE_ATR_INVALID:
        ...
        break;
    case CR_ERROR_BEFORE_RETRACT_PULL:                  //30-00-00-00(FT#0009)
        iRet = ERR_IDC_CARDPULLOUT;                     //30-00-00-00(FT#0009)
        break;                                          //30-00-00-00(FT#0009)
    case CR_ERROR_ICC_OR_SAM_ATR_COMM_ERROR:            //30-00-00-00(FT#0058)
        iRet = ERR_IDC_NOCHIP;                          //30-00-00-00(FT#0058)
        break;                                          //30-00-00-00(FT#0058)
    default:
        iRet = ERR_IDC_HWERR;
        break;
    }
...
INT CDevIDC_CRT::SendCmd(const char *pszCmd, const char *lpData, int nLen, const char *pszCallFunc)
{
    ...
    if (m_wSignalCom == 1)
    {
        nRet = SendSinglePacket_COM(pszCmd, lpData, nLen, ThisModule);
        if (nRet < 0)
        {
            return nRet;
        }
...
INT CDevIDC_CRT::SendSinglePacket(const char* pszCmd, const char *lpData, int nLen, const char *pszCallFunc)
{
    ...
    for (int iIndex = 0; iIndex < 4; iIndex++)
    {
        ...
        if (nRet < 0)
        {
            if (nRet == ERR_DEVPORT_NOTOPEN || nRet == ERR_DEVPORT_WRITE)   //30-00-00-00(FT#0019)
            {
                Close();                                //30-00-00-00(FT#0019)
                int nRetTemp = Open(m_strMode.c_str()); //30-00-00-00(FT#0019)
                if ((nRetTemp == ERR_DEVPORT_SUCCESS) && (iIndex < 3))
                {                                       //30-00-00-00(FT#0019)
                    m_bReOpen = TRUE;                   //30-00-00-00(FT#0019)
                    continue;
                }                                       //30-00-00-00(FT#0019)
            }
            m_bReOpen = FALSE; //重连失败                //30-00-00-00(FT#0019)
            return ConvertErrorCode(nRet);
        }
        else
            break;
...
INT CDevIDC_CRT::SendMultiPacket(const char *pszCmd, const char *lpData, int nLen, const char *pszCallFunc)
{
    ...
    BYTE dwReportID = {0};
    //发送的数据格式
    //ReportID(1 byte) + LEN(2 byte) + CMD(3 byte) + TEXT(58 byte)
    int nRet = 0;

    // Multi packet
    dwReportID = CRT_MULTI_REPORT_ID;
    buf[0] = (char)dwReportID;
    buf[1] = (char)((nLen + 3) / 256);
    buf[2] = (char)((nLen + 3) % 256);
    memcpy(buf + 3, pszCmd, 3);
    ...
    for(int nWriteIdx = 0; nWriteIdx < nPackCount; nWriteIdx++){
        ...
        int nlpDataOffset = nWriteIdx * CRT_PACK_MAX_CMP_LEN; //58
...
INT CDevIDC_CRT::GetResponse(char *pszResponse, int nLen, int nTimeout, const char *pszCallFunc)
{
    ...
    if (m_wSignalCom == 1)
    {
        return GetResponse_COM(pszResponse, nLen, nTimeout, pszCallFunc);
    }
