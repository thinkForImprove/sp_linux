...
int CDevIDC_CRT::GetDevStat(STCRDDEVSTATUS &stStat)
{
    ...
    if (nRcvDataLen < 0)
    {
        Log(ThisModule, __LINE__, "读取设备状态: 接收应答: 命令<%s>->GetResponse_COM(%s, %d, %d)失败, Return: %d.",
...
INT CDevIDC_CRT::GetResponse_COM(char *pszResponse, int nLen, int nTimeout, const char *pszCallFunc)
{
    ...
    // 读操作超时+写操作超时+通讯错误:->读数据超时                                     // 40-00-00-00(FT#0014)
    if (nRet == ERR_DEVPORT_RTIMEOUT || nRet == ERR_DEVPORT_WTIMEOUT || nRet == ERR_DEVPORT_FAIL)// 40-00-00-00(FT#0014)
    {                                                                           // 40-00-00-00(FT#0014)
        return ERR_IDC_READTIMEOUT;                                             // 40-00-00-00(FT#0014)
    }                                                                           // 40-00-00-00(FT#0014)
