...
#define TIMEOUT_WAIT_ACTION                                     (2*1000)
...
#define CRT_PACK_MAX_CMP_LEN        (58)    // 一帧数据长度: 64 - ReportID(1byte) - LEN(2byte) - "CXX"(3byte) = 58
...
#define SND_CMD_DEV_RESET           "CP0"   // 设备复位
#define SND_CMD_DEV_UPDATA          "CP1"   // 进入固件升级模式
...
public: // 接口函数封装
    ...
    INT     WriteCardSlotInfo(WORD wSlotNo, CHAR szInfo[68+1]);     // 6. 写指定暂存仓信息
    INT     ReadCardSlotInfo(WORD wSlotNo, CHAR szInfo[68+1]);      // 7. 读指定暂存仓信息
    INT     ClearCardSoltInfo(WORD wSlotNo = 0);                    // 8. 清除暂存仓信息
    INT     GetSoftVersion(CHAR szVersion[8+1]);                    // 9. 获取软件版本信息
    ...
    INT     GetDeviceSerialNumber(CHAR szSerialNum[18+1]);          // 14. 获取设备序列号
...
private: // USB命令收发
    INT SendCmd(const char *pszCmd, const char *lpData, int nLen, const char *pszCallFunc);     // 主下发数据
    INT GetResponse(char *pszResponse, int nLen, int nTimeout, const char *pszCallFunc);        // 主接收数据
    INT SendSinglePacket(const char* pszCmd, const char *lpData, int nLen, const char *pszCallFunc);    // 下发单帧数据
    INT SendMultiPacket(const char *pszCmd, const char *lpData, int nLen, const char *pszCallFunc);     // 下发多帧数据
