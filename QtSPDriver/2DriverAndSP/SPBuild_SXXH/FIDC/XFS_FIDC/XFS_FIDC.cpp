...
CXFS_FIDC::CXFS_FIDC() : m_bNeedRepair(FALSE)
{
    ...
    m_bMultiCard = FALSE;
    m_bCmdExecute = false;                  //40-00-00-00(FT#0003)
    m_bStartCheckCard = false;
    m_bExecChipPoweron = false;
...
//        CAutoControlLed AutoCtrLed(this);
        ...
        else
        {
            ...
            FireCardInserted();
            SetDevBusyStatus();                     //40-00-00-00(FT#0004)
...
long CXFS_FIDC::UpdateCardStatus()
{
    ...
    if (iRetStatus >= 0)
    {
        m_Cardstatus = m_bStartCheckCard ? m_Cardstatus : IDCSTAUTS_NOCARD;
        switch (m_Cardstatus)
        {
        ...
        fwChipPower = m_bExecChipPoweron ? fwChipPower : WFS_IDC_CHIPUNKNOWN;
    }
    else
    {
...
//40-00-00-00(FT#0004)    if (fwMedia == WFS_IDC_MEDIANOTPRESENT && wLastMedia == WFS_IDC_MEDIAENTERING)
    if (fwMedia == WFS_IDC_MEDIANOTPRESENT &&                                           //40-00-00-00(FT#0004)
        (wLastMedia == WFS_IDC_MEDIAENTERING || wLastMedia == WFS_IDC_MEDIAPRESENT))    //40-00-00-00(FT#0004)
    {
        UnsetDevBusyStatus();                       //40-00-00-00(FT#0004)
        UpdateDevStatus(iRetStatus);                //40-00-00-00(FT#0004)
        //关闭非接灯
		ControlLED(0);
        FireMediaRemoved();
        m_bStartCheckCard = false;
...
void CXFS_FIDC::UpdateDevStatus(int iRet)
{
    ...
    {                                               //40-00-00-00(FT#0003)
        if(m_bCmdExecute){                          //40-00-00-00(FT#0003)
            fwDevice = WFS_IDC_DEVBUSY;
        } else {                                    //40-00-00-00(FT#0003)
...
//40-00-00-00(FT#0003)
void CXFS_FIDC::SetDevBusyStatus()
{
    m_bCmdExecute = true;
    m_Status.fwDevice = WFS_IDC_DEVBUSY;
}

//40-00-00-00(FT#0003)
void CXFS_FIDC::UnsetDevBusyStatus()
{
     m_bCmdExecute = false;
}
#endif
