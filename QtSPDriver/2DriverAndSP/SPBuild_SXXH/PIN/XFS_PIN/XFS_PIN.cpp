...
//Del    if (lpImport->lpsKey == nullptr || (lpImport->fwUse != 0 && lpImport->lpxValue == nullptr))
    ...
    if((lpImport->lpxValue->usLength == 0) || (lpImport->lpxValue->usLength % 8 != 0)){
...
HRESULT CXFS_PIN::GetPIN(const LPWFSPINGETPIN lpGetPin, LPWFSPINENTRY &lpEntry, DWORD dwTimeOut)
{
    ...
    CAutoSetBusyStatus autoSetBusyStatus(this);
...
bool CXFS_PIN::UpdateStatus(const DEVPINSTATUS &stStatus)
{
    ...
    case DEVICE_ONLINE:
        {
            fwDevice = bCmdExecuting() ? WFS_PIN_DEVBUSY : WFS_PIN_DEVONLINE;
...
void CXFS_PIN::setBCmdExecuting(bool bCmdExecuting)
{
    m_bCmdExecuting = bCmdExecuting;
    if(m_bCmdExecuting){
        //仅在设备ONLINE时设置BUSY状态
        if(m_stStatus.fwDevice == WFS_PIN_DEVONLINE){
            m_stStatus.fwDevice = WFS_PIN_DEVBUSY;
        }
    } else {
        if(m_stStatus.fwDevice == WFS_PIN_DEVBUSY){
            m_stStatus.fwDevice = WFS_PIN_DEVONLINE;
        }
    }
}

bool CXFS_PIN::bCmdExecuting() const
{
    return m_bCmdExecuting;
}
