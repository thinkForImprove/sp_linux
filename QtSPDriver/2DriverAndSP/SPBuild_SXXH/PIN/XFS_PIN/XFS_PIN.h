...
public:
    ...
    virtual HRESULT EncIo(const LPWFSPINENCIO lpEncIO, LPWFSPINENCIO &lpxEncIOData);         //40-00-00-00(FS#0002)
    virtual HRESULT DeriveKey(const LPWFSPINDERIVE lpDerive, LPWFSXDATA &lpCryptData);       //40-00-00-00(FS#0002)
    ...
    bool bCmdExecuting() const;
    void setBCmdExecuting(bool bCmdExecuting);
...
protected:
    ...
    //获取RSA公钥加密的密钥数据
    long AnalyKeyDataFromFormatedData(UINT uPKeyEppNo, UINT uSKeyEppNo, RSADATAALGOTYPE eRSADataAlgo,
                                      QByteArray formatedData, QByteArray &keyData);
private:
    ...
    CWFSPINEXPORTRSAEPPSIGNEDITEMOUTPUT m_cExportRsaEppSignedItem;      //30-00-00-00(FS#0006)
    CWFSPINSTARTKEYEXCHANGE             m_cStartKeyExchange;            //30-00-00-00(FS#0006)
 ...
 //30-00-00-00(FS#0003)  start
    ...
    PROTCHNIMPORTSM2PUBLICKEYOUT   m_stImportSM2PublicKeyOut;           //40-00-00-00(FS#0002)
 ...
 //30-00-00-00(FS#0003)  end
    ...
    bool                              m_bPinBlockDataPrependZero;
    bool                              m_bFireEnterEventBefMinLen;//30-00-00-00(FS#0008)
    bool                              m_bRSARandomMode;         //30-00-00-00(FS#0006)
    string                            m_strRSAKeyPairName;      //30-00-00-00(FS#0006)

    bool                              m_bCmdExecuting;
};

//自动设置命令执行状态
class CAutoSetBusyStatus
{
public:
    CAutoSetBusyStatus(CXFS_PIN *pObj):m_pObj(pObj){
        if(m_pObj){
            m_pObj->setBCmdExecuting(true);
        }
    }
    ~CAutoSetBusyStatus(){
        if(m_pObj){
            m_pObj->setBCmdExecuting(false);
        }
    }
private:
    CXFS_PIN *m_pObj;
