...
class CWFSPINEXPORTRSAEPPSIGNEDITEMOUTPUT
{
public:
    CWFSPINEXPORTRSAEPPSIGNEDITEMOUTPUT(){
        memset(&m_stExportRsaEppSignedItem, 0, sizeof(m_stExportRsaEppSignedItem));
        memset(&m_stValue, 0, sizeof(m_stValue));
        memset(&m_stSignature, 0, sizeof(m_stSignature));
        memset(&m_stSelfSignature, 0, sizeof(m_stSelfSignature));
    }
    ~CWFSPINEXPORTRSAEPPSIGNEDITEMOUTPUT(){
        //内存释放
        Clear();
    ...
    void Clear(){
        ...
        if(m_stSelfSignature.lpbData != nullptr){
            delete m_stSelfSignature.lpbData ;
        }
    ...
    void AddValueData(const LPBYTE lpbData, USHORT usLength)
    {
        m_stExportRsaEppSignedItem.lpxValue = &m_stValue;
        if(lpbData != nullptr && usLength != 0){
            m_stValue.lpbData = new BYTE[usLength];
            if(m_stValue.lpbData){
                m_stValue.usLength = usLength;
    ...
    void AddSignatureData(const LPBYTE lpbData, USHORT usLength)
    {
        m_stExportRsaEppSignedItem.lpxSignature = &m_stSignature;
        if(lpbData != nullptr && usLength != 0){
            m_stSignature.lpbData = new BYTE[usLength];
            if(m_stSignature.lpbData){
                m_stSignature.usLength = usLength;
    ...
    void AddSelfSignatureData(const LPBYTE lpbData, USHORT usLength){
        m_stExportRsaEppSignedItem.lpxSignature = &m_stSelfSignature;
        if(lpbData != nullptr && usLength != 0){
            m_stSignature.lpbData = new BYTE[usLength];
            if(m_stSignature.lpbData){
                m_stSignature.usLength = usLength;
                memcpy(m_stSelfSignature.lpbData, lpbData, usLength);
            }
        }
    }
    LPWFSPINEXPORTRSAEPPSIGNEDITEMOUTPUT GetData()
    {
        return &m_stExportRsaEppSignedItem;
    }
private:
    WFSPINEXPORTRSAEPPSIGNEDITEMOUTPUT m_stExportRsaEppSignedItem;
    WFSXDATA m_stValue;
    WFSXDATA m_stSignature;
    WFSXDATA m_stSelfSignature;
};
//////////////////////////////////////////////////////////////////////////
class CWFSPINSTARTKEYEXCHANGE
{
public:
    CWFSPINSTARTKEYEXCHANGE(){
        memset(&m_stData, 0, sizeof(m_stData));
        memset(&m_stStartKeyExchange, 0, sizeof(m_stStartKeyExchange));
    }
    ~CWFSPINSTARTKEYEXCHANGE(){
        Clear();
    }
public:
    ...
    void Add(const LPBYTE lpbData, USHORT usLength){
        m_stStartKeyExchange.lpxRandomItem = &m_stData;
        if(lpbData != nullptr && usLength != 0){
    ...
    LPWFSPINSTARTKEYEXCHANGE GetData(){
        return &m_stStartKeyExchange;
    }
private:
    WFSPINSTARTKEYEXCHANGE m_stStartKeyExchange;
    WFSXDATA  m_stData;
};
