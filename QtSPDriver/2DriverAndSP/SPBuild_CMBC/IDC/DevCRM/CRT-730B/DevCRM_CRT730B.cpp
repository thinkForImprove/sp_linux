...
int CDevCRM_CRT730B::GetStatus(STCRMSTATUS &stStatus)
{
    ...
    if (nDevStat[1] == IMPL_STAT_CRDSLOT_NOTHAVE)       // 暂存仓1无卡
    {
        m_stCardSlotInfo.Clear(1);
    }
    if (nDevStat[2] == IMPL_STAT_CRDSLOT_NOTHAVE)       // 暂存仓2无卡
    {
        m_stCardSlotInfo.Clear(2);
    }
    if (nDevStat[3] == IMPL_STAT_CRDSLOT_NOTHAVE)       // 暂存仓3无卡
    {
        m_stCardSlotInfo.Clear(3);
    }
    if (nDevStat[4] == IMPL_STAT_CRDSLOT_NOTHAVE)       // 暂存仓4无卡
    {
        m_stCardSlotInfo.Clear(4);
    }
    if (nDevStat[5] == IMPL_STAT_CRDSLOT_NOTHAVE)       // 暂存仓5无卡
    {
