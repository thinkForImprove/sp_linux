...
long CCashUnit::InnerSetXFSData(LPWFSCIMCASHIN pData)
{
    ...
    if(bAllowedSetValue) VERIFY_FUNC_CALL(SetItemType(pData->fwItemType));
    VERIFY_FUNC_CALL(SetUnitID(pData->cUnitID));
    VERIFY_FUNC_CALL(SetCurrencyID(pData->cCurrencyID));
    //应用非回收箱传入为0，此处临时对策，不设置面值
//    VERIFY_FUNC_CALL(SetValues(bAllowedSetValue ? pData->ulValues : 0));
    ...
    VERIFY_FUNC_CALL(SetMaximum(pData->ulMaximum));
    VERIFY_FUNC_CALL(SetAppLock(pData->bAppLock));
    if(pData->lpNoteNumberList == nullptr || pData->lpNoteNumberList->usNumOfNoteNumbers == 0
       || pData->lpNoteNumberList->lppNoteNumber == nullptr){
        //如果上层未传入lpNoteNumberList,则不进行设置,仅对未更改面值的情况适应，如果更改面值，需要做对应处理,
        //由程序生成对应的NoteNumberList
    } else {
