// XFSBufferLogger.cpp: implementation of the CXFSBufferLogger class.
//
//////////////////////////////////////////////////////////////////////
...
string GetTypeNameOfArrayName(const char *pTypeName, int &nArraySize)
{
    char szTypeName[MAX_TYPE_NAME_LEN];
...
string GetTypeNameOfPointerName(const char *pTypeName)
{
    ...
    assert(szTypeName[0] == 'L' && szTypeName[1] == 'P');
    return szTypeName + 2;
}
...
//              CXFSBufferLogger
//////////////////////////////////////////////////////////////////////
CXFSBufferLogger::CXFSBufferLogger(CStringBuffer &StringBuffer)
    : m_StringBuffer(StringBuffer), m_eMsgType(MT_EVENT), m_dwMsgID(0)
{
...
const char *CXFSBufferLogger::GetMsgTypeDesc(MSGTYPE eMsgType)
{
    switch (eMsgType)
    {
    case MT_EVENT: return "MT_EVENT";
    case MT_GI: return "MT_GI";
    case MT_GC: return "MT_GC";
    case MT_EX: return "MT_EX";
    case MT_EC: return "MT_EC";
    default:
        {
...
int CXFSBufferLogger::GetZeroEndPtrArraySize(
const char *pNonPtrTypeName, const char *pDataName, LPCTSTR pData)
{
    ...
    try
    {
        ...
        {
            //查看数据是否为全0
            for (int i = 0; i < it->second && pData[i] == 0; i++);
            if (i == it->second)
                break;
...
string CXFSBufferLogger::GetMemberFullName(const char *pMemberName) const
{
    assert(m_stackStructCalled.size() > 0); //必须位于结构中

    //形成成员变量完整名字
    string s = m_stackStructCalled.top()->pName;
    s += "___";
    s += pMemberName;
    return s;
}
...
//形成日志字串
//返回以\0结束的字串
void CXFSBufferLogger::Log(MSGTYPE eMsgType, DWORD dwID,
                           const char *pDataName, LPCTSTR pData, int nTabNum)
{
    m_eMsgType = eMsgType;
    m_dwMsgID = dwID;
    ...
    {
        m_StringBuffer.AddTab(nTabNum).AddF("ERROR: msg not found(id=%d, type=%s)",
                                            dwID, GetMsgTypeDesc(eMsgType)).EndLine();
        return;
    }
...
{
    assert(!TypeNameIsPtr(pTypeName));
    assert(GetAtomDataLen(pTypeName) == 0);

    //查找结构定义
    STR_NAME2DEF::const_iterator it = m_mapStrName2Def.find(pTypeName);
    assert(it != m_mapStrName2Def.end());  //
    assert(it->first == pTypeName);
...
int CXFSBufferLogger::GetDataTypeLen(const char *pTypeName) const
{
    ...
    {
        int nArraySize = 0;
        string sTypeName = GetTypeNameOfArrayName(pTypeName, nArraySize);
        assert(nArraySize > 0);
        return nArraySize * GetDataTypeLen(sTypeName.c_str());
    }
...
int CXFSBufferLogger::GetPointerMemberDataLen(const char *pMemberName) const
{
    ...
    string sMemberFullName = GetMemberFullName(pMemberName);
...
const char *CXFSBufferLogger::ConvertMemberValue2Desc(
const char *pMemberFullName, int nValue) const
{
...
void CXFSBufferLogger::LogAtomType(const char *pDataName, const char *pTypeName,
                                   LPCTSTR pData, int nTabNum)
{
    ...
    //形成成员变量完整名字
    string sMemberFullName = GetLenDefineName(pDataName);
    ...
    try
    {
        ...
        {
            nValue = *(WORD *)pData;
            LogWORDOrDWORDValue(sMemberFullName.c_str(), nValue, TRUE);
        }
        else if (strcmp(pTypeName, "DWORD") == 0)
        {
            nValue = *(DWORD *)pData;
            LogWORDOrDWORDValue(sMemberFullName.c_str(), nValue, FALSE);
        }
        ...
        else if (strcmp(pTypeName, "SYSTEMTIME") == 0)
        {
            ...
            m_StringBuffer.AddF("%04.4d-%02.2d-%02.2d %02.2d:%02.2d:%02.2d.%03.3d",
...
string CXFSBufferLogger::GetLenDefineName(const char *pDataName) const
{
    string sLenDefName;
    ...
    {
        sLenDefName = GetMemberFullName(pDataName);
    }
    return sLenDefName;
}
...
STR_END_TYPE CXFSBufferLogger::GetStringEndType(const char *pDataName) const
{
    ...
    string sLenDefName = GetLenDefineName(pDataName);
...
void CXFSBufferLogger::LogLPVOIDData(const char *pDataName, const char *pTypeName,
                                     LPCTSTR pData, int nTabNum, int nDataLen)
{
    LogPtrValue(pDataName, pData, nTabNum);
    int nIndex = 0;
...
BOOL CXFSBufferLogger::LogStringType(const char *pDataName, const char *pTypeName,
                                     LPCTSTR pData, int nTabNum)
{
...
void CXFSBufferLogger::LogNullEndPointer(const char *pDataName, const char *pTypeName,
                                         LPCTSTR pData, int nTabNum)
{
    string sTypeName = GetTypeNameOfPointerName(pTypeName);

    assert(!TypeNameIsPtrOfPtr(sTypeName.c_str())); //去掉一重指针后不是双重指针
    assert(TypeNameIsPtr(sTypeName.c_str()));       //去掉一重指针后一定是指针
    if (TypeNameIsPtrOfPtr(sTypeName.c_str()) ||
        !TypeNameIsPtr(sTypeName.c_str()))
    {
        assert(FALSE);
        m_StringBuffer.AddTab(nTabNum).
        AddF("ERROR: LogNullEndPointer: %s is PtrOfPtr or not ptr(pDataName=%s)",
             sTypeName.c_str(), pDataName).EndLine();
    ...
    try
    {
        while (*ppPtrArray != NULL)
        {
            ...
            LogPointerType(szDataName, sTypeName.c_str(), *ppPtrArray, nTabNum + 1);
            ppPtrArray++;
            nPtrIndex++;
        }
    }
...
void CXFSBufferLogger::LogZeroEndPointer(const char *pDataName, const char *pTypeName,
                                         LPCTSTR pData, int nTabNum)
{
    string sTypeName = GetTypeNameOfPointerName(pTypeName);

    ////以0结束的指针不可以是指向指针的指针，并且是原子数据类型
    assert(GetAtomDataLen(sTypeName.c_str()) > 0);
    assert(!TypeNameIsPtr(sTypeName.c_str()));
    if (GetAtomDataLen(sTypeName.c_str()) == 0 ||
        TypeNameIsPtr(sTypeName.c_str()))
    {
        assert(FALSE);
        m_StringBuffer.AddTab(nTabNum).
    ...
    int nArraySize = GetZeroEndPtrArraySize(sTypeName.c_str(), pDataName, pData);
    assert(nArraySize >= 0);

    LogArray(pDataName, sTypeName.c_str(), pData, nTabNum, nArraySize);
}

void CXFSBufferLogger::LogPointerType(const char *pDataName, const char *pTypeName,
                                      LPCTSTR pData, int nTabNum)
{
    assert(TypeNameIsPtr(pTypeName));
    ...
    //处理指针的数据长度由其他成员指定的情况
    string sTypeName = GetTypeNameOfPointerName(pTypeName); //去掉类型前的LP或后面的*
    ...
    if (nLen != -1) //由其他成员指定其长度
    {
        ...
        LogArray(pDataName, sTypeName.c_str(), pData, nTabNum, nLen);
        return;
    }

    //处理ZERO_END_PTR
    if (m_setZeroEndPtrMemName.find(GetLenDefineName(pDataName)) != m_setZeroEndPtrMemName.end())
    {
    ...
    {
        if (m_stackStructCalled.size() == 0 ||
            m_setNullEndMemName.find(GetLenDefineName(pDataName)) != m_setNullEndMemName.end())
        {
    ...
    if (GetAtomDataLen(sTypeName.c_str()) > 0)
    {
        LogArray(pDataName, sTypeName.c_str(), pData, nTabNum, 1);
        return;
    }

    //其他情况：处理指针指向结构的情况
    LogStruct(pDataName, sTypeName.c_str(), pData, nTabNum);
}

BOOL CXFSBufferLogger::LogArrayToSingleLine(
const char *pTypeName, LPCTSTR pData, int nArraySize)
{
    if (stricmp(pTypeName, "char") == 0)
    {
    ...
    if (stricmp(pTypeName, "BOOL") == 0)
    {
        ...
        for (int i = 0; i < nArraySize; i++)
        {
            if ((pBOOLData != NULL && pBOOLData[i]) ||
                (pboolData != NULL && pboolData[i]))
...
void CXFSBufferLogger::LogArray(const char *pDataName, const char *pTypeName,
                                LPCTSTR pData, int nTabNum, int nArraySize)
{
    int nLen = GetDataTypeLen(pTypeName);
    assert(nLen > 0);
    assert(!TypeNameIsArray(pTypeName));    //类型名不能又是一个数组
...
void CXFSBufferLogger::LogStruct(const char *pDataName, const char *pTypeName,
                                 LPCTSTR pData, int nTabNum)
{
    //判定必须为结构类型
    assert(GetStructLen(pTypeName) > 0);
    assert(pData != NULL);

    //处理结构类型
    STR_NAME2DEF::const_iterator itStr = m_mapStrName2Def.find(pTypeName);
    assert(itStr != m_mapStrName2Def.end());
    ...
    while (pMember->pName != NULL)
    {
        ...
        {
            int nArraySize;
            string sTypeName = GetTypeNameOfArrayName(pMember->pType, nArraySize);
            LogArray(pMember->pName, sTypeName.c_str(),
        ...
        else if (TypeNameIsPtr(pMember->pType))     //处理指针的情况
        {
            ...
            try
            {
                if (!IsBadReadPtr(pData + nPtrOffset, sizeof(LPVOID)))
                {
...
{
    string s = GetLenDefineName(pDataName);
    m_StringBuffer.EndLine().AddF("EXCEPTION: %s=0x%08.8X", s.c_str(), pData).EndLine();
}
