...
#define FNUSB_DLL_NAME      "FNUSB.dll"             //默认FL DLL名称
...
int _tag_brm_config_param::LoadParam(LPCSTR lpszFileName)
{
    ...
    sFlDLLName = m_SPConfigFile.GetString("BRMInfo", "FlDLLName", FNUSB_DLL_NAME);
    if (sFlDLLName.size() == 0)
    {
        Log(ThisModule, -1,
            "配置项FlDLLName未正确配置，长度为0");
        return WFS_ERR_INTERNAL_ERROR;
    }
    ...
    dwRollbackWaitItemtakenTime = m_SPConfigFile.GetInt("BRMInfo", "RollbackWaitItemtakenTime", 10);
    bEPDownAutoReset = (m_SPConfigFile.GetInt("BRMInfo", "EPDownAutoReset", 0) != 0);           //40-00-00-00(FT#0015)
