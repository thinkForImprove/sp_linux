...
HRESULT XFS_CRSImp::OnOpen(LPCSTR lpLogicalName)
{
    ...
    m_ReaderConfigFile.LoadINIFile(m_strConfigFile);                                    //test#11
    CINIReader cINI = m_ReaderConfigFile.GetReaderSection("BRMInfo");                   //test#11
    string m_strCashInShutterPort = (string)cINI.GetValue("CashInShutterPort",           //test#11
    ...
    QLibrary *fnusb_lib = NULL;
    fnusb_lib = new QLibrary(m_Param.GetFlDLLName().c_str());   //加载fnusb库
    fnusb_lib->load();
    if(!fnusb_lib->isLoaded())
    {
        string s = string("LoadLibrary error! (") + m_Param.GetFlDLLName() + ")";
        VERIFY_RESULT(s.c_str(), iRet);
    }
    //定义函数指针
    typedef unsigned int (*Fun)(unsigned int, PSTR_SETPRM);

    Fun pSETFNSETTING = (Fun)fnusb_lib->resolve("_F_SetFnSetting");
    unsigned int uiRet = 0;
    if(pSETFNSETTING){
        //12 sheets per sec, need license.
        STR_SETPRM StrSetPrm;
        memset(&StrSetPrm, NULL, sizeof(StrSetPrm));
        StrSetPrm.usParam = 1;
        StrSetPrm.pvDataInBuffPtr = NULL;
        StrSetPrm.uiDataInBuffSz = 0;
        uiRet = pSETFNSETTING(18,&StrSetPrm);
        StrSetPrm.usParam = 0;
    }
...
HRESULT XFS_CRSImp::OnStatus()
{
    ...
    // 空闲更新状态
    UpdateStatus();
    if ((m_pAdapter->IsConnected() == FALSE) &&                 //40-00-00-00(FT#0015)
            (m_Param.GetEPDownAutoReset())) {                   //40-00-00-00(FT#0015)
        if (ReOpen(TRUE) == WFS_SUCCESS) {                      //40-00-00-00(FT#0015)
            UpdateStatus();                                     //40-00-00-00(FT#0015)
        }                                                       //40-00-00-00(FT#0015)
    }                                                           //40-00-00-00(FT#0015)
    return WFS_SUCCESS;
}
...
HRESULT XFS_CRSImp::CIMGetStatus(LPWFSCIMSTATUS &lpStatus)
{
    ...
    ADPCDMCAPS CDMCaps;                                     //test#10
    m_pAdapter->GetCDMCapabilities(&CDMCaps);               //test#10
 ...
 //test#10   InOutpos[1].fwPosition = CIM_OUTPUT_POSITION(CIMCaps.fwPositions);
    InOutpos[1].fwPosition = CIM_OUTPUT_POSITION(CDMCaps.fwPositions);                  //test#10
...
HRESULT XFS_CRSImp::CIMCashInStart(const LPWFSCIMCASHINSTART lpData)
{
    ...
    if (SHUTTER_CONTROL_BY_SP(CIMCaps) || (m_usTellerID == 0) || (m_usTellerID == 1))   //test#13 START
    {
        long iRet = 0;
...
//test#13    iRet = m_pAdapter->ValidateAndCounting(arycResult, ulRejectCount, eRejectCause);
    ...
    //入钞门和入钞口状态同时变化时，获取一次状态只能获得一种状态变化，需要调用两次获取      //40-00-00-00(FT#0019)
    UpdatePostionStatus();
    if (iRet < 0)
    {
        Log(ThisModule, iRet,
...
HRESULT XFS_CRSImp::CIMCashInRollBack(LPWFSCIMCASHINFO &lpCUInfo)
{
    ...
    if (m_Param.IsCheckCSTSEmptyWhenRollBack())
    {
        ...
        //检查ESC无钞的情况
        if (ADPStatus.stStacker == ADP_STACKER_EMPTY)                                                       //test#24
        {                                                                                                   //test#24
            Log(ThisModule, -1, "ESC not notes: stStacker:%s",ADPStatus.stStacker == 0 ? "Empty" : "");     //test#24
            return WFS_ERR_CIM_NOITEMS;                                                                     //test#24
        }                                                                                                 //test#24
    }
    ...
    long iRet = 0;  //test#24
//    CAutoFireDeviceStatus::IgnoreItemTakenInsertedOnce(TRUE, FALSE); //忽略一次ITEM插入取出事件
    while(ADPStatus.stStacker != ADP_INPOS_EMPTY){                //test#24
        iRet = m_pAdapter->RollBack(arywCUError);
        FireCUErrorAndSetErrorNum(arywCUError, FALSE);
            ...
            if(ADPStatus.stOutPutPosition == ADP_OUTPOS_EMPTY){
                ...
                if (SHUTTER_CONTROL_BY_SP(CIMCaps))
                {
                        ...
                        && m_adapterinfo.bHaveOutShutter)
                    {
                        ...
                        {
                            Log(ThisModule, iRet,
                                "Rollback item taken CloseShutter()失败");
                        }
                    }
                }
                break;
            }else if(iWaitTime >= m_Param.GetRollbackWaitItemtakenTime()){
                //由SP控制门的情况处理
                if (SHUTTER_CONTROL_BY_SP(CIMCaps))
                {
                    if (ADPStatus.stOutShutter == ADP_OUTSHUTTER_OPEN
                        && m_adapterinfo.bHaveOutShutter)
                    {
                        ...
                        {
                            Log(ThisModule, iRet,
                                "Rollback item taken timeout CloseShutter()失败");
                        }
                    }
                }
                break;
...
WORD XFS_CRSImp::ComputeDispenseState(const ADPSTATUSINFOR &ADPStatus, BOOL *pbStopByHardwareError, BOOL bLog, BOOL bCheck)
{
    ...
    STOPDENOMINATE_BY_HARDWARE(TRUE);
    //处理STACKER不正常的情况
//test by guojy    CHECK_DISPENSE_ACCEPTOR_STATE_ERROR(ADPStatus.stStacker == ADP_STACKER_UNKNOWN, WFS_CDM_DISPCUSTOP);
...
WORD XFS_CRSImp::ComputeAcceptorState(const ADPSTATUSINFOR &ADPStatus, BOOL bLog)
{
    ...
    //处理STACKER不正常的情况
 //TEST BY GUOJY   CHECK_DISPENSE_ACCEPTOR_STATE_ERROR(ADPStatus.stStacker == ADP_STACKER_UNKNOWN, WFS_CIM_ACCCUSTOP);
