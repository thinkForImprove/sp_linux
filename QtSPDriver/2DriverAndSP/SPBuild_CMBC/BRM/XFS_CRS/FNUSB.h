...
typedef struct _STR_DRV {							// 霓撞兽DLL婲摦梡峔憿懱
        unsigned long		ulDrvHnd ;							// 霓撞兽瘦霓?
        unsigned short		usParam ;							// 蔬滓袄
        unsigned short		usExParam ;							// 奼挘蔬滓袄
        void *  		pvDataInBuffPtr ;					// 棉袄奿擺In兽卟堇
        unsigned long		ulDataInBuffSz ;					// 棉袄奿擺In兽w掕徊睫
        void * 		pvDataOutBuffPtr ;					// 棉袄奿擺Out兽卟堇
        unsigned long		ulDataOutReqSz ;					// 棉袄奿擺Out兽v媮徊睫
        unsigned long		ulDataOutBuffSz ;					// 棉袄奿擺Out兽w掕徊睫
        unsigned long		ulTimer ;							// 啦蠏l
        long   		hwWndHnd ;							// 敞菽蕹瘦霓?
        unsigned int		uiWndMsg ;							// 巢菽蕹爷景嫁
} STR_DRV, *PSTR_DRV ;								//
