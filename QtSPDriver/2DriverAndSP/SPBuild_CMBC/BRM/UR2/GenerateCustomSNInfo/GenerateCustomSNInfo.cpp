...
void CSaveCusSNInfo::GenerateFSNInfo(const LPSTSNINFODETAILS lpSNInfoDetails, LPSTFSNBody lpFSNBody)
{
    ...
    m_stProductionInfo.Money_Type =
            ((lpSNInfoDetails->NoteCateGory != NOTELEVEL_LEVEL4) ? MONEY_TYPE_DOUBTFUL_BILL : MONEY_TYPE_COMMON_INTACT_BILL);   //test#26
...

    {
        ...
        sprintf(szMachineSNo, "BOC%s/CFES/%s", &szYear[2], m_szMachineSNo);             //test#26
...
void CSaveCusSNInfo::InitConfig()
{
    ...
    // add start test#26
    memset(m_szMachineSNo, 0x00, sizeof(m_szMachineSNo));
    string szMachineSType = (LPCSTR)cINI.GetValue("MachineSType", "URT");
    string szMachineSNo = (LPCSTR)cINI.GetValue("MachineSNo", "00000001");
    size_t uiMachineSType = szMachineSType.size();
    size_t uiMachineSNo = szMachineSNo.size();

    strcpy(m_szMachineSNo, szMachineSType.c_str());
    strcat(m_szMachineSNo, szMachineSNo.c_str());
    memset(&m_stProductionInfo, 0x00, sizeof(m_stProductionInfo));
    m_stProductionInfo.Machine_Length = uiMachineSType;
    m_stProductionInfo.MachineSNo_Length = qMin(uiMachineSType + uiMachineSNo, (size_t)24 - 7 - 4);
    m_stProductionInfo.Machine_Type = MACHINE_TYPE_CASHIN_AND_DISPENSE;
    Log("InitConfig", 1, "MachineSNo :%s", m_szMachineSNo);
    // add end test#26
