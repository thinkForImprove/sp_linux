...
long CUR2Adapter::InitADPConfig()
{
    ...
    m_stOperationalInfo.bStorDiffSizeNotesInDeposit = (int)cINI.GetValue("StorDiffSizeNotesInDeposit", "1"); //test#24
    ...
    m_sDevConfigInfo.bRB5ToEscSupp = (int)cINI.GetValue("RB5ToEscSupp", "0") == 1; //test#24
...
//test#12    m_sCDMCapabilities.bShutterControl = TRUE;//固定值 见SR001006
    m_sCDMCapabilities.bShutterControl = FALSE;                 //test#12
...
//    ST_HW_CONFIG stHWConfig;
    ...
    if (!m_bBRMInitialized)
    {
        for (int i = 0; i < MAX_CASSETTE_NUM ; i++)
        {
            ...
            if (i == 3)
            {
                stCassType[i].iCassNoteHandInfo = DESTINATION_REJECT_CASH2_COMBINE;
            ...
            if((i == 4) && (m_sDevConfigInfo.bRB5ToEscSupp == TRUE)){           //test#24
                stCassType[i].iCassNoteHandInfo = DESTINATION_REJCET_DEFAULT;        //test#24
                stCassType[i].iCassType = CASSETTE_TYPE_RB;                     //test#24
                stCassType[i].iCassOper = RB_OPERATION_ESCW;                    //test#24
                stCassType[i].iDenoCode = DENOMINATION_CODE_100_C;                //test#24
                continue;                                                       //test#24
            }                                                                   //test#24
...
//test#13             || !stDevStatusInfo.bESCInPos
             || ((m_sDevConfigInfo.bRB5ToEscSupp == TRUE) && !stDevStatusInfo.bESCInPos)  //test#24
        ...
        || m_sCIMCapabilities.bShutter)
    {
        ...
        if (m_bCloseShutterJammed || m_bOpenShutterJammed)
        {
            ...
            if(stDevStatusInfo.iOutShutterStatus == UR2SHUTTER_STATUS_CLOSED ||
               stDevStatusInfo.iOutShutterStatus == UR2SHUTTER_STATUS_UNKNOWN ){                //test#13
                m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_CLOSED;                            //test#13
            }else{                                                                            //test#13
                m_sDevStatus.stOutShutter = ADP_OUTSHUTTER_OPEN;                              //test#13
            }                                                                                 //test#13
            if(stDevStatusInfo.CashInShutterStatus == UR2SHUTTER_STATUS_CLOSED ||
               stDevStatusInfo.CashInShutterStatus == UR2SHUTTER_STATUS_UNKNOWN){              //test#13
                m_sDevStatus.stInShutter = ADP_INSHUTTER_CLOSED;                              //test#13
            }else{                                                                            //test#13
                m_sDevStatus.stInShutter = ADP_INSHUTTER_OPEN;                                //test#13
            }                                                                                 //test#13
            if(stDevStatusInfo.InPutStatus == UR2INPUTPOS_STATUS_EMPTY||                      //40-00-00-00(FT#0019)
                    stDevStatusInfo.InPutStatus == UR2INPUTPOS_STATUS_UNKNOWN){               //40-00-00-00(FT#0019)
                m_sDevStatus.stInPutPosition = ADP_INPOS_EMPTY;                               //40-00-00-00(FT#0019)
            }                                                                                 //40-00-00-00(FT#0019)
            else {                                                                            //40-00-00-00(FT#0019)
                m_sDevStatus.stInPutPosition = ADP_INPOS_NOTEMPTY;                            //40-00-00-00(FT#0019)
            }                                                                                 //40-00-00-00(FT#0019)
        }                                                                                     //test#13
 ...
 /*   if (stDevStatusInfo.bCashAtCS
    ...
    if(stDevStatusInfo.bCashAtCSErrPos
       || stDevStatusInfo.bCashAtShutter||stDevStatusInfo.InPutStatus==UR2INPUTPOS_STATUS_NOTEMPTY){    //40-00-00-00(FT#0019)
//       m_sDevStatus.stOutPutPosition = ADP_OUTPOS_NOTEMPTY;
        m_sDevStatus.stInPutPosition = ADP_INPOS_NOTEMPTY;
    }else{
//           m_sDevStatus.stOutPutPosition = ADP_OUTPOS_EMPTY;
    ...
    if (stDevStatusInfo.bCashAtCS
        || stDevStatusInfo.bCashLeftInCS||stDevStatusInfo.InPutStatus==UR2INPUTPOS_STATUS_NOTEMPTY)     //40-00-00-00(FT#0019)
    {
          m_sDevStatus.stInPutPosition = ADP_INPOS_NOTEMPTY;
    }else{
    ...
    // 获取暂存区状态
    if(m_sDevConfigInfo.bRB5ToEscSupp == TRUE){                             //test#24 start
...
long CUR2Adapter::QueryDevInfo()
{
    ...
    //判断UR/BV EP DOWN
    if ((iRet == ERR_UR_DRV_DRVHND_DIFFER) || (iRet == ERR_UR_DRV_DRV_REMOVE) ||                //40-00-00-00(FT#0015)
        (i == ERR_UR_DRV_DRVHND_DIFFER) || (i == ERR_UR_DRV_DRV_REMOVE)) {                      //40-00-00-00(FT#0015)
        if (m_bBVConnectNormal) {                                                               //40-00-00-00(FT#0015)
            m_pUR2Dev->CloseBVConn();                                                           //40-00-00-00(FT#0015)
            m_bBVConnectNormal = FALSE;                                                         //40-00-00-00(FT#0015)
        }                                                                                       //40-00-00-00(FT#0015)
        if (m_bConnectNormal) {                                                                 //40-00-00-00(FT#0015)
            m_pUR2Dev->CloseURConn();                                                           //40-00-00-00(FT#0015)
            m_bConnectNormal = FALSE;                                                           //40-00-00-00(FT#0015)
        }                                                                                       //40-00-00-00(FT#0015)
    }                                                                                           //40-00-00-00(FT#0015)
...
long CUR2Adapter::ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject, ADP_CUERROR arywCUError[],WORD wCashInCnt)  //test#13
{
    ...
    int iRet = m_pUR2Dev->CashCount(m_sDevConfigInfo.CashInNoteType,  iValue, iAmount, iPowerIndex,
                                    bArryAcceptDeno, m_sDevConfigInfo.stBVDependentMode,
                                    m_sDevConfigInfo.bRB5ToEscSupp,                                 //test#24
                                    iTotalCashInURJB,
...
long CUR2Adapter::StoreCash(char ppNoteCounts[ADP_MAX_CU_SIZE][256], ADP_CUERROR arywCUError[ADP_MAX_CU_SIZE])
{
	...
	//test#13 start
    if((cFirmHardInfo == HARD_INFO_TCR) && (m_sDevConfigInfo.bRB5ToEscSupp == FALSE)){                         //test#24
