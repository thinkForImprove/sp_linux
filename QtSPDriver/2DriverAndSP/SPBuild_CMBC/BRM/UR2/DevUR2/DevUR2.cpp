...
const char *ThisModule)
{
    ...
    m_stDevStatusInfo.bCashAtOutCS         = (szResp[iDataPos] & 0x10) ? TRUE : FALSE;

    //TEST#24    m_stDevStatusInfo.bESCFull             = (szResp[iDataPos + 1] & 0x80) ? TRUE : FALSE;
    //TEST#24     m_stDevStatusInfo.bESCEmpty            = (szResp[iDataPos + 1] & 0x40) ? TRUE : FALSE;
        if((szResp[iDataPos + 12] & 0xFF)  == 0x50){//TEST#24
            m_stDevStatusInfo.bESCFull = TRUE;//TEST#24
        }else{
            m_stDevStatusInfo.bESCFull = FALSE;
        }//TEST#24
        if((szResp[iDataPos + 12] & 0xFF)  == 0x10){//TEST#24
            m_stDevStatusInfo.bESCEmpty = TRUE;//TEST#24
        }else{
            m_stDevStatusInfo.bESCEmpty = FALSE;//TEST#24
        }//TEST#24
        if((szResp[iDataPos + 12] & 0xFF)  == 0x40){//TEST#24
            m_stDevStatusInfo.bESCNearFull = TRUE;//TEST#24
        }else{
            m_stDevStatusInfo.bESCNearFull = FALSE;//TEST#24
        }//TEST#24

//TEST#24   m_stDevStatusInfo.bESCFull             = (szResp[iDataPos + 1] & 0x80) ? TRUE : FALSE;
//TEST#24   m_stDevStatusInfo.bESCEmpty            = (szResp[iDataPos + 1] & 0x40) ? TRUE : FALSE;
//TEST#24   m_stDevStatusInfo.bESCNearFull         = (szResp[iDataPos + 2] & 0x40) ? TRUE : FALSE;
...
//test#24    m_stDevStatusInfo.bESCInPos       = (szResp[iDataPos + 6] & 0x40) ? TRUE : FALSE;
    m_stDevStatusInfo.bESCInPos       = (szResp[iDataPos + 5] & 0x08) ? TRUE : FALSE;    //test#24
...
//    iNumStackedToESC = MAKEWORD(szResp[iDataPos + 3], szResp[iDataPos + 2]);
    iNumStackedToESC = MAKEWORD(szResp[iDataPos + 15], szResp[iDataPos + 14]);  //TEST#24
    for (int i = 0; i < MAX_CASSETTE_NUM - 1; i++)
    {
    ...
    for (int i = 0; i < MAX_CASSETTE_NUM; i++)
    {
        if (CASSETTE_TYPE_RB == stCassType[i].iCassType)//RB箱类型
        {
            if ((RB_OPERATION_RECYCLE  == stCassType[i].iCassOper)   ||
                ...
                (RB_OPERATION_ESCW  == stCassType[i].iCassOper)) //test#24
            {
                continue;
            }
...
//			}
    ...
    for (i = 0; i < MAX_CASSETTE_NUM - 1; i++)
    {
        ...
        case RB_OPERATION_DISPENSE:
            szCassType[2] = 3;
            break;
        case RB_OPERATION_ESCW:             //test#24
            szCassType[2] = 5;              //test#24
            break;                          //test#24
        default:
            szCassType[2] = 0;
            break;
 ...
 //test by guojy   szCmdParam[12] = 0x70;
...
//test#13     char szCmdData[56 + 1] = {0};
    char szCmdData[94 + 1] = {0};                                   //test#24
    if (iValidateMode == VALIDATION_MODE_REAL)
    {
        if(bRB5ToEscSupp == TRUE){                                  //test#24
           MOSAICCMND(szCmdData, iCmdLen, 0x1000);                  //test#24
        }else{                                                      //test#24
    ...
    {
        if(bRB5ToEscSupp == TRUE){                                  //test#24
            MOSAICCMND(szCmdData, iCmdLen, 0x1900);                 //test#24
        }else{                                                      //test#24
...
/*//test by guojy    switch (usRespCode)
    {
...
//      LOGDEVACTION();                                                                   //test#11
      long lRet = m_pDev->Open(m_strCashInShutterPort.c_str());  //test by guojy
      if (0 != lRet)                                                                    //test#11
      {                                                                                 //test#11
          Log(ThisModule, __LINE__, "打开串口失败:%d 串口[%s]", lRet, m_strCashInShutterPort.c_str()); //test#11
...
//   40-00-00-00(FT#0019)
            char buf[32] = {0};
            int iShutterStatusIndex = 1;
            int iInPosStatusIndex = 5;
            memcpy(buf, szReadData, dwReadLen);

            if(dwReadLen < 8){
                int i = 0;
                int length = dwReadLen;
                while(i < 6)
                {
                    ...
                    iRet = m_pDev->Read(szReadData, dwReadLen, (DWORD)lTimeOut);
                    if(iRet == 0) {
                        memcpy(buf+length, szReadData, dwReadLen);
                        length = length + dwReadLen;
                    }
                    if(length >= 8) {
                        break;
                    }
            ...
            if(buf[iInPosStatusIndex] == SHUTTER_SENSOR_ON) {
                m_stDevStatusInfo.InPutStatus = UR2INPUTPOS_STATUS_NOTEMPTY;
            } else if(buf[iInPosStatusIndex] == SHUTTER_SENSOR_OFF) {
                m_stDevStatusInfo.InPutStatus = UR2INPUTPOS_STATUS_EMPTY;
            }
            if(buf[iShutterStatusIndex] == SHUTTER_SENSOR_ON) {
                // S2 is Ope
                m_stDevStatusInfo.CashInShutterStatus = UR2SHUTTER_STATUS_OPEN;
             } else if(buf[iShutterStatusIndex] == SHUTTER_SENSOR_OFF) {
               // S1 is Close
                m_stDevStatusInfo.CashInShutterStatus = UR2SHUTTER_STATUS_CLOSED;
             }
        }else{
           m_stDevStatusInfo.CashInShutterStatus = UR2SHUTTER_STATUS_UNKNOWN;
           m_stDevStatusInfo.InPutStatus = UR2INPUTPOS_STATUS_UNKNOWN;
        }
//40-00-00-00(FT#0019)
    }
    return TRUE;
                        ...
                        bool bPDL)
{
    ...
    if (ERR_UR_SUCCESS != lRes) {               //40-00-00-00(FT#0015)
        return lRes;                            //40-00-00-00(FT#0015)
    }                                           //40-00-00-00(FT#0015)
...
{
    CINIFileReader ini;
    ini.LoadINIFile(string(SPETCPATH) + "/" + "BRMSPConfig.ini");
    CINIReader iniReader = ini.GetReaderSection("BRMInfo");
    m_strCashInShutterPort = (LPCSTR)iniReader.GetValue("CashInShutterPort", "COM:/dev/ttyUSB0:57600,N,8,1");
