...
CHX_DevImpl::CHX_DevImpl()
{
    ...
    m_pLIVESCAN_GetFigTemplete              = nullptr;  // 21. 合成指纹模板
...
BOOL CHX_DevImpl::LoadFprCapIntf()
{
    ...
    // 21. 合成指纹模板
    m_pLIVESCAN_GetFigTemplete = (FIGLIVESCANGETFIGTEMPLETE)m_FprCapLibrary.resolve("getFigTemplate");
    FUNC_POINTER_ERROR_RETURN_CAP(m_pLIVESCAN_GetFigTemplete, "getFigTemplate");
}
...
BOOL CHX_DevImpl::Cap_GetFigTemplete(unsigned char *pcBuf1, unsigned char *pcBuf2, unsigned char *pcBuf3, int nBufLen1, int nBufLen2, int BufLen3, unsigned char *pcBuf4, int *pnBufLen)
{
    int nRet = 0;
    ...
    nRet = m_pLIVESCAN_GetFigTemplete(pcBuf1, pcBuf2, pcBuf3, nBufLen1, nBufLen2, BufLen3, pcBuf4, pnBufLen);
    if (nRet != 0)
    {
        Log(ThisFile, 1, "LIVESCAN_setBufferEmpty fail");
        return FALSE;
