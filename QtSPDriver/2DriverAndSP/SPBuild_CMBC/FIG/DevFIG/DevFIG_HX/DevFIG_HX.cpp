...
CDevFIG_HX::CDevFIG_HX(LPCSTR lpDevType)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        memset(m_ppTemplateData[i], 0x00, MAX_FEATURE_SIZE);
    }
}
...
{
    int nRet = 0;
    int len = FIG_IMAGE_RAW_DATA_SIZE;
    BYTE pExrFeature[MAX_FEATURE_SIZE] = {0};
    ...
    if (m_pszRawData == nullptr || !m_bIsPressed ||
       ((nRet = FeatureExtract(m_pszRawData, m_pszBmpData, pExrFeature, &len, 0)) < 0))
    {
...
//    nRet = FeatureExtract(m_pszRawData, m_pszBmpData, pData, &len, 0);
//    if (nRet < 0)
//    {
//        m_wBuildModelFailTimes++;
//        if (m_wBuildModelFailTimes > 3)
...
//            for (int i = 0; i < 3; i++)
//            {
//                memset(m_ppTemplateData[i], 0x00, FIG_IMAGE_RAW_DATA_SIZE);
//            }
//            return ERR_FIG_OTHER;
//        }
//        Log(ThisFile, 0, "第%02d次采集指纹错误，模板合成失败, 重新采集此次指纹, ErrorCode: %d", m_wBuildModelPressedTimes + 1, nRet);
...
//    }
    ...
    if (m_wBuildModelPressedTimes < 3)
        memcpy(m_ppTemplateData[m_wBuildModelPressedTimes], pExrFeature, MAX_FEATURE_SIZE);
    ...
    if (m_wBuildModelPressedTimes > 3)
    {
        ...
        m_wBuildModelFailTimes = 0;
        m_wBuildModelPressedTimes = 0;
        len = MAX_FEATURE_SIZE;
        nRet = Cap_GetFigTemplete(m_ppTemplateData[0], m_ppTemplateData[1], m_ppTemplateData[2], MAX_FEATURE_SIZE, MAX_FEATURE_SIZE, MAX_FEATURE_SIZE, pFeature, &len);
        if (nRet < 0 || len <= 0)
        {
...
int CDevFIG_HX::CompareFPData(LPBYTE pFeatureData1, LPBYTE pFeatureData2)
{
    ...
    nRet = Fpr_FeatureMatch(pFeatureData1, pFeatureData2, &fSimilarity);
...
int CDevFIG_HX::SetFPPrintData(STPRINTDATA stPrintData)
{
    ...
    nRet = Cap_SetBright(nChannel, stPrintData.nBright);
    if (nRet < 0)
    {
...
int CDevFIG_HX::setBufferEmpty(LPBYTE pImageData, long imageLength)
{
    ...
    nRet = Cap_setBufferEmpty(pImageData, imageLength);
    if (nRet < 0)
    {
        return ERR_FIG_OTHER;
    }
...
int CDevFIG_HX::GetQualityScore(LPBYTE pFingerImgBuf, LPBYTE pszScore)
{
    ...
    nRet = Fpr_GetQualityScore(pFingerImgBuf, pszScore);
    if (nRet < 0)
    {
        return ERR_FIG_OTHER;
    }
