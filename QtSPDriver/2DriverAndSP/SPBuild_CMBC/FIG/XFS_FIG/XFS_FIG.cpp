...
HRESULT CXFS_FIG::ReadImage(const LPWFSPTRIMAGEREQUEST lpImgRequest, LPWFSPTRIMAGE *&lppImage, DWORD dwTimeOut)
{
    ...
    BYTE pszModelFeatureData[MAX_FEATURE_SIZE];                     // 指纹模板数据
    ...
    memset(pszModelFeatureData, 0x00, MAX_FEATURE_SIZE);
    ...
    if ((wImageSource & WFS_PTR_CODELINE) == WFS_PTR_CODELINE)
    {
        ...
        while(nIndex < 3)
        {
            ...
            if (m_bIsPressed == FINGER_PRESENT)
            {
                ...
                nRet = m_pFinger->GetFingerTemplateData(pszBmpData, pszModelFeatureData, nLen);
                UpdateDevStatus(nRet);
...
//        解码
//        int out = 0;
//        BASE64Data = BASE64Decode((LPCSTR)pszModelFeatureData, MAX_FEATURE_SIZE, out);
//        if (out <= MAX_FEATURE_SIZE)
//            memcpy((LPSTR)pszModelFeatureData, BASE64Data.c_str(), BASE64Data.length());

        // 保存模板数据
        UpdateStatus();
        if (lpReadImage->lpszFrontImageFile)
        {
            if (pszModelFeatureData[0] != 0x00 && pszModelFeatureData[1] != 0x00)
            {
                nRet = SaveDataToImage(pszModelFeatureData, lpReadImage->lpszFrontImageFile, MAX_FEATURE_SIZE);
                if (nRet < 0)
                    goto END;
            }
        }
        ...
        memcpy(m_pszFeaData, pszModelFeatureData, MAX_FEATURE_SIZE);
        ...
        {
            BASE64Data.clear();
            BASE64Data = BASE64Encode(pszModelFeatureData, MAX_FEATURE_SIZE);
            ...
            m_pData.Add(wImageSource, WFS_PTR_DATAOK, MAX_FEATURE_SIZE, pszModelFeatureData);
            Log(ThisModule, __LINE__, "ReadImage返回数据|ret=WFS_SUCCESS|ImageSource:<WFS_PTR_CODELINE>|Status:<WFS_PTR_DATAOK>|Size:<512>|Data:<%s>",
                GetData(pszModelFeatureData, MAX_FEATURE_SIZE));
        }
...
//            goto END;
    ...
    else if((wImageSource & WFS_PTR_IMAGEBACK) == WFS_PTR_IMAGEBACK)
    {
        ...
        LPBYTE pData = m_stFeatureData.top();
        if (pData != nullptr)
            m_stFeatureData.pop();
        else
            goto END;
        ...
        {
            memset(pszCapFeatureData, 0x00, MAX_FEATURE_SIZE);
            memcpy(pszCapFeatureData, pData, MAX_FEATURE_SIZE);
            nRet = m_pFinger->CompareFPData(pszFPFeatureData, pszCapFeatureData);
        }
        else if (last == COLLECT_CODELINE)
        {
            memset(pszModelFeatureData, 0x00, MAX_FEATURE_SIZE);
            memcpy(pszModelFeatureData, pData, MAX_FEATURE_SIZE);
            nRet = m_pFinger->CompareFPData(pszFPFeatureData, pszModelFeatureData);
        }
        else
        {
            goto APIEND;
...
APIEND:
    ...
    lppImage = (LPWFSPTRIMAGE*)m_pData;
    Log(ThisModule, __LINE__, "ReadImage返回数据|ret=WFS_SUCCESS|ImageSource:<%d>|Status:<WFS_PTR_DATASRCMISSING>|Size:<0>|Data:<%s>",
...
int CXFS_FIG::InitConifig()
{
    ...
    m_stConfig.wReturnOpenVal = (WORD)m_cXfsReg.GetValue("RETVALUE", "ReturnOpenVal", (INT)0);
