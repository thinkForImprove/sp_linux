...
CASHUNIT_STATUS CCashUnit::ComputeStatusFromMinMax() const
{
    ...
    ICashUnitConfigItem *pCfg = m_pCfgItem->GetConfig()->GetCommonConfig();     //30-00-00-00(FT#0036)
    if (m_usType == ADP_CASSETTE_BILL ||
        m_usType == ADP_CASSETTE_RECYCLING ||
        m_usType == ADP_CASSETTE_RETRACT)
    {
        if (m_ulCount == 0 && (pCfg->GetCUValue(0, "EmptyStatusBaseLogicCount") == 1))  //30-00-00-00(FT#0036)
        {
...
void CCashUnit::InitData()
{
    ...
    memset(&m_CDMCashUnit, 0, sizeof(m_CDMCashUnit));
    memset(&m_CIMCashUnit, 0, sizeof(m_CIMCashUnit));
...
long CCashUnit::InnerSetXFSData(LPWFSCDMCASHUNIT pData)
{
    ...
    VERIFY_FUNC_CALL(SetMaximum(pData->ulMaximum));
...
CASHUNIT_STATUS CCashUnit::ComputeStatusFromCfg(CASHUNIT_STATUS eStatus) const
{
    ...
    {
        ULONG ulCDMBoxLeftCount = (ULONG)pCfg->GetCUValue(0, "CDMBoxLeftCount");
        if(ulCDMBoxLeftCount > 0){
            if ((eStatus != ADP_CASHUNIT_MISSING)
                && (eStatus != ADP_CASHUNIT_INOP)
                && (eStatus != ADP_CASHUNIT_MANIP)
                && m_ulCount <= ulCDMBoxLeftCount)
            {
    ...
    {
        ULONG ulRecyclingBoxLeftCount = pCfg->GetCUValue(0, "RecyclingBoxLeftCount");
        if (ulRecyclingBoxLeftCount > 0 && m_ulCount <= ulRecyclingBoxLeftCount)
        {
            return ADP_CASHUNIT_EMPTY;
        }
...
long CCashUnit::SetValues(ULONG ulValues)
{
    ...
    VERIFY_FUNC_CALL(VerifyData(ulValues, (ULONG)0, (ULONG)1, (ULONG)2, (ULONG)5, (ULONG)10, (ULONG)20, (ULONG)50, (ULONG)100, (ULONG)200, (ULONG)500, (ULONG)1000, (ULONG)10000, (ULONG)-1));
...
long CCashUnit::SetStatus(CASHUNIT_STATUS usStatus, BOOL bForce)
{
    ...
    if (usStatus != m_usStatus) //如果新状态不同于老状态
    {
        ...
        {
            //Missing可改变MANIP状态
            if(usStatus != ADP_CASHUNIT_MISSING){
                return 0;
            }
        }
...
//30-00-00-00(FT#0003)                }
        ...
        if(m_usStatus == ADP_CASHUNIT_INOP && usStatus != ADP_CASHUNIT_MISSING){          //30-00-00-00(FS#0020)
            Log("SetStatus", __LINE__, "当前状态为INOP，新状态非MISSING保持不变");             //30-00-00-00(FS#0020)
            return 0;                                                                     //30-00-00-00(FS#0020)
        }                                                                                 //30-00-00-00(FS#0020)
