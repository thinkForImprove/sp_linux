...
void CThreadGenerateSN::SetResult(int iResult)
{
    m_iResult = iResult;
}
...
//30-00-00-00(FT#0016)            ReplaceChar(pSNInfo->cSerialNumber, '*', '$');
        ...
        if(strlen(pSNInfo->cSerialNumber) == 0){                                //30-00-00-00(FT#0016)
            for(int i = 0; i < 10; i++){                                        //30-00-00-00(FT#0016)
                pSNInfo->cSerialNumber[i] = m_cUnrecognizeDisplayChar;        //30-00-00-00(FT#0016)
            }                                                                   //30-00-00-00(FT#0016)
        } else {                                                                //30-00-00-00(FT#0016)
            ReplaceChar(pSNInfo->cSerialNumber, '*', m_cUnrecognizeDisplayChar);  //30-00-00-00(FT#0016)
        }                                                                       //30-00-00-00(FT#0016)
                                            ...
                                            const ST_MEDIA_INFORMATION_INFO *pTraceInfo, char lpImageName[MAX_PATH], bool bFront)
{
    ...
    //时间
    sprintf(&lpImageName[0], "%04d%02d%02d%02d%02d%02d", (int)stCurr.wYear, (int)stCurr.wMonth, (int)stCurr.wDay,
            (int)stCurr.wHour, (int)stCurr.wMinute, (int)stCurr.wSecond);
    //索引
    sprintf(&lpImageName[14], "%05d", uIndex);
    //正反面
    sprintf(&lpImageName[19], "_%s.bmp", bFront ? "A" : "B");
                                               ...
                                               const ST_MEDIA_INFORMATION_INFO *pTraceInfo, char lpImageName[MAX_PATH])
{
    ...
    case SAVESERIALOPERAT_B:  lpImageName[14] = 'B'; break;
    case SAVESERIALOPERAT_J:  lpImageName[14] = 'J'; break;
    case SAVESERIALOPERAT_SELFCOUNT: lpImageName[14] = 'E'; break;
    ...
    //索引
    sprintf(&lpImageName[19], "%05d", uIndex);

    //币种
    memcpy(&lpImageName[24], "CNY", 3);

    //面额
    switch (pTraceInfo->iMediaInfoDnoCode)
    {
    case DENOMINATION_CODE_100_B:     memcpy(&lpImageName[27], "1001999", 7); break;
    case DENOMINATION_CODE_100_C:     memcpy(&lpImageName[27], "1002005", 7); break;
    case DENOMINATION_CODE_100_D:     memcpy(&lpImageName[27], "1002015", 7); break;
    case DENOMINATION_CODE_20_B:      memcpy(&lpImageName[27], "0201999", 7); break;
    case DENOMINATION_CODE_20_C:      memcpy(&lpImageName[27], "0202005", 7); break;
    case DENOMINATION_CODE_20_D:      memcpy(&lpImageName[27], "0202015", 7); break;
    case DENOMINATION_CODE_10_B:      memcpy(&lpImageName[27], "0101999", 7); break;
    case DENOMINATION_CODE_10_C:      memcpy(&lpImageName[27], "0102005", 7); break;
    case DENOMINATION_CODE_10_D:      memcpy(&lpImageName[27], "0102015", 7); break;
    case DENOMINATION_CODE_50_B:      memcpy(&lpImageName[27], "0502001", 7); break;
    case DENOMINATION_CODE_50_C:      memcpy(&lpImageName[27], "0502005", 7); break;
    case DENOMINATION_CODE_50_D:      memcpy(&lpImageName[27], "0502015", 7); break;
    case DENOMINATION_CODE_5_B:       memcpy(&lpImageName[27], "0051999", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_5_C:       memcpy(&lpImageName[27], "0052005", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_5_D:       memcpy(&lpImageName[27], "0052020", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_1_B:       memcpy(&lpImageName[27], "0011999", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_1_D:       memcpy(&lpImageName[27], "0012019", 7); break;        //30-00-00-00(FS#0018)
    case DENOMINATION_CODE_ALL:
    case DENOMINATION_CODE_00:
    default:                                memcpy(&lpImageName[27], "0000000", 7); break;
    }
    ...
    case NOTE_CATEGORY_4:
        {
            ...
            {
                memcpy(&lpImageName[34], "4F", 2);
            }
            else
            {
                memcpy(&lpImageName[34], "1V", 2);
            }
            break;
        }
    case NOTE_CATEGORY_4B:                  memcpy(&lpImageName[34], "4U", 2); break;
    case NOTE_CATEGORY_1:
    case NOTE_CATEGORY_2:
    case NOTE_CATEGORY_3:                   memcpy(&lpImageName[34], "1V", 2); break;
    case NOTE_CATEGORY_UNKNOWN:
    default:                                memcpy(&lpImageName[34], "0U", 2); break;
    }
...
//30-00-00-00(FT#0016)        sprintf(&lpImageName[34], "_%10s.bmp", pSNInfo->cSerialNumber);
        sprintf(&lpImageName[36], "_%10s.bmp", cSerialNumber);  //30-00-00-00(FT#0016)
    }
    else
    {
        sprintf(&lpImageName[36], "_%10s.bmp", "$$$$$$$$$$");
    }
