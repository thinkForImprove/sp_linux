...
using namespace std;
#include <string.h>
#include "IDevSIU.h"                        //30-00-00-00(FS#0004)

#define ZeroMemory(a, l)    memset(a, 0, l)

//ERRCODE
#define NORMAL_ERRCODE          "00000000"
#define UP_UNIT_NOT_IN_POS      "05111101"
#define LOW_UNIT_NOT_IN_POS     "05111201"
#define NEED_RESET              "05F00001"
...
typedef struct _DevConfigInfo
{
    ...
    ST_BV_DEPENDENT_MODE     stCashInBVDependentMode;           //30-00-00-00(FS#0030)
    ST_BV_DEPENDENT_MODE     stCashInEndBVDependentMode;        //30-00-00-00(FS#0030)
    ST_BV_DEPENDENT_MODE     stDispenseBVDependentMode;         //30-00-00-00(FS#0030)
    ST_BV_DEPENDENT_MODE     stRetractBVDependentMode;          //30-00-00-00(FS#0030)
    ST_BV_DEPENDENT_MODE     stRejectBVDependentMode;           //30-00-00-00(FS#0030)
    ST_BV_DEPENDENT_MODE     stSelfCountBVDependentMode;        //30-00-00-00(FS#0030)
    ...
    //是否启用钞箱补正处理
    BOOL  bRetractCashCountCalibration;                     //30-00-00-00(FT#0037)
...
public:
    ...
    //功能：验钞
    //输入：ulMaxNumber:入金限制最大张数
    //输入: ulMaxAmount:入金限制最大金额
    ...
    //返回：XFS通用错误码及CIM错误码
    //说明：
    virtual long ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject, ULONG ulMaxNumber, ULONG ulMaxAmount);   //30-00-00-00(FS#0002)
...
private:
    ...
    //返回：XFS通用错误码
    //说明：内部函数
    void SaveLastErrCode(int iRet, LPCSTR lpModule, BOOL bForceUpdate = FALSE);
...
private:
    ...
    int m_iNumStackedToESC;                                                                 //30-00-00-00(FT#0037)
    ...
    //获取出钞钞箱单元错误信息
    bool GetCUErrorInfoForDispense(ST_DISP_MISSFEED_ROOM &stDispMissfeedRoom, ADP_CUERROR arywCUError[ADP_MAX_CU_SIZE]);    //30-00-00-00(FS#0020)
    //全画像取得和冠字号关联的BV设定信息取得
    void GetFullImgSNBVSettingInfo();                                                               //30-00-00-00(FS#0030)
    void SetActionFullImgSNBVSettingInfo(string strSetting, ST_BV_DEPENDENT_MODE &stBVSettings);    //30-00-00-00(FS#0030)
    //获取钞箱存入取出优先级设定信息
...
private:
    ...
    //AP 入金限额相配置文件相关信息
    char       m_szApAmountLimitINIItem[3][MAX_PATH];   //ini文件名,小节名,key名       //30-00-00-00(FS#0009)
    ...
    CQtDLLLoader<IDevSIU>             m_pDevSIU;            //30-00-00-00(FS#0004)
    BOOL                              m_bSIUOpen;           //30-00-00-00(FS#0004)
