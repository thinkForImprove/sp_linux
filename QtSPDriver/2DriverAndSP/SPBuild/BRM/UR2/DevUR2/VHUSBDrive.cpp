...
#define STR_FUNC_DLLNAME "/hots/lib/libFNUSB.so"                    //30-00-00-00(FS#0016)
#else
...
CVHUSBDrive::CVHUSBDrive(const char *lpDevName, bool bUseFuncLib/* = false*/) : m_Log(lpDevName),               //30-00-00-00(FS#0016)
                                                                                m_bUseFuncLib(bUseFuncLib)      //30-00-00-00(FS#0016)
{
    m_pFnATMUSB = nullptr;
    ...
    m_pFnFGetInitResult = nullptr;                  //30-00-00-00(FS#0016)
    m_pFnFFnATMUSB = nullptr;                       //30-00-00-00(FS#0016)
    m_pFnFInfATMUSB = nullptr;                      //30-00-00-00(FS#0016)
    m_pFnFSetFnSetting = nullptr;                   //30-00-00-00(FS#0016)
}
...
bool CVHUSBDrive::VHUSBDllLoad()
{
    ...
    if(!m_bUseFuncLib){                                             //30-00-00-00(FS#0016)
    ...
    } else {                                            //30-00-00-00(FS#0016)
        //30-00-00-00(FS#0016) add start
        if (!m_hdll.isLibrary(STR_FUNC_DLLNAME))
        {
            QString qstrerr = m_hdll.errorString();
            Log(ThisModule, __LINE__, "FNUSB.dll is not Lib：GetLastError=%s", qstrerr.toStdString().c_str());
            return false;
        }

        m_hdll.setFileName(STR_FUNC_DLLNAME);
        if (!m_hdll.isLoaded())
        {
            if (!m_hdll.load())
            {
                QString qstrerr = m_hdll.errorString();
                Log(ThisModule, __LINE__, "FNUSB.dll加载失败：GetLastError=%s", qstrerr.toStdString().c_str());
                return false;
            }
        }

        if (nullptr == m_pFnFFnATMUSB || nullptr == m_pFnFInfATMUSB||
                nullptr == m_pFnFGetInitResult || nullptr == m_pFnFSetFnSetting)
        {
            m_pFnFGetInitResult = (FN_F_GetInitResult)m_hdll.resolve("_F_GetInitResult");
            m_pFnFFnATMUSB = (FN_F_FnATMUSB)m_hdll.resolve("_F_FnATMUSB");
            m_pFnFInfATMUSB = (FN_F_InfATMUSB)m_hdll.resolve("_F_InfATMUSB");
            m_pFnFSetFnSetting = (FN_F_SetFnSetting)m_hdll.resolve("_F_SetFnSetting");
            ...
            //设定快速验钞模式
            bool bFastValidateMode = false;
            CINIFileReader ini;
            if(ini.LoadINIFile(string(SPETCPATH) + "/" + "BRMSPConfig.ini")){
                CINIReader iniReader = ini.GetReaderSection("BRMInfo");
                bFastValidateMode = (int)iniReader.GetValue("FastValidateMode", 0) == 1;
            }
            SetFuncSetting(18, bFastValidateMode ? 0x01 : 0x00);
        }
        //30-00-00-00(FS#0016) add end
    }                                                       //30-00-00-00(FS#0016)
    return true;
}
...
//30-00-00-00(FS#0016)    if (m_pFnATMUSB == nullptr || m_pInfATMUSB == nullptr)
    if(((m_pFnATMUSB == nullptr || m_pInfATMUSB == nullptr) && !m_bUseFuncLib) ||       //30-00-00-00(FS#0016)
       ((m_pFnFFnATMUSB == nullptr || m_pFnFInfATMUSB == nullptr) && m_bUseFuncLib))    //30-00-00-00(FS#0016)
    {
    ...
    case USB_DRV_FN_DATASENDRCV:
        ...
        if(!m_bUseFuncLib){             //30-00-00-00(FS#0016)
            ...
            pFunc =m_pFnFFnATMUSB;      //30-00-00-00(FS#0016)
        }                               //30-00-00-00(FS#0016)
        break;
    ...
    case USB_DRV_INF_INFGET:
        ...
        if(!m_bUseFuncLib){             //30-00-00-00(FS#0016)
            ...
            pFunc = m_pFnFInfATMUSB;    //30-00-00-00(FS#0016)
        }                               //30-00-00-00(FS#0016)
        break;
    default:
...
//30-00-00-00(FS#0016)
DWORD CVHUSBDrive::SetFuncSetting(UINT uiFncNo, USHORT usParam, const char *pszSendData/* = nullptr*/, UINT uiSendLen/* = 0*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    if(!m_bUseFuncLib){
        Log(ThisModule, __LINE__, "未使用FNUSB库");
        return ERR_UR_SUCCESS;
    }

    if (nullptr == m_pFnFSetFnSetting)
    {
        Log(ThisModule, __LINE__, "参数指针为NULL，或没有加载USB驱动库");
        return ERR_UR_USB_CONN_ERR;
    }
    ...
    STR_SETPRM stSetPrm;
    memset(&stSetPrm, 0, sizeof(stSetPrm));
    stSetPrm.usParam = usParam;
    stSetPrm.pvDataInBuffPtr = (void *)pszSendData;
    stSetPrm.uiDataInBuffSz = uiSendLen;
    DWORD dwRet = m_pFnFSetFnSetting(uiFncNo, &stSetPrm);
    if (dwRet != ERR_DRV_USB_SUCCESS)
    {
        Log(ThisModule, __LINE__, "调用命令返回失败：[wCmd=%d,sParam=%d]lRet=0x%02X[ %s ]", \
            uiFncNo, usParam, dwRet, GetErrDesc(dwRet));
        return dwRet;
    }
