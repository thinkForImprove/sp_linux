...
#define HT_RESP_TIMEOUT_SELFCOUNT 60 * 60
...
//30-00-00-00(FS#0022) #define HT_RESP_LEN_ZERO      0xA000
#define HT_RESP_LEN_ZERO      0xFFF6                 //30-00-00-00(FS#0022)
...
#define PACKET_ID_SELFCOUNTPERROOM      0x05D9  //30-00-00-00(FS#0022)
...
#define PACKET_ID_TEST_DISP_ROOM_INFO   0x05E8
...
CUR2Drv::CUR2Drv()
{
    ...
    ReadConfig();                                           //30-00-00-00(FS#0016)

    m_pUSBDrv = new CVHUSBDrive("UR2", m_bUseFuncLib);      //30-00-00-00(FS#0016)
    ...
    for (USHORT i = iPacketIDStart; i <= iPacketIDEnd ; i++)
    {
            ...
            iRepNumDeno == 128)
        {
            ...
            for (int j = 0; j < iRepNumDeno; j++)
            {
                ...
                if (memcmp(szRespDeno, szRespDenoEmpty, 4) == 0 || memcmp(szRespDeno, szRespDenoInvalid, 4) == 0)       //30-00-00-00(FS#0022)
                {
                    break;
                }
...
void CUR2Drv::OPInfoToArray(const ST_OPERATIONAL_INFO stOperationalInfo, char bArrayOPInfo[4])
{
    ...
    //使能全画像获取
    bArrayOPInfo[3] |= 0x40;                //30-00-00-00(FS#0022)
}
...
)
{
    ...
    szCmdParam[2] = 0x00;
    szCmdParam[3] = 0x3E;
    memset(szCmdParam + 10, 0xFF, 20);
    szCmdParam[50] = static_cast<char>((iTotalCashInURJB >> 8) & 0xFF);
    szCmdParam[51] = static_cast<char>(iTotalCashInURJB & 0xFF);
...
//30-00-00-00(FS#0022) add start
    ...
    if(stHWConfig.bSelfCountRB5CfgAutoSwitch) {
        szCmdParam[6] |= 0x80;
    }
    if(stHWConfig.bSelfCountRB5SameDenoLimit) {
        szCmdParam[6] |= 0x10;
    }
    if(!stHWConfig.bSelfCountDiffDenoAbort) {
        szCmdParam[6] |= 0x04;
    }
    memcpy(szCmdData + iCmdLen, szCmdParam, 22);
    iCmdLen += 22;

    //30-00-00-00(FS#0022) add end
...
//30-00-00-00(FS#0022)    szCmdParam[12] = 0x70;
        ...
        && usArryCassCashOutPrioritySet != nullptr)
    {
        ...
        for (i = 0; i < MAX_ROOM_NUM; i++)              //30-00-00-00(FS#0022)
        {
...
ST_BV_VERIFICATION_LEVEL &stDispenseLevel)
{
    ...
    */
    ...
    for (i = 0; i < MAX_CASSETTE_NUM - 1; i++)
    {
        ...
        else if (szRoomDenoCode[0] & 0x01)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_1_B;
            stCassType[i].iCashValue = 1;
            stCassType[i].ucVersion = 0x42;
        }
        else if (szRoomDenoCode[1] & 0x80)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_5_C;
            stCassType[i].iCashValue = 5;
            stCassType[i].ucVersion = 0x43;
        }
        else if (szRoomDenoCode[1] & 0x40)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_10_B;
            stCassType[i].iCashValue = 10;
            stCassType[i].ucVersion = 0x42;
        }
        else if (szRoomDenoCode[1] & 0x20)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_20_B;
            stCassType[i].iCashValue = 20;
            stCassType[i].ucVersion = 0x42;
        }
        else if (szRoomDenoCode[1] & 0x10)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_50_B;
            stCassType[i].iCashValue = 50;
            stCassType[i].ucVersion = 0x42;
        }
        else if(szRoomDenoCode[1] & 0x08) {
            stCassType[i].iDenoCode = DENOMINATION_CODE_100_D;
            stCassType[i].iCashValue = 100;
            stCassType[i].ucVersion = 0x44;
        }
        else if (szRoomDenoCode[1] & 0x04) {
            stCassType[i].iDenoCode = DENOMINATION_CODE_100_B;
            stCassType[i].iCashValue = 100;
            stCassType[i].ucVersion = 0x42;
        }
        else if (szRoomDenoCode[1] & 0x02)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_5_B;
            stCassType[i].iCashValue = 5;
            stCassType[i].ucVersion = 0x42;
        }
        else if (szRoomDenoCode[1] & 0x01)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_10_D;
            stCassType[i].iCashValue = 10;
            stCassType[i].ucVersion = 0x44;
        }
        else if (szRoomDenoCode[2] & 0x80)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_20_D;
            stCassType[i].iCashValue = 20;
            stCassType[i].ucVersion = 0x44;
        }
        else if (szRoomDenoCode[2] & 0x40)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_50_D;
            stCassType[i].iCashValue = 50;
            stCassType[i].ucVersion = 0x44;
        }
        else if (szRoomDenoCode[2] & 0x20)
        {
            stCassType[i].iDenoCode = DENOMINATION_CODE_1_D;
            stCassType[i].iCashValue = 1;
            stCassType[i].ucVersion = 0x44;
        }
...
)
{
    ...
    {
        MOSAICCMND(szCmdData, iCmdLen, 0x1000);
    }
    else
    {
        MOSAICCMND(szCmdData, iCmdLen, 0x1900);
    }
    iCmdLen += 6;

    char szCmdParam[50 + 1] = {0};
    //2.Deposit Limit(6 bytes) 可变包
    if (iNumDepositLimit > 0)
    {
        if (iNumDepositLimit > 300)
        {
            iNumDepositLimit = 300;
        }
        szCmdParam[0] = 0x05;
        szCmdParam[1] = 0x30;
        szCmdParam[2] = 0x00;
    ...
    char szResp[HT_RESP_LEN_MAX] = {0};
    USHORT dwRespLen = HT_RESP_LEN_MAX; // 输入：缓冲区长度，输出：实际接收响应字节数
    int iRet = ExecuteCmd(szCmdData, iCmdLen, szResp, dwRespLen, 4408, HT_RESP_TIMEOUT_180, ThisModule);
    if (ERR_UR_SUCCESS != iRet)
    {
        Log(ThisModule, iRet, "ExecuteCmd(%s) failed", ThisModule);
        return iRet;
...
)
{
    ...
    if (iValidateMode == VALIDATION_MODE_REAL)
    {
        if (pArrDispDeno)
        {
            MOSAICCMND(szCmdData, iCmdLen, 0x2001);
        }
        else
        {
    ...
    else
    {
        if (pArrDispDeno)
        {
            MOSAICCMND(szCmdData, iCmdLen, 0x2F01);
        }
        else
        {
    ...
    else if (pArrDispDeno != nullptr) //3.Number of Dispense notes per Denomination(52 bytes)
    {
        memset(szCmdParam, 0, sizeof(szCmdParam));
        szCmdParam[0] = 0x05;
        szCmdParam[1] = 0x41;
        ...
        {
            if(pArrDispDeno[i].iCashNumber <= 0)                    //30-00-00-00(FT#0073)
            {
//30-00-00-00(FT#0073)                Log(ThisModule, ERR_UR_PARAM, "pArrDispDeno[%d].iCashNumber:%d错误", i, pArrDispDeno[i].iCashNumber);
				continue; 
			}
...
30-00-00-00(FT#0073) del end*/
    ...
    char szResp[HT_RESP_LEN_MAX] = {0};
    USHORT dwRespLen = HT_RESP_LEN_MAX; // 输入：缓冲区长度，输出：实际接收响应字节数
    int iRet = ExecuteCmd(szCmdData, iCmdLen, szResp, dwRespLen, 4640, HT_RESP_TIMEOUT_180, ThisModule);
    if (ERR_UR_SUCCESS != iRet)
    {
        Log(ThisModule, iRet, "ExecuteCmd(%s) failed", ThisModule);
        return iRet;
    ...
    if (iDataPos > 0)
    {
        ...
        stDispMissfeedRoom.bArryRoom[3] = FALSE; //AB箱固定值   //30-00-00-00(FS#0020)
        if (szResp[iDataPos] & 0x10)
            stDispMissfeedRoom.bArryRoom[0] = TRUE;     //30-00-00-00(FS#0020)
        if (szResp[iDataPos] & 0x08)
            stDispMissfeedRoom.bArryRoom[1] = TRUE;     //30-00-00-00(FS#0020)
        if (szResp[iDataPos] & 0x04)
            stDispMissfeedRoom.bArryRoom[2] = TRUE;     //30-00-00-00(FS#0020)
...
long CUR2Drv::GetNoteSeiralInfoFullImage(DWORD dwIndex, LPSNOTESERIALINFOFULLIMAGE lpNoteSerialInfoFullImage)
{
    ...
    BOOL bHeadData = TRUE;
    char szCmdData[HT_CMD_LEN_ZERO] = {0};
//30-00-00-00(FS#0022)    char szRespData[HT_RESP_LEN_ZERO * 2] = {0};
    char szRespData[HT_RESP_LEN_ZERO] = {0};            //30-00-00-00(FS#0022)
    szCmdData[0] = 0x00;
    szCmdData[1] = 0x24;
    szCmdData[5] = 0x01;
...
//30-00-00-00(FS#0022)        memset(szRespData, 0, sizeof(char)*HT_RESP_LEN_ZERO * 2);
//30-00-00-00(FS#0022)        dwRespLen = static_cast<USHORT>(HT_RESP_LEN_ZERO * 2);
        memset(szRespData, 0, sizeof(char) * HT_RESP_LEN_ZERO);         //30-00-00-00(FS#0022)
        dwRespLen = static_cast<USHORT>(HT_RESP_LEN_ZERO);              //30-00-00-00(FS#0022)
        iRet = ExecuteCmd(szCmdData, HT_CMD_LEN_ZERO, szRespData, dwRespLen, HT_RESP_LEN_ZERO,  HT_RESP_TIMEOUT_10, ThisModule, CONNECT_TYPE_ZERO);
...
//30-00-00-00(FS#0007)
int CUR2Drv::TestCashUnits(VALIDATION_MODE iValidateMode,
                           BYTE byRoomExclusion[2][5],
                           BYTE byRetryCnt,
                           TESTCASHUNITSOUTPUT &stTestCashUnitsOutput)
{
    const char *ThisModule = "TestCashUnits";
    char szCmdData[512] = {0};
    int iCmdLen = 2;
    //$0001
    if(iValidateMode == VALIDATION_MODE_REAL){
        MOSAICCMND(szCmdData, iCmdLen, 0x7800);
    } else {
        MOSAICCMND(szCmdData, iCmdLen, 0x7900);
    }
    iCmdLen += 6;

    //$0536
    if(byRetryCnt > 0){
        szCmdData[iCmdLen++] = 0x05;
        szCmdData[iCmdLen++] = 0x36;
        szCmdData[iCmdLen++] = 0x00;
        szCmdData[iCmdLen++] = 0x04;
        szCmdData[iCmdLen++] = byRetryCnt;
    }

    //$0550
    szCmdData[iCmdLen++] = 0x05;
    szCmdData[iCmdLen++] = 0x50;
    szCmdData[iCmdLen++] = 0x00;
    szCmdData[iCmdLen++] = 0x22;
    typedef struct {
        BYTE byReserved1[3];
        BYTE byRoomExclusion[10];
        BYTE byReserved2[10];
        BYTE byURJB;
        BYTE byReserved3[8];
    }ROOMEXCLUSIONPKG, *LPROOMEXCLUSIONPKG;
    LPROOMEXCLUSIONPKG lpRoomExclusionPkg = (LPROOMEXCLUSIONPKG)(szCmdData + iCmdLen);
    lpRoomExclusionPkg->byURJB = 1;
    for(int i = 0; i < 2; i++){
        for(int j = 0; j < 5; j++){
            if(byRoomExclusion[i][j] == 1){
                lpRoomExclusionPkg->byRoomExclusion[i * 5 + j] = 0x01;
            }
        }
    }
    iCmdLen += 0x20;

    szCmdData[0] = (iCmdLen >> 8) & 0xFF;
    szCmdData[1] = iCmdLen & 0xFF;

    char szResp[HT_RESP_LEN_MAX] = {0};
    USHORT usRecvLenInOut = sizeof(szResp);
    int iRet = ExecuteCmd(szCmdData, iCmdLen, szResp, usRecvLenInOut, 70, HT_RESP_TIMEOUT_60, ThisModule);
    //解析返回数据
    if (ERR_UR_SUCCESS != iRet)
    {
        Log(ThisModule, iRet, "ExecuteCmd(%s) failed", ThisModule);
    ...
    PACKETMAP mapPacket;
    int iMinPacketCount = 12;
    if (!CheckRespEachPacket(szResp, iMinPacketCount, mapPacket, ThisModule))
    {
        Log(ThisModule, iRet, "%s response packet count check error", ThisModule);
        return ERR_UR_RESPDATA_ERR;
    }
    ...
    BYTE byRespCode = usRespCode & 0xFF;
    switch (byRespCode)
    {
    case 0x00:
    case 0x40:
...
//        break;  //Warn
    ...
    case 0xFF:
    {
        Log(ThisModule, ERR_UR_INVALID_COMMAN, "Invalid Command");
        return ERR_UR_INVALID_COMMAN;
    }
        break;  //Invalid Command
    default:
    ...
    lpData = szResp + iDataPos + 6;
    for(int i = 0; i < 2; i++){
        for(int j = 0; j < 5; j++){
            int iOffSet = 2 * (i * 5 + j);
            stTestCashUnitsOutput.wCountToRoom[i][j] = MAKEWORD(*(lpData + iOffSet + 1), *(lpData + iOffSet));
        }
    }

    //6.Number of Fed Notes(36 bytes)
    iDataPos = FindMapPacketDataPosforID(mapPacket, PACKET_ID_NUMFEDNOTES, ThisModule);
    if(iDataPos <= 0){
        return ERR_UR_RESPDATA_ERR;
    }
    lpData = szResp + iDataPos + 6;
    for(int i = 0; i < 2; i++){
        for(int j = 0; j < 5; j++){
            int iOffSet = 2 * (i * 5 + j);
            stTestCashUnitsOutput.wCountFromRoom[i][j] = MAKEWORD(*(lpData + iOffSet + 1), *(lpData + iOffSet));
        }
    }
    ...
    lpData = szResp + iDataPos;
    for(int i = 0; i < 2; i++){
        for(int j = 0; j < 5; j++){
            int iOffSet = 2 * (i * 5 + j);
            stTestCashUnitsOutput.wNumOfCSPerRoom[i][j] = MAKEWORD(*(lpData + iOffSet + 1), *(lpData + iOffSet));
        }
    }
...
//    }
    ...
    //10.Number of Reject Notes per BV Denomination and Destination(68,132,260,516 bytes)
    int iRetParse = ParseRespNumStackedNotesPerDeno(szResp, mapPacket, REJECT_BV_NOTES_DENO_INFO_TYPE, stTestCashUnitsOutput.stPerDenomiNumofRJ, ThisModule);
    if (iRetParse < 0)
    {
        return iRetParse;
    }
    ...
    //11.各room出金结果
    iDataPos = FindMapPacketDataPosforID(mapPacket, PACKET_ID_TEST_DISP_ROOM_INFO, ThisModule);
    lpData = szResp + iDataPos;
    for(int i = 0; i < 2; i++){
        for(int j = 0; j < 5; j++){
            BYTE byFailCause = *(lpData + (i * 5 + j));
            TESTDISPFAILCAUSE eFailCause = NO_ERROR;
            if(byFailCause != 0){
                eFailCause = ERROR_OTHER;
                if(byFailCause & 0x80){
                    if(byRoomExclusion[i][j] == 0x00 && j != 3){
                        eFailCause = EXECUTE_ABNORMAL;
                    } else {
                        eFailCause = NO_ERROR;
                    }
                } else if(byFailCause & 0x10){
                    eFailCause = ALL_REJECT;
                } else if(byFailCause & 0x08){
                    eFailCause = CST_EMPTY;
                } else if(byFailCause & 0x04){
                    eFailCause = MISS_FEED;
                } else if(byFailCause & 0x02){
                    eFailCause = DENOMINATION_ERROR;
                }
            }
            stTestCashUnitsOutput.eRoomError[i][j] = eFailCause;
        }
    }
...
//30-00-00-00(FS#0022)
int CUR2Drv::SelfCount(VALIDATION_MODE iValidateMode,                   //真币/测试币模式
                       bool bEnableRoom[5][2],                             //精查对象room
                       ULONG ulNumSelfCountPerCass[MAX_CASSETTE_NUM],      //每个朝向精查的结果张数
                       ULONG ulNumFeedPerCass[MAX_CASSETTE_NUM],           //每个钞箱出的张数
                       ST_TOTAL_STACKE_NOTES_DENO_INFO &stRejectDenoDstInfo,
                       ST_TOTAL_STACKE_NOTES_DENO_INFO &stRejectDenoSrcInfo,
                       const ST_BV_DEPENDENT_MODE stBVDependentMode,
                       bool  bContinueAfterBVDataFull/* = false*/)
{
    const char *ThisModule = "SelfCount";
    char szCmdData[512] = {0};
    int iCmdLen = 2;
    //$0001
    if(iValidateMode == VALIDATION_MODE_REAL){
        if(bContinueAfterBVDataFull){
            MOSAICCMND(szCmdData, iCmdLen, 0x3720);
        } else {
            MOSAICCMND(szCmdData, iCmdLen, 0x3700);
        }
    } else {
        if(bContinueAfterBVDataFull){
            MOSAICCMND(szCmdData, iCmdLen, 0x3D20);
        } else {
            MOSAICCMND(szCmdData, iCmdLen, 0x3D00);
        }
    }
    iCmdLen += 6;
    ...
    for(int i = 0; i < 2; i++){
        for(int j = 0; j < 5; j++){
            //0x00:不禁止 0x01:禁止
            szCmdData[iCmdLen + 3 + i * 5 + j] = bEnableRoom[j][i] ? 0x00 : 0x01;
        }
    }
    //回收箱固定不禁止
    szCmdData[iCmdLen + 3 + 3] = 0x00;
    //URJB
    szCmdData[iCmdLen + 3 + 10 + 10] = 0x01;
    ...
    char szResp[HT_RESP_LEN_MAX] = {0};
    USHORT usRecvLenInOut = sizeof(szResp);
    int iRet = ExecuteCmd(szCmdData, iCmdLen, szResp, usRecvLenInOut, 70, HT_RESP_TIMEOUT_SELFCOUNT, ThisModule);
    //解析返回数据
    if (ERR_UR_SUCCESS != iRet)
    {
        Log(ThisModule, iRet, "ExecuteCmd(%s) failed", ThisModule);
    ...
    case 0xFF:
    {
        Log(ThisModule, ERR_UR_INVALID_COMMAN, "命令格式错误");
        return ERR_UR_INVALID_COMMAN;
    }
        break;  //Invalid Command
    default:
        Log(ThisModule, iRet, "数据解析错误, 返回码%X未定义", usRespCode);
        return ERR_UR_RESPDATA_ERR;
    }
    ...
    //5.Number of Stacked Notes(64 bytes)
//    iDataPos = FindMapPacketDataPosforID(mapPacket, PACKET_ID_NUMSTACKEDNOTES, ThisModule);
//    if(iDataPos <= 0){
//        return ERR_UR_RESPDATA_ERR;
//    }
    ...
    lpData = szResp + iDataPos;
    for(int i = 0; i < MAX_CASSETTE_NUM - 1; i++){
        ulNumFeedPerCass[i] = MAKEWORD(lpData[6 + i * 2 + 1], lpData[6 + i * 2]);
    }

    //7.Number of reject notes(6 bytes)

    //8.Number of self-counting notes per room(28 bytes)
    iDataPos = FindMapPacketDataPosforID(mapPacket, PACKET_ID_SELFCOUNTPERROOM, ThisModule);
    if (iDataPos <= 0)
    {
        return ERR_UR_RESPDATA_ERR;
    }
    lpData = szResp + iDataPos;
    for(int i = 0; i < MAX_CASSETTE_NUM - 1; i++){
        ulNumSelfCountPerCass[i] = MAKEWORD(lpData[i * 2 + 1], lpData[i * 2]);
    }

    //9.Number of Stacked Notes per Denomination and Destination(68,132,260,516 bytes)
//    iRetParse = ParseRespNumStackedNotesPerDeno(szResp, mapPacket, STACKE_NOTES_DENO_INFO_TYPE, stStackeNotesDenoInfo, ThisModule);
//    if (iRetParse < 0)
//    {
//        return iRetParse;
//    }
    ...
    //9.Number of Reject Notes per Denomination and Source(68 bytes)
    int iRetParse = ParseRespNumStackedNotesPerDeno(szResp, mapPacket, REJECT_SOURCE_NOTES_DENO_INFO_TYPE, stRejectDenoSrcInfo, ThisModule);
    if (iRetParse < 0)
    {
        return iRetParse;
    }
    ...
    //10.Number of Reject Notes per BV Denomination and Destination(68,132,260,516 bytes)
    iRetParse = ParseRespNumStackedNotesPerDeno(szResp, mapPacket, REJECT_BV_NOTES_DENO_INFO_TYPE, stRejectDenoDstInfo, ThisModule);
    if (iRetParse < 0)
    {
        return iRetParse;
    }
