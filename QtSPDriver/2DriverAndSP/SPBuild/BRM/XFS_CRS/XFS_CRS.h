...
enum WAIT_TAKEN_FLAG
{
    ...
    WTF_CIM_TK_EVT = 3,         //只检测taken事件，不关门                //30-00-00-00(FT#0060)
    WTF_CDM_TK_EVT = 4          //只检测taken事件，不关门                //30-00-00-00(FT#0060)
};
...
protected:
    ...
    virtual HRESULT CDMTestCashUnits(const LPWFSCDMITEMPOSITION lpItemPosition, LPWFSCDMCUINFO &lpCUInfo);  //30-00-00-00(FS#0007)
    ...
    virtual HRESULT CIMCashUnitCount(const LPWFSCIMCOUNT lpCIMCount) override;       //30-00-00-00(FS#0022)
private:
...
private:
    ...
    WORD ComputeDispenseState(const ADPSTATUSINFOR &ADPStatus, BOOL *pbStopByHardwareError = NULL, BOOL bLog = FALSE);
    ...
    HRESULT OnDenominate(USHORT usTellerID, USHORT usMixNumber, ICDMDenomination *pDenoInOut);
    ...
    HRESULT CheckDenominateParam(const LPWFSCDMDENOMINATE lpDenoData);                  //30-00-00-00(FT#0026)
    HRESULT CheckDispenseParam(const LPWFSCDMDISPENSE lpDisData);                       //30-00-00-00(FT#0026)
    void RelieveBoxManipStatus();                                                       //30-00-00-00(FT#0042)
private:
    //FIRE EVENT
...
private:
    ...
    //入金限制
    ULONG              m_ulMaxNumber;       //最大入金张数 //30-00-00-00(FS#0002)
    ULONG              m_ulMaxAmount;       //最大入金总额 //30-00-00-00(FS#0002)
    bool               m_bCashInExecuted;   //30-00-00-00(FS#0002)
