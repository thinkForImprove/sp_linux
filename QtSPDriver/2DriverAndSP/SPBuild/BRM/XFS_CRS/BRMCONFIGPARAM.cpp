...
{
    char szBuf[1024] = {0};
    string strTemp = m_SPConfigFile.GetString(pKeyName, pValueName, "");
    strcpy(szBuf, strTemp.c_str());
...
int _tag_brm_config_param::LoadParam(LPCSTR lpszFileName)
{
    ...
    bPresenSPCloseShutter = m_SPConfigFile.GetInt("BRMInfo", "PresentSPCloseShutter", 0) == 1;   //30-00-00-00(FT#0005)
    wCashInStatusWhenCsNotEmpty = m_SPConfigFile.GetInt("BRMInfo", "CashInStatusWhenCsNotEmpty", 0);  //30-00-00-00(FT#0014)
    bCashinRefuseCountAdd = m_SPConfigFile.GetInt("BRMInfo", "CashinRefuseCountAdd", 0) == 1;    //30-00-00-00(FT#0034)
    //装载币种
    int i = 0;
    ...
    for (int i = 0; i < msCurrencyList.GetCount(); i++)
    {
        ...
        memcpy(lppCurrencyExp[i]->cCurrencyID, msCurrency.GetAt(0), 3);
    ...
    for (i = 0; i < msNoteTypeList.GetCount(); i++)
    {
        ...
        memcpy(NoteType.cCurrencyID, msNoteType.GetAt(0), 3);
        ...
        if (NoteType.ulValues == 0 && NoteType.usNoteID != 128)     //30-00-00-00(FS#0021)
        {
            Log(ThisModule, -1,
    ...
    ulCassMiniGuarantee = m_SPConfigFile.GetInt("BRMInfo", "CassMiniGuarantee", 0);
    bResetRelieveManipSupp = m_SPConfigFile.GetInt("BRMInfo", "ResetRelieveManipSupp", 1) == 1;     //30-00-00-00(FT#0042)
    bSetUnitInfoRelieveManipSupp = m_SPConfigFile.GetInt("BRMInfo", "SetUnitInfoRelieveManipSupp", 1) == 1; //30-00-00-00(FT#0042)
    bPresentTakenEventAfterCloseShutter = m_SPConfigFile.GetInt("BRMInfo", "PresentTakenEventAfterCloseShutter", 1) == 1;
    bShutterClosedErrorSupp = m_SPConfigFile.GetInt("BRMInfo", "ShutterClosedErrorSupp", 0);
    bShutterClosedErrorSupp = m_SPConfigFile.GetInt("BRMInfo", "ShutterOpenedErrorSupp", 0);

    //招商银行特殊需求
    bCashInBoxReturnNOITEMS = m_SPConfigFile.GetInt("SPEC", "CashInBoxReturnNOITEMS", 0) == 1;  //30-00-00-00(FT#0054)
    return 0;
}
