...
//        wAcceptor != wOldAcceptor)
    if(m_ADPStatus.stDevice != ADPStatus.stDevice)
    {
...
//        ADPStatus.stShutterPosition == ADP_SHUTPOS_EMPTY)
            ...
            (m_pSP->m_eWaitTaken != WTF_NONE || ADPStatus.stOutShutter != ADP_OUTSHUTTER_CLOSED))
    {
        ...
        m_pSP->m_pAdapter->GetCIMCapabilities(&CIMCaps);
        if(m_pSP->m_bCdmCmd){
            if(m_pSP->m_eWaitTaken == WTF_CDM && m_pSP->m_Param.GetPresentTakenEventAfterCloseShutterFlag()){
        ...
        if(m_pSP->m_eWaitTaken == WTF_CIM_TK_EVT || m_pSP->m_eWaitTaken == WTF_CDM_TK_EVT){     //30-00-00-00(FT#0060)
            m_pSP->m_eWaitTaken = WTF_NONE;                                                     //30-00-00-00(FT#0060)
        }                                                                                       //30-00-00-00(FT#0060)
        m_pSP->Log(ThisModule, 0,
