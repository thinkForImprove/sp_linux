...
extern "C"
{
	...
	///
    /// @param	[in] hDev	   	    Device handle.
    /// @param	[in] nLineHeight	Specifies the high number of rows. The valid range is 0-255. The distance of each point depends on the print head resolution.
	///
    /// <b>The sample code\n</b>
    /// See details @ref EG_PrintTextExample
    /// 
	...
	///
	/// @param	[in] hDev	   	    设备端口
	/// @param	[in] nLineHeight	指定行高点数。有效范围为0-255。每点的距离与打印头分辨率相关。
	///
	/// <b>示例代码\n</b>
	/// 详见 @ref EG_PrintTextExample
	/// 
    ...
    /// 			3     | Half cut
    /// @param [in] nDistance 			The number of points that specify the length of the feed. \n valid range is 0-255. The distance of each point depends on the print head resolution.
    ///
    /// <b>The sample code\n</b>
    /// See details @ref EG_BasicSetExample
    /// 
...
typedef int (POSSDKCALL_MODE *pEnumDeviceInfo)(int nOperationType, char *szDeviceNameBuf, int nBufLen, int *npNumber);
...
typedef int (POSSDKCALL_MODE *pClosePort)(DEVICEHANDLE hDev);
