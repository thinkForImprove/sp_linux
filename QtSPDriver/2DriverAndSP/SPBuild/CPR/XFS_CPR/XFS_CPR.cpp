...
HRESULT CXFS_CPR::OnClose()
{
    ...
    if (m_pPrinter != nullptr)
    {
        Log(ThisModule, __LINE__, "关闭设备连接成功.");
        m_pPrinter->Close();
    }
...
HRESULT CXFS_CPR::PrintForm(const LPWFSPTRPRINTFORM lpPrintForm, DWORD dwTimeOut)
{
    ...
    {
        Log(ThisModule, __LINE__, "Media != PRESENT,通道内无票, Return: %d.", WFS_ERR_PTR_NOMEDIAPRESENT);
...
{    
    ...
    HRESULT hRet = InnerReadForm(pIn);
    if (hRet == WFS_SUCCESS)
    {
...
HRESULT CXFS_CPR::ResetCount(const LPUSHORT lpusBinNum)
{
    ...
    WORD wRetBoxNo = 0;             // 入参箱号对应的索引
    INT nCount = 0;                 // 回收箱内张数

    // Check: 是否有可用的回收箱
    if (m_stNoteBoxList.nIsRetractBox() < 1)
    {
        Log(ThisModule, __LINE__, "INI没有设置可用的回收箱, Return: %d.", WFS_ERR_UNSUPP_COMMAND);
        return WFS_ERR_UNSUPP_COMMAND;
    }

    // Check: 入参>0为指定回收箱号,检查指定回收箱是否存在
    if (wRetractBox > 0)
    {
        if (m_stNoteBoxList.nBoxIsHave(wRetractBox, BOX_RETRACT) == BOX_NOHAVE)
        {
            Log(ThisModule, __LINE__, "指定回收箱号[%d]不存在, Return: %d.", wRetractBox, WFS_ERR_UNSUPP_DATA);
            return WFS_ERR_UNSUPP_DATA;
        }
        wRetBoxNo = wRetractBox;
    } else
    {
        // 取第一个回收箱号
        wRetBoxNo = m_stNoteBoxList.nGetRetainNum();
        if (wRetBoxNo == BOX_NOHAVE)
        {
            Log(ThisModule, __LINE__, "设置回收箱计数并记录到INI->取第一个回收箱号未找到.");
            return WFS_ERR_UNSUPP_DATA;
        }
    }

    // 取回收箱号对应的箱内张数
    nCount = m_stNoteBoxList.nGetBoxCount(wRetBoxNo, BOX_RETRACT);

    // 统计: 票据计数: 0,写入INI记录
    SetRetractBoxCount(wRetBoxNo, nCount, FALSE);
...
HRESULT CXFS_CPR::Reset(const LPWFSPTRRESET lpReset)
{
    ...
    {
        Log(ThisModule, __LINE__, "Input dwMediaControl[%d] Is Invalid, Return: %d.", pIn->dwMediaControl, WFS_ERR_UNSUPP_DATA);
        return WFS_ERR_UNSUPP_DATA;
    }

    if ((pIn->dwMediaControl & WFS_PTR_CTRLEXPEL) == WFS_PTR_CTRLEXPEL)
    {
        Log(ThisModule, __LINE__, "Input dwMediaControl[%d] Is NoSupp, Return: %d.", WFS_ERR_UNSUPP_DATA);
        return WFS_ERR_UNSUPP_DATA;
    }
    ...
    {
        Log(ThisModule, __LINE__, "入参dwMediaControl[%d] Is Invalid, 参数冲突, Return: %d.",
    ...
    {
        hRet = InnerControlMedia(pIn->dwMediaControl);
        if (hRet != WFS_SUCCESS && hRet != WFS_ERR_PTR_NOMEDIAPRESENT)
        {
            Log(ThisModule, __LINE__, "Input dwMediaControl[%d] Is EjectMedia: ->ControlMedia(%d) Is Fail, Return: %d.",
                pIn->dwMediaControl, pIn->dwMediaControl, hRet);
            return hRet;
        }
    } else
    {
        hRet = InnerControlMedia(WFS_PTR_CTRLRETRACT);
        if (hRet != WFS_SUCCESS && hRet != WFS_ERR_PTR_NOMEDIAPRESENT)
        {
            Log(ThisModule, __LINE__, "Input dwMediaControl[%d] Is ControlMedia: ->ControlMedia(%d) Is Fail, Return: %d.",
                pIn->dwMediaControl, pIn->dwMediaControl,  hRet);
            return hRet;
        }
    }
    ...
    if (nRet != PTR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "->ResetEx(%d) Fail, ErrCode = %d, Return: %d.",
...
HRESULT CXFS_CPR::RetractMedia(const LPUSHORT lpusBinNum, LPUSHORT &lpusBinNumOut)
{
    ...
    if (wRetractBox > 0)
    {
        if (m_stNoteBoxList.nBoxIsHave(wRetractBox, BOX_RETRACT) == BOX_NOHAVE)
        {
            Log(ThisModule, __LINE__, "指定回收箱索引[%d]不存在, Return: %d.", wRetractBox, WFS_ERR_UNSUPP_DATA);
            return WFS_ERR_UNSUPP_DATA;
        }
    }
    ...
    {
        Log(ThisModule, __LINE__, "指定回收箱[%d]已满, Return: %d.", wRetractBox, WFS_PTR_RETRACTBINFULL);
    ...
    if (nRet != PTR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "票据回收->MeidaControl(%d, %d) Fail, ErrCode = %d, Return: %d.",
...
HRESULT CXFS_CPR::DispensePaper(const LPWORD lpPaperSource)
{
    ...
    {
        Log(ThisModule, __LINE__, "INI没有设置可用的票箱, Return: %d.", WFS_ERR_UNSUPP_COMMAND);
        return WFS_ERR_UNSUPP_COMMAND;
    }
    ...
    {
        Log(ThisModule, __LINE__, "Input[%d] Is Invalid, Return: %d.", *lpPaperSource, WFS_ERR_UNSUPP_DATA);
        return WFS_ERR_UNSUPP_DATA;
    }
    ...
    if (nRet == BOX_NOHAVE)
    {
        Log(ThisModule, __LINE__, "指定票箱索引[%d]不存在, Return: %d.",
    ...
    {
        Log(ThisModule, __LINE__, "指定票箱索引[%d]为空, INI计数[%d] < 1 || Status.Paper[%d] = PAPEROUT, Return: %d.",
    ...
    // 出票前指定票据类型
    WORD wNoteType = 0;
    if (m_stConfig.nDriverType == IXFSCPR_TYPE_BT8500M) // 8500M设备
    {
        // 取INI指定票箱中票据类型转换为BT8500M分纸类型
        wNoteType = BoxNoteType2DevSDK(m_stNoteBoxList.nGetNoteTypeIsBox(wStorageBox),
                                       IXFSCPR_TYPE_BT8500M);
        m_pPrinter->SetData((void*)&wNoteType, DTYPE_NOTE_TYPE);
    }
    // 执行: 票箱出票
    nRet = m_pPrinter->MeidaControl(MEDIA_CTR_ATPBACKWARD, wStorageBox);
    if (nRet != PTR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "票箱出票->MeidaControl(%d, %d) Fail, ErrCode = %d, Return: %d.",
