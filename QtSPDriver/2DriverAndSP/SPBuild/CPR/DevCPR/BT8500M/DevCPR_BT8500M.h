/***************************************************************
* 文件名称：DevCPR_BT8500M.h
* 文件描述：BT-8500M票据发放模块功能处理接口封装 头文件
*
* 版本历史信息
* 变更说明：建立文件
* 变更日期：2021年4月4日
...
#define LOG_NAME_DEVCPR     "DevCPR_BT8500M.log"
...
//-------------------------------------------------------------------------------
class CDevCPR_BT8500M : public IDevPTR, public CLogManage, public ConvertVar
{
...
private:
    INT     ConvertErrorCode(INT nRet);                                                 // IMPL错误码转换为PTR错误码
    void    DiffDevStat(DEVICESTATUS stStat, DEVICESTATUS stOLD);

private:
    CSimpleMutex                m_cMutex;
    CDevImpl_BT8500M            m_devBT8500M;
    DEVICESTATUS                m_stDevStatOLD;                                         // 记录上一次取到的状态
