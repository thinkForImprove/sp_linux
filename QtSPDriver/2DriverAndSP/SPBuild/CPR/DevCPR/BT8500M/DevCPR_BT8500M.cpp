...
int CDevCPR_BT8500M::ReadImage(DEVPTRREADIMAGEIN stImageIn, DEVPTRREADIMAGEOUT &stImageOut)
{
    ...
    if (stImageIn.wInMode == IMAGE_MODE_CODELINE)
    {
        ...
        if (m_usBankNo == BANK_NO_CSBC) // 长沙银行
        {
            for (i = 0; i < wCount; i ++)
            {
        ...
        } else
        {
            ...
            for (i = 0; i < wCount; i ++)
            {
                if (cJsonTmpData[i](BT_JSON_FIELDNAME).compare(BT_JSON_CHECKNO) == 0)   // 票号
                {
                    stdCardNo.append(cJsonTmpData[i](BT_JSON_FIELDNAME));
                }
                if (cJsonTmpData[i](BT_JSON_FIELDNAME).compare(BT_JSON_ACCOUNT) == 0)   // 账号
                {
                    stdAccount.append(cJsonTmpData[i](BT_JSON_FIELDNAME));
                }
                if (cJsonTmpData[i](BT_JSON_FIELDNAME).compare(BT_JSON_AMOUNT) == 0)    // 金额
                {
                    stdAmount.append(cJsonTmpData[i](BT_JSON_FIELDNAME));
                }
            }
            stdCardNo.append("|");
            stdCardNo.append(stdAccount);
            stdCardNo.append("|");
            stdCardNo.append(stdAmount);

            SET_KEYNAME(JSON_KEY_IDEN_NAME, 1, szKeyName);
            cJsonScanOut.Add(szKeyName, std::string("Data"));
            SET_KEYNAME(JSON_KEY_IDEN_VALUE, 1, szKeyName);
            cJsonScanOut.Add(JSON_KEY_IDEN_CNT, 1);
        }
    }
        ...
        (stImageIn.wInMode & IMAGE_MODE_AUX) == IMAGE_MODE_AUX)
    {
        ...
        {
            cJsonSndData.Clear();
            if (m_usBankNo == BANK_NO_JIANGNAN)
            {
                cJsonSndData.Parse(cJsonScanIn.ToString());
            } else
            {
                cJsonSndData.Add(BT_JSON_USEAREA, 0);
            }
...
int CDevCPR_BT8500M::MeidaControl(MEDIA_ACTION enMediaAct, unsigned short usParam)
{
    ...
    {
        if ((nRet = m_devBT8500M.nFeedCheck(usParam, m_usNoteType)) != IMP_SUCCESS)
        {
            Log(ThisModule, __LINE__, "介质控制: 分纸: ->nFeedCheck(%d, %d) Fail, ErrCode=%d, Return %s.",
               usParam, m_usNoteType, nRet, ConvertErrCodeToStr(ConvertErrorCode(nRet)));
            return ConvertErrorCode(nRet);
        }
    } else
    {
...
int CDevCPR_BT8500M::SetData(void *vInitPar, WORD wDataType)
{
    switch(wDataType)
    {
        ...
        case DTYPE_BANKNO:      // 设置银行编码
        {
            m_usBankNo = *((USHORT*)vInitPar);
            break;
        }
        case DTYPE_NOTE_TYPE:   // 设置票据类型
        {
            m_usNoteType = *((USHORT*)vInitPar);
            break;
        }
        default:
            break;
