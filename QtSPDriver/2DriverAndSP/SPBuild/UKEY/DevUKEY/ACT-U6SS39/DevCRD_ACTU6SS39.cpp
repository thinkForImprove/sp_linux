#include "DevCRD_ACTU6SS39.h"

#include <qnamespace.h>
#include <unistd.h>
#include <QObject>

static const char *ThisModule = "DevCRD_ACTU6SS39.cpp";

//////////////////////////////////////////////////////////////////////////

CDevCRD_ACTU6SS39::CDevCRD_ACTU6SS39() : m_devACTU6SS39(LOG_NAME_DEVCRD)
{
    SetLogFile(LOG_NAME_DEVCRD, ThisModule);  // 设置日志文件名和错误发生的文件
}

CDevCRD_ACTU6SS39::~CDevCRD_ACTU6SS39()
{
    Close();
}

void CDevCRD_ACTU6SS39::Release()
{
...
// 建立CRD连接
int CDevCRD_ACTU6SS39::Open(LPCSTR lpMode)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    // 建立连接
    if (m_devACTU6SS39.IsDeviceOpen() != TRUE)
    {
        if ((nRet = m_devACTU6SS39.DeviceOpen((LPSTR)lpMode)) != IMP_SUCCESS)
        {
    ...
    // 读机器参数
    INT nParam[12] = { 0 };
    nRet = m_devACTU6SS39.GetDeviceParam(MODE_SCAN_PARAM, m_nParam);
    if (nRet != IMP_SUCCESS)
    {
        Log(ThisModule, __LINE__, "读机器参数: ->GetDeviceParam(%d) Fail, ErrCode=%d, Not Return.",
            MODE_SCAN_PARAM, nRet);
    } else
    {
        Log(ThisModule, __LINE__,
            "读机器参数: ->GetDeviceParam(%d) Succ, 扫描一步升距: %d, 扫描一步后升距: %d, "
            "上升次数: %d, 移出卡口超时: %d.",
            m_nParam[0], m_nParam[1], m_nParam[2], m_nParam[3]);
    }
...
// CRD初始化
int CDevCRD_ACTU6SS39::Init(INT eActFlag)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
// 关闭CRD连接
int CDevCRD_ACTU6SS39::Close()
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);

    m_devACTU6SS39.DeviceClose();
...
// 设备复位
int CDevCRD_ACTU6SS39::Reset(int nMode, int nParam)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    // 下发复位
    if ((nRet = m_devACTU6SS39.DeviceReset(usReset)) != IMP_SUCCESS)
    {
...
// 读取设备状态
int CDevCRD_ACTU6SS39::GetDevStat(STCRDDEVSTATUS &stStat)
{
    THISMODULE(__FUNCTION__);
    //AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    // 获取状态    
    nRet = m_devACTU6SS39.GetDeviceStat(0, nStatus);
    /*if (nRet != IMP_SUCCESS)
    {
...
// 读取设备信息
int CDevCRD_ACTU6SS39::GetUnitInfo(STCRDUNITINFO &stInfo)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
// 发卡
int CDevCRD_ACTU6SS39::DispenseCard(const int nUnitNo, const int nMode)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    if (nUnitNo == 1)
    {
        ...
        if (nMode == DC_BOX2FSCAN)          // 移动操作:箱1UKEY移动到前端扫描位置
        {
        ...
        {
            usInPar = MODE_MOVE_BOX1_SCAN1_N;
        }
    } else
    if (nUnitNo == 2)
    {
        ...
        if (nMode == DC_BOX2FSCAN_NOS)
        {
            usInPar = MODE_MOVE_BOX2_SCAN1_N;
        }
    } else
    {
    ...
    // 调用接口
    if ((nRet = m_devACTU6SS39.MoveMedia(usInPar)) != IMP_SUCCESS)
    {
...
// 弹卡
int CDevCRD_ACTU6SS39:: EjectCard(const int nMode)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
// 回收卡
int CDevCRD_ACTU6SS39::RetainCard(const int nMode)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    // 调用接口
    if ((nRet = m_devACTU6SS39.MoveMedia(MODE_MOVE_UKEY_RETAIN)) != IMP_SUCCESS)
    {
...
// 设置数据
int CDevCRD_ACTU6SS39::SetData(void *vData, int nSize,  WORD wDataType)
{
    switch(wDataType)
    {
        case DTYPE_SET_SNDTIMEOUT:      // 设置报文发送超时时间
        {
            m_devACTU6SS39.SetTimeOut(*((UINT*)(vData)), 0);
            break;
        }
        case DTYPE_SET_RCVTIMEOUT:      // 设置报文接收超时时间
        {
            m_devACTU6SS39.SetTimeOut(0, *((UINT*)(vData)));
            break;
        }
        case DTYPE_SET_POWEROFFMODE:// 掉电处理模式
        {
            ...
            if (nSize == 1)
            {
                return m_devACTU6SS39.SetDeviceParam(MODE_ST_POWOFF_NOACT, nParam);
            } else
            if (nSize == 1)
            {
                return m_devACTU6SS39.SetDeviceParam(MODE_ST_POWOFF_RETAIN, nParam);
            } else
            if (nSize == 1)
            {
                return m_devACTU6SS39.SetDeviceParam(MODE_ST_POWOFF_EJECT, nParam);
            }
            break;
        }
        default:
...
// 获取数据
int CDevCRD_ACTU6SS39::GetData(void *vData, int *nSize, WORD wDataType)
{
    switch(wDataType)
    {
        case DATATYPE_INIT:
...
void CDevCRD_ACTU6SS39::GetVersion(char* szVer, long lSize, ushort usType)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    if (usType == 2)
    {
        m_devACTU6SS39.GetDeviceFW((LPSTR)szVer, lSize);
    }
...
// 信息扫描
INT CDevCRD_ACTU6SS39::GetScanUKey(LPSTR lpData, INT *nDataLen)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    // 清除上一次扫描信息
    if ((nRet = m_devACTU6SS39.ScanMedia(MODE_SCAN_CLR_INFO, lpData, nDataLen)) != IMP_SUCCESS)
    {
    ...
    // 启动扫描
    if ((nRet = m_devACTU6SS39.ScanMedia(MODE_SCAN_START, lpData, nDataLen)) != IMP_SUCCESS)
    {
    ...
    // 获取扫描信息
    /*if ((nRet = m_devACTU6SS39.ScanMedia(MODE_SCAN_GET_INFO, lpData, nDataLen)) != IMP_SUCCESS)
    {
...
// Impl错误码转换为DevCRD错误码
INT CDevCRD_ACTU6SS39::ConvertErrorCode(INT nRet)
{
    switch(nRet)
    {
...
// DevCRD错误码含义
CHAR* CDevCRD_ACTU6SS39::ConvertErrCodeToStr(INT nRet)
{
