...
public: // 接口函数封装
    ...
    BOOL bEnumResolutions(SHORT deviceIndex, CWResolution** resolutionLists);
    // 1.14 枚举相机支持的所有分辨率(VIDPID)
    BOOL bEnumResolutionsEx(LPSTR vid, LPSTR pid, CWResolution** resolutionLists);
    // 1.15 开启活体检测
    BOOL bStartLiveDetect(BOOL isContinue);
    // 1.16 停止连续活体检测
    BOOL bStopLiveDetect();
    // 1.17 抓拍当前画面帧
    BOOL bCaptureCurrentFrame(CWSize size, BOOL isBase64, CWLiveImage* image);
    // 1.18 保存当前帧至指定文件路径参数
    BOOL bSaveCurrentFrame(LPSTR fileName);
    // 1.19 开启语音提示功能
    BOOL bEnableVoiceTip(BOOL bEnable, LPCSTR lpTipFile);// 30-00-00-00(FT#0031)
    // 1.20 获取本次活体检测最佳人脸图像
    BOOL bGetBestFace(float ratio, BOOL isBase64, CWLiveImage* image, int type = 1);
    // 1.21 保存活体最佳人脸到指定路径
    ...
    BOOL bSleepCamera(SHORT index, BOOL bIsSleep);
    // 1.29 通过相机硬件信息VID/PID休眠相机
    BOOL bSleepCameraEx(LPSTR vid, LPSTR pid, BOOL bIsSleep);
