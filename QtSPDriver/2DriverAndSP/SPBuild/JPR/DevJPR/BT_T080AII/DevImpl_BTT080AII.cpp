...
//#include "BTT080AIIDef.h"
#include "../XFS_JPR/def.h"

static const char *ThisFile = "BTT080AII_DevImpl.cpp";
...
CDevImpl_BTT080AII::CDevImpl_BTT080AII()
{
    SetLogFile(LOG_NAME, ThisFile);  // 设置日志文件名和错误发生的文件
    Init();
}

CDevImpl_BTT080AII::CDevImpl_BTT080AII(LPSTR lpLog)
{
...
void CDevImpl_BTT080AII::Init()
{
    ...
    memset(m_nRetErrOLD, 0, 12);
    m_nOpenMode = 0;                                                    // 打开模式(缺省0)
    m_nDeviceID = -1;                                                   // 内部ID(缺省-1)

    // 设定动态库路径
    QString strDllName(QString::fromLocal8Bit(DLL_DEVLIB_NAME));
    strDllName.prepend("JPR/BTT080AII/");
    #ifdef Q_OS_WIN
...
INT CDevImpl_BTT080AII::DeviceOpen()
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
    ...
    // 设备初始化
    nRet = InitPtr(BK_SERIES_PRINTER_NAME); // TKIOSK获取状态会有问题
    if (nRet != IMP_SUCCESS)
    {
...
INT CDevImpl_BTT080AII::DeviceClose()
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::InitPtr(char *pszPrinterName)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::Connect()
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
    ...
    // Open模式: 0 >> 首先以class mode方式打开,失败则尝试以API模式打开
    //          1 >> 指定内部ID以以API模式打开
    if (m_nOpenMode == 0)
    {
        // 尝试以class mode方式打开
        nRet = EnumDeviceInfo(PORT_USBDEVICE_CLASS, szDeviceNameListStr, sizeof(szDeviceNameListStr),
                                  &nDeviceNameNum);
    ...
    if (m_nOpenMode == 1)
    {
        nRet = OpenUsbApiPort(m_hPrinter, m_nDeviceID);
        if (nRet != ERR_SUCCESS)
        {
            if (m_nRetErrOLD[6] != nRet)
            {
                Log(ThisModule, __LINE__,
                    "API模式连接设备: 指定内部ID打开设备: OpenUsbApiPort(%d) fail. Return: %s.",
                    m_nDeviceID, ConvertErrCodeToStr(nRet));
                m_nRetErrOLD[6] = nRet;
            }
            return nRet;
        } else
        {
            Log(ThisModule, __LINE__,
                "API模式连接设备: 指定内部ID打开设备: OpenUsbApiPort(%d) succ.", m_nDeviceID);
        }
    } else
    {
...
INT CDevImpl_BTT080AII::GetRtStatus(char *pStaBuf, ULONG *pulBufSz)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::GetNonRtStatus(char *pStaBuf, ULONG *pulBufSz)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::GetAutoStatus(char *pStaBuf, ULONG *pulBufSz)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::GetFwVersion(LPSTR pFwVerBuff, ULONG *ulFwVerSize)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::GetSoftwareVersion(LPSTR pVerBuff, ULONG *ulVerBuffSize)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::GetHardSerialNo(LPSTR pBuff, ULONG *ulVerBuffSize)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::GetPrinterName(LPSTR pBuff, ULONG *ulVerBuffSize)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::GetVendorInfo(LPSTR pBuff, ULONG *ulVerBuffSize)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::ControlMedia(int nFeedLength)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
    ...
    if (nRet != ERR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "切纸+走纸: CutPaper(2, %d|%d) fail. Return: %s.",
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::ControlMedia(BOOL bDetectBlackStripe, int nFeedLength)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::LineModePrintText(LPSTR lpPrintString, WORD wStringSize, WORD wStartX, BOOL bFeedLine)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::LineModePrintTextUniversal(LPSTR lpPrintString, WORD wStringSize, WORD wStartX,
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::LineModePrintImage(LPSTR lpImageStr, WORD lpImageStrSize, WORD wStartX, BOOL bFeedLine)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::LineModePrintImageRAM(LPSTR lpImageStr, WORD lpImageStrSize, WORD wStartX, BOOL bFeedLine)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::LineModePrintImageByMode(LPSTR lpImageStr, WORD lpImageStrSize, WORD wStartX, BOOL bFeedLine)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::LineModePrintBarcode(LPSTR lpData, WORD wDataSize, WORD wStartX, WORD wType,
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::LineModePrintPDF417(LPSTR lpData, WORD wDataSize, WORD wStartX, WORD wBasicWidth,
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::LineModePrintQrcode(LPSTR lpData, WORD wDataSize, WORD wStartX, WORD wBasicWidth,
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::PrtFeedLine(INT nLine)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::PrintFrontSetFormat()
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::BK_SetPrintMode(WORD wPrintMode)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
    ...
    if (nRet != ERR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "设置打印模式: PrintSetMode(%d:0行/2页) fail. Return: %s.",
            wPrintMode, ConvertErrCodeToStr(nRet));
        return FALSE;
    }
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::BK_SetTextLineHight(WORD wLineHeight, BOOL IsSpot /* = FALSE */)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::BK_SetTextCharacterSpace(WORD nCharSpace, BOOL IsSpot /* = FALSE */)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
// 设置物理黑标位移
INT CDevImpl_BTT080AII::BK_SetBlackMove(UINT uMoveSize)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::BK_SetTextBold(WORD wBold /* = 0 */)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
/**
 ...
 @返回：	TRUE/FALSE
 */
INT CDevImpl_BTT080AII::BK_SetTextDoubleWH(WORD wWidth, WORD wHeight)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
INT CDevImpl_BTT080AII::SetReConFlag(BOOL bFlag)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
void CDevImpl_BTT080AII::SetLibPath(LPCSTR lpPath)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
...
void CDevImpl_BTT080AII::SetOpenMode(INT nMode, INT nDevID)
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
...
ULONG CDevImpl_BTT080AII::SplitString(LPCSTR lpszStr, CHAR cSplit, CHAR lpszStrArray[][MAX_PATH], ULONG ulArrayCnt)
{
    ULONG ulCnt = 0;
    string str = "", s = "";
...
BOOL CDevImpl_BTT080AII::bLoadLibrary()
{
    THISMODULE(__FUNCTION__);
    // AutoLogFuncBeginEnd();
    AutoMutex(m_MutexAction);
...
void CDevImpl_BTT080AII::vUnLoadLibrary()
{
...
BOOL CDevImpl_BTT080AII::bLoadLibIntf()
{
    m_bLoadIntfFail = FALSE;

    //
    LOAD_LIBINFO_FUNC(pEnumDeviceInfo1, EnumDeviceInfo, "EnumDeviceInfo");
    LOAD_LIBINFO_FUNC(pOpenUsbClassPort, OpenUsbClassPort, "OpenUsbClassPort");
    LOAD_LIBINFO_FUNC(pInit, Init_, "Init");
    LOAD_LIBINFO_FUNC(pClosePort11, ClosePort, "ClosePort");
...
INT CDevImpl_BTT080AII::ConvertErrorCode(INT nRet)
{
    return nRet;
}

CHAR* CDevImpl_BTT080AII::ConvertErrCodeToStr(INT nRet)
{
