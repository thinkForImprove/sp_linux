...
long CDevPIN_XZF35::SetActiveKey(DWORD dwActiveFuncKey, DWORD dwActiveFDKKey)
{
    ...
    stActivateKey.dwActiveFDKs = dwActiveFDKKeyConvert;
...
long CDevPIN_XZF35::PinInput(UINT uMinLen, UINT uMaxLen, bool bClearKeyAsBackspace/* = false*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_XZF35::DataCrypt(UINT uKeyNo, UINT uMode, LPCSTR lpCryptData, BYTE bPadding, LPCSTR lpIVData, QByteArray &vtResultData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    lRet = SendReadCmd(QUERY_SYMMETRIC_ENC_KEY_INFO, vtCmd, vtResult);
    if(lRet != ERR_SUCCESS){
...
long CDevPIN_XZF35::DataMAC(UINT uKeyNo, UINT uMode, LPCSTR lpMacData, LPCSTR lpIVData, QByteArray &vtResultData, BYTE byPad/* = 0xFF*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_XZF35::SM4ImportMKey(UINT uMKeyNo, LPCSTR lpKeyVal, LPSTR lpKCV, UINT uEncKeyNo/* = 0xFF*/)
{
//    THISMODULE(__FUNCTION__);
//    AutoLogFuncBeginEnd();
//    AutoMutex(m_cMutex);
...
long CDevPIN_XZF35::SM4RemoteImportKey(UINT uKeyNo, UINT uEppSKeyNo, UINT uHostSM2PKeyNo,
                                       ...
                                       LPCSTR lpSignatureVal, LPSTR lpKCV)
{
    ...
    if (uKeyNo >= DES_OR_SM4_KEY_MAX_NUM)
    {
        Log(ThisModule, __LINE__, "无效保存主密钥ID：uMKeyNo=%02X", uKeyNo);
        return ERR_PARAM;
    }

    long lRet = SwitchModeDESOrSM4(false);
    if(lRet != ERR_SUCCESS){
        Log(ThisModule, __LINE__, "SM主密钥导入失败[切换对称密钥处理模式失败]：lRet=%d[uMKeyNo=%d]", lRet, uKeyNo);
        return lRet;
    }

    UINT uLen;

    if(m_iImportSm4KeyEncBySm2Mode == 1) {
        SM2DATAOPT stSm2DataOpt = {0};
        //使用SM2公钥加密,仅测试时使用
        stSm2DataOpt.wKeyId = EXWORDHL(uEppSKeyNo);
        stSm2DataOpt.byKeyType = 0x50;
        char szData[256] = "11223344556677889900AABBCCDDEEFF";
        uLen = sizeof(stSm2DataOpt.byData);
        CAutoHex::Hex2Bcd(szData, stSm2DataOpt.byData, uLen);
        uLen = sizeof(stSm2DataOpt) - (sizeof(stSm2DataOpt.byData) - uLen);
        vtCmd.append((LPSTR)&stSm2DataOpt, uLen);
        lRet = SendReadCmd(SM2_DATA_ENC_OR_DENC, vtCmd, vtResult);
        vtCmd.clear();
        vtResult.clear();

        //使用SM2私钥解密
        memset(&stSm2DataOpt, 0, sizeof(stSm2DataOpt));
        stSm2DataOpt.wKeyId = EXWORDHL(uEppSKeyNo);
        stSm2DataOpt.byKeyType = 0x53;
        uLen = sizeof(stSm2DataOpt.byData);
        CAutoHex::Hex2Bcd(lpCipherTextKeyVal, stSm2DataOpt.byData, uLen);
        uLen = sizeof(stSm2DataOpt) - (sizeof(stSm2DataOpt.byData) - uLen);
        vtCmd.append((LPSTR)&stSm2DataOpt, uLen);
        lRet = SendReadCmd(SM2_DATA_ENC_OR_DENC, vtCmd, vtResult);
        if (lRet != ERR_SUCCESS)
        {
            Log(ThisModule, __LINE__, "SM2私钥解密失败：lRet=%d[uDKeyNo=%02X]", lRet, uEppSKeyNo);
            return lRet;
        }

        //下载SM4 Key
        CAutoHex autoHex((LPBYTE)vtResult.constData(), vtResult.size());
        lRet = SM4ImportWKey(uKeyNo, 0xFF, uKeyUse, autoHex.GetHex(), lpKCV);
        if(lRet != ERR_SUCCESS){
            return lRet;
        }
    } else {
        // 下载密文主密钥,该方法返回0x31
        vtResult.clear();
        IMPORTSM4KEYENCBYSM2 stImportSm4KeyEncBySm2;
        memset(&stImportSm4KeyEncBySm2, 0, sizeof(stImportSm4KeyEncBySm2));
        stImportSm4KeyEncBySm2.wSM4KeyId = EXWORDHL(uKeyNo);
        stImportSm4KeyEncBySm2.wSM2KeyId = EXWORDHL(uEppSKeyNo);
        stImportSm4KeyEncBySm2.dwKeyAttr = RVSDWORD(uKeyUse);
        uLen = sizeof(stImportSm4KeyEncBySm2.byKeyValue);
        CAutoHex::Hex2Bcd(lpCipherTextKeyVal, stImportSm4KeyEncBySm2.byKeyValue, uLen);
        uLen = sizeof(stImportSm4KeyEncBySm2) - (sizeof(stImportSm4KeyEncBySm2.byKeyValue) - uLen);
        vtCmd.append((LPSTR)&stImportSm4KeyEncBySm2, uLen);

        lRet = SendReadCmd(SM4_KEY_ENC_BY_SM2_LOAD, vtCmd, vtResult);
        if (lRet != ERR_SUCCESS)
        {
            Log(ThisModule, __LINE__, "SM主密钥导入失败：lRet=%d[uMKeyNo=%02X]", lRet, uKeyNo);
            return lRet;
        }

        //获取国密密钥KCV
        lRet = ReadSymmKeyKCV(uKeyNo, 2, lpKCV);
        if(lRet != ERR_SUCCESS) {
            Log(ThisModule, __LINE__, "获取国密密钥KCV失败:=%d[uMKeyNo=%02X]", lRet, uKeyNo);
            return lRet;
        }
    }
...
long CDevPIN_XZF35::SM4CryptData(UINT uKeyNo, UINT uMode, LPCSTR lpCryptData, BYTE bPadding, LPCSTR lpIVData, QByteArray &vtResultData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_XZF35::SM4MACData(UINT uKeyNo, UINT uMode, LPCSTR lpMacData, LPCSTR lpIVData, QByteArray &vtResultData, BYTE byPad/* = 0xFF*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_XZF35::SM2EncryptData(UINT uPKeyNo, LPCSTR lpData, LPSTR lpCryptData)
{

}

long CDevPIN_XZF35::SM2DecryptData(UINT uSKeyNo, LPCSTR lpCryptData, LPSTR lpData)
{

}

long CDevPIN_XZF35::SM2SignData(UINT uSKeyNo, LPCSTR lpZA, LPCSTR lpData, LPSTR lpSignData)
{

}

long CDevPIN_XZF35::SM2VerifySign(UINT uPKeyNo, LPCSTR lpZA, LPCSTR lpData, LPCSTR lpSignData)
{
...
long CDevPIN_XZF35::SM3CaculateZAData(LPCSTR lpUserData, UINT uPKNum, QByteArray &resultData)
{
...
long CDevPIN_XZF35::RSAImportRootHostPK(UINT uPKeyNo, UINT uSignKeyNo, int iSignAlgo, QByteArray strHostPKData, QByteArray strSignData)
{
...
long CDevPIN_XZF35::RSAExportEppEncryptPK(UINT uEppPKeyNo, UINT uEppSKeyNo, RSASIGNALGOTYPE &eRSASignAlgo,
...
long CDevPIN_XZF35::RSAExportEppSN(UINT uEppSKeyNo, RSASIGNALGOTYPE &eRSASignAlgo, QByteArray &strSN, QByteArray &strSignSN)
{
...
long CDevPIN_XZF35::RSAImportDesKey(UINT uKeyNo, UINT uEppSKeyNo, UINT uHostPKeyNo, RSADATAALGOTYPE eRSAEncAlgo,
                                    RSASIGNALGOTYPE eRSASignAlgo, QByteArray strKeyVal, QByteArray strSignVal,
...
long CDevPIN_XZF35::RSAGenerateKeyPair(UINT uPKeyNo, UINT uSKeyNo, int iModuleLen, int iExponentValue)
{

}

//30-00-00-00(FS#0006)
long CDevPIN_XZF35::RSAEncryptData(UINT uRSAPKeyNo, RSADATAALGOTYPE eRSAEncAlgo, QByteArray data, QByteArray &result)
{
    return ERR_SUCCESS;
}

//30-00-00-00(FS#0006)
long CDevPIN_XZF35::RSADecryptData(UINT uRSASKeyNo, RSADATAALGOTYPE eRSAEncAlgo, QByteArray data, QByteArray &result)
{
    return ERR_SUCCESS;
}

long CDevPIN_XZF35::SetDesKeyLenInfo(map<UINT, BYTE> &keyIdMapKeyLen)
{
    return ERR_SUCCESS;
}
...
void CDevPIN_XZF35::ConvertFDKEnableBitData(DWORD dwActiveFDKKey, DWORD &dwActiveFDKKeyConvert)
{
    dwActiveFDKKeyConvert = 0;
    //F1->F1 F2->F8 F3->F2 F4->F7 F5->F4 F6->F5 F7->F3 F8->F6
 ...
 */
long CDevPIN_XZF35::ReadSymmKeyKCV(UINT uKeyNo, BYTE byKcvMode, LPSTR lpKcv, BYTE bySymmKeyType/* = 0*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    LOGDEVACTION();
...
void CDevPIN_XZF35::ReadConfig()
{
    ...
    m_iImportSm4KeyEncBySm2Mode = iniReader.GetValue("ImportSm4KeyEncBySm2Mode", (DWORD)0);

    return;
}
//30-00-00-00(FS#0003) add end

// History = 2021/04/18,SP  30-00-00-00,CFES(Guojy): 30-00-00-00(FS#0003) for 光大远程密钥
long CDevPIN_XZF35::SM2GenerateKey(UINT uSKeyNo, UINT uPKeyNo)
{
    ...
    //检查密钥索引(0~63)
    if(uSKeyNo >= DES_OR_SM4_KEY_MAX_NUM){
        Log(ThisModule, __LINE__, "无效密钥ID：uKeyNo=%02X", uSKeyNo);
        return ERR_PARAM;
    }
...
//    WORD wEncKeyNo = (uMKeyNo == 0xFF) ? 0xFFFF : uMKeyNo;
 ...
 //   dwKeyUse |= KEY_ATTR_VECTOR_DECRYPT;
    ...
    stImportSm2KeyPair.byKeyAttr = 0x03;                //功能密钥属性
    vtCmd.append((LPSTR)&stImportSm2KeyPair, sizeof(stImportSm2KeyPair));
    lRet = SendReadCmd(GENERATE_SM2_KEY, vtCmd, vtResult);   //下载SM2 密钥对
    if (lRet != ERR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "密钥对导入失败：lRet=%d[uSKeyNo=%02X]", lRet, uSKeyNo);
        return lRet;
    }
    Log(ThisModule, 1, "密钥对导入成功:uSKeyNo=%04X", uSKeyNo);
    return ERR_SUCCESS;
}

// History = 2021/04/18,SP  30-00-00-00,CFES(Guojy): 30-00-00-00(FS#0003) for 光大远程密钥
long CDevPIN_XZF35::SM2ExportPKeyForGD(UINT uKeyNo, LPSTR lpData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    stExportSM2EppSignKey.byKeyAttr = 0x50;
    vtCmd.append((LPSTR)&stExportSM2EppSignKey, sizeof(stExportSM2EppSignKey));
    lRet = SendReadCmd(EXPORT_SM2_KEY, vtCmd, vtResult);   //导出SM2公钥
    if (lRet != ERR_SUCCESS)
    {
