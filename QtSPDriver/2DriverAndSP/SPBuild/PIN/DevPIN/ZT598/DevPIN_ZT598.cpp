...
long CDevPIN_ZT598::PinInput(UINT uMinLen/* = 4*/, UINT uMaxLen/* = 6*/, bool bClearKeyAsBackspace/* = false*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
//        }
        ...
        if(m_iClearKeyMode == 1){
            bClearKeyAsBackspace = true;
        }

        lRet = SetClearKeyFunc(bClearKeyAsBackspace);
        if(lRet != ERR_SUCCESS){
            break;
        }
...
//30-00-00-00(FS#0010)        QByteArray vtCmd("\x36\x30\x30\x30\x31\x32");
        ...
        if(bClearKeyAsBackspace){                        //30-00-00-00(FS#0010)
...
long CDevPIN_ZT598::DataCrypt(UINT uKeyNo, UINT uMode, LPCSTR lpCryptData, BYTE bPadding, LPCSTR lpIVData, QByteArray &vtResultData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    vtResultData = vtResultTotal;
    return ERR_SUCCESS;
}

long CDevPIN_ZT598::DataMAC(UINT uKeyNo, UINT uMode, LPCSTR lpMacData, LPCSTR lpIVData, QByteArray &vtResultData, BYTE byPad/* = 0xFF*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_ZT598::ReadSymmKeyKCV(UINT uKeyNo, BYTE byKcvMode, LPSTR lpKcv, BYTE bySymmKeyType/* = 0*/)
{
    return ERR_SUCCESS;
}

//30-00-00-00(FS#0003)
long CDevPIN_ZT598::SM2ExportPKeyForGD(UINT uKeyNo, LPSTR lpData)
{
...
long CDevPIN_ZT598::SM4ImportMKey(UINT uMKeyNo, LPCSTR lpKeyVal, LPSTR lpKCV, UINT uEncKeyNo/* = 0xFF*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_ZT598::SM4CryptData(UINT uKeyNo, UINT uMode, LPCSTR lpCryptData, BYTE bPadding, LPCSTR lpIVData, QByteArray &vtResultData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_ZT598::SM4MACData(UINT uKeyNo, UINT uMode, LPCSTR lpMacData, LPCSTR lpIVData, QByteArray &vtResultData, BYTE byPad/* = 0xFF*/)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_ZT598::SM4PinBlock(UINT uKeyNo, UINT uFormat, LPCSTR lpCustomerData, BYTE bPadding, LPSTR lpPinBlock)
{
    ...
    if (lRet != ERR_SUCCESS)
    {
        if(lRet == -0xEF){                  //30-00-00-00(FT#0022)
            lRet = ERR_NO_PIN;              //30-00-00-00(FT#0022)
        }                                   //30-00-00-00(FT#0022)
...
long CDevPIN_ZT598::SM2ImportKey(UINT uKeyNo, UINT uVendorPKeyNo, BOOL bPublicKey, LPCSTR lpZA, LPCSTR lpKeyVal, LPCSTR lpSignKeyVal)
{
    ...
    if((bPublicKey && strlen(lpKeyVal) != 128) ||
       (!bPublicKey && strlen(lpKeyVal) != 64)){
        return ERR_PARAM;
    }
    ...
    vtCmd.resize(7);
    sprintf(vtCmd.data(), "\x66%02X%02X%02X", uVendorPKeyNo, uKeyNo, bPublicKey ? 0x01 : 0x00);
    if(lpZA == nullptr){
        vtCmd.append(64, '0');
    } else {
        vtCmd.append(lpZA);
...
//30-00-00-00(FS#0005)
long CDevPIN_ZT598::SM2EncryptData(UINT uPKeyNo, LPCSTR lpData, LPSTR lpCryptData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    vtCmd.resize(3);
    sprintf(vtCmd.data(), "\x68%02X", uPKeyNo);
    vtCmd.append(lpData);
    ...
    memcpy(lpCryptData, vtResult.constData(), vtResult.size());
    return ERR_SUCCESS;
}

//30-00-00-00(FS#0005)
long CDevPIN_ZT598::SM2DecryptData(UINT uSKeyNo, LPCSTR lpCryptData, LPSTR lpData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    vtCmd.resize(3);
    sprintf(vtCmd.data(), "\x69%02X", uSKeyNo);
...
//30-00-00-00(FS#0005)
long CDevPIN_ZT598::SM2SignData(UINT uSKeyNo, LPCSTR lpZA, LPCSTR lpData, LPSTR lpSignData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    vtCmd.resize(3);
    sprintf(vtCmd.data(), "\x6A%02X", uSKeyNo);
    vtCmd.append(lpZA);
    vtCmd.append(lpData);
    ...
    memcpy(lpSignData, vtResult.constData(), vtResult.size());
    return ERR_SUCCESS;
}

long CDevPIN_ZT598::SM2VerifySign(UINT uPKeyNo, LPCSTR lpZA, LPCSTR lpData, LPCSTR lpSignData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
long CDevPIN_ZT598::SM2ExportPKey(UINT uKeyNo, UINT uSignKeyNo, LPSTR lpZA, LPSTR lpSM2PKeyData, LPSTR lpSignData)
{
    ...
    vtCmd.resize(5);
    sprintf(vtCmd.data(), "\x71%02X%02X", uKeyNo, uSignKeyNo);
    long lRet = SendReadCmd(vtCmd, vtResult);
    if (lRet != ERR_SUCCESS)
    {
    ...
    memcpy(lpSM2PKeyData, vtResult.constData() + 64, 128);
    if(uSignKeyNo != 0xFF){
        memcpy(lpZA, vtResult.constData(), 64);
        memcpy(lpSignData, vtResult.constData() + 64 + 128, 128);
    }
    return ERR_SUCCESS;
}
...
long CDevPIN_ZT598::SM2GenerateKey(UINT uSKeyNo, UINT uPKeyNo)
{
    ...
    vtCmd.resize(5);
    sprintf(vtCmd.data(), "\x67%02X%02X", uSKeyNo, uPKeyNo);
    long lRet = SendReadCmd(vtCmd, vtResult);
    if (lRet != ERR_SUCCESS)
    {
...
//30-00-00-00(FS#0005)
long CDevPIN_ZT598::SM3CaculateZAData(LPCSTR lpUserData, UINT uPKNum, QByteArray &resultData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    vtCmd.resize(5);
    sprintf(vtCmd.data(), "\x7B%02X%02X", 0x30, uPKNum);
    vtCmd.append(128, '0');
    vtCmd.append(lpUserData);
    long lRet = SendReadCmd(vtCmd, vtResult);
    if (lRet != ERR_SUCCESS)
    {
...
long CDevPIN_ZT598::SM4SaveKeyType(UINT uType)
{
    ...
    QByteArray vtCmd;
    vtCmd.resize(5);
    sprintf(vtCmd.data(), "\x35\x44\x33%x", nCmd);
    vtCmd.chop(1);
    long lRet = SendReadCmd(vtCmd, vtResult);
    if (lRet != ERR_SUCCESS)
    {
...
long CDevPIN_ZT598::RSAImportRootHostPK(UINT uPKeyNo, UINT uSignKeyNo, int iSignAlgo, QByteArray strHostPKData, QByteArray strSignData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    WORD wModLen = vtKeyData.size() + 1;        //Modulus Len (With prefixed 0x00)
...
//    } else {
        ...
        *lpBuffer = (BYTE)uPKeyNo;
        lpBuffer += 1;
        *lpBuffer = (uSignKeyNo == 0xFF) ? 0x25 : uSignKeyNo;
        lpBuffer += 1;
//    }

    BYTE bySignAlgo = 0x30;
    if(iSignAlgo == RSA_SIGN_SSA_PKCS1_V1_5){
        bySignAlgo = 0x31;
    } else if(iSignAlgo == RSA_SIGN_SSA_PSS){
        bySignAlgo = 0x32;
    }
    memset( lpBuffer, bySignAlgo, 1);
    lpBuffer += 1;
    //Create Tag
    ...
    if(bPublicKeyValueType == FALSE){
        ...
        vtKeyData.push_front((char)0x00);                     //prefixed with 0x00
        memcpy( lpBuffer, vtKeyData.constData(), vtKeyData.size() );
        lpBuffer += vtKeyData.size();
...
long CDevPIN_ZT598::RSAExportEppEncryptPK(UINT uEppPKeyNo, UINT uEppSKeyNo, RSASIGNALGOTYPE &eRSASignAlgo,
...
long CDevPIN_ZT598::RSAExportEppSN(UINT uEppSKeyNo, RSASIGNALGOTYPE &eRSASignAlgo, QByteArray &strSN, QByteArray &strSignSN)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    vtCmd.push_back(uEppSKeyNo);
    vtCmd.push_back(bySignAlgo);
    vtCmd.push_back(byDigestAlgo);
...
long CDevPIN_ZT598::RSAExportEppRandData(QByteArray &strRandData)
{
    ...
    // 随机数(只需要８位随机数)
    strRandData.append((LPSTR)vtResult.constData(), 16);
...
//30-00-00-00(FS#0006)
long CDevPIN_ZT598::RSAImportDesKey(UINT uKeyNo, UINT uEppSKeyNo, UINT uHostPKeyNo,
...
//30-00-00-00(FS#0006)
long CDevPIN_ZT598::RSAGenerateKeyPair(UINT uPKeyNo, UINT uSKeyNo, int iModuleLen, int iExponentValue)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
...
//30-00-00-00(FS#0006)
long CDevPIN_ZT598::RSAEncryptData(UINT uRSAPKeyNo, RSADATAALGOTYPE eRSAEncAlgo, QByteArray data, QByteArray &result)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    vtCmd.push_back(byEncAlgo);
    vtCmd.append(data);

    long lRet = SendReadCmd(vtCmd, vtResult);
    if (lRet != ERR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "RSA加密数据失败：lRet=%d, PKeyNo:%d", lRet, uRSAPKeyNo);
        return lRet;
    }

    result.append(vtResult);
    return ERR_SUCCESS;
}

//30-00-00-00(FS#0006)
long CDevPIN_ZT598::RSADecryptData(UINT uRSASKeyNo, RSADATAALGOTYPE eRSAEncAlgo, QByteArray data, QByteArray &result)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    vtCmd.push_back(0x84);
    vtCmd.push_back(uRSASKeyNo);
    BYTE byEncAlgo = 0x30;
    switch(eRSAEncAlgo){
    case RSA_ENC_PKCS1_V1_5:
        byEncAlgo = 0x30;
    ...
    if (lRet != ERR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "RSA解密数据失败：lRet=%d, PKeyNo:%d", lRet, uRSASKeyNo);
        return lRet;
    }
...
long CDevPIN_ZT598::SetDesKeyLenInfo(map<UINT, BYTE> &keyIdMapKeyLen)
{
    return ERR_SUCCESS;
}
...
//30-00-00-00(FS#0005)    if (byRTN < 0x60 || (byRTN > 0x9F && byRTN != 0xA3))
    if (byRTN < 0x60 || (byRTN > 0x9F && byRTN != 0xA3 && byRTN != 0xA0 && byRTN != 0xA1 && byRTN != 0xAB &&
        !(byRTN >= 0xB0 && byRTN <= 0xB7)))      //30-00-00-00(FS#0005)//30-00-00-00(FS#0006)
    {
        if((m_byCmdCode == 0x37 || m_byCmdCode == 0x6F) && (byRTN == 0xEF)){

        } else {
            UpdateStatus(DEVICE_HWERROR, byRTN);
        }
        return ConvertErrCode(byRTN, ThisModule);
    }

    if(m_byCmdCode != 0x49){                  //排除取状态命令
        UpdateStatus(DEVICE_ONLINE, 0);
    }
...
long CDevPIN_ZT598::SetPinInputLen(UINT uMinLen /*= 0*/, UINT uMaxLen /*= 12*/)
{
    ...
    //长度大于9时的特殊处理
    if(uMaxLen > 9){                    //30-00-00-00(FT#0025)
        uMaxLen += 6;                   //30-00-00-00(FT#0025)
    }                                   //30-00-00-00(FT#0025)
    if(uMinLen > 9){                    //30-00-00-00(FT#0025)
        uMinLen += 6;                   //30-00-00-00(FT#0025)
    }                                   //30-00-00-00(FT#0025)

    QByteArray vtResult;
    QByteArray vtCmd;
    vtCmd.resize(5);
    sprintf(vtCmd.data(), "\x35\x34%02X", uMinLen);             //30-00-00-00(FT#0025)
    vtCmd.chop(1);
    long lRet = SendReadCmd(vtCmd, vtResult);
    if (lRet != ERR_SUCCESS)
    {
    ...
    vtCmd.resize(5);
    sprintf(vtCmd.data(), "\x35\x35%02X", uMaxLen);             //30-00-00-00(FT#0025)
    vtCmd.chop(1);
    lRet = SendReadCmd(vtCmd, vtResult);
    if (lRet != ERR_SUCCESS)
    {
...
long CDevPIN_ZT598::BuildAsn1Tag(WORD wTagType, WORD wDataLen, LPWORD lpwTagLen, LPBYTE lpbyTagBuffer)
{
    long lRet = -1;
...
long CDevPIN_ZT598::ExtractAsn1Data( WORD wFieldLen, LPBYTE lpbyField, WORD wTagType,
...
void CDevPIN_ZT598::ReadConfig()
{
    ...
    m_iIllegalHoldKeyTime = (int)cIR.GetValue("IllegalHoldKeyTime", 0);

    return;
}

//30-00-00-00(FS#0010)
long CDevPIN_ZT598::SetClearKeyFunc(bool bClearKeyAsBackspace)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    ...
    if (lRet != ERR_SUCCESS)
    {
        Log(ThisModule, __LINE__, "设置CLEAR键功能[%s]失败", bClearKeyAsBackspace ? "BACKSPACE" : "CLEAR");
        return lRet;
    }
...
bool CDevPIN_ZT598::SetIlligalHoldKeyTime(int iSecond)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();

    LOGDEVACTION();
    if(iSecond < 0 ||iSecond > 15){
        Log(ThisModule, __LINE__, "设置非法长按键时间值非法：%d", iSecond);
        return false;
    }

    QByteArray vtResult;
    QByteArray vtCmd("\x35\x41\x31");

    char cTime[2] = {0};
    sprintf(cTime, "%X", iSecond);
    vtCmd.push_back(cTime);
