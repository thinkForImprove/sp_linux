...
/*
 * @func   获取非对称密钥的密钥号
 * @param  iKeyType:4-SM2密钥 5-RSA密钥
 * @return
 */
PINKEYDATA CKeySlotHelperXZ::FindAsymmKeySlot(QByteArray strName, int iKeyType/* = 4*/)
{
    AutoMutex(m_cMutex);
    PINKEYDATA stKey;
    if (FindAsymmKeyByName(strName, stKey))
        return stKey;

    strcpy(stKey.szName, strName);
    stKey.uNum = KEY_NOT_NUM;
    stKey.dwType = iKeyType;

    mapUUINT vtKeyNum;
    for (UINT uNum = 0; uNum < XZ_ASYMMETRIC_KEY_MAX; uNum++)
    {
        vtKeyNum[uNum] = uNum;
    }
    for (auto &it : m_ltKeyData)
    {
        if(it.dwType == iKeyType){
            if (vtKeyNum.find(it.uNum) != vtKeyNum.end())
                vtKeyNum.erase(it.uNum);
        }
    }
...
//30-00-00-00（FS#0003）
PINKEYDATA CKeySlotHelperXZ::FindAsymmKey(QByteArray strName, int iKeyType/* = 4*/)
{
    AutoMutex(m_cMutex);
    PINKEYDATA stKey;
    if (!FindAsymmKeyByName(strName, stKey, iKeyType)){
        stKey.uNum = KEY_NOT_NUM;
    }
...
//30-00-00-00（FS#0003）
long CKeySlotHelperXZ::DeleteAsymmKey(QByteArray strName, int iKeyType/* = 4*/)
{
    AutoMutex(m_cMutex);
    for (auto it = m_ltKeyData.begin(); it != m_ltKeyData.end(); it++)
    {
        if (it->dwType == iKeyType && qstricmp(strName.constData(), it->szName) == 0)
        {
            m_ltKeyData.erase(it);
            return SaveKeyFile();
        }
...
//30-00-00-00（FS#0003）
void CKeySlotHelperXZ::GetAllAsymmKey(listPINKEYDATA &ltKeyData)
{
    AutoMutex(m_cMutex);
    ltKeyData.clear();
    for(auto &it : m_ltKeyData){
...
//30-00-00-00（FS#0003）
bool CKeySlotHelperXZ::FindAsymmKeyByName(QByteArray strName, PINKEYDATA &stKey, int iKeyType/* = 4*/)
{
    for (auto &it : m_ltKeyData)
    {
        if (it.dwType == iKeyType && qstricmp(strName, it.szName) == 0)
        {
            memcpy(&stKey, &it, sizeof(PINKEYDATA));
            return true;
        }
...
/*-----------------------------------CKeySlotHelper C90---------------------------*/
CKeySlotHelperC90::CKeySlotHelperC90()
{
    ReadKeyFile();
}
...
//查找主密钥
PINKEYDATA CKeySlotHelperC90::FindMKeySlot(QByteArray strName, LPCSTR strEncKeyName/* = ""*/, bool bIsDesKey/* = true*/)
{
    AutoMutex(m_cMutex);
    PINKEYDATA stKey;
    if (FindKeyByName(strName, stKey, bIsDesKey))
    ...
    {
        if (vtKeyNum.find(it.uNum) != vtKeyNum.end()){
            if((bIsDesKey && it.dwType == 0) ||
               (!bIsDesKey && it.dwType == 2)){
                 vtKeyNum.erase(it.uNum);
            }
        }
    }
...
//查找工作密钥
PINKEYDATA CKeySlotHelperC90::FindWKeySlot(QByteArray strName, bool bIsDesKey/* = true*/, int iKeyType/* = PKU_CRYPT*/)
{
    AutoMutex(m_cMutex);
    PINKEYDATA stKey;
    if (FindKeyByName(strName, stKey, bIsDesKey))
    ...
    {
        if (vtKeyNum.find(it.uNum) != vtKeyNum.end()){
            if((bIsDesKey && it.dwType == 1) ||
               (!bIsDesKey && it.dwType == 3)){
                vtKeyNum.erase(it.uNum);
            }
        }
    }
...
//查找密钥统一接口
PINKEYDATA CKeySlotHelperC90::FindKey(QByteArray strName, bool bIsDesKey/* = true*/)
{
    AutoMutex(m_cMutex);
    QByteArray strNewName;
    PINKEYDATA stKey;
...
void CKeySlotHelperC90::AddKey(const PINKEYDATA &stKeyData)
{
    AutoMutex(m_cMutex);
    for (auto &it : m_ltKeyData)
    {
...
long CKeySlotHelperC90::DeleteKey(QByteArray strName, bool bIsDesKey/* = true*/)
{
    AutoMutex(m_cMutex);
    int nPos = 0;
    QByteArray strNewName;
...
long CKeySlotHelperC90::DeleteAllKey()
{
    AutoMutex(m_cMutex);
    // 清空缓存
    m_ltKeyData.clear();
...
bool CKeySlotHelperC90::IsInvalidKey(const PINKEYDATA &stKeyData)
{
    return (stKeyData.uNum == KEY_NOT_NUM) ? true : false;
}

bool CKeySlotHelperC90::IsNewKey(const PINKEYDATA &stKeyData)
{
    if (stKeyData.uNum == KEY_NOT_NUM)
        return false;
    return (stKeyData.bNewKey) ? true : false;
...
bool CKeySlotHelperC90::IsMaxKey(const PINKEYDATA &stKeyData)
{
    return (stKeyData.uNum == KEY_NOT_NUM) ? true : false;
}

bool CKeySlotHelperC90::IsMKey(const PINKEYDATA &stKeyData)
{
    if (stKeyData.uNum == KEY_NOT_NUM)
        return false;
    return (stKeyData.dwType % 2 == 0) ? true : false;
...
bool CKeySlotHelperC90::IsWKey(const PINKEYDATA &stKeyData)
{
    if (stKeyData.uNum == KEY_NOT_NUM)
        return false;
    return (stKeyData.dwType % 2 != 0) ? true : false;
...
bool CKeySlotHelperC90::IsSMKey(const PINKEYDATA &stKeyData)
{
    if (stKeyData.uNum == KEY_NOT_NUM)
        return false;
    return (stKeyData.dwType > 1) ? true : false;
...
void CKeySlotHelperC90::GetAllKey(listPINKEYDATA &ltKeyData)
{
    AutoMutex(m_cMutex);
    QByteArray strName;
    ltKeyData.clear();
...
bool CKeySlotHelperC90::IsEmpty()
{
    AutoMutex(m_cMutex);
    return m_ltKeyData.empty();
}

bool CKeySlotHelperC90::IsAllMKey()
{
    AutoMutex(m_cMutex);
    for (auto &it : m_ltKeyData)
    {
...
//30-00-00-00(FS#0005)
/*
 *@param bEppPKey:false Host公钥
 *                true  EPP公钥
 *
 */
PINKEYDATA CKeySlotHelperC90::FindSM2KeySlot(QByteArray strName, bool bEppPKey/* = true*/)
{
    AutoMutex(m_cMutex);
    PINKEYDATA stKey;
    if (FindSM2Key(strName, stKey))
    ...
    mapUUINT vtKeyNum;
    UINT uKeyIdBase = bEppPKey ? 0x00 : 0x30;
    UINT uKeyIdMax = bEppPKey ? 0x1F : 0x3F;
    for (UINT uNum = uKeyIdBase; uNum <= uKeyIdMax; uNum++)
    {
        vtKeyNum[uNum] = uNum;
    }
...
//查找SM2密钥
//30-00-00-00(FS#0005)
bool CKeySlotHelperC90::FindSM2Key(QByteArray strName, PINKEYDATA &stKey)
{
    for (auto &it : m_ltKeyData)
    {
        if(it.dwType == 4){
...
//删除SM2密钥
//30-00-00-00(FS#0005)
long CKeySlotHelperC90::DeleteSM2Key(QByteArray strName)
{
    AutoMutex(m_cMutex);
    for (auto it = m_ltKeyData.begin(); it != m_ltKeyData.end(); it++)
    {
...
void CKeySlotHelperC90::GetAllSM2Key(listPINKEYDATA &ltKeyData)
{
    AutoMutex(m_cMutex);
    ltKeyData.clear();
    for(auto &it : m_ltKeyData){
...
PINKEYDATA CKeySlotHelperC90::FindRSAKeySlot(QByteArray strName, bool bPkey/* = true*/)
{
    AutoMutex(m_cMutex);
    PINKEYDATA stKey;
    if (FindRSAKey(strName, stKey))
...
bool CKeySlotHelperC90::FindRSAKey(QByteArray strName, PINKEYDATA &stKey)
{
    for (auto &it : m_ltKeyData)
    {
        if(it.dwType == 5){
...
long CKeySlotHelperC90::DeleteRSAKey(QByteArray strName)
{
    AutoMutex(m_cMutex);
    for (auto it = m_ltKeyData.begin(); it != m_ltKeyData.end(); it++)
    {
...
std::map<UINT, BYTE>& CKeySlotHelperC90::GetDesKeyLenInfo()
{
    AutoMutex(m_cMutex);
    m_keyIdMapKeyLen.clear();
    for(auto &it : m_ltKeyData){
        if(it.dwType == 0 || it.dwType == 1){
            m_keyIdMapKeyLen[it.uNum] = it.dwKeyLen;
        }
    }
    return m_keyIdMapKeyLen;
}

long CKeySlotHelperC90::ReadKeyFile()
{
    // 读取文件
    m_ltKeyData.clear();
    fstream cFile(KEYSAVEFILE, ios::in | ios::binary);
...
long CKeySlotHelperC90::SaveKeyFile()
{
    // 每次都是新文件
    fstream cFile(KEYSAVEFILE, ios::out | ios::trunc | ios::binary);
    if (!cFile.is_open())
...
void CKeySlotHelperC90::XOR(const char *pX, const char *pR, UINT uLen, char *pXOR)
{
    for (UINT i = 0; i < uLen; i++)
    {
        pXOR[i] = pX[i] ^ pR[i];
...
bool CKeySlotHelperC90::FindKeyByName(QByteArray strName, PINKEYDATA &stKey, bool bIsDesKey/* = true*/)
{
    for (auto &it : m_ltKeyData)
    {
        if((bIsDesKey && it.dwType <= 1) || (!bIsDesKey && it.dwType > 1)){
