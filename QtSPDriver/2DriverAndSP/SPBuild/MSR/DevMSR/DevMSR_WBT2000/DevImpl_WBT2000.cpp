        ...
        if (strlen(stOpenMode.szDevPath) < 1)
        {
            ...
            if (m_DeviceId <= 0)
            {
                Log(ThisModule, __LINE__,
                    ...
                    nullptr, 0, m_DeviceId, ConvertErrCodeToStr(IMP_ERR_NOTOPEN));
                return IMP_ERR_NOTOPEN;
            } else
            {
                Log(ThisModule, __LINE__,
        ...
        } else
        {
            ...
            if (m_DeviceId <= 0)
            {
                Log(ThisModule, __LINE__, "打开指定设备端口(串口): ->OpenPort(%s, %d) fail. DeviceId: %d, Return: %s",
                    stOpenMode.szDevPath, stOpenMode.wBaudRate, m_DeviceId, ConvertErrCodeToStr(IMP_ERR_NOTOPEN));
                return IMP_ERR_NOTOPEN;
            } else
            {
...
INT CDevImpl_WBT2000::nReset()
{
    ...
    if (nRet != IMP_SUCCESS)
    {
        Log(ThisModule, __LINE__, "复位设备: Reset(%d) fail. Return: %s",
            m_DeviceId, ConvertErrCodeToStr(nRet));
        return nRet;
    }
...
BOOL CDevImpl_WBT2000::nGetFWVersion(LPSTR lpFwVer)
{
    ...
    if (nRet != IMP_SUCCESS)
    {
        Log(ThisModule, __LINE__, "获取固件版本: GetFirmVer(%d, %s) fail. Return: %s",
            m_DeviceId, lpFwVer, ConvertErrCodeToStr(nRet));
        return nRet;
    }
                                  ...
                                  LPINT lpInOutLen3, INT nTimeOut)
{
    ...
    if (nRet != IMP_SUCCESS)
    {
        Log(ThisModule, __LINE__,
            ...
            lpTrack3, *lpInOutLen3, nTimeOut, ConvertErrCodeToStr(nRet));
        return nRet;
    }
...
LPSTR CDevImpl_WBT2000::ConvertErrCodeToStr(INT nErrCode)
{
    memset(m_szErrStr, 0x00, sizeof(m_szErrStr));
    switch(nErrCode)
    {
