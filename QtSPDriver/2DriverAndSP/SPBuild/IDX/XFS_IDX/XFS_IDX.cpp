...
HRESULT CXFS_IDX::ReadRawData(LPWORD lpReadData, LPWFSIDCCARDDATA *&lppCardData, DWORD dwTimeOut)
{
    ...
    //if ((wOption & 0x803F) == 0)
    if (m_stIdxIniConfig.wBankNo == 2)  // 陕西信合入参和通用不一致                 // 40-00-00-00(FT#0010)
    {                                                                           // 40-00-00-00(FT#0010)
        DWORD nTrack = WFS_IDC_CHIP | WFS_IDC_FRONTIMAGE | WFS_IDC_BACKIMAGE;   // 40-00-00-00(FT#0010)
        if ((wOption & nTrack) == 0)                                            // 40-00-00-00(FT#0010)
        {                                                                       // 40-00-00-00(FT#0010)
            Log(ThisModule, -1, "接收ReadRawData参数无效");                       // 40-00-00-00(FT#0010)
            return WFS_ERR_INVALID_DATA;                                        // 40-00-00-00(FT#0010)
        }                                                                       // 40-00-00-00(FT#0010)
    } else  // 通用参数                                                          // 40-00-00-00(FT#0010)
    {                                                                           // 40-00-00-00(FT#0010)
...
long CXFS_IDX::ReadTrackData(DWORD dwReadOption)
{
    ...
    INT             nReadMode = RT_DEF;                                         // 40-00-00-00(FT#0012)
    // ReadRawData前先将磁道初始化避免KAL出错崩溃
    if (m_stIdxIniConfig.wBankNo == 2)  // 陕西信合入参和通用不一致                 // 40-00-00-00(FT#0010)
    {                                                                           // 40-00-00-00(FT#0010)
        if (dwReadOption & WFS_IDC_CHIP)                                        // 40-00-00-00(FT#0010)
        {                                                                       // 40-00-00-00(FT#0010)
            SetTrackInfo(WFS_IDC_CHIP, WFS_IDC_DATAINVALID, 0, NULL);           // 40-00-00-00(FT#0010)
            nReadMode = (nReadMode | RT_CHIP);                                  // 40-00-00-00(FT#0012)
        }                                                                       // 40-00-00-00(FT#0010)
        if (dwReadOption & WFS_IDC_FRONTIMAGE)                                  // 40-00-00-00(FT#0010)
        {                                                                       // 40-00-00-00(FT#0010)
            SetTrackInfo(WFS_IDC_FRONTIMAGE, WFS_IDC_DATAINVALID, 0, NULL);     // 40-00-00-00(FT#0010)
            nReadMode = (nReadMode | RT_FRONTIMAGE);                            // 40-00-00-00(FT#0012)
        }                                                                       // 40-00-00-00(FT#0010)
        if (dwReadOption & WFS_IDC_BACKIMAGE)                                   // 40-00-00-00(FT#0010)
        {                                                                       // 40-00-00-00(FT#0010)
            SetTrackInfo(WFS_IDC_BACKIMAGE, WFS_IDC_DATAINVALID, 0, NULL);      // 40-00-00-00(FT#0010)
            nReadMode = (nReadMode | RT_BACKIMAGE);                             // 40-00-00-00(FT#0012)
        }                                                                       // 40-00-00-00(FT#0010)
    } else  // 通用                                                              // 40-00-00-00(FT#0010)
    {
        if (dwReadOption & WFS_IDC_TRACK1)
        {
            SetTrackInfo(WFS_IDC_TRACK1, WFS_IDC_DATAINVALID, 0, NULL);
            nReadMode = (nReadMode | RT_TRACK1);                                // 40-00-00-00(FT#0012)
        }
        if (dwReadOption & WFS_IDC_TRACK2)
        {
            SetTrackInfo(WFS_IDC_TRACK2, WFS_IDC_DATAINVALID, 0, NULL);
            nReadMode = (nReadMode | RT_TRACK2);                                // 40-00-00-00(FT#0012)
        }
        if (dwReadOption & WFS_IDC_TRACK3)
        {
            SetTrackInfo(WFS_IDC_TRACK3, WFS_IDC_DATAINVALID, 0, NULL);
            nReadMode = (nReadMode | RT_TRACK3);                                // 40-00-00-00(FT#0012)
        }
    }                                                                           // 40-00-00-00(FT#0010)

    int nRet = m_pDev->ReadTracks(trackInfo, nReadMode);                        // 40-00-00-00(FT#0012)
    if (nRet == 0)
    {
        ...
        {                                                                       // 40-00-00-00(FT#0010)
            if (dwReadOption & WFS_IDC_CHIP)                                    // 40-00-00-00(FT#0010)
            {                                                                   // 40-00-00-00(FT#0010)
                if (trackInfo.TrackData[0].bTrackOK)                            // 40-00-00-00(FT#0010)
                {                                                               // 40-00-00-00(FT#0010)
                    SetTrackInfo(WFS_IDC_CHIP, WFS_IDC_DATAOK, strlen(trackInfo.TrackData[0].szTrack), (BYTE *)trackInfo.TrackData[0].szTrack);// 40-00-00-00(FT#0010)
                }                                                               // 40-00-00-00(FT#0010)
            }                                                                   // 40-00-00-00(FT#0010)
            if (dwReadOption & WFS_IDC_FRONTIMAGE)                              // 40-00-00-00(FT#0010)
            {                                                                   // 40-00-00-00(FT#0010)
                if (trackInfo.TrackData[1].bTrackOK)                            // 40-00-00-00(FT#0010)
                {                                                               // 40-00-00-00(FT#0010)
                    SetTrackInfo(WFS_IDC_FRONTIMAGE, WFS_IDC_DATAOK, strlen(trackInfo.TrackData[1].szTrack), (BYTE *)trackInfo.TrackData[1].szTrack);// 40-00-00-00(FT#0010)
                }                                                               // 40-00-00-00(FT#0010)
            }                                                                   // 40-00-00-00(FT#0010)
            if (dwReadOption & WFS_IDC_BACKIMAGE)                               // 40-00-00-00(FT#0010)
            {                                                                   // 40-00-00-00(FT#0010)
                if (trackInfo.TrackData[2].bTrackOK)                            // 40-00-00-00(FT#0010)
                {                                                               // 40-00-00-00(FT#0010)
                    SetTrackInfo(WFS_IDC_BACKIMAGE, WFS_IDC_DATAOK, strlen(trackInfo.TrackData[2].szTrack), (BYTE *)trackInfo.TrackData[2].szTrack);// 40-00-00-00(FT#0010)
                }                                                               // 40-00-00-00(FT#0010)
            }                                                                   // 40-00-00-00(FT#0010)
        } else                                                                  // 40-00-00-00(FT#0010)
        {                                                                       // 40-00-00-00(FT#0010)
            if (dwReadOption & WFS_IDC_TRACK1)
            {
...
int CXFS_IDX::InitConfig()
{
    ...
    // -----[DEVICE_CONFIG]下参数------------
    // 设备类型(0/新北洋BS-ID81)
    m_stIdxIniConfig.wDeviceType = (WORD)m_cXfsReg.GetValue("DEVICE_CONFIG", "DeviceType", (DWORD)0);


    // -----[DEVICE_SET_0]下参数------------
    memset(szIniAppName, 0x00, sizeof(szIniAppName));
    sprintf(szIniAppName, "DEVICE_SET_%d", m_stIdxIniConfig.wDeviceType);

    // 设备SDK库路径
    strcpy(m_stIdxIniConfig.szSDKPath, m_cXfsReg.GetValue(szIniAppName, "SDK_Path", ""));

    // SDK版本,缺省0
    m_stIdxIniConfig.wSDKVersion = (WORD)m_cXfsReg.GetValue(szIniAppName, "SDK_Version", (DWORD)0);

    // -----[OPEN_CONFIG]下参数------------
    // Open时是否执行Reset动作(0不执行/1执行,缺省0)
    m_stIdxIniConfig.wOpenResetSupp = (WORD)m_cXfsReg.GetValue("OPEN_CONFIG", "OpenResetSupp", (DWORD)0);
    ...
    // Open失败时返回值(0原样返回/1返回SUCCESS,缺省0)
    m_stIdxIniConfig.nOpenFailRet = m_cXfsReg.GetValue("OPEN_CONFIG", "OpenFailRet", (DWORD)0);
    ...
    m_stIdxIniConfig.wResetFailReturn = (WORD)m_cXfsReg.GetValue("RESET_CONFIG", "ResetFailReturn", (DWORD)0);
    if (m_stIdxIniConfig.wResetFailReturn != 0 &&
    ...
    m_stIdxIniConfig.wEjectMode = (WORD)m_cXfsReg.GetValue("EJECT_CONFIG", "EjectMode", (DWORD)0);
    ...
    m_stIdxIniConfig.wRetainSupp = (WORD)m_cXfsReg.GetValue("RETAIN_CONFIG(", "RetainSupp", (DWORD)0);
    ...
    m_stIdxIniConfig.wReadEndRunEject = (WORD)m_cXfsReg.GetValue("READRAWDATA_CONFIG", "ReadEndRunEject", (DWORD)0);
    ...
    m_stIdxIniConfig.stIdxInitParamInfo.fScanImgSaveZoomSc = QString(szTmp).toFloat();

    // 证件扫描图像是否以人像信息面为正面(0否/1是,缺省1)
    m_stIdxIniConfig.stIdxInitParamInfo.wScanImageFrontIsInfor =
    ...
    m_stIdxIniConfig.wBankNo = (WORD)m_cXfsReg.GetValue("BANK_CONFIG", "BankNo", (DWORD)0);   // 40-00-00-00(FT#0010)
...
HRESULT CXFS_IDX::StartOpen()
{
    ...
    // Open前下传初始参数(非断线重连)
    // 设置SDK路径
    if (strlen(m_stIdxIniConfig.szSDKPath) > 0)
    {
        m_pDev->SetData(m_stIdxIniConfig.szSDKPath, DTYPE_LIB_PATH);
    }
    // 设置SDK版本
    m_pDev->SetData(&(m_stIdxIniConfig.wSDKVersion), DTYPE_LIB_VER);
