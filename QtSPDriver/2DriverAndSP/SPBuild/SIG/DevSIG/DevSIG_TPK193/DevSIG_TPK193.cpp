...
#define LOGNAME "DevSIG_TPK193.log"
...
CDevSIG_TPK193::CDevSIG_TPK193(LPCSTR lpDevType)
{
    ...
    SetLogFile(LOGNAME, ThisFile, lpDevType);
}
...
int CDevSIG_TPK193::GetStatus(DEVCAMSTATUS &stStatus)
{
    ...
    if (nRet == 0)
    {
        ...
        m_stStatus.fwCameras[CAM_EXTRA] = STATUS_OK;
    }
    else if (nRet == 1)
        m_stStatus.fwCameras[CAM_EXTRA] = STATUS_INOP;
    else
    {
        m_stStatus.fwMedia[CAM_EXTRA] = MEDIA_UNKNOWN;
        m_stStatus.fwCameras[CAM_EXTRA] = STATUS_UNKNOWN;
    }
...
int CDevSIG_TPK193::GetSignature(LPSTR pszKey, LPSTR pszEncKey, LPSTR pszPictureFile, LPSTR pszSignatureFile, DWORD wnd, WORD wEncypt, LPSTR pszCamData, LPWSTR pswzUnicodeCamData)
{
    ...
    Log(ThisFile, 0, "begin GetSignature... signature path: %s, image path: %s, Encypt: %d", pszSignatureFile, pszPictureFile, wEncypt);
    ...
    if (wEncypt == ENCRYPT_NONE)
    {
        ...
        if (nRet < 0)
        {
            Log(ThisFile, 0, "灌注主密钥失败. ret: %d, key: %s", nRet, pszKey);
            return ERR_OTHER;
        }
        // 输出签名数据到指定路径。
        ...
        Log(ThisFile, 0, "明文签名数据已存放到指定路径. data path: %s, image path: %s", pszSignatureFile, pszPictureFile);
    } else {
        // 灌注主密钥
        ...
        if (nRet < 0)
        {
            Log(ThisFile, 0, "灌注工作密钥. ret: %d, key: %s, enckey: %s", nRet, pszKey, pszEncKey);
            return ERR_OTHER;
        }
        ...
        Log(ThisFile, 0, "获取加密签字数据成功, 开始转换图片... iWorkKey:KEY_INDEX_3, ret: %d, len: %d", nRet, nLen);
...
//        return nRet;
    ...
    if (nRet < 0)
    {
        Log(ThisFile, 0, "SaveDataToImage fail. ret: %d, len: SIGNATURE_DATA_SIZE, path: %s", nRet, pszSignatureFile);
        return nRet;
    }

    Log(ThisFile, 0, "GetSignature success.");
    return CAM_SUCCESS;
}
