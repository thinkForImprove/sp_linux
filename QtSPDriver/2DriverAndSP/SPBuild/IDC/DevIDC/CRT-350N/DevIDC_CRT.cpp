...
//    LOGDEVACTION();
    ...
    if(m_stIDC.bSkimmingSupport){                           //30-00-00=00(FS#0014)
        if((iRet == ERR_IDC_SUCCESS) && m_bSkimmingError){  //30-00-00=00(FS#0014)
            iRet = ERR_IDC_HWERR;                           //30-00-00=00(FS#0014)
        }                                                   //30-00-00=00(FS#0014)
    }                                                       //30-00-00=00(FS#0014)
...
INT CDevIDC_CRT::GetFWVersion(char pFWVersion[], unsigned int &uLen)
{
    ...
    char reply1[512] = {0};
    nRet = SendCmd("CA2", nullptr, 0, ThisModule);
    if (nRet != 0)
    {
        Log(ThisModule, __LINE__, "SendCmd()失败");
        return nRet;
    }
    nRet = GetResponse(reply1, sizeof(reply1), TIMEOUT_WAIT_ACTION, ThisModule);
    if (nRet < 0)
    {
        Log(ThisModule, __LINE__, "GetResponse()失败");
        return nRet;
    ...
    if (reply1[3] != 'P')
    {
        nRet = CardReaderError(reply1 + 6, ThisModule);
        return HWErrToStdErr(nRet);
    }
    ...
    nRet = GetResponse(reply2, sizeof(reply2), TIMEOUT_WAIT_ACTION, ThisModule);
    if (nRet < 0)
    {
        Log(ThisModule, __LINE__, "GetResponse()失败");
        return nRet;
    ...
    if (reply2[3] != 'P')
    {
        nRet = CardReaderError(reply2 + 6, ThisModule);
        return HWErrToStdErr(nRet);
    }
    char MT[MAX_LEN_FWVERSION] = "MT:";
    memcpy(MT+3, reply2+8, 22);
    nRet = strcmp(MT, "MT:**********************");
    if(nRet != 0)
    {
...
//30-00-00-00(FS#0014)
CUSBDrive &CDevIDC_CRT::cUSBDrive()
{
    return m_cUSBDrive;
}
//30-00-00-00(FS#0014)
IOMCIDCPARAM CDevIDC_CRT::stIDC() const
{
    return m_stIDC;
}

//30-00-00-00(FS#0014)
BOOL CDevIDC_CRT::bSkimmingError() const
{
    return m_bSkimmingError;
}

//30-00-00-00(FS#0014)
void CDevIDC_CRT::setBSkimmingError(const BOOL &bSkimmingError)
{
    m_bSkimmingError = bSkimmingError;
}
...
string CDevIDC_CRT::GetIOMCHWType()
{
    string strHWType = "HT";
