...
INT CDevImpl_CRT730B::DeviceOpen(LPSTR lpMode)
{
    ...
    if (m_pDev == nullptr)
    {
        if (m_pDev.Load("AllDevPort.dll", "CreateIAllDevPort", "CRM", "DevCRM_CRT730B") != 0)
        {
    ...
    for (int nWriteIdx = 0; nWriteIdx < nPackCount; nWriteIdx++)
    {
        ...
        for (int iIndex = 0; iIndex < 4; iIndex++)
        {
            ...
            if (nWriteIdx == 0)
            {
                ...
                // 下发到Device
                nRet = m_pDev->Send(buf, nBufOffset, TIMEOUT_WAIT_ACTION);
            } else
            if (nWriteIdx == nPackCount - 1)// 最后一组帧数据
            {
                ...
                // 下发到Device
                nRet = m_pDev->Send(buf + nBufOffset, nOverAgeLen + 2, TIMEOUT_WAIT_ACTION);
            } else  // 中间分组帧数据
            {
                // CRT_PACK_MAX_LEN = 65
                ...
                // 下发到Device
                nRet = m_pDev->Send(buf + nBufOffset, CRT_PACK_MAX_LEN - 1, TIMEOUT_WAIT_ACTION);
            }
