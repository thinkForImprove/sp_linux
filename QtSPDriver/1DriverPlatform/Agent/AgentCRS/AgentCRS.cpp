...
static const char *ThisFile = "AgentCRS.cpp";
...
HRESULT CAgent_CRS::GetInfoOut(DWORD dwCategory, LPVOID lpQueryDetails, LPWFSRESULT &lpCopyCmdData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    if (!LoadWFMDll())
    ...
    case WFS_INF_CDM_STATUS:
        hRet = Get_WFS_INF_CDM_STATUS_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CDM_CAPABILITIES:
        hRet = Get_WFS_INF_CDM_CAPABILITIES_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CDM_CASH_UNIT_INFO:
        hRet = Get_WFS_INF_CDM_CASH_UNIT_INFO_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CDM_MIX_TYPES:
        hRet = Get_WFS_INF_CDM_MIX_TYPES_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CDM_PRESENT_STATUS:
        hRet = Get_WFS_INF_CDM_PRESENT_STATUS_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CDM_CURRENCY_EXP:
        hRet = Get_WFS_INF_CDM_CURRENCY_EXP_Out(lpQueryDetails, lpCopyCmdData);
        break;

    case WFS_INF_CIM_STATUS:
        hRet = Get_WFS_INF_CIM_STATUS_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CIM_CAPABILITIES:
        hRet = Get_WFS_INF_CIM_CAPABILITIES_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CIM_CASH_UNIT_INFO:
        hRet = Get_WFS_INF_CIM_CASH_UNIT_INFO_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CIM_BANKNOTE_TYPES:
        hRet = Get_WFS_INF_CIM_BANKNOTE_TYPES_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CIM_CASH_IN_STATUS:
        hRet = Get_WFS_INF_CIM_CASH_IN_STATUS_Out(lpQueryDetails, lpCopyCmdData);
        break;
    case WFS_INF_CIM_CURRENCY_EXP:
        hRet = Get_WFS_INF_CIM_CURRENCY_EXP_Out(lpQueryDetails, lpCopyCmdData);
        break;
    default:
        break;
    }
...
HRESULT CAgent_CRS::ExecuteOut(DWORD dwCommand, LPVOID lpCmdData, LPWFSRESULT &lpCopyCmdData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    if (!LoadWFMDll())
    ...
    case WFS_CMD_CDM_DISPENSE:
        hRet = Exe_WFS_CMD_CDM_DENOMINATE_Out(lpCmdData, lpCopyCmdData);
        break;
    case WFS_CMD_CDM_START_EXCHANGE:
    case WFS_CMD_CDM_TEST_CASH_UNITS:
        hRet = ExeWFS_CMD_CDM_START_EXCHANGE_Out(lpCmdData, lpCopyCmdData);
        break;
    ...
    case WFS_CMD_CIM_CASH_UNIT_COUNT:
    case WFS_CMD_CIM_CASH_IN_START:
            hRet = WFS_SUCCESS;
            break;
    case WFS_CMD_CIM_CASH_IN:
        hRet = Exe_WFS_CMD_CIM_CASH_IN_Out(lpCmdData, lpCopyCmdData);
        break;
    case WFS_CMD_CIM_CASH_IN_END:
    case WFS_CMD_CIM_START_EXCHANGE:
    case WFS_CMD_CIM_CASH_IN_ROLLBACK:
        hRet = Exe_WFS_CMD_CIM_CASH_IN_END_Out(lpCmdData, lpCopyCmdData);
        break;
...
HRESULT CAgent_CRS::CopyEventStruct(UINT uMsgID, DWORD dwEventID, LPVOID lpData, LPWFSRESULT &lpResult)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    if (!LoadWFMDll())
    ...
    case WFS_EXECUTE_EVENT:
        {
            switch (dwEventID)
            {
                case WFS_EXEE_CDM_CASHUNITERROR:
                    hRet = Fmt_WFS_EXEE_CDM_CASHUNITERROR(lpData, lpResult);
                    break;
                case WFS_EXEE_CIM_CASHUNITERROR:
                    hRet = Fmt_WFS_EXEE_CIM_CASHUNITERROR(lpData, lpResult);
                    break;
                case WFS_EXEE_CIM_INPUTREFUSE:
                    hRet = Fmt_WFS_EXEE_CIM_INPUTREFUSE(lpData, lpResult);
                    break;
                case WFS_EXEE_CIM_SUBCASHIN:
                    hRet = Fmt_WFS_EXEE_CIM_SUBCASHIN(lpData, lpResult);
                    break;
                default:
                    break;
            }
    ...
    case WFS_SERVICE_EVENT:
        {
           ...
           case WFS_SRVE_CDM_ITEMSTAKEN:
                hRet = Fmt_WFS_SRVE_CDM_ITEMSTAKEN(lpData, lpResult);
               break;
           case WFS_SRVE_CDM_CASHUNITINFOCHANGED:
               hRet = Fmt_WFS_SRVE_CDM_CASHUNITINFOCHANGED(lpData, lpResult);
               break;
           case WFS_SRVE_CIM_CASHUNITINFOCHANGED:
                hRet = Fmt_WFS_CIM_CASH_IN(lpData, lpResult);
               break;
           case WFS_SRVE_CDM_COUNTS_CHANGED:
           case WFS_SRVE_CIM_COUNTS_CHANGED:
               hRet = Fmt_WFS_CDM_CIM_COUNTS_CHANGED(lpData, lpResult);
               break;
           case WFS_SRVE_CDM_MEDIADETECTED:
                hRet = Fmt_WFS_SRVE_CDM_MEDIADETECTED(lpData, lpResult);
               break;
           case WFS_SRVE_CIM_MEDIADETECTED:
               hRet = Fmt_WFS_SRVE_CIM_MEDIADETECTED(lpData, lpResult);
               break;
           default:
               break;
           }
    ...
    case WFS_SYSTEM_EVENT:
        {
            ...
            case WFS_SYSE_HARDWARE_ERROR:
                hRet = Fmt_WFS_SYSE_HARDWARE_ERROR(lpData, lpResult);
                break;
            case WFS_SYSE_DEVICE_STATUS:
                hRet = Fmt_WFS_SYSE_DEVICE_STATUS(lpData, lpResult);
                break;
            default:
                break;
            }
...
HRESULT CAgent_CRS::CIMExecute(DWORD dwCommand, LPVOID lpCmdData, LPVOID &lpCopyCmdData)
{
    ...
    case WFS_CMD_CIM_CASH_UNIT_COUNT:                                       //30-00-00-00(FS#0022)
        hRet = Exe_WFS_CMD_CIM_CASH_UNIT_COUNT(lpCmdData, lpCopyCmdData);   //30-00-00-00(FS#0022)
...
//30-00-00-00(FS#0022)
HRESULT CAgent_CRS::Exe_WFS_CMD_CIM_CASH_UNIT_COUNT(LPVOID lpCmdData, LPVOID &lpCopyCmdData)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    lpCopyCmdData = nullptr;

    LPWFSCIMCOUNT lpNewData = nullptr;
    auto lpData = static_cast<LPWFSCIMCOUNT>(lpCmdData);
    if (lpData != nullptr){
        HRESULT hRet = WFS_ERR_INTERNAL_ERROR;
        CAutoWFMFreeBuffer _auto(m_pIWFM, &hRet);

        hRet = m_pIWFM->WFMAllocateBuffer(sizeof(WFSCIMCOUNT), WFS_MEM_FLAG, (LPVOID *)&lpNewData);
        if (hRet != WFS_SUCCESS)
            return hRet;
        _auto.push_back(lpNewData);
        memcpy(lpNewData, lpData, sizeof(WFSCIMCOUNT));

        lpNewData->lpusCUNumList = nullptr;
        if (lpData->usCount > 0)
        {
            if(lpData->lpusCUNumList != nullptr){
                hRet = m_pIWFM->WFMAllocateMore(sizeof(USHORT) * lpData->usCount, lpNewData, (LPVOID *)&lpNewData->lpusCUNumList);
                if (hRet != WFS_SUCCESS)
                    return hRet;
                memcpy(lpNewData->lpusCUNumList, lpData->lpusCUNumList, sizeof(USHORT) * lpData->usCount);
            } else {
                lpNewData->usCount = 0;
            }
        }
