...
protected:
    ...
    void FreeBufferMGR(SPCMDDATA &stCmd);
    void FreeBufferMGR(SPRESULTDATA &stResult);

    // 转换消息为字符
    LPCSTR ConvertMSGID(UINT uMSGID);

    bool AllocateBufferLFSToMGR(bool isComEvent, SPRESULTDATA stLFSResult, SPRESULTDATA &stMGRResult);     //30-00-00-00(FS#0011)
private:
