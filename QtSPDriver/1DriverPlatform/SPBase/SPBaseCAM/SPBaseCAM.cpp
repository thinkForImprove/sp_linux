...
HRESULT CSPBaseCAM::Execute(DWORD dwCommand, LPVOID lpCmdData, DWORD dwTimeOut, LPWFSRESULT &lpResult)
{
    ...
    case WFS_CMD_CAM_DISPLAY:
        {
            ...
            hRet = Fmt_WFSCAMDISPLAY(lpDispInput, lpResult);
        }
        break;;
    case WFS_CMD_CAM_DISPLAYEX:
        {
            auto lpDispInput = static_cast<LPWFSCAMDISPEX>(lpCmdData);
            if (lpDispInput == nullptr)
                break;
            hRet = m_pCmdFunc->DisplayEx(*lpDispInput, dwTimeOut);
            if (hRet != WFS_SUCCESS)
                break;
        }
        break;
    ...
    case WFS_CMD_CAM_GET_SIGNATURE:
        {
            auto lpGetSIGInput = static_cast<LPWFSCAMGETSIGNATURE>(lpCmdData);
            if (lpGetSIGInput == nullptr)
                break;

            WFSCAMSIGNDATA stSignData;
            hRet = m_pCmdFunc->GetSignature(*lpGetSIGInput, stSignData, dwTimeOut);
            if (hRet != WFS_SUCCESS)
                break;

            hRet = Fmt_WFSCAMGETSIGN(&stSignData, lpResult);
        }
        break;
...
HRESULT CSPBaseCAM::Fmt_WFS_EXEE_CAM_LIVEERROR(LPVOID lpData, LPWFSRESULT &lpResult)
{
    HRESULT            hRet = WFS_ERR_INTERNAL_ERROR;
    CAutoWFMFreeBuffer _auto(m_pIWFM, &hRet);
    do
    {
        ...
        LPWFSCAMLIVEERROR lpNewData = nullptr;
        hRet = m_pIWFM->WFMAllocateMore(sizeof(WFSCAMLIVEERROR), lpResult, (LPVOID *)&lpNewData);
        if (hRet != WFS_SUCCESS)
            break;
        _auto.push_back(lpNewData);
        memcpy(lpNewData, lpwResetOut, sizeof(WFSCAMLIVEERROR));
        lpNewData->wLiveError = lpwResetOut->wLiveError;
...
HRESULT CSPBaseCAM::Fmt_WFSCAMDISPLAY(LPVOID lpData, LPWFSRESULT &lpResult)
{
    HRESULT hRet = WFS_ERR_INTERNAL_ERROR;
    CAutoWFMFreeBuffer _auto(m_pIWFM, &hRet);
    do
    {
        auto lpCaps = static_cast<LPWFSCAMDISP>(lpData);
        if (lpCaps == nullptr)
            break;

        LPWFSCAMDISP lpNewData = nullptr;
        hRet = m_pIWFM->WFMAllocateMore(sizeof(WFSCAMDISP), lpResult, (LPVOID *)&lpNewData);
        if (hRet != WFS_SUCCESS)
            break;

        _auto.push_back(lpNewData);
        memcpy(lpNewData, lpCaps, sizeof(WFSCAMDISP));

#if defined(SET_BANK_CMBC)
        if (lpCaps->pszTexData != nullptr)
        {
            hRet = m_pIWFM->WFMAllocateMore(strlen(lpCaps->pszTexData) + 1, lpResult, (LPVOID *)&lpNewData->pszTexData);
            if (hRet != WFS_SUCCESS)
                break;

            _auto.push_back(lpNewData->pszTexData);
            memcpy(lpNewData->pszTexData, lpCaps->pszTexData, strlen(lpCaps->pszTexData) + 1);
        }
#endif
...
HRESULT CSPBaseCAM::Fmt_WFSCAMGETSIGN(LPVOID lpData, LPWFSRESULT &lpResult)
{
    HRESULT hRet = WFS_ERR_INTERNAL_ERROR;
    CAutoWFMFreeBuffer _auto(m_pIWFM, &hRet);
    do
    {
        auto lpCaps = static_cast<LPWFSCAMSIGNDATA>(lpData);
        if (lpCaps == nullptr)
            break;

        LPWFSCAMSIGNDATA lpNewData = nullptr;
        hRet = m_pIWFM->WFMAllocateMore(sizeof(WFSCAMSIGNDATA), lpResult, (LPVOID *)&lpNewData);
        if (hRet != WFS_SUCCESS)
            break;

        _auto.push_back(lpNewData);
        memcpy(lpNewData, lpCaps, sizeof(WFSCAMSIGNDATA));

        if (lpCaps->data != nullptr)
        {
            hRet = m_pIWFM->WFMAllocateMore(strlen((LPCSTR)lpCaps->data) + 1, lpResult, (LPVOID *)&lpNewData->data);
            if (hRet != WFS_SUCCESS)
                break;

            _auto.push_back(lpNewData->data);
            memcpy(lpNewData->data, lpCaps->data, strlen((LPCSTR)lpCaps->data) + 1);
        }
