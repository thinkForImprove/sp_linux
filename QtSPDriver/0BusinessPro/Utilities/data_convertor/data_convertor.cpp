...
unsigned long DataConvertor::string_to_ulong(const char* str, int convert_length/* = 0*/, int radix/* = 10*/)
{
    ...
    if (radix == 8)
    {
        ret = sscanf(str_data, "%o", &ulong_data);
    } else
    if (radix == 16)
    {
        ...
        {
            if (strncmp(str_data + i * 2, "0x", strlen("0x")) == 0) {
                ret = sscanf(str_data + i * 2, "%0x02X", &ulTmp);
            } else {
                ret = sscanf(str_data + i * 2, "%02X", &ulTmp);
            }
            ...
            ulong_data = (ulong_data << (i * 8)) + ulTmp;
        }
    } else
    {
        ret = sscanf(str_data, "%lu", &ulong_data);
    }
...
double DataConvertor::str2number(char *str)
{
    ...
    // 入参检查: 只有'-', 返回0.0; 第一位非数字返回0.0
    if ((strlen(str) == 1 && str[0] == '-') ||
        (strlen(str) == 1 && (str[0] < '0' || str[0] > '9')))
    {
        return 0.0;
    }
    ...
    for(; i < strlen(str); i ++)
    {
        ...
        if (nRadixSum == 1)
        {
            duRet = (duRet + (str[i] - 48) / 10);
        } else
        {
            duRet = (duRet * 10 + (str[i] - 48));
        }
    }

    if (str[0] == '-')
    {
        return (double)(0.0 - duRet);
    } else
    {
        return (double)duRet;
    }
