...
//////////////////////////////////////////////////////////////////////////
CAtmStartPower::CAtmStartPower(QWidget *parent) : QWidget(parent),
                                                  ui(new Ui::CAtmStart),
                                                  m_cDev("IOMC"),
                                                  m_cDevPW("IOMC_PW"),
                                                  ...
                                                  m_cIOMCShareMemoryR(IOMC_RECV_SHARE_MEMORY_MUTEX_NAME)
{
    ...
    //创建共享内存(仅自研IOMC)
    if(strlen(m_stPW.szPortParam) > 0){
        if(false == m_cIOMCShareMemoryS.Open(IOMC_SEND_SHARE_MEMORY_KEY_NAME, true)){
            Log(__FUNCTION__, __LINE__, "创建发送用共享内存%s失败", IOMC_SEND_SHARE_MEMORY_KEY_NAME);
        } else {
            if(false == m_cIOMCShareMemoryR.Open(IOMC_RECV_SHARE_MEMORY_KEY_NAME, true)){
                Log(__FUNCTION__, __LINE__, "创建接收用共享内存%s失败", IOMC_SEND_SHARE_MEMORY_KEY_NAME);
            }
        }
    }
...
//            break;
        ...
        while (!m_bQuitRun)
        {
            if(CQtTime::GetSysTick() - ulStartTime > 10*1000){
                do
                {
                    pIcon = &cIconStop;
                ...
                ulStartTime = CQtTime::GetSysTick();
//                QtSleep(10*1000);
            } else {
                //处理其他进程的IOMC命令
                if(strlen(m_stPW.szPortParam) > 0){
...
long CAtmStartPower::ReadConfig()
{
    ...
    cIR = m_cINI.GetReaderSection("DEVICE");
    strcpy(m_stPW.szPortParam, (LPCSTR)cIR.GetValue("PortParam", ""));
...
long CAtmStartPower::Open()
{
    ...
    long lRet = 0;
    if(0 == strlen(m_stPW.szPortParam)){
        // 加载USB库，并打开连接
        lRet = m_cDev.USBDllLoad();
        if (lRet != 0)
        {
        ...
        m_cDev.SetIomcType(IOMC_TYPE_CFES);
        // 加载USB库，并打开连接
        lRet = m_cDev.USBDllLoad();
        if (lRet != 0)
        {
        ...
        lRet = m_cDev.USBOpen(m_stPW.szPortParam);
        if (lRet != 0)
        {
            Log(ThisModule, __LINE__, "打开连接失败: lRet=%d", lRet);
            return -1;
...
long CAtmStartPower::SendAndReadCmd(const STR_IOMC &strIomc, IOMCRespInfT &stRespInf, const char *ThisModule)
{
    ...
    {
        pDev = (strlen(m_stPW.szPortParam) == 0 ? &m_cDevPW : &m_cDev);
    }
        break;
    default:
        break;
...
void CAtmStartPower::HdlOtherProcIOMCCmd()
{
    if(!m_cIOMCShareMemoryS.IsOpened() || !m_cIOMCShareMemoryR.IsOpened()){
        MLog(__FUNCTION__, __LINE__, "IOMC 共享内存未打开");
        return;
    }

    long lRet = 0;
    char cReadBuff[1024];
    char cWriteBuff[1024];
    memset(cReadBuff, 0, sizeof(cReadBuff));
    memset(cWriteBuff, 0, sizeof(cWriteBuff));
    //读取共享内存命令
    if(!m_cIOMCShareMemoryS.Read(cReadBuff, sizeof(cReadBuff))){
        return;
    }
    ...
    stDrvParam.pvDataInBuffPtr = cReadBuff;
    stDrvParam.uiDataInBuffSz = wLen;
    stDrvParam.pvDataOutBuffPtr = cWriteBuff;
    stDrvParam.uiDataOutReqSz = sizeof(cWriteBuff);
    ...
    if(lRet != 0){
        //命令执行失败，写入两字节\0\0
        bRet = m_cIOMCShareMemoryR.Write(cWriteBuff, 2);
    } else {
        bRet = m_cIOMCShareMemoryR.Write(stDrvParam.pvDataOutBuffPtr, stDrvParam.uiDataOutBuffSz);
    }

    if(!bRet){
        MLog(__FUNCTION__, __LINE__, "IOMC 命令结果数据写入共享内存失败");
    }
