...
struct  IDevPIN
{
    ...
    // 密文输入
    virtual long PinInput(UINT uMinLen = 4, UINT uMaxLen = 6, bool bClearKeyAsBackspace = false) = 0;
    ...
    /*****************************************************
     * @func   RKL方式导入SM4密钥.
     * @param  uKeyNo:下载的SM4密钥号
     *         uEppSKeyNo:解密用EPP SM2私钥号
     *         uHostSM2PKeyNo:验签用行方SM2公钥号
     *         uKeyUse:SM4密钥属性
     *         lpUserId:用户ID
     *         lpCipherTextKeyVal:SM2公钥加密的SM4密钥密文值
    ...
    virtual long SM4RemoteImportKey(UINT uKeyNo, UINT uEppSKeyNo, UINT uHostSM2PKeyNo,
                                    ...
                                    LPCSTR lpSignatureVal, LPSTR lpKCV) = 0;
#endif
    // 加解密
    ...
    // 公钥加密数据
    virtual long SM2EncryptData(LPCSTR lpSM2PKeyData, LPCSTR lpData, LPCSTR lpCryptData) = 0;
    // 私钥解密数据
    virtual long SM2DecryptData(LPCSTR lpSM2SKeyData, LPCSTR lpCryptData, LPCSTR lpData) = 0;
    // 私钥签名
    ...
    // 私钥解密数据
    virtual long SM2DecryptData(UINT uSKeyNo, LPCSTR lpCryptData, LPSTR lpData) = 0;
    // 私钥签名
    virtual long SM2SignData(LPCSTR lpSM2SKeyData, LPCSTR lpZA, LPCSTR lpData, LPSTR lpSignData) = 0;
    // 公钥验签
    virtual long SM2VerifySign(LPCSTR lpSM2PKeyData, LPCSTR lpZA, LPCSTR lpData, LPCSTR lpSignData) = 0;
    ...
    // 公钥验签
    virtual long SM2VerifySign(UINT uPKeyNo, LPCSTR lpZA, LPCSTR lpData, LPCSTR lpSignData) = 0;
#endif    
	// 导出公钥和公钥签名
    ...
    virtual long RSAImportRootHostPK(UINT uPKeyNo, UINT uSignKeyNo, int iSignAlgo,
                                     QByteArray strHostPKData, QByteArray strSignData)// = 0;           //30-00-00-00(FS#0006)
    {
        return ERR_NOT_SUPPORT;
    }
    // 导入Host签名公钥(HOST私钥签名)
    ...
    virtual long RSAExportEppSN(UINT uEppSKeyNo, RSASIGNALGOTYPE &eRSASignAlgo,
                                QByteArray &strSN, QByteArray &strSignSN)// = 0;       //30-00-00-00(FS#0006)
    {
        return ERR_NOT_SUPPORT;
    }
    // 生成8位的导入MK随机数据
    ...
    virtual long RSAImportDesKey(UINT uKeyNo, UINT uEppSKeyNo, UINT uHostPKeyNo,
                                 ...
                                 QByteArray strKeyVal, QByteArray strSignVal,
                                 QByteArray &strKCV, bool bUseRandom = false,
                                 int iSymmKeyType = 0)// = 0; //30-00-00-00(FS#0006)
    {
        return ERR_NOT_SUPPORT;
    }
#endif
    ...
    // RSA密钥加密数据
    virtual long RSAEncryptData(UINT uRSAPKeyNo, RSADATAALGOTYPE eRSAEncAlgo, QByteArray data, QByteArray &result)// = 0; //30-00-00-00(FS#0006)
    {
        return ERR_NOT_SUPPORT;
    }
    // RSA密钥解密数据
    virtual long RSADecryptData(UINT uRSASKeyNo, RSADATAALGOTYPE eRSAEncAlgo, QByteArray data, QByteArray &result)// = 0; //30-00-00-00(FS#0006)
    {
        return ERR_NOT_SUPPORT;
    }

    // 设置DES密钥长度信息
    virtual long SetDesKeyLenInfo(std::map<UINT, BYTE> &keyIdMapKeyLen)// = 0;
    {
        return ERR_NOT_SUPPORT;
    }
};
