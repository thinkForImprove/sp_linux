...
public:
    ...
    ISPPrinterField *FindField(const char *lpszName)
    {
        ...
        for (DWORD i = 0; i < pForm->GetSubItemCount(); i++)
        {
            ...
            ISPPrinterSubform *pSubform = nullptr;
...
public:
    ...
    {
        m_bBack     = false;
        m_Overflow  = OF_TERMINATE; // 30-00-00-00(FT#0063)
        m_Style     = 0;
        m_HAlign    = LEFT;
        m_VAlign    = BOTTOM;
...
public:
    ...
    {
        return m_Header.empty() ? nullptr : m_Header.c_str();
    }

    LPCSTR GetFooter() const
    {
        return m_Footer.empty() ? nullptr : m_Footer.c_str();
    }
    ...
    OVER_FLOW GetOverflow() const   // 30-00-00-00(FT#0063)
    {
        return m_Overflow;
    }
...
protected:
    ...
    string      m_Footer;
    BOOL        m_bBack;
    OVER_FLOW    m_Overflow;    // 30-00-00-00(FT#0063)
    DWORD       m_Style;
    HORIZONTAL  m_HAlign;
    VERTICAL    m_VAlign;
...
public:
    ...
    {
        return m_FontName.empty() ? nullptr : m_FontName.c_str();
    }
    ...
    {
        return m_Format.empty() ? nullptr : m_Format.c_str();
    }

    LPCSTR GetInitValue() const
    {
        return m_InitValue.empty() ? nullptr : m_InitValue.c_str();
    }
...
protected:
    BOOL        m_bLoadSucc;
    FORMALIGN   m_Align;                // 排列方式
    SIZE        m_BaseSize;             // UNIT X,Y
    FORMBASE    m_Base;                 // UNIT TYPE
    FORMORIENTATION m_Orientation;      // Form类型:缺省PORTRAIT
    BYTE        m_VerMajor, m_VerMinor; // VERSION
    WORD        m_LangID;               // LANGUAGE: 语言识别码
    DWORD       m_Skew;
    string      m_Title;
    string      m_Copyright;
    string      m_Comment;
