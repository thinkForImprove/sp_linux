...
void CStlSixThread::StartThread()
{
    ...
    if (!m_thRunParallelExe.joinable()){                                                //30-00-00-00(FS#0002)
        m_thRunParallelExe = std::thread(&CStlSixThread::Run_ParallelExecute, this);    //30-00-00-00(FS#0002)
    }                                                                                   //30-00-00-00(FS#0002)
    return;
}
...
void CStlSixThread::StopThread()
{
    ...
    if (m_thRunStatus.joinable())
        m_thRunStatus.join();
    if (m_thRunParallelExe.joinable())                                                  //30-00-00-00(FS#0002)
        m_thRunParallelExe.join();                                                      //30-00-00-00(FS#0002)
