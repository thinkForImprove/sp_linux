...
protected:
    ...
    // Add by CLin (递归部分)
    // 格式化函数以及节点分级函数
    bool x_FormatDoc(MCD_STR &strDoc, int iLevel);
...
{
public:
    CAutoIntoElem(CMarkup *pXML): m_pXML(pXML) {if (m_pXML)m_pXML->IntoElem();}
    ~CAutoIntoElem() {if (m_pXML)m_pXML->OutOfElem();}
private:
    CMarkup *m_pXML;
};
//////////////////////////////////////////////////////////////////////////
