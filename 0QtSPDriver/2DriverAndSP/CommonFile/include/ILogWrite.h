...
{
public:
    CAutoLogFuncBeginEndEx(CLogManage *pLog, LPCSTR ThisFile, LPCSTR ThisModule, long lLine, int nFlag = 0, LPCSTR lpLogFile = LOGFILE,
                           LPCSTR lpPrefixType = nullptr)
    {
        m_nFlag = nFlag;
        ...
        m_pLog->Log(m_szModule, m_nFlag, "%s->开始调用->[LINE:%d]", m_szModule, lLine);
    }
    ~CAutoLogFuncBeginEndEx() { m_pLog->Log(m_szModule, m_nFlag, "%s->结束调用[耗时 %d 毫秒]", m_szModule, m_qTime.elapsed()); }
...
{
public:
    CAutoLogFuncBeginEnd(CLogManage *pLog, LPCSTR ThisModule, long lLine) : m_pLog(pLog)
    {
        strcpy(m_szModule, ThisModule);
        m_qTime = QTime::currentTime();
        m_pLog->Log(m_szModule, 0, "%s->开始调用->[LINE:%d]", m_szModule, lLine);
    }
    ~CAutoLogFuncBeginEnd() { m_pLog->Log(m_szModule, 0, "%s->结束调用[耗时 %d 毫秒]", m_szModule, m_qTime.elapsed()); }
...
#define AutoLogFuncBeginEnd()        CAutoLogFuncBeginEnd _auto_log_begin_end(this, ThisModule, __LINE__)
#define AutoLogFuncBeginEndEx()      CAutoLogFuncBeginEndEx _auto_log_begin_end_ex(&gLog, ThisFile, ThisModule, __LINE__)
#define AutoLogFuncBeginEndExFile(X) CAutoLogFuncBeginEndEx _auto_log_begin_end_ex(&gLog, ThisFile, ThisModule, __LINE__, 0, X)
//////////////////////////////////////////////////////////////////////////
