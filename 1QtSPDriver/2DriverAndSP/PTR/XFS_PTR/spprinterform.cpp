...
#define LONG_MAX                    2147483647L //2015.8.6
...
STRLIST Split(LPCSTR s, LPCSTR SepStr)
{
    ...
    while (*p)
    {
        BOOL bFound;     // 2015.8.24
        if (NULL == strchr(SepStr, *p)) //2015.8.24
        {
            bFound = FALSE;     //2015.8.24
        }
        else
        {
            bFound = TRUE;      //2015.8.24
        }
...
BOOL CSPPrinterItem<T>::LoadAttrs(char *&p, int &nLine, KeyRule *pRule, char *pResults[], int nResultCount)
{
    ...
    for (nAttr = 1, itAttr = al.begin(); itAttr != al.end(); nAttr++, itAttr++)
    {
        if (nAttr >= nResultCount)  // 有的属性没有设置
        {
            ...
            {
                log_write(EVENT_LOG, ThisFile, ThisModule, -1, IDS_ERR_REQUIRED_ATTR, \
                          ItemTypeToDesc(GetItemType()), GetName(), pRule->m_Name.c_str(), nAttr + 1, nLine);
                return FALSE;
            }
            continue;
        }
        ...
        else
        {
            ...
            else
            {
                ...
                //                  }
                //              }
                //modified by zhongjs 20120315 Bug#39 配置media中UNIT关键字base的值为负数，执行WFS_INF_PTR_QUERY_MEDIA返回结果错误
                //  modify start
                errno = 0;
                ...
                if (ERANGE == errno)
                {
                    log_write(EVENT_LOG, ThisFile, ThisModule, errno, IDS_ERR_ATTR_INVALID_VALUE, \
                ...
                else if (0 == dwValue)
                {
                    if ('\0' != (*endptr))
                    {
                        log_write(EVENT_LOG, ThisFile, ThisModule, -1, IDS_ERR_ATTR_INVALID_VALUE, \
...
BOOL CSPPrinterItem<T>::Load(char *&p, const char *lpszName, int &nLine)
{
    ...
    else
    {
        ...
        {
            log_write(EVENT_LOG, ThisFile, ThisModule, -1, IDS_ERR_READ_NAME, \
    ...
    while (1)
    {
        ...
        if (0 == strcmp(pResults[0], "BEGIN"))
        {
            ...
            {
                log_write(EVENT_LOG, ThisFile, ThisModule, -1, IDS_ERR_REBEGIN, \
        ...
        if (!bBegin)
        {
            log_write(EVENT_LOG, ThisFile, ThisModule, -1, IDS_ERR_NOBEGIN, \
                      ItemTypeToDesc(GetItemType()), GetName(), nLine);
            return FALSE;
        }
        ...
        if (it == rl.end())
        {
            log_write(EVENT_LOG, ThisFile, ThisModule, -1, IDS_ERR_ERR_KEY, \
    ...
    for (int i = 0; i < s.size(); i++)
    {
        ...
        {
            log_write(EVENT_LOG, ThisFile, ThisModule, -1, IDS_ERR_REQUIRED_KEY, \
...
KeyRules *CSPPrinterMedia::GetKeyRules()
{
    ...
    {
        //MediaKeyRules.AddRule("TYPE", false).AALV(m_MediaType, "GENERIC,MULTIPART,PASSBOOK");
        MediaKeyRules.AddRule("TYPE", FALSE).AALV(m_MediaType, "GENERIC,PASSBOOK,MULTIPART");//modify by lvcb 2013.0307
