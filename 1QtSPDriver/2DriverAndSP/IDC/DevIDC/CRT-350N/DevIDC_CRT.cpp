...
LONG CDevIDC_CRT::ReadConfig()
{
    ...
    m_stIDC.iESUStrength = (int)cIR.GetValue("ESUStrength", 0);
...
INT CDevIDC_CRT::EatCard()
{
    ...
    char reply[2068] = {0};
//30-00-00-00(FT#0030)    nRet = SendCmd("C31", nullptr, 0, ThisModule);
...
INT CDevIDC_CRT::EjectCard()
{
    ...
    // C30 退卡速度更快，但快速取卡会很费力(hsyh)
    //nRet = SendCmd("C30", nullptr, 0, ThisModule);        //30-00-00-00(FT#0056)
...
INT CDevIDC_CRT::ICCChipIO(ICCardProtocol eProFlag, char *pInOutData, unsigned int &nInOutLen, DWORD dwBuffSz)
{
    ...
    char szCmd[2068] = {0};
    //    if (ICCARD_PROTOCOL_T0 == eProFlag)                                      //30-00-00-00(FT#0051)
    //    {                                                                        //30-00-00-00(FT#0051)
    //        memcpy(szCmd, "CI3", 3);    //IC 卡 T=0 通讯                          //30-00-00-00(FT#0051)
    //    }                                                                        //30-00-00-00(FT#0051)
    //    else if (ICCARD_PROTOCOL_T1 == eProFlag)                                 //30-00-00-00(FT#0051)
    //    {                                                                        //30-00-00-00(FT#0051)
    //        memcpy(szCmd, "CI4", 3);    //IC 卡 T=1 通讯                          //30-00-00-00(FT#0051)
    //    }                                                                        //30-00-00-00(FT#0051)
    memcpy(szCmd, "CI9", 3);
...
INT CDevIDC_CRT::SetESUMode(int iMode)
{
    ...
    if (reply[3] != 'P')
    {
        ...
        if(reply[8] & 0x20){            //支持磁干扰功能
            ...
            //设置干扰强度
            QByteArray vtParam = "U9";
            vtParam.push_back((char)0x30 + m_stIDC.iESUStrength);
            nRet = SendCmd("CpC", vtParam.constData(), vtParam.size(), ThisModule);
            if (nRet != 0)
            {
                return nRet;
            }
