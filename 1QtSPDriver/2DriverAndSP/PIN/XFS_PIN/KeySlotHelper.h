...
public:
    ...
    PINKEYDATA FindAsymmKeySlot(QByteArray strName, int iKeyType = 0);		//30-00-00-00(FS#0003)
    // 查询密钥
    PINKEYDATA FindKey(QByteArray strName);
    PINKEYDATA FindSMKey(QByteArray strName);
    PINKEYDATA FindAsymmKey(QByteArray strName);							//30-00-00-00(FS#0003)
    ...
    long DeleteAsymmKey(QByteArray strName);								//30-00-00-00(FS#0003)
    long DeleteAllKey();
    // 无此密钥
    bool IsInvalidKey(const PINKEYDATA &stKeyData);
    // 是否为新密钥序号
...
private:
    ...
    bool FindAsymmKeyByName(QByteArray strName, PINKEYDATA &stKey);		//30-00-00-00(FS#0003)
