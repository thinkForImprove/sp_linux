...
PINKEYDATA CKeySlotHelper::FindSM2KeySlot(QByteArray strName)
{
    ...
    mapUUINT vtKeyNum;
    for (UINT uNum = SM2KEY_BASE; uNum <= SM2KEY_BASE; uNum++)
    {
        vtKeyNum[uNum] = uNum;
    }
    for (auto &it : m_ltKeyData)
...
//30-00-00-00（FS#0003）
PINKEYDATA CKeySlotHelperXZ::FindAsymmKeySlot(QByteArray strName, int iKeyType)
{
    AutoMutex(m_cMutex);
    PINKEYDATA stKey;
...
//30-00-00-00（FS#0003）
PINKEYDATA CKeySlotHelperXZ::FindAsymmKey(QByteArray strName)
{
    AutoMutex(m_cMutex);
    PINKEYDATA stKey;
    if (!FindAsymmKeyByName(strName, stKey)){
...
//30-00-00-00（FS#0003）
long CKeySlotHelperXZ::DeleteAsymmKey(QByteArray strName)
{
    AutoMutex(m_cMutex);
    for (auto it = m_ltKeyData.begin(); it != m_ltKeyData.end(); it++)
    {
        if (it->dwType == 4 && qstricmp(strName.constData(), it->szName) == 0)
        {
            m_ltKeyData.erase(it);
            return SaveKeyFile();
        }
...
//30-00-00-00（FS#0003）
bool CKeySlotHelperXZ::FindAsymmKeyByName(QByteArray strName, PINKEYDATA &stKey)
{
    for (auto &it : m_ltKeyData)
    {
        if (it.dwType == 4 && qstricmp(strName, it.szName) == 0)
