...
/*            if(lpPublicKey->dwRSASignatureAlgorithm == WFS_PIN_SIGN_RSASSA_PKCS1_V1_5){
                eRSASignAlgo = RSA_SIGN_SSA_PKCS1_V1_5;
            } else if(lpPublicKey->dwRSASignatureAlgorithm == WFS_PIN_SIGN_RSASSA_PSS){
...
*/
            long lRet = m_pDev->RSAImportRootHostPK(stPKHost.uNum, 0xFF, eRSASignAlgo, valueHex, signatureHex);
            if(lRet != 0){
...
HRESULT CXFS_PIN::ImportRSASignedDesKey(const LPWFSPINIMPORTRSASIGNEDDESKEY lpSignedDESKey, LPWFSPINIMPORTRSASIGNEDDESKEYOUTPUT &lpSignedDESKeyOutput)
{
    ...
    if(m_strDevName == "XZ"){
        ...
        if(m_bRSAUseSM2Key){
            ...
            //下载DES主密钥
            QByteArray vtKcvHex;
            long lRet = m_pDev->RSAImportDesKey(stKey.uNum, stSKEpp.uNum, stPKHost.uNum, eRSADataAlgo, eRSASignAlgo,
                                                value, signature, vtKcvHex);
            if(lRet != 0){
...
long CXFS_PIN::GetKeyPress(vectorWFSPINKEY &vtKey, CInputDataHelper &cInput, DWORD dwTimeOut)
{
    ...
    {
        //Get_Data与Pin_crypt并行处理,使其他线程可获得cpu处理机会
        CQtTime::Sleep(10);
    ...
    if(true){
        ...
        */
        ...
        if(iDataLen == 0){
            ...
            if(iDataLen >= 12){
                vtData = vtData.right(12);
            } else {
                vtData.prepend(12 - iDataLen, 0x30);
            }
        }
    }
