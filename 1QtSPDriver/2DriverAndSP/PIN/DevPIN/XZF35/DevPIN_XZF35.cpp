...
CDevPIN_XZF35::CDevPIN_XZF35(LPCSTR lpDevType)
{
    ...
    InitErrCode();
    m_bIsSendRead = false;                  //30-00-00-00(FS#0002)
    m_bIsGetPressedKey = false;             //30-00-00-00(FS#0002)
}
...
long CDevPIN_XZF35::TextInput()
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
//    AutoMutex(m_cMutex);              //30-00-00-00(FS#0002)
...
long CDevPIN_XZF35::ReadKeyPress(EPP_KEYVAL &stKeyVal, DWORD dwTimeOut)
{
    ...
    if(m_bIsSendRead){                          //30-00-00-00(FS#0002)
        return 0;                               //30-00-00-00(FS#0002)
    }                                           //30-00-00-00(FS#0002)
    CAutoSetStatus autoSetStatus(&m_bIsGetPressedKey);                  //30-00-00-00(FS#0002)
    LOGDEVACTION();
    BYTE szKeyBuff[64] = {0};
...
long CDevPIN_XZF35::RSAImportDesKey(UINT uKeyNo, UINT uEppSKeyNo, UINT uHostPKeyNo, RSADATAALGOTYPE eRSAEncAlgo, RSASIGNALGOTYPE eRSASignAlgo, QByteArray strKeyVal, QByteArray strSignVal, QByteArray &strKCV)
{
...
long CDevPIN_XZF35::SendReadCmd(WORD wCmdCode, const QByteArray &vtCmd, QByteArray &vtResult, DWORD dwTimeOut)
{
    ...
    AutoMutex(m_cMutex);
    //AutoLogFuncBeginEnd();
    while (m_bIsGetPressedKey);                                      //30-00-00-00(FS#0002)
    CAutoSetStatus autoSetSendReadStatus(&m_bIsSendRead);            //30-00-00-00(FS#0002)
...
long CDevPIN_XZF35::SendReadData(const QByteArray &vtCmd, QByteArray &vtResult, DWORD dwTimeOut)
{
    ...
    do
    {
        ...
        if (lRet == 0 && dwReadLen > 0)
        {
            ...
            //检查按键数据并添加到按键数据列表中
            {
                int iStartTagPos = vtResult.indexOf(RSP_STX);
                QByteArray pressKeyData;
                if(iStartTagPos != -1){
                    if(iStartTagPos != 0 && iStartTagPos > iCheckEndPos){
                        pressKeyData = vtResult.mid(iCheckEndPos + 1, iStartTagPos - iCheckEndPos);
                    } else {
                        QByteArray endTag;
                        endTag.append(0x0D);
                        endTag.append(0x0A);
                        int iEndTagPos = vtResult.indexOf(endTag);
                        if(iEndTagPos != -1){
                            iEndTagPos += 1;
                            if(iEndTagPos < iCheckEndPos){
                                pressKeyData = vtResult.mid(iCheckEndPos + 1);
                            }
                        }
                    }
                } else {
                ...
                iCheckEndPos = vtResult.size() -1;

                //添加数据到按键数据列表中
                if(pressKeyData.size() > 0){
                    for (int i = 0; i < pressKeyData.size(); i++)
                    {
                        EPP_KEYVAL stKeyVal;
                        memset(&stKeyVal, 0, sizeof(stKeyVal));
                        if (ConvertKeyVal(pressKeyData[i], stKeyVal))
                        {
                            m_vtKeyVal.push_back(stKeyVal);
                        }
                    }
...
long CDevPIN_XZF35::SM4ImportMKeyForGD(UINT uMKeyNo, UINT uDKeyNo, UINT uKeyUse, LPCSTR lpKeyVal, LPSTR lpKCV)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
    AutoMutex(m_cMutex);
    ...
    if(m_iImportSm4KeyEncBySm2Mode == 1) {
        ...
        //使用SM2公钥加密,仅测试时使用
        stSm2DataOpt.wKeyId = EXWORDHL(uDKeyNo);
        ...
        uLen = sizeof(stSm2DataOpt.byData);
        CAutoHex::Hex2Bcd(lpKeyVal, stSm2DataOpt.byData, uLen);
        uLen = sizeof(stSm2DataOpt) - (sizeof(stSm2DataOpt.byData) - uLen);
        vtCmd.append((LPSTR)&stSm2DataOpt, uLen);
        lRet = SendReadCmd(SM2_DATA_ENC_OR_DENC, vtCmd, vtResult);
        if (lRet != ERR_SUCCESS)
        {
            Log(ThisModule, __LINE__, "SM2私钥解密失败：lRet=%d[uDKeyNo=%02X]", lRet, uDKeyNo);
            return lRet;
        }

        //下载SM4 Key
        CAutoHex autoHex((LPBYTE)vtResult.constData(), vtResult.size());
        lRet = SM4ImportWKey(uMKeyNo, 0xFF, uKeyUse, autoHex.GetHex(), lpKCV);
        if(lRet != ERR_SUCCESS){
            return lRet;
        }
    } else {
        ...
        stImportSm4KeyEncBySm2.wSM4KeyId = EXWORDHL(uMKeyNo);
        stImportSm4KeyEncBySm2.wSM2KeyId = EXWORDHL(uDKeyNo);
        stImportSm4KeyEncBySm2.dwKeyAttr = RVSDWORD(uKeyUse);
        uLen = sizeof(stImportSm4KeyEncBySm2.byKeyValue);
        CAutoHex::Hex2Bcd(lpKeyVal, stImportSm4KeyEncBySm2.byKeyValue, uLen);
        ...
        //获取国密密钥KCV
        lRet = ReadSymmKeyKCV(uMKeyNo, 2, lpKCV);
        if(lRet != ERR_SUCCESS) {
            Log(ThisModule, __LINE__, "获取国密密钥KCV失败:=%d[uMKeyNo=%02X]", lRet, uMKeyNo);
            return lRet;
        }
    }
