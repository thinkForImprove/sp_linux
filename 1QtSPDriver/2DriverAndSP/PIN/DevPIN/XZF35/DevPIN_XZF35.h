...
public:
    ...
    virtual long SM4ImportMKeyForGD(UINT uMKeyNo, UINT uDKeyNo, UINT uKeyUse, LPCSTR lpKeyVal, LPSTR lpKCV);				//30-00-00-00(FS#0003)
    ...
    virtual long RSAImportRootHostPK(UINT uPKeyNo, UINT uSignKeyNo, int iSignAlgo, QByteArray strHostPKData, QByteArray strSignData);           //30-00-00-00(FS#0006)
    // 导入Host签名公钥(HOST私钥签名)
    ...
    virtual long RSAImportDesKey(UINT uKeyNo, UINT uEppSKeyNo, UINT uHostPKeyNo,
                                 ...
                                 QByteArray &strKCV);               //30-00-00-00(FS#0006)
    // 生成RSA密钥对
    virtual long RSAGenerateKeyPair(UINT uPKeyNo, UINT uSKeyNo, int iModuleLen, int iExponentValue);            //30-00-00-00(FS#0006)
private:
    // 打包发送命令
...
private:
	...
	int                         m_iImportSm4KeyEncBySm2Mode;		//30-00-00-00(FS#0003)
public:
    bool                        m_bIsSendRead;              //30-00-00-00(FS#0002)
    bool                        m_bIsGetPressedKey;         //30-00-00-00(FS#0002)
};

//30-00-00-00(FS#0002) add start
class CAutoSetStatus{
public:
    CAutoSetStatus(bool *bIsSendRead):m_bIsSendRead(bIsSendRead){
        if(m_bIsSendRead){
           *m_bIsSendRead = true;
        }
    }
