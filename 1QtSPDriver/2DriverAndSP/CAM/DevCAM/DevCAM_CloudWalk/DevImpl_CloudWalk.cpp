...
CDevImpl_CloudWalk::CDevImpl_CloudWalk()
{
    ...
    sprintf(m_szCloudWalkDllPath, "%s", strDllName.data());
    m_CloudWalkLibrary.setFileName(strDllName);
    ...
    if (m_CloudWalkLibrary.isLoaded() != true)
    {
        if (m_CloudWalkLibrary.load() != true)
        {
            Log(ThisFile, 1, "加载动态库<%s> fail. ReturnCode:%s.",
                m_szCloudWalkDllPath, m_CloudWalkLibrary.errorString().data());
            return FALSE;
        }
    }
                                       ...
                                       SHORT height)      // 请求图像的高度
{
    ...
    if(code != CW_LIVENESS_SUCCESSFUL)
    {
        Log(ThisFile, 1, "按VID/PID开启相机: bOpenCameraEx()->cwEngineOpenCameraEx(%s, %s, %d, %d) fail. ReturnCode:%s",
...
LPSTR CDevImpl_CloudWalk::GetErrorStr(LONG lErrCode)
{
    ...
    switch(lErrCode)
    {
        ...
        case 75002004 :
            ...
            break;
        case 75002005 :
            sprintf(m_szErrStr,  "%d|%s", lErrCode, "非云从相机");
            break;
        case 75002006 :
