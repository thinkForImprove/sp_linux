...
void CCDES::AscToBcd(unsigned char *charbcd, unsigned char *charasc, int len)
{
...
void CCDES::BcdToAsc(unsigned char *charasc, unsigned char *charbcd, int len)
{
    int i;
    unsigned char cbuf[300], ubuf[300];
...
void CCDES::key_encrypt_Asc(unsigned char *input, unsigned char *key)
{
...
void CCDES::key_encrypt_Bcd(unsigned char *input, unsigned char *key)
{
    int i = 0;
...
void CCDES::setkey(unsigned char *key)
{
    // ADD for include
...
void CCDES::encrypt(unsigned char *block, int edflag)
{
    // ADD include
...
void CCDES::expand(unsigned char *in, unsigned char *out)
{
    int i, j;
...
void CCDES::compress(unsigned char *in, unsigned char *out)
{
    int temp;
    int i, j;
...
void CCDES::key_uncrypt_Asc(unsigned char *input, unsigned char *key)
{
    AscToBcd(cdeskey, key, 8);
    AscToBcd(cdesinput, input, 8);
...
void CCDES::key_uncrypt_Bcd(unsigned char *input, unsigned char *key)
{
    int     i;
...
void CCDES::pan_encrypt_Bcd(unsigned char *input, unsigned char *pan, unsigned char *key)
{
...
void CCDES::pan_encrypt_Asc(unsigned char *input, unsigned char *pan, unsigned char *key)
{
    AscToBcd(cdeskey, key, 8);
    AscToBcd(cdesinput, input, 8);
...
void CCDES::pan_uncrypt_Bcd(unsigned char *input, unsigned  char *pan, unsigned char *key)
{
    int     i, len;
...
void CCDES::pan_uncrypt_Asc(unsigned char *input, unsigned char *pan, unsigned char *key)
{
    AscToBcd(cdeskey, key, 8);
    AscToBcd(cdesinput, input, 8);
...
long CCDES::gDes(char *DataInput, char *key, char *DataOutput)
{
    int  nKeyLen, nInputLen;
    char left_key[17] = {0}, cen_key[17] = {0}, right_key[17] = {0}, tmpstr1[17] = {0};

    nKeyLen = strlen(key);
    nInputLen = strlen(DataInput);
    if (nKeyLen == 16)
    {
        for (int i = 0; i < (nInputLen + 1) / 16; i++)
        {
            key_encrypt_Asc((unsigned char *)DataInput + 16 * i, (unsigned char *)key);
            strcpy(DataOutput + 16 * i, (char *)cascoutput);
        }
    }
    else if (nKeyLen == 32)
    {
        memset(left_key, '\0', 17);
        memset(right_key, '\0', 17);
        memcpy(left_key, key, 16);
        memcpy(right_key, key + 16, 16);
        memset(tmpstr1, '\0', 17);
        for (int i = 0; i < (nInputLen + 1) / 16; i++)
        {
            key_encrypt_Asc((unsigned char *)DataInput + 16 * i, (unsigned char *)left_key);
            strcpy(tmpstr1, (char *)cascoutput);
            key_uncrypt_Asc((unsigned char *)tmpstr1, (unsigned char *)right_key);
            memset(tmpstr1, '\0', 17);
            strcpy(tmpstr1, (char *)cascoutput);
            key_encrypt_Asc((unsigned char *)tmpstr1, (unsigned char *)left_key);
            strcpy(DataOutput + 16 * i, (char *)cascoutput);
        }
    }
    else if (nKeyLen == 48)
    {
        ...
        memset(right_key, '\0', 17);
        memcpy(left_key, key, 16);
        memcpy(cen_key, key + 16, 16);
        memcpy(right_key, key + 32, 16);
        memset(tmpstr1, '\0', 17);
        for (int i = 0; i < (nInputLen + 1) / 16; i++)
        {
            key_encrypt_Asc((unsigned char *)DataInput + 16 * i, (unsigned char *)left_key);
            strcpy(tmpstr1, (char *)cascoutput);
            key_uncrypt_Asc((unsigned char *)tmpstr1, (unsigned char *)cen_key);
            strcpy(tmpstr1, (char *)cascoutput);
            key_encrypt_Asc((unsigned char *)tmpstr1, (unsigned char *)right_key);
            strcpy(DataOutput + 16 * i, (char *)cascoutput);
        }
    }
...
long  CCDES::gUndes(char *DataInput, char *key, char *DataOutput)
{
    int  nKeyLen, nInputLen;
    char left_key[17], cen_key[17], right_key[17], tmpstr1[17];
    ...
    if (nKeyLen == 16)
    {
        for (int i = 0; i < (nInputLen + 1) / 16; i++)
        {
            key_uncrypt_Asc((unsigned char *)DataInput + 16 * i, (unsigned char *)key);
            strcpy(DataOutput + 16 * i, (char *)cascoutput);
        }
    }
    else if (nKeyLen == 32)
    {
        ...
        for (int i = 0; i < (nInputLen + 1) / 16; i++)
        {
            key_uncrypt_Asc((unsigned char *)DataInput + 16 * i, (unsigned char *)left_key);
            strcpy(tmpstr1, (char *)cascoutput);
            key_encrypt_Asc((unsigned char *)tmpstr1, (unsigned char *)right_key);
            memset(tmpstr1, '\0', 17);
            strcpy(tmpstr1, (char *)cascoutput);
            key_uncrypt_Asc((unsigned char *)tmpstr1, (unsigned char *)left_key);
            strcpy(DataOutput + 16 * i, (char *)cascoutput);
        }
    }
    else if (nKeyLen == 48)
    {
        ...
        for (int i = 0; i < (nInputLen + 1) / 16; i++)
        {
            key_uncrypt_Asc((unsigned char *)DataInput + 16 * i, (unsigned char *)right_key);
            strcpy(tmpstr1, (char *)cascoutput);
            key_encrypt_Asc((unsigned char *)tmpstr1, (unsigned char *)cen_key);
            strcpy(tmpstr1, (char *)cascoutput);
            key_uncrypt_Asc((unsigned char *)tmpstr1, (unsigned char *)left_key);
            strcpy(DataOutput + 16 * i, (char *)cascoutput);
        }
