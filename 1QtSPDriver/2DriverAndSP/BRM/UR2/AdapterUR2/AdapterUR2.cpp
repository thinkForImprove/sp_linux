...
long CUR2Adapter::InitADPConfig()
{
    ...
    //是否支持计数修正功能（0：不支持，1：支持 默认为0）
    m_sDevConfigInfo.bCountCorrectionSupp = (int)cINI.GetValue("CountCorrectionSupp", "0");                     //30-00-00-00(FT#0040)
    ...
    //获取是否支持生成ＦＳＮ文件
    string m_strConfigFilePathTemp = SPETCPATH;
    m_strConfigFilePathTemp += "/";
    m_strConfigFilePathTemp += "SNConfig.ini";
    CINIFileReader ReaderConfigFileTemp;
    ReaderConfigFileTemp.LoadINIFile(m_strConfigFilePathTemp.c_str());

    CINIReader cINI2 = ReaderConfigFileTemp.GetReaderSection("DEFAULT");
    //FSNGENMODE2 iFGMode = (FSNGENMODE2)(int)cINI.GetValue("FSNGenerateMode", 1);
    int iFGMode = (int)cINI2.GetValue("FSNGenerateMode", 1);

    m_strConfigFileForPAB = (LPCSTR)cINI2.GetValue("WSSPConfigFilePath", "/etc/xfs/logical_services/WSAPConfig.ini");    //30-00-00-00(FS#0019)

    //设置是否支持ｆｓｎ
    CINIFileReader ReaderConfigFile;
    ReaderConfigFile.LoadINIFile(m_strConfigFileForPAB.c_str());
    CINIWriter cINIFORPAB = ReaderConfigFile.GetWriterSection("DEFAULT");
    cINIFORPAB.SetValue("IsSPSupportFSN", iFGMode);
...
//    ST_HW_CONFIG stHWConfig;
    ...
    if (!m_bBRMInitialized)
    {
        ...
        int iRet = m_pUR2Dev->SetUnitInfo(iTotalCashInURJB, /*tTime,*/ stOperationalInfo,   m_stHWConfig, stCassType, bArryAcceptDenoCode,
                                          ...
                                          m_sDevConfigInfo.bURJBSupp, m_sDevConfigInfo.bCountCorrectionSupp);               //30-00-00-00(FT#0040)
        UpdateADPDevStatus(iRet);
    }
/*
...
//30-00-00-00(FS#0009) add start
    ...
    if(strlen(m_szApAmountLimitINIItem[0]) > 0 && strlen(m_szApAmountLimitINIItem[1]) > 0 &&
       strlen(m_szApAmountLimitINIItem[2]) > 0){
        ...
        if (info.exists())
        {
            ...
            if(iniFile.LoadINIFile(m_szApAmountLimitINIItem[0])){
                CINIReader iniReader = iniFile.GetReaderSection(m_szApAmountLimitINIItem[1]);
                LPCSTR lpAmountLimitForAPStr = (LPCSTR)iniReader.GetValue(m_szApAmountLimitINIItem[2], "0");

                if(strlen(lpAmountLimitForAPStr) > 0){
                    for(int i = 0; i < strlen(lpAmountLimitForAPStr); i++){
                        if(lpAmountLimitForAPStr[i] < '0' || lpAmountLimitForAPStr[i] > '9'){
                            if(i == 0 && lpAmountLimitForAPStr[i] != '-'){
                                return WFS_ERR_INVALID_DATA;
                            }
                        }
                    }

                    iAmountLimitForAPTmp = atoi(lpAmountLimitForAPStr);                             //test#5
                    if(iAmountLimitForAPTmp < 0){
                        if(iAmountLimitForAPTmp == -1){
                            iAmountLimitForAPTmp = 0;
                        } else {
                            return WFS_ERR_INVALID_DATA;
                        }
	...
	Log(ThisModule, 0, "AP设定的入金限额：%d", iAmountLimitForAPTmp);                  //test#5
	return WFS_SUCCESS;
}
...

{
    ...
    //清除冠字号相关文件
    CThreadGenerateSN threadSN(SAVESERIALOPERAT_R, m_eLfsCmdId, iRet);
