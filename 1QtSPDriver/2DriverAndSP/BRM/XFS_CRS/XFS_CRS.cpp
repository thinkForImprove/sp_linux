    ...
    else
    {
        if (ADPStatus.stOutPutPosition == ADP_OUTPOS_EMPTY) //钞口无钞
        {
            ...
            if (iRet < 0)
            {
                ...
                {
                    //发送Taken事件
                    if(m_Param.GetTakenEventAfterCloseShutterFlag()){
                        ADPCDMCAPS CDMCaps;
...
//        }
    ...
    AddRetractDetailToExtra(pPresentStatus);
...
//    CAutoFireDeviceStatus::IgnoreItemTakenInsertedOnce(); //忽略一次ITEM插入取出事件
    ...
    //检验并保存结果数据
    if(m_Param.GetCashinRefuseCashCountAddFlag()){
        m_CashInInfor.AddRefusedCount((USHORT)ulRejectCount);
    } else {
        m_CashInInfor.SetRefusedCount((USHORT)ulRejectCount);
    }
...
HRESULT XFS_CRSImp::CIMOpenShutter(WORD wPos)
{
    ...
    HRESULT hRet = OpenShutter(ThisModule, FALSE, FALSE);
    if (hRet < 0)
    {
        return hRet;
    }
...
HRESULT XFS_CRSImp::CIMCloseShutter(WORD wPos)
{
    ...
    HRESULT hRet = CloseShutter(ThisModule, FALSE, FALSE);
    if (hRet < 0)
    {
        return hRet;
    }
...
void XFS_CRSImp::UpdateRetractCountToCUManager(BOOL bCDM, USHORT usPCUIndex)
{
    ...
    if (m_Param.GetRetractCountMode() == 0 && m_Param.GetCashUnitType() == 0)
    {
        ...
        if(bCDM) m_retractDetail = NoteIDCountArray[usPCUIndex - 1];
    }
...
void XFS_CRSImp::AddRetractDetailToExtra(ICDMPresentStatus *pPresentStatus)
{
    if(m_retractDetail.size() == 0){
        return;
    }
    ...
    GCNTCDM_DEPOSIT_ALLOWED(m_Param.GetCfgNoteTypeCantDepositMode()) ? //允许进
    m_Param.GetNoteTypeList() : GetNoteTypeListDipositable();
    if(nullptr == pNoteTypeList){
        return;
    }

    NOTEID_COUNT_ARRAY retractDetail;
    for(int i = 0; i < m_retractDetail.size(); i++){
        ULONG ulValue = 0;

        NOTEID_COUNT noteIdCnt = m_retractDetail[i];
        int iRet = FindNoteIDInNoteTypeList(noteIdCnt.first, pNoteTypeList);
        if(iRet != -1){
            ulValue = pNoteTypeList->lppNoteTypes[iRet]->ulValues;
        }

        NOTEID_COUNT valueCnt;
        valueCnt.first = ulValue;
        valueCnt.second = noteIdCnt.second;
        int j = 0;
        for(j = 0; j < retractDetail.size(); j++){
            NOTEID_COUNT valueCntTemp = retractDetail[i];
            if(valueCntTemp.first == valueCnt.first){
                valueCnt.second += valueCntTemp.second;
                retractDetail[i] = valueCnt;
                break;
            }
        }

        if(j >= retractDetail.size()){
            retractDetail.push_back(valueCnt);
        }
    }

    //设置Extra域内容
    for(int i = 0; i < retractDetail.size(); i++){
        NOTEID_COUNT valueCnt = retractDetail[i];

        char szKey[MAX_PATH] = "NG";
        char szValue[MAX_PATH] = {0};
        if(valueCnt.first != 0) sprintf(szKey, "%d", valueCnt.first);
        sprintf(szValue, "%d", valueCnt.second);

        pPresentStatus->AddExtra(szKey, szValue);
    }
