/***************************************************************************
* 文件名称: XFS_BCR_IDC.cpp
* 文件描述: 条码阅读模块命令处理接口(IDC命令系)
*
* 版本历史信息
* 变更说明: 建立文件
* 变更日期: 2022年7月13日
...
static const char *DEVTYPE = "BCR_IDC";
#define LOG_NAME     "XFS_BCR_IDC.log"
...
HRESULT CXFS_BCR::OnOpen(LPCSTR lpLogicalName)
{
        ...
        //m_stConfig.wDeviceType != XFS_EC2G)
    {
        ...
        SetErrorDetail((LPSTR)EC_ERR_DevNotSupp);
        ...
        if (hRet != WFS_SUCCESS)
        {
            if (m_nRetErrOLD[0] != hRet)
            {
                Log(ThisModule, __LINE__,
                    "BCR(IDC): 断线重连中: OnStatus->InnerOpen(%d) fail, ErrCode: %d.", TRUE, hRet);
                m_nRetErrOLD[0] = hRet;
            }
        } else
        {
            ...
            {
                Log(ThisModule, __LINE__,
                    "BCR(IDC): 断线重连完成: OnStatus->InnerOpen(%d) succ, RetCode:%d.", TRUE, hRet);
                m_nRetErrOLD[0] = hRet;
            }
        }
    }
...
//----------------------------IDC类型接口----------------------------
// 查询状态
HRESULT CXFS_BCR::GetStatus(LPWFSIDCSTATUS &lpStatus)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
...
// 查询能力值
HRESULT CXFS_BCR::GetCapabilities(LPWFSIDCCAPS &lpCaps)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();
...
// 读卡
HRESULT CXFS_BCR::ReadRawData(LPWORD lpReadData, LPWFSIDCCARDDATA *&lppCardData, DWORD dwTimeOut)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();

    WORD wReadDataChk = WFS_IDC_TRACK_WM;      // 0x8000
    ...
    {
        Log(ThisModule, __LINE__, "读卡(扫码): 入参lpReadData[%d] 无效, Return: %d",
