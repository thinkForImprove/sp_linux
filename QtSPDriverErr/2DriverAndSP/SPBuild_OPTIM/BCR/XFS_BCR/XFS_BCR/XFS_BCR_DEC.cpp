...
BOOL CXFS_BCR::LoadDevBCRDll(LPCSTR ThisModule)
{
    if (m_pBCRDev == nullptr)
    {
        if (m_pBCRDev.Load(m_stConfig.szDevDllNameBCR, "CreateIDevBCR", DEVTYPE2STR(m_stConfig.wDeviceType)) != 0)
        {
            ...
            SetErrorDetail((LPSTR)EC_ERR_DevXXXLoadFail);
            return false;
        } else
        {
...
INT CXFS_BCR::InitConfig()
{
    ...
    *********************************************************************/
    ...
    if (m_stConfig.wDeviceType == XFS_NT0861)
    {
        ...
        stDevOpenModeTmp.nOtherParam[0] = XFS_NT0861;  // 设备类型
        ...
        if (stDevOpenModeTmp.nOtherParam[3] < 0 || stDevOpenModeTmp.nOtherParam[3] > 4)
        {
            stDevOpenModeTmp.nOtherParam[3] = 0;
        }

        // 扫码读码时是否设置条码类型限制(0:不设置,按硬件所有类型识别, 1:根据入参设置类型识别限制, 缺省0)
        stDevOpenModeTmp.nOtherParam[4] =
                (WORD)m_cXfsReg.GetValue(szIniAppName, "ReadBcrSymModeSet", (DWORD)0);
        if (stDevOpenModeTmp.nOtherParam[4] != 0 &&
            stDevOpenModeTmp.nOtherParam[4] != 1)
        {
            stDevOpenModeTmp.nOtherParam[4] = 0;
        }
...
#define PRINT_INI_BUF2(STR1, STR2, STR3) \
    ...
    PRINT_INI_BUF2("\n\t\t\t\t 命令下发超时时间(单位:毫秒): DEVICE_SET_%d->SndTimeOut = %d", m_stConfig.wDeviceType, m_stConfig.stDevOpenMode.nOtherParam[1]);
    PRINT_INI_BUF2("\n\t\t\t\t 命令接收超时时间(单位:毫秒): DEVICE_SET_%d->RcvTimeOut = %d", m_stConfig.wDeviceType, m_stConfig.stDevOpenMode.nOtherParam[2]);
    PRINT_INI_BUF2("\n\t\t\t\t 扫码识读模式(0:硬件默认,1:手动识读,2:命令连续识读,3:上电连续识读,4:感应识读): DEVICE_SET_%d->ScanSymMode = %d", m_stConfig.wDeviceType, m_stConfig.stDevOpenMode.nOtherParam[3]);
    PRINT_INI_BUF2("\n\t\t\t\t 扫码读码时是否设置条码类型限制(0:不设置,按硬件所有类型识别,1:根据入参设置类型识别限制): DEVICE_SET_%d->ReadBcrSymModeSet = %d", m_stConfig.wDeviceType, m_stConfig.stDevOpenMode.nOtherParam[4]);
...
HRESULT CXFS_BCR::InnerReadBcr(LPWORD lpwSymList, DWORD dwTimeOut)
{
    ...
    if (m_clReadBcrOut.GetSize() == 0)
    {
        if (m_clReadBcrOut.NewBuff(uCount) != TRUE)
        {
            ...
            SetErrorDetail((LPSTR)EC_ERR_MemoryApply);
                ...
                if (m_stConfig.wReadBcrRetDataCode == 0)    // INI配置返回条码数据编码格式为UTF8
                {
                    ...
                    } else                      // 数据为GBK编码,转换为UTF8
                    {
                        ...
                        if (dwBcrDataResultSize < 1)
                        {
                            ...
                            SetErrorDetail((LPSTR)EC_ERR_CodeChange);
                            return WFS_ERR_OUT_OF_MEMORY;
                        }
                    }
