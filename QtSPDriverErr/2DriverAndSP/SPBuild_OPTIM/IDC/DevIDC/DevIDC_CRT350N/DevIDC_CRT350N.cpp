    ...
    m_pDevImpl(LOG_NAME_DEV, lpDevType)
{
    ...
    m_bIsChipHave = FALSE;                          // 卡是否有芯片初始化:F
    ...
    m_clErrorDet.SetDevName(EC_DEV_IDC);
...
int CDevIDC_CRT350N::Reset(MEDIA_ACTION enMediaAct, unsigned short usParam)
{
    ...
    if (AND_IS1(m_nTamperSensorStat, IMPL_TAMPER_PRESS))
    {
        ...
        if(nRet != IMP_SUCCESS)
        {
            ...
            m_clErrorDet.SetDevErrCode((LPSTR)EC_IDC_ERR_TamperRelease);
            return ConvertImplErrCode2IDC(nRet);
        }
...
int CDevIDC_CRT350N::GetStatus(STDEVIDCSTATUS &stStatus)
{
    ...
    if (IsTeaseCardProtect() != 0)
    {
        ...
        stStatus.wMedia = MEDIA_STAT_UNKNOWN;
        stStatus.wChipPower = CHIP_STAT_UNKNOWN;        
        m_clErrorDet.SetDevErrCode(EC_IDC_ERR_TeaseCard);
        return ERR_IDC_OTHER;
    }
    ...
    switch(nStat[1])
    {
        ...
        case IMPL_STAT_CARD_ISINSIDE:   // 卡在内部
        {
            ...
            {
                stStatus.wChipPower = CHIP_STAT_POWEREDOFF;// 存在未上电
                // 硬件无法检测卡是否有芯片存在, 通过读卡获取的结果
                if (m_bIsChipHave == FALSE)
                {
                    stStatus.wChipPower = CHIP_STAT_NOCARD;// 存在未上电
                }
                // 硬件检测卡无芯片
                if (nStat[5] == IMPL_STAT_CHIPNOTHAVE)
                {
                    stStatus.wChipPower = CHIP_STAT_NOCARD;// 无芯片
                }
            }
            break;
        }
            ...
            AND_IS1(stMediaData.dwRWType, RW_CHIP))
        {
            ...
            } else  // 读磁道
            {
                ...
                } else  // 读多个磁道,解析
                {
                    ...
                    {
                        // 组织证件数据写入应答
                        if (qtSplitstr[2].toStdString().length() < 1)
                        {
            ...
            if (AND_IS1(stMediaData.dwRWType, RW_CHIP))
            {
                ...
                {                    
                    ...
                    {
                        stMediaData.SetData(3, RW_RESULT_INV, nullptr, 0);
                        return IDC_SUCCESS;
                    }
                }
                m_bICPress = TRUE;
    ...
    switch(usType)
    {
        ...
        case SET_DEV_OPENMODE:      // 设置设备打开模式
        {
            if (vData != nullptr)
            {
                ...
                // 设置命令收发超时
                if (m_stOpenMode.nOtherParam[1] > 0 || m_stOpenMode.nOtherParam[2] > 0)
                {
                    m_pDevImpl.SetSndRcvTimeOut(m_stOpenMode.nOtherParam[1],
                                                m_stOpenMode.nOtherParam[2]);
                }
...
INT CDevIDC_CRT350N::AcceptMedia(MEDIA_ACTION enMediaAct, DWORD dwTimeOut)
{
    ...
    if (m_nAuxParam[0] == 1)    // INI配置启用防盗钩
    {
        ...
        if (m_nTamperSensorStat == IMPL_TAMPER_PRESS)   // 未释放
        {
            ...
            m_clErrorDet.SetDevErrCode((LPSTR)EC_IDC_ERR_TamperNotRelease);
...
#define CASE_SET_HW_DETAIL(IMP, DEVID) \
    ...
    switch(nRet)
    {
        ...
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_02 - 1, m_wDeviceType)            // 命令无法执行
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_03 - 1, m_wDeviceType)            // 硬件不支持
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_04 - 1, m_wDeviceType)            // 命令数据错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_05 - 1, m_wDeviceType)            // IC触点未释放
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_06 - 1, m_wDeviceType)            // 密钥不存在
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_07 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_08 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_09 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_10 - 1, m_wDeviceType)            // 卡堵塞
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_11 - 1, m_wDeviceType)            // Shuter错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_12 - 1, m_wDeviceType)            // 传感错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_13 - 1, m_wDeviceType)            // 不规则卡长度(过长)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_14 - 1, m_wDeviceType)            // 不规则卡长度(过短)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_15 - 1, m_wDeviceType)            // FRAM错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_16 - 1, m_wDeviceType)            // 卡位置移动
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_17 - 1, m_wDeviceType)            // 重进卡时卡堵塞
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_18 - 1, m_wDeviceType)            // SW1,SW2错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_19 - 1, m_wDeviceType)            // 卡没有从后端插入
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_20 - 1, m_wDeviceType)            // 读磁卡错误(奇偶校验错)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_21 - 1, m_wDeviceType)            // 读磁卡错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_22 - 1, m_wDeviceType)            // 写磁卡错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_23 - 1, m_wDeviceType)            // 读磁卡错误(没有数据内容,只有STX起始符,ETX结束符和LRC)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_24 - 1, m_wDeviceType)            // 读磁卡错误(没有磁条或没有编码-空白轨道)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_25 - 1, m_wDeviceType)            // 写磁卡校验错误(品质错误)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_26 - 1, m_wDeviceType)            // 读磁卡错误(没有SS)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_27 - 1, m_wDeviceType)            // 读磁卡错误(没有ES)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_28 - 1, m_wDeviceType)            // 读磁卡错误(LRC错误)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_29 - 1, m_wDeviceType)            // 写磁卡校验错误(数据不一致)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_30 - 1, m_wDeviceType)            // 电源掉电
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_31 - 1, m_wDeviceType)            // DSR信号为OFF
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_32 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_40 - 1, m_wDeviceType)            // IC触点或触点传感器错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_42 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_43 - 1, m_wDeviceType)            // 无法走到IC卡位
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_44 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_45 - 1, m_wDeviceType)            // 卡机强制弹卡
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_46 - 1, m_wDeviceType)            // 前端卡未在指定时间内取走
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_47 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_48 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_49 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_50 - 1, m_wDeviceType)            // 回收卡计数溢出
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_51 - 1, m_wDeviceType)            // 马达错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_52 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_53 - 1, m_wDeviceType)            // 数字解码读错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_54 - 1, m_wDeviceType)            // 防盗钩移动错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_55 - 1, m_wDeviceType)            // 防盗钩已经设置,命令不能执行
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_56 - 1, m_wDeviceType)            // 芯片检测传感器错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_57 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_58 - 1, m_wDeviceType)            // 防盗钩正在移动
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_59 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_60 - 1, m_wDeviceType)            // IC卡或SAM卡Vcc条件异常
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_61 - 1, m_wDeviceType)            // IC卡或SAM卡ATR通讯错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_62 - 1, m_wDeviceType)            // IC卡或SAM卡在当前激活条件下ATR无效
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_63 - 1, m_wDeviceType)            // IC卡或SAM卡通讯过程中无响应
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_64 - 1, m_wDeviceType)            // IC卡或SAM卡通讯错误(除无响应外)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_65 - 1, m_wDeviceType)            // IC卡或SAM卡未激活
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_66 - 1, m_wDeviceType)            // IC卡或SAM卡不支持(仅对于非EMV激活)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_67 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_68 - 1, m_wDeviceType)            //
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_69 - 1, m_wDeviceType)            // IC卡或SAM卡不支持(仅对于EMV激活)
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_76 - 1, m_wDeviceType)            // ESU模块和卡机通讯错误
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_95 - 1, m_wDeviceType)            // ESU模块损坏或无连接
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_99 - 1, m_wDeviceType)            // ESU模块过流
        CASE_SET_HW_DETAIL(IMP_ERR_DEVRET_B0 - 1, m_wDeviceType)            // 未接收到初始化命令
    }
