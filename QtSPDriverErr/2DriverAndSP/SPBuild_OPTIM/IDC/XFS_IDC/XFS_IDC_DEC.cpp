...
BOOL CXFS_IDC::LoadDevIDCDll(LPCSTR ThisModule)
{
    if (m_pIDCDev == nullptr)
    {
        if (m_pIDCDev.Load(m_stConfig.szDevDllNameIDC, "CreateIDevIDC", DEVTYPE2STR(m_stConfig.wDeviceType)) != 0)
        {
            ...
            SetErrorDetail(0, (LPSTR)EC_ERR_DevXXXLoadFail);
            return false;
        } else
        {
...
INT CXFS_IDC::InitConfig()
{
    ...
    if (m_stConfig.wDeviceType == XFS_CRT350N)
    {
        ...
        stDevOpenModeTmp.nOtherParam[0] = XFS_CRT350N;  // 设备类型
...
void CXFS_IDC::UpdateExtra()
{
    ...
    CHAR szDevVerBuff[256] = { 0x00 };
    CHAR szFWVerBuff[256] = { 0x00 };

    // 组织状态扩展数据
    //m_cStatExtra.Clear();
    m_cStatExtra.AddExtra("VRTCount", "1");
    m_cStatExtra.AddExtra("VRTDetail[00]", (char*)byXFSVRTU);

    sprintf(szDevVerBuff + strlen(szDevVerBuff), "IDC:%s", byDevIDCVRTU);
    if (m_stCRMConfig.bCRMDeviceSup == TRUE)
    {
       sprintf(szDevVerBuff + strlen(szDevVerBuff), ", CRM:%s", byDevCRMVRTU);
    }
    m_cStatExtra.AddExtra("VRTCount", "2");
    m_cStatExtra.AddExtra("VRTDetail[01]", (char*)szDevVerBuff);

    // 取固件版本写入扩展数据
    memcpy(szFWVerBuff, "FW: ", 4);
    m_pIDCDev->GetVersion(GET_VER_FW, szFWVersion, 64);
    if (strlen(szFWVersion) > 0)
    {
        sprintf(szFWVerBuff + strlen(szFWVerBuff), "IDC:%s", szFWVersion);
    }

    if (m_stCRMConfig.bCRMDeviceSup == TRUE)
    {
       //sprintf(szDevVerBuff + strlen(szDevVerBuff), "%s, CRM:", byDevCRMVRTU);
    }
    m_cStatExtra.AddExtra("VRTCount", "3");
    m_cStatExtra.AddExtra("VRTDetail[02]", szFWVerBuff);
...
HRESULT CXFS_IDC::InnerAcceptAndReadTrack(DWORD dwReadOption, DWORD dwTimeOut)
{
    ...
    if (m_stConfig.wRetainCardCount >= m_stConfig.wRetainFull)     // 回收满
    {
        ...
        } else
        {
            ...
            SetErrorDetail(0, (LPSTR)EC_ERR_BoxFull);
    ...
    if (bIsCheck)
    {
        ...
        if (wAction == WFS_IDC_RETAIN && bIsHaveCard == TRUE)
        {
            if (m_stConfig.wRetainSupp == 0)    // INI设置不支持吞卡
            {
                ...
                SetErrorDetail(0, (LPSTR)EC_ERR_RetainNoSupp);
...
INT CXFS_IDC::SetErrorDetail(WORD wDevType, LPSTR lpCode)
{
    if (wDevType == 0)  // IDC
    {
        m_clErrorDet.SetXFSName(EC_XFS_IDC);
    } else
    if (wDevType == 1)  // CRM
    {
        m_clErrorDet.SetXFSName(EC_XFS_CRM);
    } else
    if (wDevType == 2)  // CRD
    {
        m_clErrorDet.SetXFSName(EC_XFS_CRD);
    }
