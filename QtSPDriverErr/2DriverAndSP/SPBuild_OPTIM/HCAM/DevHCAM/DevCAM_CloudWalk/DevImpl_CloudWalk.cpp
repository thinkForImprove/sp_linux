...
void CDevImpl_CloudWalk::Init()
{
    ...
    m_LoadLibrary.setFileName(m_szCloudWalkDllPath);
    ...
    cwEngineEnumResolutions = NULL;        // 1.13 枚举相机支持的所有分辨率(序号)
    cwEngineEnumResolutionsEx = NULL;      // 1.14 枚举相机支持的所有分辨率(VIDPID)
    ...
    cwEngineGetBestFace1 = NULL;           // 1.20 获取本次活体检测最佳人脸图像
    cwEngineSaveBestFace1 = NULL;          // 1.21 保存活体最佳人脸到指定路径
    ...
    cwEngineSleepCamera = NULL;            // 1.28 依照相机在设备上的序号休眠相机
    cwEngineSleepCameraEx = NULL;          // 1.29 依照相机硬件信息VID/PID 休眠相机
    ...
    if (m_LoadLibrary.isLoaded() != true)
    {
        if (m_LoadLibrary.load() != true)
        {
            Log(ThisFile, 1, "加载动态库<%s> fail. ReturnCode:%s.",
                m_szCloudWalkDllPath, m_LoadLibrary.errorString().toStdString().c_str());
            return FALSE;
        }
    }
...
BOOL CDevImpl_CloudWalk::LoadCloudWalkIntf()
{
    ...
    // 1.1 创建检测对象
    FUNC_LOADLIBRARY(pcwEngineCreateDetector, cwEngineCreateDetector, "cwEngineCreateDetector");

    // 1.2 释放检测对象及COM服务
    FUNC_LOADLIBRARY(pcwEngineReleaseDetector, cwEngineReleaseDetector, "cwEngineReleaseDetector");

    // 1.3 获取COM服务及算法版本号
    FUNC_LOADLIBRARY(pcwEnginGetVersion, cwEnginGetVersion, "cwEngineGetVersion");

    // 1.4 注册相机回调回显函数
    FUNC_LOADLIBRARY(pcwEngineRegisterPreviewHandle, cwEngineRegisterPreviewHandle, "cwEngineRegisterPreviewHandle");

    // 1.5 注册检测回调函数对象
    FUNC_LOADLIBRARY(pcwEngineRegisterDetectHandle, cwEngineRegisterDetectHandle, "cwEngineRegisterDetectHandle");

    // 1.6 加载检测模型及设置检测参数
    FUNC_LOADLIBRARY(pcwEngineInitDetectorParam, cwEngineInitDetectorParam, "cwEngineInitDetectorParam");

    // 1.7 是否进行人脸追踪显示
    FUNC_LOADLIBRARY(pcwEngineEnableFaceTrace, cwEngineEnableFaceTrace, "cwEngineEnableFaceTrace");

    // 1.9 依据相机在设备显示的序号开启相机
    FUNC_LOADLIBRARY(pcwEngineOpenCamera, cwEngineOpenCamera, "cwEngineOpenCamera");

    // 1.10 依据相机硬件信息开启相机
    FUNC_LOADLIBRARY(pcwEngineOpenCameraEx, cwEngineOpenCameraEx, "cwEngineOpenCameraEx");

    // 1.11 关闭当前程序打开的所有相机设备
    FUNC_LOADLIBRARY(pcwEngineCloseAllCameras, cwEngineCloseAllCameras, "cwEngineCloseAllCameras");

    // 1.12 枚举当前设备上的所有相机	// 30-00-00-00(FT#0031)
    FUNC_LOADLIBRARY(pcwEngineEnumCameras, cwEngineEnumCameras, "cwEngineEnumCameras");

    // 1.13 枚举相机支持的所有分辨率(序号)
    FUNC_LOADLIBRARY(pcwEngineEnumResolutions, cwEngineEnumResolutions, "cwEngineEnumResolutions");

    // 1.14 枚举相机支持的所有分辨率(VIDPID)
    FUNC_LOADLIBRARY(pcwEngineEnumResolutionsEx, cwEngineEnumResolutionsEx, "cwEngineEnumResolutionsEx");

    // 1.15 开启活体检测
    FUNC_LOADLIBRARY(pcwEngineStartLiveDetect, cwEngineStartLiveDetect, "cwEngineStartLiveDetect");

    // 1.16 停止连续活体检测
    FUNC_LOADLIBRARY(pcwEngineStopLiveDetect, cwEngineStopLiveDetect, "cwEngineStopLiveDetect");

    // 1.17 抓拍当前画面帧
    FUNC_LOADLIBRARY(pcwEngineCaptureCurrentFrame, cwEngineCaptureCurrentFrame, "cwEngineCaptureCurrentFrame");

    // 1.18 保存当前帧至指定文件路径参数
    FUNC_LOADLIBRARY(pcwEngineSaveCurrentFrame, cwEngineSaveCurrentFrame, "cwEngineSaveCurrentFrame");

    // 1.19 开启语音提示功能	// 30-00-00-00(FT#0031)
    FUNC_LOADLIBRARY(pcwEngineEnableVoiceTip, cwEngineEnableVoiceTip, "cwEngineEnableVoiceTip");

    if (m_wSDKVersion == 1)
    {
        // 1.20获取本次活体检测最佳人脸图像
        FUNC_LOADLIBRARY(pcwEngineGetBestFace1, cwEngineGetBestFace1, "cwEngineGetBestFace");

        // 1.21 保存活体最佳人脸到指定路径
        FUNC_LOADLIBRARY(pcwEngineSaveBestFace1, cwEngineSaveBestFace1, "cwEngineSaveBestFace");
    } else
    {
        // 1.20获取本次活体检测最佳人脸图像
        FUNC_LOADLIBRARY(pcwEngineGetBestFace, cwEngineGetBestFace, "cwEngineGetBestFace");

        // 1.21 保存活体最佳人脸到指定路径
        FUNC_LOADLIBRARY(pcwEngineSaveBestFace, cwEngineSaveBestFace, "cwEngineSaveBestFace");
    }

    // 1.22 通过序号得到相机状态
    FUNC_LOADLIBRARY(pcwEngineGetCameraStatus, cwEngineGetCameraStatus, "cwEngineGetCameraStatus");

    // 1.23 通过相机硬件信息VID/PID得到相机状态
    FUNC_LOADLIBRARY(pcwEngineGetCameraStatusEx, cwEngineGetCameraStatusEx, "cwEngineGetCameraStatusEx");

    // 1.26 将图片转换成base64编码格式
    FUNC_LOADLIBRARY(pcwEngineImage2Base64Format, cwEngineImage2Base64Format, "cwEngineImage2Base64Format");

    // 1.28 依照相机在设备上的序号休眠相机
    FUNC_LOADLIBRARY(pcwEngineSleepCamera, cwEngineSleepCamera, "cwEngineSleepCamera");

    // 1.29 依照相机硬件信息VID/PID休眠相机
    FUNC_LOADLIBRARY(pcwEngineSleepCameraEx, cwEngineSleepCameraEx, "cwEngineSleepCameraEx");

    // 1.32 获得相机固件版本信息，依照相机在设备上的序号
    FUNC_LOADLIBRARY(pcwEngineGetFirmwareVersion, cwEngineGetFirmwareVersion, "cwEngineGetFirmwareVersion");

    // 1.33 获得相机固件版本信息，依照相机在设备VID/PID
    FUNC_LOADLIBRARY(pcwEngineGetFirmwareVersionEx, cwEngineGetFirmwareVersionEx, "cwEngineGetFirmwareVersionEx");
...
// 1.13 枚举相机支持的所有分辨率(序号)
BOOL CDevImpl_CloudWalk::bEnumResolutions(SHORT deviceIndex, CWResolution** resolutionLists)
{
    LONG code = cwEngineEnumResolutions(m_cwDetector, deviceIndex, resolutionLists);
    if(code != CW_LIVENESS_SUCCESSFUL)
    {
        Log(ThisFile, 1, "枚举相机支持的所有分辨率(序号): bEnumResolutions()->cwEngineEnumResolutions() fail. ReturnCode:%s", GetErrorStr(code));
        return FALSE;
    }
...
// 1.14 枚举相机支持的所有分辨率(VIDPID)
BOOL CDevImpl_CloudWalk::bEnumResolutionsEx(LPSTR vid, LPSTR pid, CWResolution** resolutionLists)
{
    LONG code = cwEngineEnumResolutionsEx(m_cwDetector, vid, pid, resolutionLists);
    if(code != CW_LIVENESS_SUCCESSFUL)
    {
        Log(ThisFile, 1, "枚举相机支持的所有分辨率(VIDPID): bEnumResolutionsEx()->cwEngineEnumResolutionsEx() fail. ReturnCode:%s", GetErrorStr(code));
        return FALSE;
    }
...
// 1.20 获取本次活体检测最佳人脸图像
BOOL CDevImpl_CloudWalk::bGetBestFace(float ratio, BOOL isBase64, CWLiveImage* image, INT nType)
{
    LONG code = CW_LIVENESS_SUCCESSFUL;

    if (m_wSDKVersion == 1)
    {
        code = cwEngineGetBestFace1(m_cwDetector, ratio, isBase64, nType, image);
    } else
    {
        code = cwEngineGetBestFace(m_cwDetector, ratio, isBase64, image);
    }

    if(code != CW_LIVENESS_SUCCESSFUL)
    {
        Log(ThisFile, 1, "获取活体检测最佳人脸图像: bGetBestFace()->cwEngineGetBestFace(%f, %d, %d) fail. ReturnCode:%s",
            ratio, isBase64, nType, GetErrorStr(code));
        return FALSE;
    }
...
// 1.28 依照相机在设备上的序号休眠相机
BOOL CDevImpl_CloudWalk::bSleepCamera(SHORT index, BOOL bIsSleep)
{
    LONG code = cwEngineSleepCamera(m_cwDetector, index, bIsSleep);
    if(code != CW_LIVENESS_SUCCESSFUL)
    {
        Log(ThisFile, 1, "依照相机在设备上的序号休眠相机: bSleepCamera()->cwEngineSleepCamera(%d, %d) fail. ReturnCode:%s",
            index, bIsSleep, GetErrorStr(code));
        return FALSE;
    }
...
BOOL CDevImpl_CloudWalk::bSleepCameraEx(LPSTR vid, LPSTR pid, BOOL bIsSleep)
{
    LONG code = cwEngineSleepCameraEx(m_cwDetector, (const char*)vid, (const char*)pid, bIsSleep);
    if(code != CW_LIVENESS_SUCCESSFUL)
    {
        Log(ThisFile, 1, "通过相机硬件信息VID/PID休眠相机: bSleepCameraEx()->cwEngineSleepCameraEx(%s, %s, %d) fail. ReturnCode:%s",
            vid, pid, bIsSleep, GetErrorStr(code));
        return FALSE;
    }
