...
#define LFSAPI              __attribute__((__cdecl__))
    //////////////////////////////////////////////////////////////////////////
//30-00-00-00(FT#0074)
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS    //30-00-00-00(FT#0074)
...
#endif
    {
        ...
        if (logical_name == nullptr)
            return WFS_ERR_INVALID_POINTER;
        CAgentXFS *gpAgent = nullptr;       //30-00-00-00(FT#0020)
        gpAgent = GetAgent(service);
        if (gpAgent == nullptr)
        {
            QDateTime qT = QDateTime::currentDateTime();
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS      //30-00-00-00(FT#0074)
        if (gpAgent->SetHWND(ThisModule, hWnd)) //30-00-00-00(FT#0074)
            return WFS_ERR_INVALID_HWND;        //30-00-00-00(FT#0074)
#else                           //30-00-00-00(FT#0074)
...
#endif                                      //30-00-00-00(FT#0074)
    {
        ...
        if (gpAgent == nullptr)
            return WFS_ERR_INTERNAL_ERROR;
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                  //30-00-00-00(FT#0074)
        if (gpAgent->SetHWND(ThisModule, hWndReg, true))    //30-00-00-00(FT#0074)
            return WFS_ERR_INVALID_HWND;    //30-00-00-00(FT#0074)
        if (gpAgent->SetHWND(ThisModule, hWnd)) //30-00-00-00(FT#0074)
            return WFS_ERR_INVALID_HWND;    //30-00-00-00(FT#0074)
#else                                       //30-00-00-00(FT#0074)
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                   //30-00-00-00(FT#0074)
    AGENTXFS_EXPORT HRESULT LFSAPI WFPDeregister(HSERVICE service, DWORD event_class, HWND hWndReg, HWND hWnd, REQUESTID req_id)//30-00-00-00(FT#0074)
#else                                       //30-00-00-00(FT#0074)
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                                   //30-00-00-00(FT#0074)
    AGENTXFS_EXPORT HRESULT LFSAPI WFPLock(HSERVICE service, DWORD timeout, HWND hWnd, REQUESTID req_id)//30-00-00-00(FT#0074)
#else                                                       //30-00-00-00(FT#0074)
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                    //30-00-00-00(FT#0074)
    AGENTXFS_EXPORT HRESULT LFSAPI WFPUnlock(HSERVICE service, HWND hWnd, REQUESTID req_id)  //30-00-00-00(FT#0074)
#else                                        //30-00-00-00(FT#0074)
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                    //30-00-00-00(FT#0074)
    AGENTXFS_EXPORT HRESULT LFSAPI WFPGetInfo(HSERVICE service, DWORD category, LPVOID query_details, DWORD timeout, HWND hWnd, REQUESTID req_id) //30-00-00-00(FT#0074)
#else                                        //30-00-00-00(FT#0074)
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                   //30-00-00-00(FT#0074)
    AGENTXFS_EXPORT HRESULT LFSAPI WFPExecute(HSERVICE service, DWORD command, LPVOID cmd_data, DWORD timeout, HWND hWnd, REQUESTID req_id)//30-00-00-00(FT#0074)
#else                                       //30-00-00-00(FT#0074)
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                   //30-00-00-00(FT#0074)
    AGENTXFS_EXPORT HRESULT LFSAPI WFPCancelAsyncRequest(HSERVICE service, REQUESTID request_id)//30-00-00-00(FT#0074)
#else                                       //30-00-00-00(FT#0074)
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                   //30-00-00-00(FT#0074)
    AGENTXFS_EXPORT HRESULT LFSAPI WFPSetTraceLevel(HSERVICE service, DWORD trace_level)//30-00-00-00(FT#0074)
#else                                       //30-00-00-00(FT#0074)
...
#ifdef EVENT_NOTIFY_POSTMESSAGE_XFS                   //30-00-00-00(FT#0074)
    AGENTXFS_EXPORT HRESULT LFSAPI WFPUnloadService()//30-00-00-00(FT#0074)
#else
...
//////////////////////////////////////////////////////////////////////////
#elif EVENT_NOTIFY_POSTMESSAGE_XFS
HRESULT CAgentXFS::SetHWND(LPCSTR ThisModule, HWND hWnd, bool bReg /*= false*/)
{
    // 判断窗口是否有效
    if (m_pQWin == nullptr)
...
void CAgentXFS::Run_Read()
{
    ...
    try
    {
        ...
        while (!m_bQuitRun)
        {
            ...
            Log(ThisModule, __LINE__, "接收到结果数据：ReqId:%d", stResult.ReqID);
        }
    }
    catch (...)
    {
...
*/
            if(m_pIWFM->WFMPostMessage(hWndResult, stResult.uMsgID, 0, (LONG)stResult.lpResult, 0)){
                Log(ThisModule, __LINE__, "发送事件数据失败:uMsgID = %d(%s)", stResult.uMsgID, ConvertMSGID(stResult.uMsgID));
                FreeBuffer(stResult);
            }
            ...
            // 内存地址转换
            if (!AllocateBufferLFSToMGR(bComEvent, stResult, stMGRResult))
            {
                Log(ThisModule, __LINE__, "LFSManager转GRGManager内存申请失败:uMsgID = %d(%s)", stResult.uMsgID, ConvertMSGID(stResult.uMsgID));
                FreeBuffer(stResult);
                continue;
            }
            if(m_pIWFM->WFMPostMessage((LPSTR)strObjectName.c_str(), stMGRResult.uMsgID, 0, (LONG)stMGRResult.lpResult, 0) != WFS_SUCCESS){
                Log(ThisModule, __LINE__, "发送事件数据失败:uMsgID = %d(%s)", stResult.uMsgID, ConvertMSGID(stResult.uMsgID));
                FreeBufferMGR(stResult);
            }
#else
            ...
            if (!m_pQWin->PostMessageDBus(strObjectName.c_str(), stResult.uMsgID, (LPVOID)stResult.lpResult))
            {
                Log(ThisModule, __LINE__, "发送事件数据失败:uMsgID = %d(%s)", stResult.uMsgID, ConvertMSGID(stResult.uMsgID));
                FreeBuffer(stResult);
                continue;
...
bool CAgentXFS::LoadDll(LPCSTR ThisModule)
{
    if (m_pAgent == nullptr)
    {
        ...
        {
            Log(ThisModule, __LINE__, "加载库失败: AgentDllName=%s[%s]", szAgentDllName, m_pAgent.LastError().toStdString().c_str());
            return false;
        }
    }
    Log(ThisModule, __LINE__,"begin load");
    if (m_pIWFM == nullptr)
    {
...
#if (defined QT_WIN_LINUX_XFS) || (defined EVENT_NOTIFY_POSTMESSAGE_XFS)    //30-00-00-00(FT#0074)
...
HRESULT CAgentXFS::CreateSPWnd(LPCSTR lpszLogicalName, LPCSTR lpszSPName, DWORD dwTimeOut)
{
    ...
    Log(ThisModule, __LINE__, "PPPID=%s", m_strAppPID.c_str());
...
void CAgentXFS::FreeBufferMGR(SPCMDDATA &stCmd)
{
    if (stCmd.lpCmdData != nullptr && m_pIWFM != nullptr)
        m_pIWFM->SIMFreeBuffer(stCmd.lpCmdData);
    stCmd.clear();
}

void CAgentXFS::FreeBufferMGR(SPRESULTDATA &stResult)
{
    if (stResult.lpResult != nullptr && m_pIWFM != nullptr)
        m_pIWFM->SIMFreeBuffer(stResult.lpResult);
    stResult.clear();
}
...
/*************************************************************************************
** Function type		= inner 													**
** Source file			= AgentXFS.cpp 												**
** Class				= CAgentXFS 												**
** Function identifier	= CAgentXFS::AllocateBufferLFSToGRG()                       **
** Name 				= AllocateBufferLFSToGRG                                    **
** Function declaration = LPWFSVERSION CAgentXFS::AllocateBufferLFSToMGR(           **
**                      const SPRESULTDATA stLFSResult, SPRESULTDATA &stGRGResult)	**
** Parameter			= SPRESULTDATA												**
** Return value 		= SPRESULTDATA												**
** Notes				=															**
** Date(YYYY/MM/DD) 	= 2021/07/23												**
** V-R-T				= 30-00-00-00(FS#0011)										**
** History				= 2021/07/23 30-00-00-00(FS#0011) for CFES                  **
*************************************************************************************/
bool CAgentXFS::AllocateBufferLFSToMGR(bool isComEvent, SPRESULTDATA stLFSResult, SPRESULTDATA &stMGRResult)
{
    THISMODULE(__FUNCTION__);
    AutoLogFuncBeginEnd();

    HRESULT hRet = WFS_SUCCESS;
    memcpy(&stMGRResult, &stLFSResult, sizeof(SPRESULTDATA));

    //拷贝WFSRESULT通用数据
    hRet = m_pIWFM->SIMAllocateBuffer(sizeof(WFSRESULT), WFS_MEM_ZEROINIT, (LPVOID *)&stMGRResult.lpResult);
    if (hRet != WFS_SUCCESS){
        Log(ThisModule, __LINE__, "AllocateBufferLFSToMGR : hRet = %d, uMsgID = %d(%d)", hRet, stLFSResult.uMsgID, stLFSResult.lpResult->u.dwCommandCode);
        return false;
    }
    memcpy(stMGRResult.lpResult, stLFSResult.lpResult, sizeof(WFSRESULT));
    stMGRResult.lpResult->lpBuffer = nullptr;

    // 拷贝lpBuffer数据
    if (isComEvent)
    {
        if (stLFSResult.uMsgID == WFS_GETINFO_COMPLETE)
        {
            hRet = m_pAgent->GetInfoOut(stLFSResult.lpResult->u.dwCommandCode, stLFSResult.lpResult->lpBuffer, stMGRResult.lpResult);
            if (hRet != WFS_SUCCESS)
                return false;
        } else if (stLFSResult.uMsgID == WFS_EXECUTE_COMPLETE)
        {
            hRet = m_pAgent->ExecuteOut(stLFSResult.lpResult->u.dwCommandCode, stLFSResult.lpResult->lpBuffer, stMGRResult.lpResult);
            if (hRet != WFS_SUCCESS){
                Log(ThisModule, __LINE__, "AllocateBufferLFSToMGR--ExecuteOut: hRet = %d, uMsgID = %d(%d)", hRet, stLFSResult.uMsgID, stLFSResult.lpResult->u.dwCommandCode);
                return false;
            }
        }
    } else {
        // 事件处理
        hRet = m_pAgent->CopyEventStruct(stLFSResult.uMsgID, stLFSResult.lpResult->u.dwEventID, stLFSResult.lpResult->lpBuffer, stMGRResult.lpResult);
        if (hRet != WFS_SUCCESS)
            return false;
    }

    FreeBuffer(stLFSResult);
    return true;
}
