...
CSPBaseClass::CSPBaseClass(LPCSTR lpLogType) : m_pCmdRun(nullptr), m_cXFSMutex("XFS_SP_Mutex"), m_pSpRMem(nullptr)
{
    ...
    m_hServiceIdCallExec = 0;                           //30-00-00-00(FT#0059)
...
bool CSPBaseClass::FireEvent(UINT uMsgID, DWORD dwEventID, LPVOID lpData)
{
    ...
    for (auto &it : listApp)
    {
        ...
        //　执行事件只向发起执行命令的app发送
        if(uMsgID == WFS_EXECUTE_EVENT){                                                //30-00-00-00(FT#0059)
            if((m_hServiceIdCallExec != 0) && (it.hService != m_hServiceIdCallExec)){   //30-00-00-00(FT#0059)
                continue;                                                               //30-00-00-00(FT#0059)
            }                                                                           //30-00-00-00(FT#0059)
        }                                                                               //30-00-00-00(FT#0059)
...
//30-00-00-00(FT#0070)    CAutoSetExecuteStatus _auto_status(&m_bExecuting);
    m_hServiceIdCallExec = stCmd.hService;              //30-00-00-00(FT#0059)
...
void CSPBaseClass::Run_Read()
{
    ...
    try
    {
        ...
        while (!m_bQuitRun)
        {
            ...
            Log(ThisModule, __LINE__, "接收到命令数据:ReqId:%d", stCmd.ReqID);
            stCmd.clear();
        }
    }
    catch (...)
...
void CSPBaseClass::Run_GetInfo()
{
    ...
    try
    {
        ...
        while (!m_bQuitRun)
        {
            ...
            Log(ThisModule, __LINE__, "开始执行信息命令：ReqId:%d", stCmd.ReqID);
            // 获取结果内存
            stResult.clear();
            HRESULT hRet = GetResultBuff(stCmd, stResult);
            if (WFS_SUCCESS != hRet)
...
void CSPBaseClass::Run_Execute()
{
    ...
    try
    {
        ...
        while (!m_bQuitRun)
        {
            ...
            Log(ThisModule, __LINE__, "开始执行执行命令：ReqId:%d", stCmd.ReqID);
			// 获取结果内存
            stResult.clear();
            HRESULT hRet = GetResultBuff(stCmd, stResult);
            if (WFS_SUCCESS != hRet)
...
void CSPBaseClass::Run_Result()
{
    ...
    try
    {
        ...
        while (!m_bQuitRun)
        {
            ...
            Log(ThisModule, __LINE__, "发送结果数据成功：MsgType:%d, CommandOrEventId:%d, ReqId:%d",
                stResult.uMsgID,
                stResult.uMsgID >= WFS_EXECUTE_EVENT ? stResult.lpResult->u.dwEventID : stResult.lpResult->u.dwCommandCode,
                stResult.ReqID);
        }
    }
    catch (...)
