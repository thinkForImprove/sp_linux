...
{
public:
    CAutoMemoryLock(QSharedMemory *pMemory): m_pMemory(pMemory)
    {
        if (m_pMemory != nullptr) m_pMemory->lock();
    }
    ~CAutoMemoryLock()
    {
        if (m_pMemory != nullptr) m_pMemory->unlock();
    }
private:
...
{
public:
    CAutoSemMutexLock(QSystemSemaphore *pMutex) : m_pMutex(pMutex)
    {
        if (m_pMutex != nullptr) m_pMutex->acquire();
    }
    ~CAutoSemMutexLock()
    {
        if (m_pMutex != nullptr) m_pMutex->release();
    }
private:
...
bool CQtShareMemoryRW::Read(void *pBuff, int nBuffSize)
{
    ...
    memcpy(pBuff, (char *)m_cMem.constData() + m_vtFlagW.size(), qMin(m_cMem.size() - m_vtFlagW.size(), nBuffSize));
