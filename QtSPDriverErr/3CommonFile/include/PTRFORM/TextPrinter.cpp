                    ...
                    case FT_TEXT: // (default)
                        {
                            ...
                            QTextCodec *qtCodecUTF = QTextCodec::codecForName("UTF-8");
                            qtCodecUTF->toUnicode(lpszText, strlen(lpszText), &qtState);

                            if (!qtState.invalidChars)   // UTF8:需要转码
                            {
                                // 转码
                                ...
                                hRes = DrawText(&pc, pBuf, &rc, index);
                                delete [] pBuf;
                            } else
                            {
                                hRes = DrawText(&pc, lpszText, &rc, index);
                            }
                        }
                        break;
                    case FT_BARCODE:
...
HRESULT CTextPrinter::DrawText(PrintContext *pContext, LPCSTR szText, LPRECT pRect, WORD wFieldIdx)
{
...
/*BOOL CTextPrinter::NeedFormatString()
{
    return FALSE;
}*/
...
HRESULT CTextPrinter::EndReadForm(ReadContext *pContext)
{
    ...
    // ReadForm结果相关:子类继承重写:根据需要进行重写
    return 0;

    /* 继承后处理可参考如下编码:
    // 根据Position X,Y 对Field数据进行排序
    ...
    {
        PRINT_ITEM *pItem = pItems->pItems[i];  // 单个可操作Field结构体
        // 以下为处理过程
    }
