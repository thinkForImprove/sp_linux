...
#else
#define NOTE_ID_UNKNOWN         ((USHORT)0x80)            //30-00-00-00(FS#0021)
#endif
...
enum STACKER_STATUS
{
    ADP_STACKER_EMPTY        = 0,
    ADP_STACKER_NOTEMPTY     = 1,
    ADP_STACKER_FULL         = 2,           //30-00-00-00(FT#0029)
    ADP_STACKER_UNKNOWN      = 3,           //30-00-00-00(FT#0029)
    ADP_STACKER_NOTSUPPORTED = 4            //30-00-00-00(FT#0029)
};
#endif
...
enum ADP_CUERROR
{
    ...
    ADP_CUERROR_CONTINUEREJECT = 6, //连续拒钞多错误      //30-00-00-00(FS#0020)
    ADP_CUERROR_TOOMANYREJECT  = 7, //太多拒钞错误      　//30-00-00-00(FS#0020)
    ADP_CUERROR_MISSFEED       = 8  //missfeed      　  //30-00-00-00(FS#0020)
};
...
enum ADP_LFS_CMD_ID{
    ...
    ADP_OTHER           = 7
#else
    ADP_CIM_SELFCOUNT   = 7,                //30-00-00-00(FS#0022)
    ADP_OTHER           = 8                 //30-00-00-00(FS#0022)
#endif
};
//test#8 add end

//测试出钞票信息
//30-00-00-00(FS#0007) add start
#define LG_BOX_NUM          5
#define LG_ROOM_NUM         2
typedef struct _DENOINFO
{
    BYTE byNoteID;
    ULONG ulCount;
}DENOINFO, *LPDENOINFO;
typedef struct _TESTDISPINFO
{
    WORD wNumofCSPerRoom[LG_ROOM_NUM][LG_BOX_NUM];			//每个room到CS的出钞张数
    WORD wCountToRoom[LG_ROOM_NUM][LG_BOX_NUM];				//每个钞箱的纸币数量变化(+)
    WORD wCountFromRoom[LG_ROOM_NUM][LG_BOX_NUM];			//每个钞箱的纸币数量变化(-)
    WORD wRBBoxRJCount[LG_ROOM_NUM][LG_BOX_NUM];            //从每个RB箱流出的RJ纸币数量
    DENOINFO stPerDenomiNumofRJ[128];                       //拒钞中每个金种的纸币和数量
}TESTDISPINFO, *LPTESTDISPINFO;
//30-00-00-00(FS#0007) add end
/*
 *
enum BRMTYPE
{
...
virtual long Dispense(const ULONG aryulItems[ADP_MAX_CU_SIZE],
    ...
    //说明：
#if defined(SET_BANK_CMBC) | defined(SET_BANK_CSCB)
    virtual long ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject, ADP_CUERROR arywCUError[], WORD wCashInCnt) = 0;  //test#13
#elif defined(SET_BANK_SXXH)
    virtual long ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject, ADP_CUERROR arywCUError[], WORD wCashInCnt, BOOL bForceMode = FALSE) = 0;  //test#13
#else
    virtual long ValidateAndCounting(char pResult[256], ULONG &ulRejCount, ADP_ERR_REJECT &iErrReject, ULONG ulMaxNumber, ULONG ulMaxAmount) = 0;   //30-00-00-00(FS#0002)
#endif
    ...
    //功能：测试钞箱
    //输出：stTestDispInfo：测试出钞命令的执行结果信息　byCashUnitError：测试出钞命令钞箱出错信息
    virtual long TestCashUnitsHT(BYTE byRoomExclusion[2][5],
                                 TESTDISPINFO &stTestDispInfo,
                                 ADP_CUERROR wCUErrors[ADP_MAX_CU_SIZE])// = 0;
    {
...
#if defined(SET_BANK_SXXH)
    virtual void vDeleteSnrinfoForce() = 0;  //test#28
#endif
    ...
    virtual void vSetCimCashinLimit(ULONG ulTotalItemsLimit,ULONG ulAmount, LPCSTR lpCurrencyID) = 0;  //test#27
#endif

    virtual void SetLFSCmdId(ADP_LFS_CMD_ID eLfsCmdId) = 0;           //test#8

#if defined(SET_BANK_CMBC)
    virtual BOOL IsConnected() = 0;                                   //40-00-00-00(FT#0015)
    virtual void ClearError() = 0;
#elif defined(SET_BANK_CMBC) | defined(SET_BANK_CSCB)
    //功能：手动清除错误/故障码　lfg
    //输入：无
    //输出：无
    //返回：无
